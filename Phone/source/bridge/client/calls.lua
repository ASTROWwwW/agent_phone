...
function Bridge.Calls.Join(channel)
    ...
    if selected == "pma" then
        ...
        if call_channel <= 0 then
            Bridge.Debug("error", "[agent_phone] Refused to join an invalid PMA call channel.", { always = true })
            return false
        end
    ...
    Bridge.Debug(
        "error",
        "[agent_phone] Configured call voice provider '%s' is not supported or not started.",
