...
function Bridge.Callbacks.Trigger(name, data)
    ...
    TriggerServerEvent("agent_phone:bridge:callback:request", name, request_id, data)
    ...
    SetTimeout(Config.Bridge.CallbackTimeout, function()
        ...
        Bridge.Debug("error", "[agent_phone] Server callback '%s' timed out.", tostring(name))
        request:resolve(nil)
    end)
...
RegisterNetEvent("agent_phone:bridge:callback:response", function(request_id, result)
