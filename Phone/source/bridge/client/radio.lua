...
function Bridge.Radio.GetSpeaker()
    ...
    if not success then
        Bridge.Debug(
            "error",
            "[agent_phone] SaltyChat could not read the radio speaker state: %s",
            tostring(enabled),
            { always = true }
        )
        return false
...
function Bridge.Radio.SetSpeaker(enabled)
    ...
    if not success then
        Bridge.Debug(
            "error",
            "[agent_phone] SaltyChat could not update the local radio speaker state: %s",
...
function Bridge.Radio.Join(primary, secondary)
    ...
    if selected == "yaca" then
        ...
        if not voice:isEnabled() then
            Bridge.Debug("error", "[agent_phone] Yaca is started but its voice system is disabled.")
            return false
        end
    ...
    Bridge.Debug("error", "[agent_phone] No supported radio voice provider is running.")
    return false
end
...
AddEventHandler("agent_phone:configurator:updated", function()
    ...
    TriggerEvent("agent_phone:client:radioProviderUpdated")
end)
