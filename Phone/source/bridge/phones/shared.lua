AgentPhoneCompatibility = {}

function AgentPhoneCompatibility.RegisterExportAlias(resource_name, export_name, handler)
    ...
    AddEventHandler(("__cfx_export_%s_%s"):format(resource_name, export_name), function(set_callback)
        set_callback(function(...)
            local debug_custom_app = AgentPhoneApps and AgentPhoneApps.Debug
...
function AgentPhoneCompatibility.EmitServerProviderStop(resource_name)
...
function AgentPhoneCompatibility.EmitServerProviderStart(resource_name)
    assert(type(resource_name) == "string" and resource_name ~= "", "Provider resource must be a non-empty string")
...
AgentPhoneCompatibility.Providers = {
    lb = "lb_phone",
    seventeen = "17mov",
    high = "high_phone",
    quasar = "quasar_v3",
...
function AgentPhoneCompatibility.NormalizeAtResourceUrl(owner_resource, url, error_prefix)
