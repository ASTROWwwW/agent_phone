...
local function query_or_error(query, parameters, context)
    ...
    if not success then
        error(("[agent_phone] Database migration failed while %s: %s"):format(context, tostring(result)))
    end
    return result
end
...
function Bridge.Database.CompleteMigration(migration_name)
    if completed_migrations[migration_name] then
        error(("[agent_phone] Database migration '%s' was completed more than once."):format(tostring(migration_name)))
    end
    ...
    Bridge.Debug("info", "[agent_phone] Database migration '%s' completed.", migration_name)
...
function Bridge.Database.AfterMigration(migration_name, callback)
    if type(callback) ~= "function" then
        error("[agent_phone] Database migration callback must be a function.")
    end
    if completed_migrations[migration_name] then
        callback()
        return
...
function Bridge.Database.AwaitMigration(migration_name)
    ...
    Bridge.Debug("error", "[agent_phone] Database migration '%s' has not completed.", tostring(migration_name))
    return false
end
