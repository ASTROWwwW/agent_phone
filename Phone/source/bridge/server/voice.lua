...
local function yaca_is_enabled()
    ...
    if not success then
        Bridge.Debug(
            "error",
            "[agent_phone] Yaca could not report its availability: %s",
            tostring(enabled),
            { always = true }
        )
        return false
...
function Bridge.Calls.Start(identifier, player_handles)
    ...
    if selected == "yaca" then
        ...
        if not caller_source or not target_source then
            Bridge.Debug(
                "error",
                "[agent_phone] Yaca refused call %s because a player source was invalid.",
                tostring(identifier),
                { always = true }
            )
            return false, selected
        ...
        if not success then
            Bridge.Debug(
                "error",
                "[agent_phone] Yaca could not start call %s: %s",
                tostring(identifier),
    ...
    if not success then
        Bridge.Debug(
            "error",
            "[agent_phone] SaltyChat could not add players to call %s: %s",
            tostring(identifier),
            tostring(error_message),
            { always = true }
        )
...
function Bridge.Calls.Stop(identifier, player_handles, provider)
    ...
    if selected == "yaca" then
        ...
        if not caller_source or not target_source then
            Bridge.Debug(
                "error",
                "[agent_phone] Yaca could not stop call %s because a player source was invalid.",
                tostring(identifier),
                { always = true }
            )
            return
        ...
        if not success then
            Bridge.Debug(
                "error",
                "[agent_phone] Yaca could not stop call %s: %s",
                tostring(identifier),
                tostring(error_message),
                { always = true }
            )
    ...
    if not success then
        Bridge.Debug(
            "error",
            "[agent_phone] SaltyChat could not remove players from call %s: %s",
            tostring(identifier),
            tostring(error_message),
            { always = true }
        )
