...
function Bridge.Housing.ResolveProvider()
    ...
    if configured ~= "auto" then
        if provider_available(configured) then
            ...
            report_provider_state(
                "offline:" .. configured,
                ("[agent_phone] Configured housing provider '%s' is unavailable."):format(configured)
            )
            return nil, nil
        end
    else
        ...
        if not selected_provider_name then
            report_provider_state("offline:auto", "[agent_phone] No supported housing provider is running.")
            return nil, nil
        end
    end
    ...
    report_provider_state(
        "online",
        ("[agent_phone] Housing provider '%s' is active."):format(selected_provider_name)
    )
    return selected_provider_name, Bridge.Housing.Providers[selected_provider_name]
end
