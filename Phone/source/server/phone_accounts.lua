...
function AgentPhoneAccounts.List(account_id, current_imei)
    local rows = Bridge.Database.Query([[
        SELECT `imei`, `device_name`, `created_at`, `updated_at`
        FROM `agent_phone_devices`
        WHERE `account_id` = ?
...
local function link_account(source, account)
    ...
    if not AgentPhoneCalls.LinkAccountData(account.id, session.imei) then
        return { success = false, error = "request_failed" }
    end
    if not Bridge.Database.Transaction({
        {
            query = "UPDATE `agent_phone_devices` SET `account_id` = ? WHERE `imei` = ?",
            params = { account.id, session.imei },
        },
        {
            query = [[
                UPDATE `agent_phone_notes`
        ...
        {
            query = [[
                UPDATE `agent_phone_music_playlist_items` AS `item`
                INNER JOIN `agent_phone_music_playlists` AS `playlist`
                    ON `playlist`.`id` = `item`.`playlist_id`
                INNER JOIN `agent_phone_music_youtube_songs` AS `local_song`
                    ON `item`.`source` = 'youtube' AND `local_song`.`id` = `item`.`song_id`
                INNER JOIN `agent_phone_music_youtube_songs` AS `cloud_song`
        ...
        {
            query = [[
                DELETE `local_song` FROM `agent_phone_music_youtube_songs` AS `local_song`
                INNER JOIN `agent_phone_music_youtube_songs` AS `cloud_song`
                    ON `cloud_song`.`account_id` = ? AND `cloud_song`.`video_id` = `local_song`.`video_id`
        ...
        {
            query = [[
                UPDATE `agent_phone_music_youtube_songs`
                SET `account_id` = ?, `device_imei` = NULL
                WHERE `device_imei` = ? AND `account_id` IS NULL
            ]],
            params = { account.id, session.imei },
        ...
        {
            query = [[
                UPDATE `agent_phone_music_playlists`
                SET `account_id` = ?, `device_imei` = NULL
                WHERE `device_imei` = ? AND `account_id` IS NULL
            ]],
            params = { account.id, session.imei },
    ...
    return {
        ...
        data = {
            email = account.email,
            devices = AgentPhoneAccounts.List(account.id, session.imei),
        },
    }
end
...
local function authenticate(source, data, registering)
    ...
    local accounts = Bridge.Database.Query(
        "SELECT `id`, `email` FROM `agent_phone_accounts` WHERE `email` = ? AND `password` = ? LIMIT 1",
        { email, password }
    )
...
for _, endpoint in ipairs({ "account:login", "mail:login" }) do
    Bridge.Callbacks.Register("agent_phone:" .. endpoint, function(source, data)
...
for _, endpoint in ipairs({ "account:logout", "mail:logout" }) do
    Bridge.Callbacks.Register("agent_phone:" .. endpoint, function(source)
        local account, error_response = AgentPhone.RequireAccount(source)
        if not account then
            return error_response
        end
        if not AgentPhoneCalls.CopyCloudToDevice(account.id, account.imei) then
            return { success = false, error = "request_failed" }
        end
        Bridge.Database.Query("UPDATE `agent_phone_devices` SET `account_id` = NULL WHERE `imei` = ?", { account.imei })
        AgentPhone.RefreshSource(source)
        return { success = true }
    end)
end

Bridge.Callbacks.Register("agent_phone:account:devices", function(source)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
        return error_response
    end
    return { success = true, data = AgentPhoneAccounts.List(account.id, account.imei) }
end)

Bridge.Callbacks.Register("agent_phone:account:remove-device", function(source, data)
    if not AgentPhone.AllowOperation(source, "remove_device", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
    ...
    if type(data) ~= "table" or not AgentPhoneImei.IsValid(data.imei) or not valid_password(data.password) then
        return { success = false, error = "invalid_request" }
    end
    if data.imei == account.imei then
        return { success = false, error = "current_device" }
    end
    local passwords = Bridge.Database.Query("SELECT `id` FROM `agent_phone_accounts` WHERE `id` = ? AND `password` = ? LIMIT 1", {
        account.id,
        data.password,
    })
    ...
    local result = Bridge.Database.Query(
        "UPDATE `agent_phone_devices` SET `account_id` = NULL WHERE `imei` = ? AND `account_id` = ?",
    ...
    AgentPhone.RefreshAccount(account.id)
    return { success = true, data = AgentPhoneAccounts.List(account.id, account.imei) }
end)
