...
Bridge.Callbacks.Register("agent_phone:map:markers", function(source)
...
Bridge.Callbacks.Register("agent_phone:map:create-marker", function(source, data)
    if not AgentPhone.AllowOperation(source, "map_marker_write", Config.MapMarkers.ActionsPerMinute, 60) then
...
Bridge.Callbacks.Register("agent_phone:map:delete-marker", function(source, data)
    if not AgentPhone.AllowOperation(source, "map_marker_write", Config.MapMarkers.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    local result = Bridge.Database.Query(
        "DELETE FROM `agent_phone_map_markers` WHERE `id` = ? AND `device_imei` = ?",
