...
local function invoice_select(where_sql, parameters)
    local rows = Bridge.Database.Query(([=[
        ...
        FROM `agent_phone_billing_invoices`
        WHERE %s
...
local function create_invoice(data)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_billing_events` (`invoice_id`, `event`, `actor_identifier`)
        VALUES (?, 'created', ?)
    ]], { id, issuer_identifier })
    notify_identifier(recipient_identifier, "agent_phone:billing:new", {
        amount = amount,
        issuer = issuer_label,
    })
    return id
...
Bridge.Callbacks.Register("agent_phone:billing:overview", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:billing:list", function(source, data)
    local identifier, error_response = require_billing_session(source)
    if not identifier then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:billing:detail", function(source, data)
    local identifier, error_response = require_billing_session(source)
    if not identifier then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:billing:markRead", function(source, data)
    local identifier, error_response = require_billing_session(source)
    if not identifier then
        return error_response
    end
    ...
    Bridge.Database.Query([[
        UPDATE `agent_phone_billing_invoices` SET `read_at` = COALESCE(`read_at`, NOW())
        WHERE `id` = ? AND `recipient_identifier` = ?
    ]], { data and data.id, identifier })
    return { success = true, data = { unreadCount = unread_count(identifier) } }
end)

Bridge.Callbacks.Register("agent_phone:billing:dispute", function(source, data)
    ...
    local result = Bridge.Database.Query([[
        UPDATE `agent_phone_billing_invoices` SET `status` = 'disputed', `read_at` = COALESCE(`read_at`, NOW())
    ...
    notify_identifier(identifier, "agent_phone:billing:changed")
    local row = find_owned_invoice(data.id, identifier)
    return { success = true, data = invoice_dto(row, identifier) }
end)

Bridge.Callbacks.Register("agent_phone:billing:pay", function(source, data)
    if not AgentPhone.AllowOperation(source, "billing_payment", Config.Billing.ActionsPerMinute, 60) then
    ...
    local claim = Bridge.Database.Query([[
        UPDATE `agent_phone_billing_invoices` SET `status` = 'processing'
        WHERE `id` = ? AND `recipient_identifier` = ? AND `status` = 'open'
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_billing_payments` (`id`, `invoice_id`, `recipient_identifier`, `amount`)
        VALUES (?, ?, ?, ?)
    ...
    if not Bridge.Framework.RemoveMoney(source, Config.Billing.PaymentAccount, amount) then
        Bridge.Database.Transaction({
            ...
            { query = "UPDATE `agent_phone_billing_payments` SET `status` = 'failed', `error_code` = 'insufficient_funds' WHERE `id` = ?", params = { payment_id } },
        })
        return { success = false, error = "insufficient_funds" }
    end
    local completed = Bridge.Database.Transaction({
        { query = "UPDATE `agent_phone_billing_invoices` SET `status` = 'paid', `paid_at` = NOW(), `read_at` = COALESCE(`read_at`, NOW()), `payment_reference` = ? WHERE `id` = ? AND `status` = 'processing'", params = { payment_id, invoice_id } },
        { query = "UPDATE `agent_phone_billing_payments` SET `status` = 'paid' WHERE `id` = ?", params = { payment_id } },
        { query = "INSERT INTO `agent_phone_billing_events` (`invoice_id`, `event`, `actor_identifier`) VALUES (?, 'paid', ?)", params = { invoice_id, identifier } },
        { query = "INSERT INTO `agent_phone_billing_accounts` (`account_key`, `balance`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `balance` = `balance` + VALUES(`balance`)", params = { row.issuer_account, amount } },
        { query = "INSERT INTO `agent_phone_bank_transactions` (`owner_identifier`, `kind`, `amount`, `label`, `reference`) VALUES (?, 'withdrawal', ?, ?, ?)", params = { identifier, amount, row.issuer_label, payment_id } },
    })
    ...
    if row.issuer_identifier ~= "" then
        notify_identifier(row.issuer_identifier, "agent_phone:billing:changed")
    end
...
exports("CancelInvoice", function(invoice_id, actor_identifier)
    ...
    local result = Bridge.Database.Query([[
        UPDATE `agent_phone_billing_invoices` SET `status` = 'cancelled'
    ...
    local rows = Bridge.Database.Query("SELECT `recipient_identifier`, `issuer_identifier` FROM `agent_phone_billing_invoices` WHERE `id` = ? LIMIT 1", { invoice_id })
    if rows[1] then
        notify_identifier(rows[1].recipient_identifier, "agent_phone:billing:changed")
        if rows[1].issuer_identifier ~= "" then
            notify_identifier(rows[1].issuer_identifier, "agent_phone:billing:changed")
        end
    end
    return true
end)
...
exports("GetBillingAccountBalance", function(account_key)
    ...
    local rows = Bridge.Database.Query("SELECT `balance` FROM `agent_phone_billing_accounts` WHERE `account_key` = ? LIMIT 1", { key })
...
exports("RemoveBillingAccountBalance", function(account_key, value)
    ...
    local result = Bridge.Database.Query([[
        UPDATE `agent_phone_billing_accounts` SET `balance` = `balance` - ?
