...
local function ensure_schema()
    local statements = {
        [[CREATE TABLE IF NOT EXISTS `agent_phone_crypto_profiles` (
            ...
            UNIQUE KEY `uniq_agent_phone_crypto_key` (`crypto_key`)
    ...
    Bridge.Database.Query("ALTER TABLE `agent_phone_crypto_profiles` ADD COLUMN IF NOT EXISTS `price_alerts` TINYINT(1) UNSIGNED NOT NULL DEFAULT 1 AFTER `password_hash`", {})
    Bridge.Database.Query("ALTER TABLE `agent_phone_crypto_profiles` ADD COLUMN IF NOT EXISTS `trade_confirmations` TINYINT(1) UNSIGNED NOT NULL DEFAULT 1 AFTER `price_alerts`", {})
    Bridge.Database.Query("ALTER TABLE `agent_phone_crypto_profiles` ADD COLUMN IF NOT EXISTS `hide_balances` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 AFTER `trade_confirmations`", {})
    Bridge.Database.Query("ALTER TABLE `agent_phone_crypto_profiles` ADD COLUMN IF NOT EXISTS `crypto_key` CHAR(22) CHARACTER SET ascii COLLATE ascii_bin NULL AFTER `handle`", {})
    Bridge.Database.Query([[ALTER TABLE `agent_phone_crypto_operations`
        MODIFY COLUMN `type` ENUM('buy','sell','deposit','withdrawal','transfer_in','transfer_out') NOT NULL]], {})
    Bridge.Database.Query("ALTER TABLE `agent_phone_crypto_operations` ADD COLUMN IF NOT EXISTS `quantity` DECIMAL(36,0) UNSIGNED NOT NULL DEFAULT 0 AFTER `market_id`", {})
    Bridge.Database.Query("ALTER TABLE `agent_phone_crypto_operations` ADD COLUMN IF NOT EXISTS `counterparty_key` CHAR(22) CHARACTER SET ascii COLLATE ascii_bin NULL AFTER `quantity`", {})
    Bridge.Database.Query("ALTER TABLE `agent_phone_crypto_operations` MODIFY COLUMN `counterparty_key` CHAR(22) CHARACTER SET ascii COLLATE ascii_bin NULL", {})
end

local function new_id()
    local row = Bridge.Database.Query("SELECT UUID() AS `id`", {})[1]
    if not row or type(row.id) ~= "string" then
        error("[agent_phone] Database did not generate a Crypto id.")
    end
    return row.id
end
...
local function new_crypto_key()
    for _ = 1, 5 do
        ...
        local duplicate = Bridge.Database.Query(
            "SELECT 1 FROM `agent_phone_crypto_profiles` WHERE `crypto_key` = ? LIMIT 1",
            { candidate }
        )[1]
    ...
    error("[agent_phone] Database did not generate a unique Crypto key.")
end

local function migrate_crypto_keys()
    local rows = Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_crypto_profiles` WHERE `crypto_key` IS NULL OR `crypto_key` = ''",
        {}
    )
    for _, row in ipairs(rows) do
        Bridge.Database.Query(
            "UPDATE `agent_phone_crypto_profiles` SET `crypto_key` = ? WHERE `id` = ? AND (`crypto_key` IS NULL OR `crypto_key` = '')",
            { new_crypto_key(), row.id }
        )
    end
    Bridge.Database.Query(
        "ALTER TABLE `agent_phone_crypto_profiles` MODIFY COLUMN `crypto_key` CHAR(22) CHARACTER SET ascii COLLATE ascii_bin NOT NULL",
        {}
    )
    local index = Bridge.Database.Query([[
        SELECT 1 FROM information_schema.statistics
        WHERE table_schema = DATABASE() AND table_name = 'agent_phone_crypto_profiles'
            AND index_name = 'uniq_agent_phone_crypto_key' LIMIT 1
    ]], {})[1]
    if not index then
        Bridge.Database.Query(
            "ALTER TABLE `agent_phone_crypto_profiles` ADD UNIQUE KEY `uniq_agent_phone_crypto_key` (`crypto_key`)",
            {}
        )
    end
end
...
local function initialize_markets()
    ...
    for _, config in ipairs(Config.Crypto.Markets) do
        if next_markets[config.Id] then
            error(("[agent_phone] Config.Crypto.Markets contains duplicate id '%s'."):format(config.Id))
        end
        ...
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_crypto_markets`
        ...
        local persisted = Bridge.Database.Query([[
            ...
            FROM `agent_phone_crypto_markets` WHERE `id` = ? LIMIT 1
        ]], { config.Id })[1]
        if not persisted
        ...
        then
            error(("[agent_phone] Crypto market scale or supply changed without a migration: %s"):format(config.Id))
        end
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_crypto_balances` (`account_id`,`asset_id`,`available`)
...
local function load_market_cache()
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_crypto_markets`
    ]], {})
    ...
    for _, market_id in ipairs(market_order) do
        ...
        if not state then
            error(("[agent_phone] Crypto market state is missing after initialization: %s"):format(market_id))
        end
        local rows_for_market = Bridge.Database.Query([[
            SELECT `price`
            FROM `agent_phone_crypto_market_ticks`
...
local function persist_market_cache()
    ...
    for _, market_id in ipairs(market_order) do
        ...
        if state and state.dirty then
            queries[#queries + 1] = {
                query = [[
                    UPDATE `agent_phone_crypto_markets`
            ...
            queries[#queries + 1] = {
                query = [[
                    INSERT INTO `agent_phone_crypto_market_ticks`
...
local function require_phone(source)
    local phone_session, error_response = AgentPhone.RequireSession(source)
...
local function profile_by_owner(identifier)
    return Bridge.Database.Query([[
        ...
        FROM `agent_phone_crypto_profiles` WHERE `owner_identifier` = ? LIMIT 1
    ]], { identifier })[1]
end
...
local function profile_by_crypto_key(wallet_key)
    return Bridge.Database.Query([[
        ...
        FROM `agent_phone_crypto_profiles` WHERE `crypto_key` = ? LIMIT 1
    ]], { wallet_key })[1]
end
...
local function verify_password(profile_id, password)
    ...
    local row = Bridge.Database.Query(
        "SELECT `password_hash` FROM `agent_phone_crypto_profiles` WHERE `id` = ? LIMIT 1",
        { profile_id }
    )[1]
...
local function balance(account, asset)
    local row = Bridge.Database.Query([[
        SELECT `available`,`locked`,`version` FROM `agent_phone_crypto_balances`
...
local function activity(profile_id)
    local rows = Bridge.Database.Query([[
        SELECT `id`,`type`,`amount`,`market_id`,`quantity`,`counterparty_key`,`status`,
            UNIX_TIMESTAMP(`created_at`) AS `created_at`
        FROM `agent_phone_crypto_operations` WHERE `profile_id` = ?
...
local function daily_total(profile_id, operation_type)
    local row = Bridge.Database.Query([[
        ...
        FROM `agent_phone_crypto_operations`
    ...
    for _, market_id in ipairs(market_order) do
        ...
        if available > 0 then
            ...
            local fill = Bridge.Database.Query([[
                ...
                FROM `agent_phone_crypto_fills` fill
                JOIN `agent_phone_crypto_operations` operation ON operation.`id` = fill.`operation_id`
...
local function audit(profile_id, identifier, event_type, detail)
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_crypto_audit_events`
...
Bridge.Callbacks.Register("agent_phone:crypto:bootstrap", function(source)
...
Bridge.Callbacks.Register("agent_phone:crypto:register", function(source, data)
    if not AgentPhone.AllowOperation(source, "crypto:register", 5, 60) then
        return { success = false, error = "rate_limited" }
    end
    local phone_session, identifier, error_response = require_phone(source)
    if not phone_session then
    ...
    local account, account_error = AgentPhone.RequireAccount(source)
    if not account then
    ...
    local duplicate = Bridge.Database.Query(
        "SELECT 1 FROM `agent_phone_crypto_profiles` WHERE `handle` = ? LIMIT 1",
        { handle }
    )
    if duplicate[1] then
    ...
    if type(password_hash) ~= "string" then
        error("[agent_phone] Crypto password provider did not return a password hash.")
    end
    local queries = {
        {
            query = [[INSERT INTO `agent_phone_crypto_profiles`
        ...
        {
            query = [[INSERT INTO `agent_phone_crypto_balances`
...
Bridge.Callbacks.Register("agent_phone:crypto:login", function(source, data)
    if not AgentPhone.AllowOperation(source, "crypto:login", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    local phone_session, identifier, error_response = require_phone(source)
    if not phone_session then
    ...
    if not verify_password(profile.id, type(data) == "table" and data.password or nil) then
        ...
        Bridge.Database.Query([[
            UPDATE `agent_phone_crypto_profiles` SET `failed_logins` = ?,
    ...
    Bridge.Database.Query(
        "UPDATE `agent_phone_crypto_profiles` SET `failed_logins` = 0, `locked_until` = NULL WHERE `id` = ?",
        { profile.id }
    )
    set_session(source, phone_session, profile, true)
...
Bridge.Callbacks.Register("agent_phone:crypto:logout", function(source)
    local session = sessions[source]
    if session then
...
Bridge.Callbacks.Register("agent_phone:crypto:update-profile", function(source, data)
    if not AgentPhone.AllowOperation(source, "crypto:update-profile", 8, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = authenticated_profile(source)
    if not profile then
    ...
    if handle_changed then
        local duplicate = Bridge.Database.Query(
            "SELECT 1 FROM `agent_phone_crypto_profiles` WHERE `handle` = ? AND `id` <> ? LIMIT 1",
            { handle, profile.id }
        )
        if duplicate[1] then
            return { success = false, error = "handle_taken" }
    ...
    if password_changed then
        ...
        Bridge.Database.Query([[
            UPDATE `agent_phone_crypto_profiles`
...
Bridge.Callbacks.Register("agent_phone:crypto:recipient", function(source, data)
    if not AgentPhone.AllowOperation(source, "crypto:recipient", 20, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = authenticated_profile(source)
    if not profile then
...
local function execute_transfer(profile, data)
    ...
    local existing = Bridge.Database.Query([[
        SELECT `status` FROM `agent_phone_crypto_operations`
    ...
    local queries = {
        {
            query = [[INSERT INTO `agent_phone_crypto_operations`
        ...
        {
            query = [[UPDATE `agent_phone_crypto_balances`
        ...
        {
            query = [[INSERT INTO `agent_phone_crypto_ledger_entries`
    ...
    for target_source, session in pairs(sessions) do
        if session.profile_id == recipient.id then
            TriggerClientEvent("agent_phone:crypto:account-changed", target_source)
        end
    end
    return { success = true, data = bootstrap(profile) }
end

Bridge.Callbacks.Register("agent_phone:crypto:transfer", function(source, data)
    if not AgentPhone.AllowOperation(source, "crypto:transfer", 12, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = authenticated_profile(source)
    if not profile then
...
Bridge.Callbacks.Register("agent_phone:crypto:quote", function(source, data)
    if not AgentPhone.AllowOperation(source, "crypto:quote", Config.Crypto.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = authenticated_profile(source)
    if not profile then
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_crypto_quotes`
...
local function execute_trade(profile, data)
    ...
    local quote = Bridge.Database.Query([[
        SELECT quote.*
        FROM `agent_phone_crypto_quotes` quote
    ...
    local expiry = Bridge.Database.Query(
        "SELECT UNIX_TIMESTAMP(`expires_at`) AS `expires_at` FROM `agent_phone_crypto_quotes` WHERE `id` = ?",
        { quote.id }
    )[1]
    ...
    if quote.side == "buy" then
        queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` - ?, `version` = `version` + 1 WHERE `account_id` = ? AND `asset_id` = 'CASH' AND `available` >= ?]], params = { net_currency, player_account, net_currency } }
        queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` + ?, `version` = `version` + 1 WHERE `account_id` = 'treasury' AND `asset_id` = 'CASH']], params = { net_currency } }
        queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` - ?, `version` = `version` + 1 WHERE `account_id` = 'treasury' AND `asset_id` = ? AND `available` >= ?]], params = { quantity, quote.market_id, quantity } }
        queries[#queries + 1] = { query = [[INSERT INTO `agent_phone_crypto_balances` (`account_id`,`asset_id`,`available`,`version`) VALUES (?, ?, ?, 1) ON DUPLICATE KEY UPDATE `available` = `available` + VALUES(`available`), `version` = `version` + 1]], params = { player_account, quote.market_id, quantity } }
    else
        queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` - ?, `version` = `version` + 1 WHERE `account_id` = ? AND `asset_id` = ? AND `available` >= ?]], params = { quantity, player_account, quote.market_id, quantity } }
        queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` + ?, `version` = `version` + 1 WHERE `account_id` = 'treasury' AND `asset_id` = ?]], params = { quantity, quote.market_id } }
        queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` - ?, `version` = `version` + 1 WHERE `account_id` = 'treasury' AND `asset_id` = 'CASH' AND `available` >= ?]], params = { net_currency, net_currency } }
        queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` + ?, `version` = `version` + 1 WHERE `account_id` = ? AND `asset_id` = 'CASH']], params = { net_currency, player_account } }
    end
    queries[#queries + 1] = { query = [[INSERT INTO `agent_phone_crypto_ledger_entries` (`operation_id`,`account_id`,`asset_id`,`delta`) VALUES (?, ?, 'CASH', ?), (?, 'treasury', 'CASH', ?), (?, ?, ?, ?), (?, 'treasury', ?, ?)]], params = {
    ...
    queries[#queries + 1] = { query = [[INSERT INTO `agent_phone_crypto_fills` (`id`,`operation_id`,`quote_id`,`market_id`,`side`,`quantity`,`price`,`gross`,`fee`,`net`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)]], params = { fill_id, operation_id, quote.id, quote.market_id, quote.side, quantity, quote.price, quote.gross, quote.fee, quote.net } }
    queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_quotes` SET `consumed_operation_id` = ? WHERE `id` = ? AND `consumed_operation_id` IS NULL]], params = { operation_id, quote.id } }
    queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_operations` SET `status` = 'completed' WHERE `id` = ?]], params = { operation_id } }
    if not Bridge.Database.Transaction(queries) then
        return { success = false, error = "request_failed" }
    end
...
Bridge.Callbacks.Register("agent_phone:crypto:execute", function(source, data)
    if not AgentPhone.AllowOperation(source, "crypto:trade", Config.Crypto.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = authenticated_profile(source)
    if not profile then
...
local function settlement_ledger_queries(operation_id, profile_id, kind, ledger_amount)
    ...
    if kind == "deposit" then
        queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` + ?, `version` = `version` + 1 WHERE `account_id` = ? AND `asset_id` = 'CASH']], params = { ledger_amount, player_account } }
        queries[#queries + 1] = { query = [[INSERT INTO `agent_phone_crypto_ledger_entries` (`operation_id`,`account_id`,`asset_id`,`delta`) VALUES (?, ?, 'CASH', ?), (?, 'external:bank', 'CASH', ?)]], params = { operation_id, player_account, ledger_amount, operation_id, -ledger_amount } }
    else
        queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `locked` = `locked` - ?, `version` = `version` + 1 WHERE `account_id` = ? AND `asset_id` = 'CASH' AND `locked` >= ?]], params = { ledger_amount, player_account, ledger_amount } }
        queries[#queries + 1] = { query = [[INSERT INTO `agent_phone_crypto_ledger_entries` (`operation_id`,`account_id`,`asset_id`,`delta`) VALUES (?, ?, 'CASH', ?), (?, 'external:bank', 'CASH', ?)]], params = { operation_id, player_account, -ledger_amount, operation_id, ledger_amount } }
    end
    queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_operations` SET `status` = 'completed' WHERE `id` = ? AND `status` = 'external_applied']], params = { operation_id } }
    queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_settlements` SET `state` = 'completed' WHERE `operation_id` = ? AND `state` = 'external_applied']], params = { operation_id } }
    return queries
end
...
local function settle(source, profile, kind, data)
    ...
    local prepared = {
        ...
        {
            query = [[INSERT INTO `agent_phone_crypto_settlements`
    ...
    Bridge.Database.Query("UPDATE `agent_phone_crypto_operations` SET `status` = 'external_pending' WHERE `id` = ?", { operation_id })
    Bridge.Database.Query("UPDATE `agent_phone_crypto_settlements` SET `state` = 'external_pending' WHERE `operation_id` = ?", { operation_id })
    ...
    if not money_success then
        Bridge.Database.Query("UPDATE `agent_phone_crypto_operations` SET `status` = 'manual_review', `detail` = 'framework_call_ambiguous' WHERE `id` = ?", { operation_id })
        Bridge.Database.Query("UPDATE `agent_phone_crypto_settlements` SET `state` = 'manual_review' WHERE `operation_id` = ?", { operation_id })
    ...
    if not money_result then
        local failed = {
            { query = [[UPDATE `agent_phone_crypto_operations` SET `status` = 'failed', `detail` = 'framework_rejected' WHERE `id` = ?]], params = { operation_id } },
            { query = [[UPDATE `agent_phone_crypto_settlements` SET `state` = 'failed' WHERE `operation_id` = ?]], params = { operation_id } },
        }
        if kind == "withdrawal" then
            failed[#failed + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` + ?, `locked` = `locked` - ?, `version` = `version` + 1 WHERE `account_id` = ? AND `asset_id` = 'CASH' AND `locked` >= ?]], params = { ledger_amount, ledger_amount, account_id(profile.id), ledger_amount } }
        end
        Bridge.Database.Transaction(failed)
        return { success = false, error = kind == "deposit" and "insufficient_funds" or "request_failed" }
    end
    Bridge.Database.Query("UPDATE `agent_phone_crypto_operations` SET `status` = 'external_applied' WHERE `id` = ?", { operation_id })
    Bridge.Database.Query("UPDATE `agent_phone_crypto_settlements` SET `state` = 'external_applied' WHERE `operation_id` = ?", { operation_id })
    ...
    if not Bridge.Database.Transaction(ledger_queries) then
        Bridge.Database.Query("UPDATE `agent_phone_crypto_operations` SET `status` = 'manual_review', `detail` = 'ledger_apply_failed' WHERE `id` = ?", { operation_id })
        Bridge.Database.Query("UPDATE `agent_phone_crypto_settlements` SET `state` = 'manual_review' WHERE `operation_id` = ?", { operation_id })
        audit(profile.id, profile.owner_identifier, "settlement_manual_review", operation_id)
        return { success = false, error = "settlement_pending" }
    end
...
for _, callback in ipairs({
    ...
    Bridge.Callbacks.Register("agent_phone:crypto:" .. callback.name, function(source, data)
        if not AgentPhone.AllowOperation(source, "crypto:settlement", 8, 60) then
            return { success = false, error = "rate_limited" }
        end
        local profile, error_response = authenticated_profile(source)
        if not profile then
...
local function reconcile_settlements(include_recent)
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_crypto_operations` operation
        JOIN `agent_phone_crypto_settlements` settlement ON settlement.`operation_id` = operation.`id`
    ...
    for _, operation in ipairs(rows) do
        ...
        if operation.status == "prepared" then
            ...
            if operation.type == "withdrawal" then
                queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_balances` SET `available` = `available` + ?, `locked` = `locked` - ?, `version` = `version` + 1 WHERE `account_id` = ? AND `asset_id` = 'CASH' AND `locked` >= ?]], params = { ledger_amount, ledger_amount, account_id(operation.profile_id), ledger_amount } }
            end
            queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_operations` SET `status` = 'cancelled', `detail` = 'reconciled_before_external_call' WHERE `id` = ? AND `status` = 'prepared']], params = { operation.id } }
            queries[#queries + 1] = { query = [[UPDATE `agent_phone_crypto_settlements` SET `state` = 'cancelled' WHERE `operation_id` = ? AND `state` = 'prepared']], params = { operation.id } }
            Bridge.Database.Transaction(queries)
        elseif operation.status == "external_pending" then
            Bridge.Database.Transaction({
                { query = [[UPDATE `agent_phone_crypto_operations` SET `status` = 'manual_review', `detail` = 'reconciled_ambiguous_external_call' WHERE `id` = ? AND `status` = 'external_pending']], params = { operation.id } },
                { query = [[UPDATE `agent_phone_crypto_settlements` SET `state` = 'manual_review' WHERE `operation_id` = ? AND `state` = 'external_pending']], params = { operation.id } },
            })
        elseif operation.status == "external_applied" then
            local entries = Bridge.Database.Query(
                "SELECT COUNT(*) AS `count` FROM `agent_phone_crypto_ledger_entries` WHERE `operation_id` = ?",
                { operation.id }
            )[1]
            if tonumber(entries and entries.count) == 0 then
                ...
                Bridge.Database.Transaction({
                    { query = [[UPDATE `agent_phone_crypto_operations` SET `status` = 'manual_review', `detail` = 'unexpected_partial_ledger' WHERE `id` = ?]], params = { operation.id } },
                    { query = [[UPDATE `agent_phone_crypto_settlements` SET `state` = 'manual_review' WHERE `operation_id` = ?]], params = { operation.id } },
                })
            end
        end
    end
    ...
    if global_market_cycle.remaining_ticks <= 0 then
        if global_market_cycle.direction == 0 then
            global_market_cycle.direction = crypto_random_int(
                0,
                2,
                "[agent_phone] Crypto entropy provider did not return a global market direction."
            ) == 0 and -1 or 1
        else
        ...
        global_market_cycle.remaining_ticks = crypto_random_int(
            ...
            "[agent_phone] Crypto entropy provider did not return a global market cycle duration."
        )
        global_market_cycle.target = global_market_cycle.direction * crypto_random_int(
            ...
            "[agent_phone] Crypto entropy provider did not return a global market trend."
        )
    end
    ...
    if dynamics.cycle_remaining_ticks <= 0 then
        if dynamics.cycle_direction == 0 then
            dynamics.cycle_direction = crypto_random_int(
                0,
                2,
                "[agent_phone] Crypto entropy provider did not return a market cycle direction."
            ) == 0 and -1 or 1
        else
        ...
        dynamics.cycle_remaining_ticks = crypto_random_int(
            ...
            "[agent_phone] Crypto entropy provider did not return a market cycle duration."
        )
        local strength = crypto_random_int(
            ...
            "[agent_phone] Crypto entropy provider did not return a market cycle strength."
        )
    ...
    CreateThread(function()
        while scheduler_generation == generation and Config.Crypto.Enabled == true do
            local tick_seconds = crypto_random_int(
                ...
                "[agent_phone] Crypto entropy provider did not return a market tick interval."
            )
            ...
            with_exchange_lock(function()
                local market_count = crypto_random_int(
                    ...
                    "[agent_phone] Crypto entropy provider did not return a market count."
                )
                ...
                for offset = 0, market_count - 1 do
                    ...
                    if row and row.status == "active" then
                        ...
                        local impulse = crypto_random_int(
                            ...
                            "[agent_phone] Crypto entropy provider did not return a market movement."
                        )
                        if impulse > 0 then
                        ...
                        local shock_roll = crypto_random_int(
                            0,
                            10000,
                            "[agent_phone] Crypto entropy provider did not return a market shock roll."
                        )
                        ...
                        if shock_roll < Config.Crypto.MarketShockChanceBasisPoints then
                            local multiplier = crypto_random_int(
                                ...
                                "[agent_phone] Crypto entropy provider did not return a market shock multiplier."
                            )
                            local direction_roll = crypto_random_int(
                                0,
                                2,
                                "[agent_phone] Crypto entropy provider did not return a market shock direction."
                            )
                ...
                if #changed_markets > 0 then
                    TriggerClientEvent("agent_phone:crypto:changed", -1, {
...
local function refresh_crypto_runtime()
    if exchange_lock then
        print("[agent_phone] Crypto runtime refresh skipped because the exchange is busy.")
        return
    end
    with_exchange_lock(function()
...
AddEventHandler("agent_phone:configurator:serverUpdated", refresh_crypto_runtime)
