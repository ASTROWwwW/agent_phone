...
Bridge.Database.Query([[
    UPDATE `agent_phone_easyshare_transfers`
...
local rich_note_prefix = "agent-note-html-v1:"
...
local function canonical_profile(device, app_id, id)
    if app_id == "picstagram" then
        local profile = first_row([[
            ...
            LEFT JOIN `agent_phone_media` media ON media.`id` = p.`avatar_media_id`
            WHERE p.`id` = ? AND p.`status` = 'active'
                AND (p.`private` = 0 OR EXISTS(
                    SELECT 1 FROM `agent_phone_picstagram_sessions` session
                    ...
                    WHERE follow.`following_id` = p.`id` AND follow.`status` = 'accepted'
                        AND follow.`follower_id` = (
                            SELECT session.`profile_id` FROM `agent_phone_picstagram_sessions` session
        ...
        if profile then
            return {
                ...
                link = "agentphone://picstagram/profile/" .. id,
            }
        end
    elseif app_id == "feather" then
        ...
        if profile then
            return {
                ...
                copyText = "@" .. profile.handle,
                imageUrl = profile.image_url,
                link = "agentphone://feather/profile/" .. id,
            }
        end
    elseif app_id == "fliptok" then
        local profile = first_row("SELECT `display_name`, `handle` FROM `agent_phone_fliptok_profiles` WHERE `id` = ? LIMIT 1", { id })
        if profile then
            return {
                title = profile.display_name,
                subtitle = "@" .. profile.handle,
                copyText = "@" .. profile.handle,
                link = "agentphone://fliptok/profile/" .. id,
            }
        end
    elseif app_id == "flare" and device.account_id then
        local profile = first_row([[
            SELECT p.`name`, p.`age`, p.`bio`, media.`url` AS `image_url`
            FROM `agent_phone_flare_profiles` p
            LEFT JOIN `agent_phone_flare_profile_photos` photo ON photo.`profile_id` = p.`id` AND photo.`sort_order` = 1
            LEFT JOIN `agent_phone_media` media ON media.`id` = photo.`media_id`
    ...
    elseif app_id == "darkchat" and device.account_id then
        ...
        if profile then
            return {
                ...
                link = "agentphone://darkchat/invite/" .. profile.invite_code,
            }
        end
    elseif app_id == "companies" then
        ...
        if definition and definition.Public then
            local profile = first_row([[
                ...
                LEFT JOIN `agent_phone_media` media ON media.`id` = profile.`logo_media_id`
            ...
            return {
                ...
                link = "agentphone://companies/profile/" .. id,
            }
        end
    end
    return nil
...
local function canonical_post(device, app_id, id)
    if app_id == "picstagram" then
        local post = first_row([[
            ...
            LEFT JOIN `agent_phone_media` media ON media.`id` = post_media.`media_id`
        ...
        if post then
            ...
            return {
                ...
                link = "agentphone://picstagram/post/" .. id,
            }
        end
    elseif app_id == "feather" then
        local post = first_row([[
            SELECT post.`body`, profile.`handle`, media.`url` AS `image_url`
            FROM `agent_phone_feather_posts` post
            JOIN `agent_phone_feather_profiles` profile ON profile.`id` = post.`profile_id`
            LEFT JOIN `agent_phone_feather_post_media` post_media
        ...
        if post then
            return {
                ...
                imageUrl = post.image_url,
                link = "agentphone://feather/post/" .. id,
            }
        end
    elseif app_id == "fliptok" then
        local post = first_row([[
            SELECT video.`caption`, profile.`display_name`, profile.`handle`, media.`url` AS `image_url`
            FROM `agent_phone_fliptok_videos` video
            JOIN `agent_phone_fliptok_profiles` profile ON profile.`id` = video.`profile_id`
            JOIN `agent_phone_media` media ON media.`id` = video.`media_id`
            WHERE video.`id` = ? AND video.`status` = 'published' AND (
                video.`visibility` = 'public' OR EXISTS(
                    SELECT 1 FROM `agent_phone_fliptok_sessions` session
                    WHERE session.`device_imei` = ? AND session.`profile_id` = profile.`id`
                    ...
                    WHERE follow.`following_id` = profile.`id` AND follow.`follower_id` = (
                        SELECT session.`profile_id` FROM `agent_phone_fliptok_sessions` session
                        WHERE session.`device_imei` = ? LIMIT 1
                    )
                ))
            ) LIMIT 1
    ...
    elseif app_id == "citymarkt" then
        local post = first_row([[
            ...
            FROM `agent_phone_marketplace_listings` listing
            LEFT JOIN `agent_phone_marketplace_images` image
        ...
        if post then
            ...
            return {
                ...
                copyText = post.title .. "\n" .. post.description,
                imageUrl = post.image_url,
                link = "agentphone://citymarkt/listing/" .. id,
            }
        end
    end
    return nil
...
local function canonical_music(device, data)
        ...
        for _, track in ipairs(Config.Music.Tracks) do
            if track.Id == data.id then
                return {
                    ...
                    link = "agentphone://music/server/" .. track.Id,
                    meta = { source = "server" },
                }
            end
        end
    elseif data.kind == "track" and type(data.meta) == "table" and data.meta.source == "youtube" then
        ...
        local track = first_row(([=[
            ...
            FROM `agent_phone_music_youtube_songs` song
        ...
        if track then
            return {
                ...
                link = "agentphone://music/youtube/" .. data.id,
                meta = { source = "youtube" },
            }
        end
    elseif data.kind == "playlist" then
        ...
        local playlist = first_row(([=[
            ...
            FROM `agent_phone_music_playlists` playlist
            LEFT JOIN `agent_phone_music_playlist_items` item ON item.`playlist_id` = playlist.`id`
        ...
        if playlist then
            return {
                ...
                link = "agentphone://music/playlist/" .. data.id,
            }
        end
    end
    return nil
...
local function canonical_document(source, device, app_id, id)
    if app_id == "calendar" and device.account_id then
        local event = first_row([[
            SELECT `title`, `note`, `starts_at`, `ends_at` FROM `agent_phone_calendar_events`
            WHERE `id` = ? AND `account_id` = ? LIMIT 1
        ]], { id, tonumber(device.account_id) })
        if event then
            return {
                ...
                link = "agentphone://calendar/event/" .. id,
    ...
    elseif app_id == "mail" and device.account_id then
        local message = first_row([[
            ...
            FROM `agent_phone_mail_entries` entry
            JOIN `agent_phone_mail_messages` message ON message.`id` = entry.`message_id`
            LEFT JOIN `agent_phone_accounts` sender ON sender.`id` = message.`sender_account_id`
...
local function canonical_text(device, app_id, id)
    ...
    local message = first_row([[
        ...
        JOIN `agent_phone_darkchat_profiles` viewer ON viewer.`id` = member.`profile_id`
        LEFT JOIN `agent_phone_darkchat_profiles` sender ON sender.`id` = message.`sender_profile_id`
...
local function visibility_for(imei)
    local rows = Bridge.Database.Query([[ 
        SELECT `visibility` FROM `agent_phone_easyshare_preferences`
...
local function visible_to(target_device, sender_device, visibility)
    ...
    local rows = Bridge.Database.Query(([[
        SELECT `id` FROM `agent_phone_contacts`
...
local function sanitize_payload(source, device, data)
    ...
    if data.kind == "contact" and type(payload.id) == "string" then
        if app_id == "phone" and payload.id == "self" then
            ...
            local rows = Bridge.Database.Query(([[
                ...
                FROM `agent_phone_contacts` WHERE `contact_id` = ? AND %s LIMIT 1
            ]]):format(condition), params)
    ...
    elseif data.kind == "note" and type(payload.id) == "string" then
        ...
        append_params(params, owner_params)
        local rows = Bridge.Database.Query(([[
            SELECT `title`, `body` FROM `agent_phone_notes`
    ...
    elseif data.kind == "photo" or data.kind == "video" then
        local url, media_error = AgentPhoneMedia.ResolveOwnedMedia(source, payload.id, data.kind)
        if not url then
        ...
        append_params(media_params, owner_params)
        local media_rows = Bridge.Database.Query(([[
            SELECT `remote_id` FROM `agent_phone_media`
            WHERE `id` = ? AND %s LIMIT 1
...
function AgentPhoneEasyShare.SanitizeChatPayload(source, data)
...
local function notify_transfer(transfer)
    TriggerClientEvent("agent_phone:easyshare:changed", transfer.sender_source, {
        transfer = transfer_for_source(transfer, transfer.sender_source),
    })
    TriggerClientEvent("agent_phone:easyshare:changed", transfer.recipient_source, {
...
local function apply_received_payload(transfer)
    local device = AgentPhone.LoadDevice(transfer.recipient_imei)
    if not device then
        return false
    end
    ...
    if transfer.payload.kind == "contact" then
        ...
        TriggerClientEvent("agent_phone:contacts:changed", transfer.recipient_source, {})
    elseif transfer.payload.kind == "note" or transfer.payload.kind == "text" or transfer.payload.kind == "document" then
        Bridge.Database.Query([[ 
            INSERT INTO `agent_phone_notes`
        ...
        if account_id then
            AgentPhone.RefreshAccount(account_id)
        else
            AgentPhone.RefreshDevice(device.imei)
        end
    elseif transfer.payload.kind == "location" then
        Bridge.Database.Query([[ 
            INSERT INTO `agent_phone_map_markers`
    ...
    elseif transfer.payload.kind == "photo" or transfer.payload.kind == "video" then
        Bridge.Database.Query([[ 
            INSERT INTO `agent_phone_media` (`account_id`, `device_imei`, `url`, `remote_id`, `media_type`)
            VALUES (?, ?, ?, ?, ?)
        ]], {
            account_id, account_id and nil or device.imei, meta.url, meta.remoteId, transfer.payload.kind,
        })
        TriggerClientEvent("agent_phone:gallery:changed", transfer.recipient_source, {})
...
local function history(device)
    local rows = Bridge.Database.Query([[ 
        ...
        FROM `agent_phone_easyshare_transfers`
...
Bridge.Callbacks.Register("agent_phone:easyshare:bootstrap", function(source)
    if not Config.EasyShare.Enabled then
        return { success = false, error = "disabled" }
    end
    if not AgentPhone.AllowOperation(source, "easyshare_bootstrap", Config.EasyShare.BootstrapRequestsPerMinute, 60) then
...
Bridge.Callbacks.Register("agent_phone:easyshare:own-contact", function(source)
    if not Config.EasyShare.Enabled then
        return { success = false, error = "disabled" }
    end
    if not AgentPhone.AllowOperation(source, "easyshare_own_contact", Config.EasyShare.BootstrapRequestsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local device, error_response = current_device(source)
    if not device then
...
Bridge.Callbacks.Register("agent_phone:easyshare:set-visibility", function(source, data)
    if not Config.EasyShare.Enabled then
        return { success = false, error = "disabled" }
    end
    if not AgentPhone.AllowOperation(source, "easyshare_visibility", Config.EasyShare.VisibilityUpdatesPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local device, error_response = current_device(source)
    if not device then
...
Bridge.Callbacks.Register("agent_phone:easyshare:request", function(source, data)
    if not Config.EasyShare.Enabled then
        return { success = false, error = "disabled" }
    end
    if not AgentPhone.AllowOperation(source, "easyshare_request", Config.EasyShare.RequestsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
...
Bridge.Callbacks.Register("agent_phone:easyshare:respond", function(source, data)
    if not Config.EasyShare.Enabled then
        return { success = false, error = "disabled" }
    end
    if not AgentPhone.AllowOperation(source, "easyshare_action", Config.EasyShare.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
...
Bridge.Callbacks.Register("agent_phone:easyshare:cancel", function(source, data)
    if not Config.EasyShare.Enabled then
        return { success = false, error = "disabled" }
    end
    if not AgentPhone.AllowOperation(source, "easyshare_action", Config.EasyShare.ActionsPerMinute, 60) then
