...
if type(config) ~= "table" then
    error("[agent_phone] Config.CityWarn must be configured.")
end
...
local function require_integer_config(name, minimum, maximum)
    ...
    if type(value) ~= "number" or value ~= math.floor(value) or value < minimum or value > maximum then
        error(("[agent_phone] Config.CityWarn.%s must be an integer between %d and %d."):format(name, minimum, maximum))
    end
end

if type(config.Enabled) ~= "boolean" or type(config.RequireDuty) ~= "boolean" then
    error("[agent_phone] CityWarn Enabled and RequireDuty must be booleans.")
end
...
if type(config.RateLimits) ~= "table" or type(config.Publishers) ~= "table" then
    error("[agent_phone] CityWarn RateLimits and Publishers must be tables.")
end
for _, name in ipairs({ "Read", "Write" }) do
    ...
    if type(value) ~= "number" or value ~= math.floor(value) or value < 1 or value > 10000 then
        error(("[agent_phone] Invalid CityWarn rate limit '%s'."):format(name))
    end
end
for job_name, publisher in pairs(config.Publishers) do
    ...
    then
        error(("[agent_phone] Invalid CityWarn publisher '%s'."):format(tostring(job_name)))
    end
    local seen = {}
    for _, category in ipairs(publisher.Categories) do
        if not categories[category] or seen[category] then
            error(("[agent_phone] Invalid CityWarn category '%s' for publisher '%s'."):format(tostring(category), job_name))
        end
        seen[category] = true
    end
end
...
local function new_uuid()
    ...
    if not valid_uuid(id) then
        error("[agent_phone] Database did not generate a valid CityWarn UUID.")
    end
    return id
end
...
local function require_phone(source, operation, maximum)
    ...
    if not AgentPhone.AllowOperation(source, "citywarn_" .. operation, maximum, 60) then
...
local function attach_updates(alerts)
    ...
    local rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_citywarn_updates`
        WHERE `alert_id` IN (%s)
...
local function expire_alerts()
    Bridge.Database.Query([[
        UPDATE `agent_phone_citywarn_alerts`
...
local function query_alerts(status)
    ...
    local rows = Bridge.Database.Query(([[
        SELECT %s
        FROM `agent_phone_citywarn_alerts` alert
        WHERE %s
...
local function load_alert(id)
    local rows = Bridge.Database.Query(([[
        SELECT %s FROM `agent_phone_citywarn_alerts` alert WHERE alert.`id` = ? LIMIT 1
...
local function broadcast(kind, alert)
    TriggerClientEvent("agent_phone:citywarn:changed", -1, {
        alert = alert,
        alertId = alert.id,
        kind = kind,
...
Bridge.Callbacks.Register("agent_phone:citywarn:bootstrap", function(source)
...
Bridge.Callbacks.Register("agent_phone:citywarn:publish", function(source, data)
    ...
    local counts = Bridge.Database.Query([[
        SELECT COUNT(*) AS `count` FROM `agent_phone_citywarn_alerts`
        WHERE `status` = 'active' AND `expires_at` > NOW()
    ]], {})
    ...
    local result = Bridge.Database.Query([[
        INSERT INTO `agent_phone_citywarn_alerts`
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_citywarn_updates`
...
Bridge.Callbacks.Register("agent_phone:citywarn:update", function(source, data)
    local _, error_response = require_phone(source, "write", config.RateLimits.Write)
    if error_response then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:citywarn:resolve", function(source, data)
    local _, error_response = require_phone(source, "write", config.RateLimits.Write)
    if error_response then
        return error_response
    end
