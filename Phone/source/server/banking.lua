...
local function transactions(identifier)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_bank_transactions`
...
local function record_transaction(identifier, kind, amount, label, reference)
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_bank_transactions`
...
local function notify_changed(source, data)
    TriggerClientEvent("agent_phone:banking:changed", source, data)
end
...
local function online_source_for_phone(number)
    local rows = Bridge.Database.Query([[
        ...
        INNER JOIN `agent_phone_devices` d ON d.`sim_id` = s.`id`
...
Bridge.Callbacks.Register("agent_phone:banking:overview", function(source)
...
Bridge.Callbacks.Register("agent_phone:banking:transfer", function(source, data)
    if not AgentPhone.AllowOperation(source, "banking_transaction", Config.Banking.ActionsPerMinute, 60) then
