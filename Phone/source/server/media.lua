...
AgentPhoneMedia = {}
AgentPhoneMediaImport.Initialize()
...
local function media_debug(message, ...)
    ...
    Bridge.Debug("debug", "[agent_phone][media-debug] " .. message, table.unpack(arguments))
end
...
local function request_presigned_url()
    local api_key = AgentPhoneMediaProviderConfig.FiveManageApiKey()
...
local function encode_remote_path(value)
    ...
    for segment in tostring(value):gmatch("[^/]+") do
        segments[#segments + 1] = AgentPhoneMediaImport.UrlEncode(segment)
    end
...
function AgentPhoneMedia.ResolveOwnedMedia(source, media_id, media_type)
    ...
    then
        Bridge.Debug(
            "warn",
            "[agent_phone] Rejected unowned %s media %s from source %s.",
            media_type,
            tostring(media_id),
            tostring(source)
        )
    ...
    if media.origin == "website_import" then
        ...
        if os.time() - verified_at >= revalidate_after then
            ...
            if type(media.remote_id) == "string" and media.remote_id:sub(1, 4) == "url:" then
                refreshed, refresh_error = AgentPhoneMediaImport.ResolveUrl(media.source_id, media.url)
            else
                refreshed, refresh_error = AgentPhoneMediaImport.Resolve(media.source_id, media.remote_id)
            end
...
local function upload_result(source, correlation_id, success, error_code, media)
    ...
    TriggerClientEvent("agent_phone:media:upload-result", source, {
...
local function delete_result(source, correlation_id, success, error_code, media_id)
    TriggerClientEvent("agent_phone:media:delete-result", source, {
        correlationId = correlation_id,
        success = success,
        error = error_code,
        id = media_id,
...
local function delete_many_result(source, correlation_id, success, error_code, deleted_ids)
    TriggerClientEvent("agent_phone:media:delete-many-result", source, {
        correlationId = correlation_id,
        success = success,
        error = error_code,
...
local function verify_remote_upload(state, remote_id, uploaded_url)
    ...
    then
        Bridge.Debug(
            "error",
            "[agent_phone][media-debug] Upload completion payload is invalid (remote-id=%s, url=%s).",
...
AgentPhoneMedia.DeleteRemoteFile = delete_remote_file
AgentPhoneMedia.RequestPresignedUrl = request_presigned_url
AgentPhoneMedia.VerifyRemoteUpload = verify_remote_upload
...
local function expire_upload(request_id)
    ...
    Bridge.Debug(
        "error",
        "[agent_phone][media-debug] Upload session expired before completion (source=%s, correlation=%s).",
...
Bridge.Callbacks.Register("agent_phone:gallery:list", function(source, data)
    local owner, error_response = session_owner(source)
    if not owner then
...
Bridge.Callbacks.Register("agent_phone:gallery:favorite", function(source, data)
    if not AgentPhone.AllowOperation(source, "media_favorite", 60, 60)
        or type(data) ~= "table"
    ...
    Bridge.Database.Query(
        ("UPDATE `agent_phone_media` SET `favorite` = ? WHERE `id` = ? AND %s"):format(condition),
        update_params
    )
    return {
        success = true,
...
Bridge.Callbacks.Register("agent_phone:gallery:counts", function(source)
    local owner, error_response = session_owner(source)
    if not owner then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:messages:gifs", function(source, data)
    if not AgentPhone.AllowOperation(source, "gif_search", 30, 60) then
    ...
    if response.status == 401 or response.status == 403 then
        Bridge.Debug("error", "[agent_phone] GIPHY rejected the configured API key with HTTP %s.", tostring(response.status))
        return { success = false, error = "gif_provider_unauthorized" }
    end
    if response.status == 429 then
        Bridge.Debug("error", "[agent_phone] GIPHY rate limit reached.")
        return { success = false, error = "gif_provider_rate_limited" }
    end
    if response.status < 200 or response.status >= 300 then
        Bridge.Debug("error", "[agent_phone] GIPHY request failed with HTTP %s.", tostring(response.status))
...
Bridge.Callbacks.Register("agent_phone:media:config", function(source)
    local owner, error_response = session_owner(source)
    if not owner then
        return error_response
    end
...
RegisterNetEvent("agent_phone:media:request-upload", function(data)
    local src = source
    data = type(data) == "table" and data or {}
    ...
    then
        Bridge.Debug(
            "error",
            "[agent_phone][media-debug] Upload request validation failed (source=%s, correlation-type=%s, correlation-length=%s, media-type=%s).",
            tostring(src),
            type(correlation_id),
    ...
    if not AgentPhone.AllowOperation(src, "media_write", 20, 60) then
        Bridge.Debug(
            "warn",
            "[agent_phone][media-debug] Upload request was rate limited (source=%s, correlation=%s).",
            tostring(src),
            diagnostic_text(correlation_id, 80)
        )
    ...
    if not owner then
        Bridge.Debug(
            "error",
            "[agent_phone][media-debug] Upload request has no valid phone session (source=%s, correlation=%s, error=%s).",
            tostring(src),
            diagnostic_text(correlation_id, 80),
    ...
    if not presigned_url then
        Bridge.Debug(
            "error",
            "[agent_phone][media-debug] Presigned upload request failed (source=%s, correlation=%s, error=%s).",
            tostring(src),
            diagnostic_text(correlation_id, 80),
    ...
    if type(request_id) ~= "string" then
        Bridge.Debug(
            "error",
            "[agent_phone][media-debug] Database did not generate an upload request ID (source=%s, correlation=%s, rows=%s, request-id=%s).",
            tostring(src),
            diagnostic_text(correlation_id, 80),
    ...
    TriggerClientEvent("agent_phone:media:upload-ready", src, {
        correlationId = correlation_id,
...
RegisterNetEvent("agent_phone:media:complete-upload", function(data)
    local src = source
    data = type(data) == "table" and data or {}
    ...
    if not state or state.source ~= src or state.completing then
        Bridge.Debug(
            "warn",
            "[agent_phone][media-debug] Rejected upload completion (source=%s, request-id=%s, session=%s, owner-match=%s, completing=%s).",
            tostring(src),
    ...
    if not owner or not owners_match(owner, state.owner) then
        ...
        Bridge.Debug(
            "error",
            "[agent_phone][media-debug] Upload owner changed before completion (source=%s, correlation=%s, session=%s, owner-match=%s, error=%s).",
            tostring(src),
            diagnostic_text(state.correlation_id, 80),
    ...
    if not verified then
        ...
        Bridge.Debug(
            "error",
            "[agent_phone][media-debug] Uploaded file verification failed (source=%s, correlation=%s, error=%s, trusted-remote=%s).",
            tostring(src),
            diagnostic_text(state.correlation_id, 80),
    ...
    if owner.account_id then
        result = Bridge.Database.Query([[
            INSERT INTO `agent_phone_media` (`account_id`, `device_imei`, `url`, `remote_id`, `media_type`, `mime_type`)
            VALUES (?, NULL, ?, ?, ?, ?)
    ...
    if not media_id then
        ...
        Bridge.Debug(
            "error",
            "[agent_phone][media-debug] Database insert did not return a media ID (source=%s, correlation=%s, result-type=%s).",
            tostring(src),
            diagnostic_text(state.correlation_id, 80),
            type(result)
        )
...
RegisterNetEvent("agent_phone:media:cancel-upload", function(data)
    local src = source
...
RegisterNetEvent("agent_phone:media:fail-upload", function(data)
    local src = source
    local request_id = type(data) == "table" and data.requestId or nil
    local state = type(request_id) == "string" and pending_uploads[request_id] or nil
    if not state or state.source ~= src or state.completing then
        Bridge.Debug(
            "warn",
            "[agent_phone][media-debug] Rejected client upload failure report (source=%s, request-id=%s, session=%s, owner-match=%s, completing=%s).",
            tostring(src),
            type(request_id) == "string" and "present" or "invalid",
            state and "present" or "missing",
            tostring(state and state.source == src),
    ...
    Bridge.Debug(
        "error",
        "[agent_phone][media-debug] Client-reported upload failure (source=%s, correlation=%s, error=%s, stage=%s, status=%s, detail=%s).",
        tostring(src),
        diagnostic_text(state.correlation_id, 80),
        diagnostic_text(error_code, 80),
...
local function is_required_flare_profile_photo(media_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_flare_profile_photos` photo
        JOIN `agent_phone_flare_profiles` profile ON profile.`id` = photo.`profile_id`
        WHERE photo.`media_id` = ?
            AND NOT EXISTS (
                SELECT 1
                FROM `agent_phone_flare_profile_photos` other_photo
                JOIN `agent_phone_media` other_media
...
local function delete_owned_media(src, owner, media_id)
    ...
    local rows = Bridge.Database.Query(([[
        SELECT `id`, `remote_id`, `origin`, `media_type`, `url` FROM `agent_phone_media`
    ...
    if row.origin == "phone_upload" then
        local references = Bridge.Database.Query(
            "SELECT COUNT(*) AS `count` FROM `agent_phone_media` WHERE `remote_id` = ?",
            { row.remote_id }
        )
    ...
    Bridge.Database.Query(("DELETE FROM `agent_phone_media` WHERE `id` = ? AND %s"):format(condition), query_params)
    pending_deletes[delete_key] = nil
    return true, nil, row.url
end

RegisterNetEvent("agent_phone:media:delete", function(data)
    local src = source
    data = type(data) == "table" and data or {}
    local correlation_id = data.correlationId
    local media_id = tonumber(data.id)
    ...
    if not AgentPhone.AllowOperation(src, "media_delete", 30, 60) then
...
RegisterNetEvent("agent_phone:media:delete-many", function(data)
    local src = source
    data = type(data) == "table" and data or {}
    local correlation_id = data.correlationId
    ...
    if not AgentPhone.AllowOperation(src, "media_delete_many", 10, 60) then
...
function AgentPhoneMedia.GetDeviceRemoteIds(imei)
    local rows = Bridge.Database.Query([[
        SELECT `id`, `remote_id` FROM `agent_phone_media`
        WHERE `account_id` IS NULL AND `device_imei` = ? AND `origin` = 'phone_upload'
    ]], { imei })
    return rows
end

function AgentPhoneMedia.CleanupRemoteFiles(rows)
