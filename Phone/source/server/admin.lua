...
local function require_admin(source, operation, maximum)
    ...
    then
        Bridge.Debug("warn", "[agent_phone] Rejected admin panel access from source %s.", tostring(source))
        return nil, { success = false, error = "not_authorized" }
    end
    if not AgentPhone.AllowOperation(source, "admin_" .. operation, maximum, 60) then
...
local function run_admin_command(command_source)
    ...
    if not player_source or player_source < 1 then
        Bridge.Debug("warn", "[agent_phone] The admin panel command can only be used by a player.")
        return
    end
    if not Config.AdminPanel.Enabled then
        TriggerClientEvent("agent_phone:admin:command-error", player_source, "disabled")
        return
    end
    if not Bridge.Framework.HasPermission(player_source, "phonepanel") then
        Bridge.Debug(
            "warn",
            "[agent_phone] Rejected admin panel command from source %s.",
            tostring(player_source)
        )
        TriggerClientEvent("agent_phone:admin:command-error", player_source, "not_authorized")
        return
    end

    TriggerClientEvent("agent_phone:admin:launch", player_source)
end
...
local function register_admin_command()
    ...
    if type(command_name) ~= "string" or command_name == "" then
        error("[agent_phone] Config.AdminPanel.Command must be a non-empty command name.")
    end
...
local function collect_device_imeis(source, identifier)
    ...
    local function add_imei(imei)
        if AgentPhoneImei.IsValid(imei) and not seen[imei] then
            seen[imei] = true
            imeis[#imeis + 1] = imei
        end
    end
    ...
    local rows = Bridge.Database.Query([[
        ...
        UNION
        SELECT device.`imei`
        FROM `agent_phone_devices` device
        JOIN `agent_phone_sims` sim ON sim.`id` = device.`sim_id`
...
local function load_app_payload(encoded)
    ...
    if type(payload) ~= "table" then
        error("[agent_phone] Stored admin target app payload is not a JSON object.")
    end
...
local function load_player_devices(source, identifier)
    ...
    local rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_devices` device
        LEFT JOIN `agent_phone_sims` sim ON sim.`id` = device.`sim_id`
        LEFT JOIN `agent_phone_accounts` account ON account.`id` = device.`account_id`
        LEFT JOIN `agent_phone_device_security` security ON security.`device_imei` = device.`imei`
        LEFT JOIN `agent_phone_device_data` app_data
...
local function app_metadata(app_id)
    if BUILTIN_APPS[app_id] then
        if not AgentPhone.IsAppEnabled(app_id) then
            return nil
        end
        return {
    ...
    if AgentPhoneApps.GetPolicy(app_id) then
...
local function write_audit(actor_source, target_source, target_identifier, imei, action, details)
    ...
    local result = Bridge.Database.Query([[
        INSERT INTO `agent_phone_admin_audit`
...
Bridge.Callbacks.Register("agent_phone:admin:bootstrap", function(source)
    ...
    local totals = Bridge.Database.Query([[
        SELECT
            (SELECT COUNT(*) FROM `agent_phone_devices`) AS `devices`,
            (SELECT COUNT(*) FROM `agent_phone_accounts`) AS `accounts`,
            (SELECT COUNT(*) FROM `agent_phone_devices` WHERE `account_id` IS NOT NULL) AS `linked_devices`,
            (SELECT COUNT(*) FROM `agent_phone_devices` WHERE `sim_id` IS NOT NULL) AS `sim_devices`,
            (SELECT COUNT(*) FROM `agent_phone_devices`
                WHERE `updated_at` >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 24 HOUR)) AS `active_devices`,
            (SELECT COUNT(*) FROM `agent_phone_sms_messages`
                WHERE `created_at` >= CURRENT_DATE) AS `messages_today`,
            (SELECT COUNT(*) FROM `agent_phone_calls`
                WHERE `started_at` >= CURRENT_DATE) AS `calls_today`,
            (SELECT COUNT(*) FROM `agent_phone_admin_audit`) AS `audit_entries`,
            (SELECT COUNT(*) FROM `agent_phone_admin_audit`
...
Bridge.Callbacks.Register("agent_phone:admin:player", function(source, data)
    local authorized, error_response = require_admin(
        source,
        "player",
        Config.AdminPanel.ReadRequestsPerMinute
...
Bridge.Callbacks.Register("agent_phone:admin:save-apps", function(source, data)
    local authorized, error_response = require_admin(
        source,
        "save_apps",
    ...
    if type(data) ~= "table"
        or not AgentPhoneImei.IsValid(data.imei)
    ...
    if rows[1] then
        local result = Bridge.Database.Query([[
            UPDATE `agent_phone_device_data`
        ...
        local result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_device_data` (`device_imei`, `namespace`, `payload`)
            VALUES (?, 'apps', ?)
        ]], { data.imei, encoded })
        if affected_rows(result) ~= 1 then
            return { success = false, error = "revision_conflict" }
    ...
    AgentPhone.RefreshDevice(data.imei)
    return { success = true, data = load_player_detail(target_source) }
end)

Bridge.Callbacks.Register("agent_phone:admin:reveal-password", function(source, data)
    local authorized, error_response = require_admin(
        source,
        "reveal_password",
    ...
    if type(data) ~= "table" or not AgentPhoneImei.IsValid(data.imei) then
        return { success = false, error = "invalid_request" }
    end
    ...
    local accounts = Bridge.Database.Query([[
        ...
        FROM `agent_phone_devices` device
        JOIN `agent_phone_accounts` account ON account.`id` = device.`account_id`
        WHERE device.`imei` = ?
        LIMIT 1
    ]], { data.imei })
...
Bridge.Callbacks.Register("agent_phone:admin:activity", function(source, data)
    local authorized, error_response = require_admin(
        source,
        "activity",
        Config.AdminPanel.ReadRequestsPerMinute
...
Bridge.Callbacks.Register("agent_phone:admin:reset-passcode", function(source, data)
    local authorized, error_response = require_admin(
        source,
        "reset_passcode",
        Config.AdminPanel.ActionRequestsPerMinute
    ...
    local result = Bridge.Database.Query(
        "DELETE FROM `agent_phone_device_security` WHERE `device_imei` = ?",
        { data.imei }
    )
    if affected_rows(result) ~= 1 then
...
Bridge.Callbacks.Register("agent_phone:admin:change-number", function(source, data)
    local authorized, error_response = require_admin(
        source,
        "change_number",
        Config.AdminPanel.ActionRequestsPerMinute
    ...
    local changed, number_or_error = AgentPhoneSim.ChangeNumber(
        target_source,
        data.imei,
        sim.id,
        data.phoneNumber
    ...
    AgentPhoneCompanies.ClearCallAvailability(target_source)
    AgentPhoneCalls.EndForSim(sim.id, "number_changed")
...
Bridge.Callbacks.Register("agent_phone:admin:factory-reset", function(source, data)
    local authorized, error_response = require_admin(
        source,
        "factory_reset",
        Config.AdminPanel.ActionRequestsPerMinute
    ...
    local reset, phone_number_or_error = AgentPhonePersistence.FactoryReset(data.imei)
    if not reset then
    ...
    if phone_number_or_error then
        TriggerEvent("agent_phone:server:factoryReset", target_source, phone_number_or_error)
    end
    AgentPhone.RefreshDevice(data.imei)
    return { success = true, data = load_player_detail(target_source) }
end)

Bridge.Callbacks.Register("agent_phone:admin:configurator", function(source)
    local authorized, error_response = require_admin(
        source,
        "configurator_read",
        Config.AdminPanel.ReadRequestsPerMinute
...
Bridge.Callbacks.Register("agent_phone:admin:save-configurator", function(source, data)
    local authorized, error_response = require_admin(
        source,
        "configurator_save",
        Config.AdminPanel.ActionRequestsPerMinute
    ...
    local response = AgentPhoneConfigurator.Save(
        data.revision,
        data.changes,
        actor_identifier,
        actor_name
