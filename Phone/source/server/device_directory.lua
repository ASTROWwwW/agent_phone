Bridge.Database.AfterMigration("agent_phone", function()
    if type(AgentPhone) ~= "table"
        or type(AgentPhone.GetEquippedPhoneNumber) ~= "function"
        or type(AgentPhone.GetSourceFromNumber) ~= "function"
        or type(AgentPhone.FindDeviceSlots) ~= "function"
    then
        error("[agent_phone] Device directory initialized before the phone core.")
    end
    ...
    local function normalize_imei(imei)
        return AgentPhoneImei.IsValid(imei) and imei or nil
    end

    local function normalize_phone_number(phone_number)
        return AgentPhoneSimNumber.Normalize(
            phone_number,
    ...
    function AgentPhoneDeviceDirectory.GetStoredDeviceByImei(imei)
    ...
    function AgentPhoneDeviceDirectory.GetStoredDeviceByPhoneNumber(phone_number)
    ...
    function AgentPhoneDeviceDirectory.GetStoredDeviceByIdentifier(identifier)
    ...
    function AgentPhoneDeviceDirectory.GetStoredSimByPhoneNumber(phone_number)
        phone_number = normalize_phone_number(phone_number)
        if not phone_number then
            return nil, "invalid_phone_number"
        end
    ...
    function AgentPhoneDeviceDirectory.GetOnlineBySource(source)
        ...
        local phone_number = AgentPhone.GetEquippedPhoneNumber(source)
        if phone_number then
            local identity = AgentPhoneDeviceDirectory.GetStoredDeviceByPhoneNumber(phone_number)
            if not identity then
                Bridge.Debug(
                    "error",
                    "[agent_phone] Equipped phone number %s for source %s has no attached device.",
                    tostring(phone_number),
                    tostring(source)
                )
                return nil, "identity_inconsistent"
            end
            if not AgentPhone.FindDeviceSlots(source, identity.imei)[1] then
        ...
        if unique_phones then
            ...
            identity = AgentPhoneDeviceDirectory.GetStoredDeviceByImei(imeis[1])
        else
            identity = AgentPhoneDeviceDirectory.GetStoredDeviceByIdentifier(identifier)
        end
        if not identity then
            return nil, "device_not_found"
        end
    ...
    function AgentPhoneDeviceDirectory.GetOnlineByPhoneNumber(phone_number)
        phone_number = normalize_phone_number(phone_number)
        if not phone_number then
            return nil, "invalid_phone_number"
        end

        local source = AgentPhone.GetSourceFromNumber(phone_number)
        if not source then
            return nil, "player_unavailable"
        end
        local identity, identity_error = AgentPhoneDeviceDirectory.GetOnlineBySource(source)
        if not identity then
    ...
    function AgentPhoneDeviceDirectory.GetOnlineByIdentifier(identifier)
        identifier = normalize_identifier(identifier)
        if not identifier then
            return nil, "invalid_identifier"
        end
        ...
        for _, player in ipairs(Bridge.Framework.GetPlayers()) do
            ...
            if current_identifier == identifier then
                if matched_source then
                    Bridge.Debug(
                        "error",
                        "[agent_phone] Multiple online sources expose identifier %s.",
                        identifier
                    )
        ...
        return AgentPhoneDeviceDirectory.GetOnlineBySource(matched_source)
    end

    function AgentPhoneDeviceDirectory.GetOnlineByImei(imei)
        imei = normalize_imei(imei)
        if not imei then
            return nil, "invalid_imei"
        end

        local stored, stored_error = AgentPhoneDeviceDirectory.GetStoredDeviceByImei(imei)
        if not stored then
            return nil, stored_error
        end
        if stored.phoneNumber then
            local source = AgentPhone.GetSourceFromNumber(stored.phoneNumber)
            if source then
                local identity, identity_error = AgentPhoneDeviceDirectory.GetOnlineBySource(source)
                if not identity then
                    return nil, identity_error
        ...
        for _, player in ipairs(Bridge.Framework.GetPlayers()) do
            local source = tonumber(player)
            if source then
                local identity = AgentPhoneDeviceDirectory.GetOnlineBySource(source)
                if identity and identity.imei == imei then
                    if matched_identity then
                        Bridge.Debug(
                            "error",
                            "[agent_phone] Device IMEI %s is equipped by multiple online sources.",
                            imei
                        )
