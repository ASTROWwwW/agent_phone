...
local function find_device_holder(imei)
    for _, player_source in ipairs(Bridge.Framework.GetPlayers()) do
        ...
        if AgentPhone.FindDeviceSlots(source, imei)[1] then
            return source
        end
    end
    return nil
...
local function airplane_mode(imei)
    local rows = Bridge.Database.Query([[
        SELECT `payload` FROM `agent_phone_device_data`
...
local function send_state(call, source, state, channel)
    ...
    if call.payphone and outgoing then
        TriggerClientEvent("agent_phone:payphone:state", source, payload)
        return
    end
    TriggerClientEvent("agent_phone:call:state", source, payload)
end

local function notify_recents(device, source)
    if device.account_id then
        AgentPhone.NotifyAccount(device.account_id, "agent_phone:calls:changed", {})
    elseif source then
        TriggerClientEvent("agent_phone:calls:changed", source)
    end
end
...
local function settle_payphone_call(call, duration)
    ...
    if total_cost > 0 then
        ...
        if not available_money then
            Bridge.Debug(
                "error",
                "[agent_phone] Payphone settlement could not read the balance for source %s.",
    ...
    if total_cost > 0 then
        ...
        if not success then
            Bridge.Debug(
                "error",
                "[agent_phone] Payphone settlement failed for source %s: %s",
                tostring(call.caller_source),
                tostring(result)
            )
        elseif not result then
            Bridge.Debug(
                "warn",
                "[agent_phone] Payphone settlement was rejected for source %s.",
                tostring(call.caller_source)
            )
        end
    end
...
local function send_payphone_visual(call, action, delay_ms)
    ...
    local event_name = ("agent_phone:payphone:visual:%s"):format(action)
...
local function finish_call(call, status)
    ...
    if not call.payphone then
        Bridge.Database.Transaction({
            ...
            {
                query = [[
                    UPDATE `agent_phone_call_entries` SET `status` = ?
...
function AgentPhoneCalls.GetForSource(source)
    if not is_player_source(source) then
        Bridge.Debug("error", "[agent_phone] Rejected invalid player source for call lookup.")
...
function AgentPhoneCalls.GetById(call_id)
    if not is_uuid(call_id) then
        Bridge.Debug("error", "[agent_phone] Rejected invalid call ID for call lookup.")
...
function AgentPhoneCalls.IsActiveForSource(source)
    if not is_player_source(source) then
        return false
    end
...
function AgentPhoneCalls.EndForSource(source)
    if not is_player_source(source) then
        Bridge.Debug("error", "[agent_phone] Rejected invalid player source for call termination.")
...
function AgentPhoneCalls.TerminateForSource(source)
    if not is_player_source(source) then
        Bridge.Debug("error", "[agent_phone] Rejected invalid player source for forced call termination.")
        return false, "invalid_source"
    end
...
function AgentPhoneCalls.EndForSim(sim_id, reason)
...
function AgentPhoneCalls.LinkAccountData(account_id, imei)
    local counts = Bridge.Database.Query([[
        SELECT
            (SELECT COUNT(*) FROM `agent_phone_contacts` WHERE `account_id` = ?) AS `contacts`,
            (SELECT COUNT(*) FROM `agent_phone_call_entries` WHERE `account_id` = ?) AS `recents`
    ]], { account_id, account_id })
    ...
    if has_cloud_data then
        return Bridge.Database.Transaction({
            { query = "DELETE FROM `agent_phone_contacts` WHERE `device_imei` = ? AND `account_id` IS NULL", params = { imei } },
            { query = "DELETE FROM `agent_phone_call_entries` WHERE `device_imei` = ? AND `account_id` IS NULL", params = { imei } },
        })
    end
    return Bridge.Database.Transaction({
        {
            query = "UPDATE `agent_phone_contacts` SET `account_id` = ?, `device_imei` = NULL WHERE `device_imei` = ? AND `account_id` IS NULL",
            params = { account_id, imei },
        },
        {
            query = "UPDATE `agent_phone_call_entries` SET `account_id` = ?, `device_imei` = NULL WHERE `device_imei` = ? AND `account_id` IS NULL",
            params = { account_id, imei },
        },
    })
end

function AgentPhoneCalls.CopyCloudToDevice(account_id, imei)
    return Bridge.Database.Transaction({
        { query = "DELETE FROM `agent_phone_contacts` WHERE `device_imei` = ? AND `account_id` IS NULL", params = { imei } },
        { query = "DELETE FROM `agent_phone_call_entries` WHERE `device_imei` = ? AND `account_id` IS NULL", params = { imei } },
        {
            query = [[
                ...
                FROM `agent_phone_contacts` WHERE `account_id` = ?
            ]],
        ...
        },
        {
            query = [[
                ...
                FROM `agent_phone_call_entries` WHERE `account_id` = ?
            ]],
            params = { imei, account_id },
        },
    })
...
Bridge.Callbacks.Register("agent_phone:contacts:list", function(source)
    ...
    local rows = Bridge.Database.Query(([[
        SELECT `contact_id` AS `id`, `name`, `notes`, `organization`, `email`, `phone_number`, `avatar_media_id`, `favorite`,
            (SELECT media.`url` FROM `agent_phone_media` media WHERE media.`id` = `avatar_media_id`) AS `avatar_url`,
            `created_at`, `updated_at`
        FROM `agent_phone_contacts` WHERE %s ORDER BY LOWER(`name`), `phone_number`
    ]]):format(condition), params)
    local system_contacts = AgentPhoneCompanies.GetSystemContacts()
    local system_ids = {}
    local contacts = {}
    ...
    for _, contact in ipairs(rows) do
        if not system_ids[contact.id] and not AgentPhoneCompanies.IsSystemContactNumber(contact.phone_number) then
            contacts[#contacts + 1] = contact
        end
    end
...
Bridge.Callbacks.Register("agent_phone:contacts:save", function(source, data)
    if not AgentPhone.AllowOperation(source, "contact_save", 30, 60) or type(data) ~= "table" then
    ...
    local number = AgentPhoneSimNumber.Normalize(data.phoneNumber, Config.Sim.NumberLength, Config.Sim.NumberPrefix)
    ...
    if (type(data.id) == "string" and data.id:sub(1, 8) == "company:")
        or AgentPhoneCompanies.IsSystemContactNumber(number)
    then
        Bridge.Debug(
            "warn",
            "[agent_phone] Source %s attempted to modify a read-only company contact.",
            tostring(source)
        )
    ...
    if avatar_media_id > 0 then
        local media_error
        avatar_url, media_error = AgentPhoneMedia.ResolveOwnedMedia(source, tostring(avatar_media_id), "photo")
    ...
    if data.id then
        ...
        local owned = Bridge.Database.Query(("SELECT `id` FROM `agent_phone_contacts` WHERE `contact_id` = ? AND %s LIMIT 1"):format(condition), owned_params)
        if not owned[1] then
        ...
        Bridge.Database.Query(([[
            UPDATE `agent_phone_contacts` SET `name` = ?, `notes` = NULLIF(?, ''), `organization` = NULLIF(?, ''), `email` = NULLIF(?, ''), `phone_number` = ?, `avatar_media_id` = NULLIF(?, 0)
            WHERE `contact_id` = ? AND %s
        ]]):format(condition), params)
    else
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_contacts` (`id`, `contact_id`, `account_id`, `device_imei`, `name`, `notes`, `organization`, `email`, `phone_number`, `avatar_media_id`)
    ...
    if scope.account_id then
        AgentPhone.NotifyAccount(scope.account_id, "agent_phone:contacts:changed", {})
    end
    return {
        success = true,
        data = {
...
Bridge.Callbacks.Register("agent_phone:contacts:favorite", function(source, data)
    if not AgentPhone.AllowOperation(source, "contact_favorite", 60, 60) or type(data) ~= "table" or type(data.id) ~= "string" or type(data.favorite) ~= "boolean" then
        return { success = false, error = "invalid_request" }
    end
    local scope, error_response = current_scope(source)
    if not scope then
    ...
    Bridge.Database.Query(("UPDATE `agent_phone_contacts` SET `favorite` = ? WHERE `contact_id` = ? AND %s"):format(condition), params)
    if scope.account_id then
        AgentPhone.NotifyAccount(scope.account_id, "agent_phone:contacts:changed", {})
    end
...
Bridge.Callbacks.Register("agent_phone:contacts:delete", function(source, data)
    ...
    if data.id:sub(1, 8) == "company:" then
        Bridge.Debug(
            "warn",
            "[agent_phone] Source %s attempted to delete a read-only company contact.",
            tostring(source)
        )
        return { success = false, error = "readonly_contact" }
    end
    ...
    Bridge.Database.Query(("DELETE FROM `agent_phone_contacts` WHERE `contact_id` = ? AND %s"):format(condition), params)
    if scope.account_id then
        AgentPhone.NotifyAccount(scope.account_id, "agent_phone:contacts:changed", {})
    end
    return { success = true }
...
Bridge.Callbacks.Register("agent_phone:calls:recents", function(source)
    local scope, error_response = current_scope(source)
    if not scope then
        return error_response
    end
    ...
    local rows = Bridge.Database.Query(([[
        SELECT e.`id`, e.`call_id`, e.`direction`, e.`status`, e.`other_number`, e.`created_at`,
            c.`duration_seconds`
        FROM `agent_phone_call_entries` e
        JOIN `agent_phone_calls` c ON c.`id` = e.`call_id`
...
local function company_call_target(company_id, caller_source, caller_sim_id, excluded_sim_ids)
    ...
    for _, candidate in ipairs(AgentPhoneCompanies.GetCallTargets(company_id)) do
        ...
        then
            ...
            then
                ...
                local device = AgentPhone.LoadDevice(candidate_imei)
                ...
                elseif AgentPhoneCompanies.IsServiceNumber(device.phone_number) then
                    dialing_by_sim[candidate_sim_id] = nil
                    Bridge.Debug(
                        "error",
                        "[agent_phone] Company call target %s uses reserved service number %s.",
...
local function ring_callee(call)
    AgentPhone.OpenDeviceForCall(call.callee_source, call.callee_device.imei)
    TriggerClientEvent("agent_phone:call:incoming", call.callee_source, {
        id = call.id,
        state = "ringing",
        direction = "incoming",
...
reroute_company_call = function(call, previous_status)
    ...
    if not call.payphone then
        ...
        local updated = Bridge.Database.Transaction({
            {
                query = "UPDATE `agent_phone_calls` SET `callee_sim_id` = ? WHERE `id` = ? AND `status` = 'ringing'",
...
local function lock_direct_target(caller_source, target, caller_sim_id)
    ...
    if caller_sim_id then
        local blocks = Bridge.Database.Query([[
            SELECT 1 FROM `agent_phone_call_blocks`
...
function AgentPhoneCalls.StartCompanyCall(source, company_id, customer_number)
    source = tonumber(source)
    if not source or type(company_id) ~= "string" then
        return { success = false, error = "invalid_request" }
    end
    if not AgentPhoneCompanies.CanPlaceCompanyCall(source, company_id) then
        return { success = false, error = "not_authorized" }
    end
    if not AgentPhone.AllowOperation(source, "company_call_customer", 15, 60) then
    ...
    local service_line = AgentPhoneCompanies.GetServiceLineForCompany(company_id)
    ...
    if AgentPhoneCompanies.IsServiceNumber(scope.device.phone_number) then
        Bridge.Debug(
            "error",
            "[agent_phone] Source %s attempted to call from a SIM using reserved service number %s.",
            tostring(source),
    ...
    local number = AgentPhoneSimNumber.Normalize(customer_number, Config.Sim.NumberLength, Config.Sim.NumberPrefix)
    if not number or AgentPhoneCompanies.IsServiceNumber(number) then
    ...
    local targets = Bridge.Database.Query([[
        ...
        FROM `agent_phone_sims` s LEFT JOIN `agent_phone_devices` d ON d.`sim_id` = s.`id`
...
Bridge.Callbacks.Register("agent_phone:calls:dial", function(source, data)
    if not AgentPhone.AllowOperation(source, "call_dial", 15, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    local service_line = type(data.company) == "string"
        and AgentPhoneCompanies.GetServiceLineForCompany(data.company)
        or AgentPhoneCompanies.GetServiceLine(data.phoneNumber)
    local number = service_line and service_line.number
        or AgentPhoneSimNumber.Normalize(data.phoneNumber, Config.Sim.NumberLength, Config.Sim.NumberPrefix)
    if not number then
        dial_locks[source] = nil
        return { success = false, error = "invalid_number" }
    end
...
if Config.Payphones.Enabled and not next(payphone_models) then
    Bridge.Debug(
        "error",
        "[agent_phone] Payphones are enabled, but Config.Payphones.Props contains no valid models; payphone calls will be rejected.",
        { always = true }
    )
end
...
local function valid_payphone_position(source, detected_booth)
    ...
    local location = AgentPhonePayphones.ValidateDetected(
        detected_booth,
        payphone_models,
        GetEntityCoords(ped),
...
Bridge.Callbacks.Register("agent_phone:payphone:dial", function(source, data)
    if type(data) ~= "table" then
        return { success = false, error = "invalid_request" }
    end
    if not Config.Payphones.Enabled or not AgentPhone.AllowOperation(source, "payphone_dial", 15, 60) then
        return { success = false, error = "rate_limited" }
    end
...
Bridge.Callbacks.Register("agent_phone:calls:answer", function(source, data)
    ...
    if call.company_service_call and not AgentPhoneCompanies.CanAnswerCompanyCall(
        source,
        call.company_id,
    ...
    if not AgentPhone.FindDeviceSlots(source, call.callee_device.imei)[1] then
        finish_call(call, "unavailable")
    ...
    if not call.payphone then
        Bridge.Database.Query([[
            UPDATE `agent_phone_calls` SET `status` = 'connected', `answered_at` = CURRENT_TIMESTAMP
            WHERE `id` = ? AND `status` = 'ringing'
        ]], { call.id })
        Bridge.Database.Query([[
            UPDATE `agent_phone_call_entries` SET `status` = 'connected'
...
Bridge.Callbacks.Register("agent_phone:calls:set-speaker", function(source, data)
    if type(data) ~= "table" or type(data.id) ~= "string" or type(data.enabled) ~= "boolean" then
        return { success = false, error = "invalid_request" }
    end
    if not AgentPhone.AllowOperation(source, "call_speaker", 30, 60) then
        return { success = false, error = "rate_limited" }
    end
...
Bridge.Callbacks.Register("agent_phone:calls:set-muted", function(source, data)
    if type(data) ~= "table" or type(data.id) ~= "string" or type(data.enabled) ~= "boolean" then
        return { success = false, error = "invalid_request" }
    end
    if not AgentPhone.AllowOperation(source, "call_mute", 30, 60) then
        return { success = false, error = "rate_limited" }
    end
...
Bridge.Callbacks.Register("agent_phone:calls:decline", function(source, data)
    local call = type(data) == "table" and calls[data.id] or nil
    if not call or call.callee_source ~= source or call.answered_at or call.rerouting then
        return { success = false, error = "call_not_found" }
    end
...
Bridge.Callbacks.Register("agent_phone:calls:hangup", function(source, data)
    local call_id = active_by_source[source]
    local call = call_id and calls[call_id] or nil
...
Bridge.Callbacks.Register("agent_phone:calls:terminate", function(source, data)
    local call_id = active_by_source[source]
    local call = call_id and calls[call_id] or nil
    if not call
        or type(data) ~= "table"
...
Bridge.Callbacks.Register("agent_phone:calls:block", function(source, data)
    if not AgentPhone.AllowOperation(source, "call_block", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    local scope, error_response = current_scope(source)
    if not scope then
    ...
    local number = type(data) == "table" and AgentPhoneSimNumber.Normalize(
        data.phoneNumber,
    ...
    local rows = Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_sims` WHERE `phone_number` = ? LIMIT 1",
        { number }
    )
    if not rows[1] then
        return { success = false, error = "recipient_not_found" }
    ...
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_call_blocks` (`blocker_sim_id`, `blocked_sim_id`)
        VALUES (?, ?)
...
Bridge.Callbacks.Register("agent_phone:payphone:hangup", function(source, data)
    local call_id = active_by_source[source]
    local call = call_id and calls[call_id] or nil
    ...
    while true do
        ...
        for call_id, call in pairs(calls) do
            ...
            if call.payphone then
                ...
                caller_valid = AgentPhone.FindDeviceSlots(call.caller_source, call.caller_device.imei)[1] ~= nil
            end
            local callee_valid = not call.callee_source
                or AgentPhone.FindDeviceSlots(call.callee_source, call.callee_device.imei)[1] ~= nil
