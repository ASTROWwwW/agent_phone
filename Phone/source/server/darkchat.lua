...
local function load_profile(account_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_darkchat_profiles`
...
local function create_profile(account_id)
    for _ = 1, 10 do
        ...
        local result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_darkchat_profiles`
...
local function load_membership(profile_id, conversation_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_darkchat_members` mine
        JOIN `agent_phone_darkchat_conversations` conversation ON conversation.`id` = mine.`conversation_id`
        JOIN `agent_phone_darkchat_members` peer_member
            ON peer_member.`conversation_id` = mine.`conversation_id` AND peer_member.`profile_id` <> mine.`profile_id`
        JOIN `agent_phone_darkchat_profiles` peer ON peer.`id` = peer_member.`profile_id`
        LEFT JOIN `agent_phone_darkchat_contacts` contact
            ON contact.`profile_id` = mine.`profile_id` AND contact.`contact_profile_id` = peer.`id`
        LEFT JOIN `agent_phone_darkchat_blocks` block_mine
            ON block_mine.`blocker_profile_id` = mine.`profile_id` AND block_mine.`blocked_profile_id` = peer.`id`
        LEFT JOIN `agent_phone_darkchat_blocks` block_peer
...
local function decode_array(value, context)
    ...
    if type(decoded) ~= "table" then
        error(("[agent_phone] Invalid DarkChat JSON in %s."):format(context))
    end
    return decoded
end
...
local function list_contacts(profile_id)
    local rows = Bridge.Database.Query([[
        SELECT target.`id`, target.`dark_id`, target.`alias`, target.`avatar_seed`, contact.`alias_override`,
            contact.`created_at`
        FROM `agent_phone_darkchat_contacts` contact
        JOIN `agent_phone_darkchat_profiles` target ON target.`id` = contact.`contact_profile_id`
...
local function list_conversations(profile_id)
    local rows = Bridge.Database.Query([[
        SELECT conversation.`id`, conversation.`disappearing_seconds`, conversation.`updated_at`,
            ...
            (SELECT COUNT(*) FROM `agent_phone_darkchat_messages` unread
        ...
        LEFT JOIN `agent_phone_darkchat_blocks` block_mine
            ON block_mine.`blocker_profile_id` = mine.`profile_id` AND block_mine.`blocked_profile_id` = peer.`id`
        LEFT JOIN `agent_phone_darkchat_messages` latest ON latest.`id` = (
            SELECT message.`id` FROM `agent_phone_darkchat_messages` message
...
local function notify_profile(profile_id, event_name, data)
    local rows = Bridge.Database.Query([[
        SELECT `account_id`, `notification_mode` FROM `agent_phone_darkchat_profiles`
        WHERE `id` = ? LIMIT 1
    ]], { profile_id })
    local target = rows[1]
    if not target then
    ...
    if event_name == "agent_phone:darkchat:new" and data.conversationId then
        local members = Bridge.Database.Query([[
            SELECT `notifications_enabled` FROM `agent_phone_darkchat_members`
    ...
    AgentPhone.NotifyAccountDevices(target.account_id, event_name, data)
end
...
Bridge.Callbacks.Register("agent_phone:darkchat:bootstrap", function(source)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
...
Bridge.Callbacks.Register("agent_phone:darkchat:create-profile", function(source)
    if not AgentPhone.AllowOperation(source, "darkchat_profile_create", 5, 60) then
...
Bridge.Callbacks.Register("agent_phone:darkchat:delete-profile", function(source)
    if not AgentPhone.AllowOperation(source, "darkchat_profile_delete", 3, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    if not Bridge.Database.Transaction({
        {
            query = [[
                DELETE conversation FROM `agent_phone_darkchat_conversations` conversation
                JOIN `agent_phone_darkchat_members` member ON member.`conversation_id` = conversation.`id`
                WHERE member.`profile_id` = ?
            ]],
            params = { profile.id },
        },
        {
            query = "DELETE FROM `agent_phone_darkchat_profiles` WHERE `id` = ? AND `account_id` = ?",
            params = { profile.id, account.id },
        },
    }) then
        error("[agent_phone] Failed to delete a DarkChat profile transaction.")
    end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:darkchat:update-profile", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_profile", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, account, error_response = require_profile(source)
    if not profile then
    ...
    Bridge.Database.Query([[
        UPDATE `agent_phone_darkchat_profiles`
...
Bridge.Callbacks.Register("agent_phone:darkchat:start", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_start", 20, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    local targets = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_darkchat_profiles`
    ...
    local existing = Bridge.Database.Query([[
        ...
        FROM `agent_phone_darkchat_members` mine
        JOIN `agent_phone_darkchat_members` peer ON peer.`conversation_id` = mine.`conversation_id`
    ...
    if not conversation_id then
        ...
        local created = Bridge.Database.Transaction({
            {
                query = "INSERT INTO `agent_phone_darkchat_conversations` (`id`) VALUES (?)",
                params = { conversation_id },
            },
            {
                query = "INSERT INTO `agent_phone_darkchat_members` (`conversation_id`, `profile_id`) VALUES (?, ?)",
        ...
        if not created then
            error("[agent_phone] Failed to create a DarkChat conversation transaction.")
        end
    end
    return { success = true, data = { conversationId = conversation_id } }
end)

Bridge.Callbacks.Register("agent_phone:darkchat:thread", function(source, data)
    local profile, _, error_response = require_profile(source)
    if not profile then
        return error_response
    end
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_darkchat_messages` message
        JOIN `agent_phone_darkchat_members` mine
        ...
        LEFT JOIN `agent_phone_darkchat_messages` reply ON reply.`id` = message.`reply_to_id`
...
Bridge.Callbacks.Register("agent_phone:darkchat:send", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_send", Config.DarkChat.SendsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    elseif message_type == "image" or message_type == "video" then
        ...
        local media_url, media_error, resolved_mime = AgentPhoneMedia.ResolveOwnedMedia(
            source,
            data.mediaAssetId,
            media_type
        )
    ...
    elseif message_type == "share" then
        local share
        local share_error
        share, share_error, media_payload = AgentPhoneEasyShare.SanitizeChatPayload(source, data.sharePayload)
        if not share then
    ...
    if reply_to ~= nil then
        local reply = Bridge.Database.Query([[
            SELECT `id` FROM `agent_phone_darkchat_messages`
    ...
    Bridge.Database.Query(
        "UPDATE `agent_phone_darkchat_conversations` SET `updated_at` = CURRENT_TIMESTAMP WHERE `id` = ?",
        { conversation_id }
    )
    local rows = Bridge.Database.Query([[
    ...
    TriggerClientEvent("agent_phone:darkchat:changed", source, { conversationId = conversation_id })
    notify_profile(tonumber(membership.peer_id), "agent_phone:darkchat:new", {
...
Bridge.Callbacks.Register("agent_phone:darkchat:media", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_media", Config.DarkChat.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, _, error_response = require_profile(source)
    if not profile then
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_darkchat_messages` message
        JOIN `agent_phone_darkchat_members` member ON member.`conversation_id` = message.`conversation_id`
...
Bridge.Callbacks.Register("agent_phone:darkchat:react", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_react", Config.DarkChat.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, _, error_response = require_profile(source)
    if not profile then
    ...
    Bridge.Database.Query("UPDATE `agent_phone_darkchat_messages` SET `reactions` = ? WHERE `id` = ?", {
        json.encode(reactions), message.id,
    })
    TriggerClientEvent("agent_phone:darkchat:changed", source, { conversationId = message.conversation_id })
    local membership = load_membership(profile.id, message.conversation_id)
    notify_profile(tonumber(membership.peer_id), "agent_phone:darkchat:changed", {
        conversationId = message.conversation_id,
    })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:darkchat:message-action", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_message_action", Config.DarkChat.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, _, error_response = require_profile(source)
    if not profile then
    ...
    local rows = Bridge.Database.Query([[
        SELECT message.* FROM `agent_phone_darkchat_messages` message
        JOIN `agent_phone_darkchat_members` member ON member.`conversation_id` = message.`conversation_id`
        WHERE message.`id` = ? AND member.`profile_id` = ? LIMIT 1
    ]], { data and data.messageId, profile.id })
    local message = rows[1]
    ...
    if data.action == "delete_me" then
        ...
        Bridge.Database.Query("UPDATE `agent_phone_darkchat_messages` SET `deleted_for_profiles` = ? WHERE `id` = ?", {
...
Bridge.Callbacks.Register("agent_phone:darkchat:update-conversation", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_conversation_settings", Config.DarkChat.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, _, error_response = require_profile(source)
    if not profile then
    ...
    Bridge.Database.Transaction({
        {
            query = "UPDATE `agent_phone_darkchat_members` SET `notifications_enabled` = ?, `read_receipts` = ? WHERE `conversation_id` = ? AND `profile_id` = ?",
            params = { data.notificationsEnabled and 1 or 0, data.readReceipts and 1 or 0, data.conversationId, profile.id },
        },
        {
            query = "UPDATE `agent_phone_darkchat_conversations` SET `disappearing_seconds` = ? WHERE `id` = ?",
            params = { timer, data.conversationId },
        },
        {
            query = "INSERT INTO `agent_phone_darkchat_messages` (`id`, `conversation_id`, `message_type`, `body`) VALUES (?, ?, 'system', ?)",
            params = { uuid(), data.conversationId, "timer_changed:" .. tostring(timer) },
        },
    })
    TriggerClientEvent("agent_phone:darkchat:changed", source, { conversationId = data.conversationId })
    notify_profile(tonumber(membership.peer_id), "agent_phone:darkchat:changed", {
...
Bridge.Callbacks.Register("agent_phone:darkchat:add-contact", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_contact", Config.DarkChat.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, _, error_response = require_profile(source)
    if not profile then
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_darkchat_contacts` (`profile_id`, `contact_profile_id`, `alias_override`)
        VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `alias_override` = VALUES(`alias_override`)
    ]], { profile.id, membership.peer_id, alias })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:darkchat:remove-contact", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_contact", Config.DarkChat.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, _, error_response = require_profile(source)
    ...
    Bridge.Database.Query([[
        DELETE FROM `agent_phone_darkchat_contacts` WHERE `profile_id` = ? AND `contact_profile_id` = ?
    ]], { profile.id, membership.peer_id })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:darkchat:block", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_block", Config.DarkChat.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, _, error_response = require_profile(source)
    if not profile then
    ...
    if data.blocked then
        Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_darkchat_blocks` (`blocker_profile_id`, `blocked_profile_id`)
            VALUES (?, ?)
        ]], { profile.id, membership.peer_id })
    else
        Bridge.Database.Query([[
            DELETE FROM `agent_phone_darkchat_blocks` WHERE `blocker_profile_id` = ? AND `blocked_profile_id` = ?
        ]], { profile.id, membership.peer_id })
    end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:darkchat:report", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_report", Config.DarkChat.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, _, error_response = require_profile(source)
    if not profile then
    ...
    local count = Bridge.Database.Query([[
        SELECT COUNT(*) AS `count` FROM `agent_phone_darkchat_reports`
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_darkchat_reports`
...
Bridge.Callbacks.Register("agent_phone:darkchat:clear", function(source, data)
    if not AgentPhone.AllowOperation(source, "darkchat_clear", Config.DarkChat.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, _, error_response = require_profile(source)
    if not profile then
    ...
    Bridge.Database.Query([[
        UPDATE `agent_phone_darkchat_members` SET `cleared_at` = CURRENT_TIMESTAMP
        WHERE `conversation_id` = ? AND `profile_id` = ?
...
CreateThread(function()
    while true do
        ...
        Bridge.Database.Query([[
            DELETE FROM `agent_phone_darkchat_messages`
