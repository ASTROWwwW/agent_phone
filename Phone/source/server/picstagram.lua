...
local function are_profiles_blocked(first_id, second_id)
    local rows = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_picstagram_blocks`
...
local function profile_for_session(source)
    ...
    local rows = Bridge.Database.Query([[
        SELECT p.* FROM `agent_phone_picstagram_sessions` s
        JOIN `agent_phone_picstagram_profiles` p ON p.`id` = s.`profile_id`
...
local function load_post_media(posts)
    ...
    local rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_picstagram_post_media` pm
        JOIN `agent_phone_media` m ON m.`id` = pm.`media_id`
...
local function list_posts(viewer_id, where_clause, values, cursor_value)
    ...
    local rows = hydrate_posts(Bridge.Database.Query(([[
        SELECT post.`id`, post.`profile_id`, post.`caption`, post.`location`, post.`comments_enabled`,
            ...
            EXISTS(SELECT 1 FROM `agent_phone_picstagram_reactions` reaction
            ...
            (SELECT COUNT(*) FROM `agent_phone_picstagram_reactions` reaction
                WHERE reaction.`post_id` = post.`id` AND reaction.`kind` = 'like') AS `like_count`,
            (SELECT COUNT(*) FROM `agent_phone_picstagram_comments` comment
                WHERE comment.`post_id` = post.`id` AND comment.`status` = 'visible') AS `comment_count`
        FROM `agent_phone_picstagram_posts` post
        JOIN `agent_phone_picstagram_profiles` profile ON profile.`id` = post.`profile_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = profile.`avatar_media_id`
        WHERE post.`status` = 'published' AND profile.`status` = 'active' AND %s%s
            AND NOT EXISTS(SELECT 1 FROM `agent_phone_picstagram_blocks` block
    ...
    local rows = Bridge.Database.Query([[
        ...
        WHERE post.`id` = ? AND post.`status` = 'published' AND profile.`status` = 'active'
            AND (profile.`private` = 0 OR post.`profile_id` = ? OR EXISTS(
                SELECT 1 FROM `agent_phone_picstagram_follows` follow
...
local function create_activity(recipient_id, actor_id, kind, post_id)
    ...
    if post_id then
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_picstagram_activities`
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_picstagram_profiles` recipient
        JOIN `agent_phone_picstagram_profiles` actor ON actor.`id` = ?
        WHERE recipient.`id` = ? LIMIT 1
    ]], { actor_id, recipient_id })
    if not rows[1] then
        error("[agent_phone] Picstagram activity references a missing profile.")
    end
    AgentPhone.NotifyAccountDevices(tonumber(rows[1].account_id), "agent_phone:picstagram:new", {
...
Bridge.Callbacks.Register("agent_phone:picstagram:register", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:register", 5, 60) then
    ...
    if Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_picstagram_profiles` WHERE `account_id` = ? LIMIT 1",
        { account.id }
    )[1] then
        return { success = false, error = "already_registered" }
    end
    if Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_picstagram_profiles` WHERE `handle` = ? LIMIT 1",
        { handle }
    )[1] then
    ...
    if not Bridge.Database.Transaction({
        {
            query = [[INSERT INTO `agent_phone_picstagram_profiles`
                (`id`, `account_id`, `handle`, `display_name`) VALUES (?, ?, ?, ?)]],
            params = { entropy.id, account.id, handle, display_name },
        },
        {
            query = [[INSERT INTO `agent_phone_picstagram_credentials`
        ...
        {
            query = [[INSERT INTO `agent_phone_picstagram_sessions` (`device_imei`, `profile_id`)
...
Bridge.Callbacks.Register("agent_phone:picstagram:login", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:login", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then
    ...
    local profiles = Bridge.Database.Query([[
        SELECT profile.`id` FROM `agent_phone_picstagram_profiles` profile
        JOIN `agent_phone_picstagram_credentials` credentials ON credentials.`profile_id` = profile.`id`
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_picstagram_sessions` (`device_imei`, `profile_id`)
        VALUES (?, ?) ON DUPLICATE KEY UPDATE `profile_id` = VALUES(`profile_id`), `updated_at` = CURRENT_TIMESTAMP
    ]], { session.imei, profiles[1].id })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:picstagram:logout", function(source)
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then
        return error_response
    end
    Bridge.Database.Query("DELETE FROM `agent_phone_picstagram_sessions` WHERE `device_imei` = ?", { session.imei })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:picstagram:bootstrap", function(source)
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then
        return error_response
    end
    ...
    local feed, feed_error = list_posts(
        profile.id,
        [[post.`profile_id` = ? OR EXISTS(SELECT 1 FROM `agent_phone_picstagram_follows` follow
            WHERE follow.`follower_id` = ? AND follow.`following_id` = post.`profile_id`
                AND follow.`status` = 'accepted')]],
...
Bridge.Callbacks.Register("agent_phone:picstagram:feed", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:picstagram:post", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    if not AgentPhone.AllowOperation(source, "picstagram:post", 60, 60) then
        return { success = false, error = "rate_limited" }
    end
...
Bridge.Callbacks.Register("agent_phone:picstagram:explore", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:picstagram:search", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:search", 40, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = profile_for_session(source)
    if not profile then
    ...
    local profiles = Bridge.Database.Query([[
        ...
        FROM `agent_phone_picstagram_profiles` candidate
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = candidate.`avatar_media_id`
...
Bridge.Callbacks.Register("agent_phone:picstagram:saved", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    local result, list_error = list_posts(
        profile.id,
        [[EXISTS(SELECT 1 FROM `agent_phone_picstagram_reactions` saved
...
Bridge.Callbacks.Register("agent_phone:picstagram:profile", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:picstagram:connections", function(source, data)
    local viewer, error_response = profile_for_session(source)
    if not viewer then
        return error_response
    end
    ...
    local rows = Bridge.Database.Query(([[
        SELECT candidate.`id`, candidate.`handle`, candidate.`display_name`, candidate.`bio`,
            ...
            avatar.`url` AS `avatar_url`,
            COALESCE((SELECT own_follow.`status` FROM `agent_phone_picstagram_follows` own_follow
                WHERE own_follow.`follower_id` = ? AND own_follow.`following_id` = candidate.`id` LIMIT 1), '') AS `follow_status`,
            (SELECT COUNT(*) FROM `agent_phone_picstagram_follows` followers
                WHERE followers.`following_id` = candidate.`id` AND followers.`status` = 'accepted') AS `followers`,
            (SELECT COUNT(*) FROM `agent_phone_picstagram_follows` following
                WHERE following.`follower_id` = candidate.`id` AND following.`status` = 'accepted') AS `following`,
            (SELECT COUNT(*) FROM `agent_phone_picstagram_posts` post
                WHERE post.`profile_id` = candidate.`id` AND post.`status` = 'published') AS `post_count`
        FROM `agent_phone_picstagram_follows` follow
        JOIN `agent_phone_picstagram_profiles` candidate ON candidate.`id` = %s
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = candidate.`avatar_media_id`
...
Bridge.Callbacks.Register("agent_phone:picstagram:update-profile", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    if Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_picstagram_profiles` WHERE `handle` = ? AND `id` <> ? LIMIT 1",
        { handle, profile.id }
    )[1] then
        return { success = false, error = "handle_taken" }
    end
    Bridge.Database.Query([[
        UPDATE `agent_phone_picstagram_profiles`
...
Bridge.Callbacks.Register("agent_phone:picstagram:publish-post", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:publish", Config.Picstagram.PostsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = profile_for_session(source)
    if not profile then
    ...
    for index = 1, #data.mediaIds do
        ...
        if not AgentPhoneMedia.ResolveOwnedMedia(source, tostring(media_id), data.mediaType) then
            return { success = false, error = "invalid_media" }
        end
        seen[media_id] = true
    ...
    local statements = {
        {
            query = [[INSERT INTO `agent_phone_picstagram_posts`
    ...
    for index = 1, #media_ids do
        statements[#statements + 1] = {
            query = [[INSERT INTO `agent_phone_picstagram_post_media`
...
Bridge.Callbacks.Register("agent_phone:picstagram:update-post", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    local result = Bridge.Database.Query([[
        UPDATE `agent_phone_picstagram_posts`
...
Bridge.Callbacks.Register("agent_phone:picstagram:set-post-status", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    local current = Bridge.Database.Query(
        "SELECT `status` FROM `agent_phone_picstagram_posts` WHERE `id` = ? AND `profile_id` = ? LIMIT 1",
        { data.id, profile.id }
    )[1]
    if not current then
        return { success = false, error = "post_not_found" }
    ...
    Bridge.Database.Query(
        "UPDATE `agent_phone_picstagram_posts` SET `status` = ? WHERE `id` = ? AND `profile_id` = ?",
        { target, data.id, profile.id }
    )
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:picstagram:react", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:react", 80, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = profile_for_session(source)
    if not profile then
    ...
    if data.active then
        local result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_picstagram_reactions` (`post_id`, `profile_id`, `kind`)
            VALUES (?, ?, ?)
        ...
        Bridge.Database.Query([[
            DELETE FROM `agent_phone_picstagram_reactions`
...
Bridge.Callbacks.Register("agent_phone:picstagram:follow", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:follow", 40, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = profile_for_session(source)
    if not profile then
    ...
    local target = Bridge.Database.Query([[
        SELECT `id`, `private` FROM `agent_phone_picstagram_profiles`
    ...
    if not data.active then
        Bridge.Database.Query(
            "DELETE FROM `agent_phone_picstagram_follows` WHERE `follower_id` = ? AND `following_id` = ?",
    ...
    local existing = Bridge.Database.Query([[
        SELECT `status` FROM `agent_phone_picstagram_follows`
        WHERE `follower_id` = ? AND `following_id` = ? LIMIT 1
    ]], { profile.id, target.id })[1]
    if not existing or existing.status ~= status then
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_picstagram_follows` (`follower_id`, `following_id`, `status`)
...
Bridge.Callbacks.Register("agent_phone:picstagram:respond-follow", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    local request = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_picstagram_follows`
    ...
    if data.accept then
        Bridge.Database.Query(
            "UPDATE `agent_phone_picstagram_follows` SET `status` = 'accepted' WHERE `id` = ?",
            { request.id }
        )
        create_activity(data.profileId, profile.id, "request_accepted", nil)
    else
        Bridge.Database.Query("DELETE FROM `agent_phone_picstagram_follows` WHERE `id` = ?", { request.id })
    end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:picstagram:comments", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    local rows = Bridge.Database.Query([[
        SELECT comment.`id`, comment.`profile_id`, comment.`parent_id`, comment.`body`,
            ...
            EXISTS(SELECT 1 FROM `agent_phone_picstagram_comment_reactions` reaction
                WHERE reaction.`comment_id` = comment.`id` AND reaction.`profile_id` = ?) AS `is_liked`,
            (SELECT COUNT(*) FROM `agent_phone_picstagram_comment_reactions` reaction
                WHERE reaction.`comment_id` = comment.`id`) AS `like_count`
        FROM `agent_phone_picstagram_comments` comment
        JOIN `agent_phone_picstagram_profiles` author ON author.`id` = comment.`profile_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = author.`avatar_media_id`
        LEFT JOIN `agent_phone_picstagram_comments` parent ON parent.`id` = comment.`parent_id`
        LEFT JOIN `agent_phone_picstagram_profiles` parent_author ON parent_author.`id` = parent.`profile_id`
...
Bridge.Callbacks.Register("agent_phone:picstagram:comment-react", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    if not AgentPhone.AllowOperation(source, "picstagram:comment-react", 60, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    if data.active then
        local result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_picstagram_comment_reactions` (`comment_id`, `profile_id`)
            VALUES (?, ?)
        ...
        Bridge.Database.Query([[
            DELETE FROM `agent_phone_picstagram_comment_reactions`
...
Bridge.Callbacks.Register("agent_phone:picstagram:comment", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:comment", Config.Picstagram.CommentsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = profile_for_session(source)
    if not profile then
    ...
    if data.replyToId then
        reply_target = Bridge.Database.Query([[
            SELECT `id`, `profile_id`, `parent_id` FROM `agent_phone_picstagram_comments`
    ...
    if parent_id then
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_picstagram_comments` (`id`, `post_id`, `profile_id`, `parent_id`, `body`)
            VALUES (?, ?, ?, ?, ?)
        ]], { id, data.id, profile.id, parent_id, body })
    else
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_picstagram_comments` (`id`, `post_id`, `profile_id`, `body`)
            VALUES (?, ?, ?, ?)
...
Bridge.Callbacks.Register("agent_phone:picstagram:remove-comment", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_picstagram_comments` comment
        JOIN `agent_phone_picstagram_posts` post ON post.`id` = comment.`post_id`
        WHERE comment.`id` = ? AND comment.`status` = 'visible' LIMIT 1
    ]], { data.id })
    ...
    Bridge.Database.Query(
        "UPDATE `agent_phone_picstagram_comments` SET `status` = 'removed' WHERE `id` = ?",
        { data.id }
    )
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:picstagram:publish-story", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:story", Config.Picstagram.StoriesPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = profile_for_session(source)
    if not profile then
    ...
    if not media_id or media_id < 1 or media_id ~= math.floor(media_id)
        ...
        or not AgentPhoneMedia.ResolveOwnedMedia(source, tostring(media_id), data.mediaType)
    then
        return { success = false, error = "invalid_story" }
    end
    local id = new_id()
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_picstagram_stories`
...
Bridge.Callbacks.Register("agent_phone:picstagram:stories", function(source)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    local rows = Bridge.Database.Query([[
        SELECT story.`id`, story.`profile_id`, story.`body`, media.`url`, media.`media_type`, author.`handle`,
            ...
            EXISTS(SELECT 1 FROM `agent_phone_picstagram_story_views` view
                WHERE view.`story_id` = story.`id` AND view.`profile_id` = ?) AS `seen`,
            (story.`profile_id` = ?) AS `is_owner`,
            (SELECT COUNT(*) FROM `agent_phone_picstagram_story_views` view
                WHERE view.`story_id` = story.`id`) AS `view_count`
        FROM `agent_phone_picstagram_stories` story
        JOIN `agent_phone_picstagram_profiles` author ON author.`id` = story.`profile_id`
        JOIN `agent_phone_media` media ON media.`id` = story.`media_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = author.`avatar_media_id`
        WHERE story.`status` = 'active' AND story.`expires_at` > CURRENT_TIMESTAMP
            AND author.`status` = 'active'
            AND (story.`profile_id` = ? OR EXISTS(SELECT 1 FROM `agent_phone_picstagram_follows` follow
...
Bridge.Callbacks.Register("agent_phone:picstagram:view-story", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:story-view", 120, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = profile_for_session(source)
    if not profile then
    ...
    local story = Bridge.Database.Query([[
        SELECT story.`id`, story.`profile_id` FROM `agent_phone_picstagram_stories` story
        JOIN `agent_phone_picstagram_profiles` author ON author.`id` = story.`profile_id`
    ...
    if story.profile_id ~= profile.id then
        Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_picstagram_story_views` (`story_id`, `profile_id`)
            VALUES (?, ?)
        ]], { story.id, profile.id })
    end
    return { success = true }
...
Bridge.Callbacks.Register("agent_phone:picstagram:story-viewers", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    if not Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_picstagram_stories` WHERE `id` = ? AND `profile_id` = ? LIMIT 1",
        { data.id, profile.id }
    )[1] then
        return { success = false, error = "story_not_found" }
    end
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_picstagram_story_views` viewed
        JOIN `agent_phone_picstagram_profiles` viewer ON viewer.`id` = viewed.`profile_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = viewer.`avatar_media_id`
...
Bridge.Callbacks.Register("agent_phone:picstagram:remove-story", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    local result = Bridge.Database.Query([[
        UPDATE `agent_phone_picstagram_stories` SET `status` = 'removed'
...
Bridge.Callbacks.Register("agent_phone:picstagram:activities", function(source)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_picstagram_activities` activity
        JOIN `agent_phone_picstagram_profiles` actor ON actor.`id` = activity.`actor_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = actor.`avatar_media_id`
        LEFT JOIN `agent_phone_picstagram_post_media` post_media
            ON post_media.`post_id` = activity.`post_id` AND post_media.`position` = 1
        LEFT JOIN `agent_phone_media` cover ON cover.`id` = post_media.`media_id`
...
Bridge.Callbacks.Register("agent_phone:picstagram:mark-activities", function(source)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    Bridge.Database.Query([[
        UPDATE `agent_phone_picstagram_activities` SET `read_at` = CURRENT_TIMESTAMP
        WHERE `recipient_id` = ? AND `read_at` IS NULL
    ]], { profile.id })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:picstagram:block", function(source, data)
    local profile, error_response = profile_for_session(source)
    if not profile then
        return error_response
    end
    ...
    if not Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_picstagram_profiles` WHERE `id` = ? AND `status` = 'active' LIMIT 1",
        { data.profileId }
    )[1] then
        return { success = false, error = "profile_not_found" }
    end
    if data.active == false then
        Bridge.Database.Query(
            "DELETE FROM `agent_phone_picstagram_blocks` WHERE `blocker_id` = ? AND `blocked_id` = ?",
    ...
    if not Bridge.Database.Transaction({
        {
            query = [[INSERT IGNORE INTO `agent_phone_picstagram_blocks` (`blocker_id`, `blocked_id`)
                VALUES (?, ?)]],
            params = { profile.id, data.profileId },
        },
        {
            query = [[DELETE FROM `agent_phone_picstagram_follows`
...
local function valid_report_target(target_type, target_id)
    ...
    local tables = {
        post = "agent_phone_picstagram_posts",
        story = "agent_phone_picstagram_stories",
        comment = "agent_phone_picstagram_comments",
    }
...
Bridge.Callbacks.Register("agent_phone:picstagram:report", function(source, data)
    if not AgentPhone.AllowOperation(source, "picstagram:report", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = profile_for_session(source)
    if not profile then
    ...
    local result = Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_picstagram_reports`
...
Bridge.Callbacks.Register("agent_phone:picstagram:admin-reports", function(source)
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_picstagram_reports` report
        JOIN `agent_phone_picstagram_profiles` reporter ON reporter.`id` = report.`reporter_id`
...
Bridge.Callbacks.Register("agent_phone:picstagram:admin-resolve-report", function(source, data)
    if not is_admin(source) then
        return { success = false, error = "not_authorized" }
    end
    ...
    local report = Bridge.Database.Query([[
        SELECT * FROM `agent_phone_picstagram_reports` WHERE `id` = ? AND `status` = 'open' LIMIT 1
    ]], { data.id })[1]
    if not report then
    ...
    if data.action ~= "dismiss" then
        local targets = {
            profile = { table = "agent_phone_picstagram_profiles", hidden = "hidden", removed = "removed", restored = "active" },
            post = { table = "agent_phone_picstagram_posts", hidden = "hidden", removed = "removed", restored = "published" },
            story = { table = "agent_phone_picstagram_stories", hidden = "removed", removed = "removed", restored = "active" },
            comment = { table = "agent_phone_picstagram_comments", hidden = "removed", removed = "removed", restored = "visible" },
        }
    ...
    statements[#statements + 1] = {
        query = [[UPDATE `agent_phone_picstagram_reports`
    ...
    statements[#statements + 1] = {
        query = [[INSERT INTO `agent_phone_picstagram_moderation_audit`
...
local function run_verify_command(source, args)
    local command_locale = AgentPhoneLocales.Resolve(Config.Bridge.Locale).PicstagramCommand
    ...
    local function send_command_feedback(message, notification_type)
        ...
        TriggerClientEvent("agent_phone:picstagram:command-feedback", source, {
            message = message,
    ...
    if source ~= 0 and not Bridge.Framework.HasPermission(source, "picstagramverify") then
        ...
        Bridge.Debug(
            "warn",
            "[agent_phone] Source %s attempted to use the Picstagram verification command without an admin group.",
            tostring(source)
        )
        return
    end
    ...
    Bridge.Database.Query(
        "UPDATE `agent_phone_picstagram_profiles` SET `verified` = ? WHERE `id` = ?",
        { state, target.id }
    )
    TriggerClientEvent("agent_phone:picstagram:verification-changed", -1, {
        profileId = target.id,
        verified = state == 1,
    })
...
local function refresh_verify_command()
    ...
    if type(command_name) ~= "string" or command_name == "" then
        error("[agent_phone] Config.Picstagram.VerifyCommand must be a non-empty command name.")
    end
