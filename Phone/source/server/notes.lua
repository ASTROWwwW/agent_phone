...
local function notify_owner(account_id, imei)
    if account_id then
        ...
        AgentPhone.RefreshDevice(imei)
    end
end

function AgentPhoneNotes.List(account_id, imei)
...
Bridge.Callbacks.Register("agent_phone:notes:list", function(source)
    local account_id, imei, error_response = session_owner(source)
    if not imei then
        return error_response
    end
    return { success = true, data = AgentPhoneNotes.List(account_id, imei) }
end)

Bridge.Callbacks.Register("agent_phone:notes:create", function(source, data)
    if not AgentPhone.AllowOperation(source, "notes_write", 120, 60) then
    ...
    if account_id then
        result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_notes`
...
Bridge.Callbacks.Register("agent_phone:notes:update", function(source, data)
    if not AgentPhone.AllowOperation(source, "notes_write", 120, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account_id, imei, error_response = session_owner(source)
    ...
    local affected = type(result) == "number" and result
        or (type(result) == "table" and tonumber(result.affectedRows) or 0)
    if affected ~= 1 then
        return { success = false, error = "conflict", data = AgentPhoneNotes.List(account_id, imei) }
    end
...
Bridge.Callbacks.Register("agent_phone:notes:delete", function(source, data)
    if not AgentPhone.AllowOperation(source, "notes_write", 120, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account_id, imei, error_response = session_owner(source)
    ...
    Bridge.Database.Query(("DELETE FROM `agent_phone_notes` WHERE `id` = ? AND %s"):format(condition), params)
    notify_owner(account_id, imei)
    return { success = true, data = AgentPhoneNotes.List(account_id, imei) }
end)
end)
