...
local function list_events(account_id, starts_at, ends_at)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_calendar_events`
        WHERE `account_id` = ?
...
Bridge.Callbacks.Register("agent_phone:calendar:list", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:calendar:create", function(source, data)
    if not AgentPhone.AllowOperation(source, "calendar_write", 60, 60) then
...
Bridge.Callbacks.Register("agent_phone:calendar:update", function(source, data)
    if not AgentPhone.AllowOperation(source, "calendar_write", 60, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    ...
    local result = Bridge.Database.Query([[
        UPDATE `agent_phone_calendar_events`
...
Bridge.Callbacks.Register("agent_phone:calendar:delete", function(source, data)
    if not AgentPhone.AllowOperation(source, "calendar_write", 60, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    ...
    Bridge.Database.Query(
        "DELETE FROM `agent_phone_calendar_events` WHERE `id` = ? AND `account_id` = ?",
        { data.id, account.id }
    )
    return { success = true }
end)
...
CreateThread(function()
    while true do
        ...
        for _, event in ipairs(due_events) do
            ...
            if affected_rows(result) == 1 then
                AgentPhone.NotifyAccountDevices(event.account_id, "agent_phone:calendar:reminder", {
                    eventId = event.id,
