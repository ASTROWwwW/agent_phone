...
if type(config) ~= "table" then
    error("[agent_phone] Config.WeazelNews must be configured.")
end
...
local function require_integer_config(name, minimum, maximum)
    ...
    if type(value) ~= "number" or value ~= math.floor(value) or value < minimum or value > maximum then
        error(("[agent_phone] Config.WeazelNews.%s must be an integer between %d and %d."):format(name, minimum, maximum))
    end
end

if type(config.Enabled) ~= "boolean" then
    error("[agent_phone] Config.WeazelNews.Enabled must be true or false.")
end
...
then
    error("[agent_phone] Weazel News minimum text lengths cannot exceed their maximums.")
end
if type(config.RateLimits) ~= "table" then
    error("[agent_phone] Config.WeazelNews.RateLimits must be a table.")
end
for _, name in ipairs({ "Read", "Write" }) do
    ...
    if type(value) ~= "number" or value ~= math.floor(value) or value < 1 or value > 10000 then
        error(("[agent_phone] Config.WeazelNews.RateLimits.%s must be an integer between 1 and 10000."):format(name))
    end
end
...
if type(config.Categories) ~= "table" then
    error("[agent_phone] Config.WeazelNews.Categories must be a table.")
end
...
local function refresh_categories()
    ...
    for _, category in ipairs(config.Categories or {}) do
        ...
        then
            error(("[agent_phone] Invalid Weazel News category '%s'."):format(tostring(category)))
        end
        next_categories[category] = true
    end
    for category in pairs(supported_categories) do
        if not next_categories[category] then
            error(("[agent_phone] Config.WeazelNews.Categories is missing supported category '%s'."):format(category))
        end
    end
    for category in pairs(next_categories) do
        if not supported_categories[category] then
            error(("[agent_phone] Config.WeazelNews.Categories contains unsupported category '%s'."):format(category))
        end
    end
...
if type(config.AllowedJobs) ~= "table" then
    error("[agent_phone] Config.WeazelNews.AllowedJobs must be a table.")
end
...
for job_name, minimum_grade in pairs(config.AllowedJobs or {}) do
    ...
    then
        error(("[agent_phone] Invalid Weazel News AllowedJobs entry '%s'."):format(tostring(job_name)))
    end
end
...
local function require_phone(source, operation, maximum)
    ...
    if not AgentPhone.AllowOperation(source, "weazel_" .. operation, maximum, 60) then
...
local function actor_identity(source)
    ...
    if type(identifier) ~= "string" or #identifier < 1 or #identifier > 80 then
        Bridge.Debug("error", "[agent_phone] Could not resolve a valid Weazel News actor for source %s.", tostring(source))
        return nil
    end
...
local function article_dto(row)
    ...
    if not created_at or not updated_at then
        error(("[agent_phone] Weazel News article '%s' has invalid timestamps."):format(tostring(row.id)))
    end
    return {
        id = row.id,
        title = row.title,
...
local function attach_article_images(articles)
    ...
    local rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_weazel_article_media` relation
...
local function load_article(id, include_drafts)
    ...
    local rows = Bridge.Database.Query(([[
        SELECT %s
        FROM `agent_phone_weazel_articles` article
        LEFT JOIN `agent_phone_media` media ON media.`id` = article.`image_media_id`
...
local function validate_article_media(source, data, retained_media_ids)
    ...
    for index, value in ipairs(values) do
        ...
        if not retained_media_ids[media_id]
            and not AgentPhoneMedia.ResolveOwnedMedia(source, tostring(media_id), "photo")
        then
            return nil
        end
        seen[media_id] = true
...
Bridge.Callbacks.Register("agent_phone:weazel-news:context", function(source)
...
Bridge.Callbacks.Register("agent_phone:weazel-news:list", function(source, data)
    local _, error_response = require_phone(source, "read", config.RateLimits.Read)
    if error_response then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:weazel-news:get", function(source, data)
    local _, error_response = require_phone(source, "read", config.RateLimits.Read)
    if error_response then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:weazel-news:manage-list", function(source, data)
    local _, error_response = require_phone(source, "read", config.RateLimits.Read)
    if error_response then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:weazel-news:create", function(source, data)
    ...
    if not valid_uuid(id) then
        error("[agent_phone] Database did not generate a Weazel News article id.")
    end
    local statements = {{
        query = [[
            INSERT INTO `agent_phone_weazel_articles`
    ...
    for position, media_id in ipairs(article.image_media_ids) do
        statements[#statements + 1] = {
            query = [[
                INSERT INTO `agent_phone_weazel_article_media` (`article_id`, `media_id`, `position`)
                VALUES (?, ?, ?)
            ]],
    ...
    if not created then
        error(("[agent_phone] Could not reload created Weazel News article '%s'."):format(id))
    end
    return { success = true, data = { article = created } }
end)

Bridge.Callbacks.Register("agent_phone:weazel-news:update", function(source, data)
    local _, error_response = require_phone(source, "write", config.RateLimits.Write)
    if error_response then
        return error_response
    end
    ...
    local retained_rows = Bridge.Database.Query([[
        SELECT `media_id`
        FROM `agent_phone_weazel_article_media`
        WHERE `article_id` = ?
    ]], { data.id })
    ...
    if not valid_uuid(mutation_id) then
        error("[agent_phone] Database did not generate a Weazel News mutation id.")
    end
    ...
    local statements = {
        {
            query = [[
                UPDATE `agent_phone_weazel_articles`
        ...
        },
        {
            query = [[
                DELETE relation
                FROM `agent_phone_weazel_article_media` relation
                JOIN `agent_phone_weazel_articles` article ON article.`id` = relation.`article_id`
...
Bridge.Callbacks.Register("agent_phone:weazel-news:delete", function(source, data)
    local _, error_response = require_phone(source, "write", config.RateLimits.Write)
    if error_response then
        return error_response
    end
