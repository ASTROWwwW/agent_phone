...
local function refresh_runtime_configuration()
    ...
    for _, track in ipairs(Config.FlipTok.MusicTracks) do
        ...
        if id == "" or title == "" or artist == "" or url == "" then
            error("[agent_phone] Every configured FlipTok music track requires Id, Title, Artist, and Url.")
        end
...
AddEventHandler("agent_phone:configurator:serverUpdated", function()
    refresh_runtime_configuration()
end)
...
local function are_profiles_blocked(first_id, second_id)
    return Bridge.Database.Query([[SELECT `id` FROM `agent_phone_fliptok_blocks` WHERE
...
local function profile_for_session(source)
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then return nil, error_response end
    local rows = Bridge.Database.Query([[SELECT p.* FROM `agent_phone_fliptok_sessions` s
        JOIN `agent_phone_fliptok_profiles` p ON p.`id` = s.`profile_id`
...
local function load_profile(profile_id, viewer_id)
    local rows = Bridge.Database.Query([[
        SELECT p.*, avatar.`url` AS `avatar_url`,
            EXISTS(SELECT 1 FROM `agent_phone_fliptok_follows` f WHERE f.`follower_id` = ? AND f.`following_id` = p.`id`) AS `is_following`,
            (SELECT COUNT(*) FROM `agent_phone_fliptok_follows` f WHERE f.`following_id` = p.`id`) AS `followers`,
            (SELECT COUNT(*) FROM `agent_phone_fliptok_follows` f WHERE f.`follower_id` = p.`id`) AS `following`,
            (SELECT COUNT(*) FROM `agent_phone_fliptok_videos` v WHERE v.`profile_id` = p.`id` AND v.`status` = 'published') AS `video_count`
        FROM `agent_phone_fliptok_profiles` p
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = p.`avatar_media_id`
        WHERE p.`id` = ? LIMIT 1
...
local function notify_profile(recipient_id, actor_id, kind, video_id)
    local rows = Bridge.Database.Query([[SELECT recipient.`account_id`, actor.`display_name` AS `actor_name`
        FROM `agent_phone_fliptok_profiles` recipient
        JOIN `agent_phone_fliptok_profiles` actor ON actor.`id` = ?
        WHERE recipient.`id` = ? LIMIT 1]], { actor_id, recipient_id })
    if not rows[1] then
        Bridge.Debug("warn", "[agent_phone] FlipTok notification target or actor no longer exists.", { always = true })
        return
    end
    AgentPhone.NotifyAccountDevices(tonumber(rows[1].account_id), "agent_phone:fliptok:new", {
...
local function hydrate_videos(rows)
    ...
    for _, video in ipairs(rows) do
        ...
        local youtube_id = AgentPhoneYouTube.ParseId(custom_music_url)
    ...
    if #video_ids > 0 then
        ...
        local media_rows = Bridge.Database.Query(([=[
            ...
            FROM `agent_phone_fliptok_video_media` relation
            JOIN `agent_phone_media` media ON media.`id` = relation.`media_id`
...
local function list_videos(viewer_id, where_clause, values, limit, offset, ranking)
    ...
    return hydrate_videos(Bridge.Database.Query(([[
        SELECT v.`id`, v.`profile_id`, v.`caption`, v.`location`, v.`comments_enabled`, v.`view_count`, v.`share_count`,
            ...
            EXISTS(SELECT 1 FROM `agent_phone_fliptok_reactions` r WHERE r.`video_id` = v.`id` AND r.`profile_id` = ? AND r.`kind` = 'like') AS `is_liked`,
            EXISTS(SELECT 1 FROM `agent_phone_fliptok_reactions` r WHERE r.`video_id` = v.`id` AND r.`profile_id` = ? AND r.`kind` = 'save') AS `is_saved`,
            EXISTS(SELECT 1 FROM `agent_phone_fliptok_follows` f WHERE f.`follower_id` = ? AND f.`following_id` = v.`profile_id`) AS `is_following`,
            (SELECT COUNT(*) FROM `agent_phone_fliptok_reactions` r WHERE r.`video_id` = v.`id` AND r.`kind` = 'like') AS `like_count`,
            (SELECT COUNT(*) FROM `agent_phone_fliptok_comments` c WHERE c.`video_id` = v.`id` AND c.`status` = 'visible') AS `comment_count`
        FROM `agent_phone_fliptok_videos` v
        JOIN `agent_phone_fliptok_profiles` p ON p.`id` = v.`profile_id`
        JOIN `agent_phone_media` m ON m.`id` = v.`media_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = p.`avatar_media_id`
        WHERE v.`status` = 'published' AND %s
            AND NOT EXISTS(SELECT 1 FROM `agent_phone_fliptok_blocks` b WHERE
...
local function feed(source, data)
    ...
    local ranking = "(v.`view_count` + v.`share_count` * 8 + (SELECT COUNT(*) FROM `agent_phone_fliptok_reactions` rr WHERE rr.`video_id` = v.`id`) * 4) DESC, v.`created_at` DESC"
    if data.mode == "following" then
        where = "v.`visibility` IN ('public', 'followers') AND EXISTS(SELECT 1 FROM `agent_phone_fliptok_follows` ff WHERE ff.`follower_id` = ? AND ff.`following_id` = v.`profile_id`)"
...
Bridge.Callbacks.Register("agent_phone:fliptok:register", function(source, data)
    if not AgentPhone.AllowOperation(source, "fliptok:register", 5, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    ...
    local profiles = Bridge.Database.Query("SELECT `id` FROM `agent_phone_fliptok_profiles` WHERE `account_id` = ? LIMIT 1", { account.id })
    local profile_id = profiles[1] and tonumber(profiles[1].id) or nil
    local duplicates = profile_id and Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_fliptok_profiles` WHERE `handle` = ? AND `id` <> ? LIMIT 1",
        { handle, profile_id }
    ) or Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_fliptok_profiles` WHERE `handle` = ? LIMIT 1",
        { handle }
    )
    if duplicates[1] then return { success = false, error = "handle_taken" } end

    if profile_id then
        local credentials = Bridge.Database.Query("SELECT `profile_id` FROM `agent_phone_fliptok_credentials` WHERE `profile_id` = ? LIMIT 1", { profile_id })
        if credentials[1] then return { success = false, error = "already_registered" } end
        Bridge.Database.Query("UPDATE `agent_phone_fliptok_profiles` SET `handle` = ?, `display_name` = ? WHERE `id` = ?", {
            handle, display_name, profile_id,
        })
    else
        local result = Bridge.Database.Query([[INSERT IGNORE INTO `agent_phone_fliptok_profiles`
        ...
        local created = Bridge.Database.Query("SELECT `id` FROM `agent_phone_fliptok_profiles` WHERE `account_id` = ? LIMIT 1", { account.id })
        if not created[1] then error("[agent_phone] FlipTok profile insert did not return the created profile.") end
    ...
    Bridge.Database.Query([[INSERT INTO `agent_phone_fliptok_sessions` (`device_imei`, `profile_id`) VALUES (?, ?)
...
Bridge.Callbacks.Register("agent_phone:fliptok:login", function(source, data)
    if not AgentPhone.AllowOperation(source, "fliptok:login", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    local session, error_response = AgentPhone.RequireSession(source)
    ...
    local profiles = Bridge.Database.Query([[SELECT p.`id` FROM `agent_phone_fliptok_profiles` p
        JOIN `agent_phone_fliptok_credentials` c ON c.`profile_id` = p.`id`
        WHERE p.`handle` = ?
...
Bridge.Callbacks.Register("agent_phone:fliptok:logout", function(source)
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then return error_response end
    Bridge.Database.Query("DELETE FROM `agent_phone_fliptok_sessions` WHERE `device_imei` = ?", { session.imei })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:bootstrap", function(source)
    local profile, error_response = require_profile(source)
    if not profile then
...
Bridge.Callbacks.Register("agent_phone:fliptok:feed", feed)

Bridge.Callbacks.Register("agent_phone:fliptok:video", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:video", 60, 60) then
        return { success = false, error = "rate_limited" }
    end
...
Bridge.Callbacks.Register("agent_phone:fliptok:discover", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
...
Bridge.Callbacks.Register("agent_phone:fliptok:music-metadata", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:music-metadata", 12, 60) then
        return { success = false, error = "rate_limited" }
    end
    local value = trim(type(data) == "table" and data.url or nil)
    local video_id = AgentPhoneYouTube.ParseId(value)
    if not video_id then return { success = false, error = "invalid_music_url" } end

    local metadata = AgentPhoneYouTube.FetchMetadata(video_id)
    return { success = true, data = {
        url = AgentPhoneYouTube.WatchUrl(video_id),
        videoId = video_id,
...
Bridge.Callbacks.Register("agent_phone:fliptok:publish", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:publish", 6, 60) then return { success = false, error = "rate_limited" } end
    ...
    for _, value in ipairs(submitted_media_ids) do
        ...
        if not normalized or normalized < 1 or normalized ~= math.floor(normalized)
            ...
            or not AgentPhoneMedia.ResolveOwnedMedia(source, tostring(normalized), media_type)
        then
    ...
    if custom_music_url ~= "" then
        ...
        local youtube_id = AgentPhoneYouTube.ParseId(custom_music_url)
        if youtube_id then
            local metadata = AgentPhoneYouTube.FetchMetadata(youtube_id)
            custom_music_url = AgentPhoneYouTube.WatchUrl(youtube_id)
    ...
    local id = new_id()
    local queries = {{ query = [[INSERT INTO `agent_phone_fliptok_videos`
    ...
    for index, current_media_id in ipairs(media_ids) do
        queries[#queries + 1] = {
            query = "INSERT INTO `agent_phone_fliptok_video_media` (`video_id`, `media_id`, `sort_order`) VALUES (?, ?, ?)",
...
Bridge.Callbacks.Register("agent_phone:fliptok:react", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:react", 60, 60) then return { success = false, error = "rate_limited" } end
    ...
    if data.active then
        local inserted = Bridge.Database.Query("INSERT IGNORE INTO `agent_phone_fliptok_reactions` (`video_id`, `profile_id`, `kind`) VALUES (?, ?, ?)", { data.id, profile.id, data.kind })
        if affected_rows(inserted) > 0 and data.kind == "like" and owner_id ~= profile.id then
            Bridge.Database.Query("INSERT INTO `agent_phone_fliptok_notifications` (`id`, `recipient_id`, `actor_id`, `video_id`, `kind`) VALUES (?, ?, ?, ?, 'like')", { new_id(), video.profile_id, profile.id, data.id })
            notify_profile(owner_id, profile.id, "like", data.id)
        end
    else
        Bridge.Database.Query("DELETE FROM `agent_phone_fliptok_reactions` WHERE `video_id` = ? AND `profile_id` = ? AND `kind` = ?", { data.id, profile.id, data.kind })
    end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:follow", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:follow", 30, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    local targets = Bridge.Database.Query("SELECT `id` FROM `agent_phone_fliptok_profiles` WHERE `id` = ? LIMIT 1", { target_id })
    ...
    if data.active then
        local inserted = Bridge.Database.Query("INSERT IGNORE INTO `agent_phone_fliptok_follows` (`follower_id`, `following_id`) SELECT ?, `id` FROM `agent_phone_fliptok_profiles` WHERE `id` = ?", { profile.id, target_id })
        if affected_rows(inserted) > 0 then
            Bridge.Database.Query("INSERT INTO `agent_phone_fliptok_notifications` (`id`, `recipient_id`, `actor_id`, `kind`) VALUES (?, ?, ?, 'follow')", { new_id(), target_id, profile.id })
            notify_profile(target_id, profile.id, "follow")
        end
    else
        Bridge.Database.Query("DELETE FROM `agent_phone_fliptok_follows` WHERE `follower_id` = ? AND `following_id` = ?", { profile.id, target_id })
    end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:comments", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    ...
    local rows = Bridge.Database.Query([[SELECT c.`id`, c.`parent_id`, c.`body`, UNIX_TIMESTAMP(c.`created_at`) * 1000 AS `created_at`,
        ...
        EXISTS(SELECT 1 FROM `agent_phone_fliptok_comment_reactions` reaction
            WHERE reaction.`comment_id` = c.`id` AND reaction.`profile_id` = ?) AS `is_liked`,
        (SELECT COUNT(*) FROM `agent_phone_fliptok_comment_reactions` reaction
            WHERE reaction.`comment_id` = c.`id`) AS `like_count`
        FROM `agent_phone_fliptok_comments` c
        JOIN `agent_phone_fliptok_profiles` p ON p.`id` = c.`profile_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = p.`avatar_media_id`
        LEFT JOIN `agent_phone_fliptok_comments` parent ON parent.`id` = c.`parent_id`
        LEFT JOIN `agent_phone_fliptok_profiles` parent_author ON parent_author.`id` = parent.`profile_id`
...
Bridge.Callbacks.Register("agent_phone:fliptok:comment", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:comment", 20, 60) then return { success = false, error = "rate_limited" } end
    ...
    local owner_id = tonumber(video.profile_id)
    if parent_id then
        local parents = Bridge.Database.Query([[SELECT c.`id`, c.`parent_id`, c.`profile_id` FROM `agent_phone_fliptok_comments` c
    ...
    Bridge.Database.Query("INSERT INTO `agent_phone_fliptok_comments` (`id`, `video_id`, `profile_id`, `parent_id`, `body`) VALUES (?, ?, ?, ?, ?)", {
        comment_id, data.id, profile.id, parent_id, body,
    })
    if owner_id ~= profile.id then
        Bridge.Database.Query("INSERT INTO `agent_phone_fliptok_notifications` (`id`, `recipient_id`, `actor_id`, `video_id`, `kind`) VALUES (?, ?, ?, ?, 'comment')", { new_id(), video.profile_id, profile.id, data.id })
        notify_profile(owner_id, profile.id, "comment", data.id)
    end
    return { success = true, data = { id = comment_id } }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:comment-react", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:comment-react", 60, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    local comments = Bridge.Database.Query([[SELECT c.`profile_id`, c.`video_id` FROM `agent_phone_fliptok_comments` c
        JOIN `agent_phone_fliptok_videos` v ON v.`id` = c.`video_id`
    ...
    if data.active then
        Bridge.Database.Query([[INSERT IGNORE INTO `agent_phone_fliptok_comment_reactions`
            (`comment_id`, `profile_id`) VALUES (?, ?)]], { comment_id, profile.id })
    else
        Bridge.Database.Query([[DELETE FROM `agent_phone_fliptok_comment_reactions`
            WHERE `comment_id` = ? AND `profile_id` = ?]], { comment_id, profile.id })
    end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:view", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:view", 120, 60) then return { success = false, error = "rate_limited" } end
    if type(data) ~= "table" or not load_accessible_video(data.id, profile.id) then
        return { success = false, error = "video_not_found" }
    end
    Bridge.Database.Query("UPDATE `agent_phone_fliptok_videos` SET `view_count` = `view_count` + 1 WHERE `id` = ? AND `status` = 'published'", { data.id })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:share", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:share", 30, 60) then return { success = false, error = "rate_limited" } end
    if type(data) ~= "table" or not load_accessible_video(data.id, profile.id) then
        return { success = false, error = "video_not_found" }
    end
    Bridge.Database.Query("UPDATE `agent_phone_fliptok_videos` SET `share_count` = `share_count` + 1 WHERE `id` = ? AND `status` = 'published'", { data.id })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:profile", function(source, data)
    ...
    local rows = handle and Bridge.Database.Query("SELECT `id` FROM `agent_phone_fliptok_profiles` WHERE `handle` = ? LIMIT 1", { handle }) or { { id = id } }
    ...
    local videos = list_videos(viewer.id, [[v.`profile_id` = ? AND (v.`visibility` = 'public' OR v.`profile_id` = ? OR
        (v.`visibility` = 'followers' AND EXISTS(SELECT 1 FROM `agent_phone_fliptok_follows` profile_follow
...
Bridge.Callbacks.Register("agent_phone:fliptok:connections", function(source, data)
    local viewer, error_response = require_profile(source)
    if not viewer then return error_response end
    local target_id = type(data) == "table" and tonumber(data.profileId) or nil
    ...
    local rows = Bridge.Database.Query(([[SELECT p.*, avatar.`url` AS `avatar_url`,
        EXISTS(SELECT 1 FROM `agent_phone_fliptok_follows` own_follow
            WHERE own_follow.`follower_id` = ? AND own_follow.`following_id` = p.`id`) AS `is_following`,
        (SELECT COUNT(*) FROM `agent_phone_fliptok_follows` followers WHERE followers.`following_id` = p.`id`) AS `followers`,
        (SELECT COUNT(*) FROM `agent_phone_fliptok_follows` following WHERE following.`follower_id` = p.`id`) AS `following`,
        (SELECT COUNT(*) FROM `agent_phone_fliptok_videos` video WHERE video.`profile_id` = p.`id` AND video.`status` = 'published') AS `video_count`
        FROM `agent_phone_fliptok_follows` f
        JOIN `agent_phone_fliptok_profiles` p ON p.`id` = %s
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = p.`avatar_media_id`
        WHERE %s = ? AND NOT EXISTS(SELECT 1 FROM `agent_phone_fliptok_blocks` block WHERE
...
Bridge.Callbacks.Register("agent_phone:fliptok:update-profile", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if type(data) ~= "table" then return { success = false, error = "invalid_request" } end
    local handle = normalize_handle(data.handle)
    ...
    if data.avatarMediaId ~= nil then
        ...
        if requested_avatar_id > 0 and not AgentPhoneMedia.ResolveOwnedMedia(source, requested_avatar_id, "photo") then
            return { success = false, error = "invalid_profile_image" }
        end
        avatar_media_id = requested_avatar_id > 0 and requested_avatar_id or nil
    end
    local duplicate = Bridge.Database.Query("SELECT `id` FROM `agent_phone_fliptok_profiles` WHERE `handle` = ? AND `id` <> ? LIMIT 1", { handle, profile.id })
    if duplicate[1] then return { success = false, error = "handle_taken" } end
    Bridge.Database.Query("UPDATE `agent_phone_fliptok_profiles` SET `handle` = ?, `display_name` = ?, `bio` = ?, `account_type` = ?, `avatar_media_id` = ? WHERE `id` = ?", {
...
Bridge.Callbacks.Register("agent_phone:fliptok:activities", function(source)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    local rows = Bridge.Database.Query([[SELECT n.`id`, n.`kind`, n.`video_id`, n.`read_at`, UNIX_TIMESTAMP(n.`created_at`) * 1000 AS `created_at`,
        p.`id` AS `profile_id`, p.`handle`, p.`display_name`, p.`verified`, avatar.`url` AS `avatar_url`
        FROM `agent_phone_fliptok_notifications` n JOIN `agent_phone_fliptok_profiles` p ON p.`id` = n.`actor_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = p.`avatar_media_id`
        WHERE n.`recipient_id` = ?
            AND (n.`kind` = 'verified' OR NOT EXISTS(SELECT 1 FROM `agent_phone_fliptok_blocks` b WHERE
...
Bridge.Callbacks.Register("agent_phone:fliptok:mark-activities", function(source)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    Bridge.Database.Query("UPDATE `agent_phone_fliptok_notifications` SET `read_at` = NOW() WHERE `recipient_id` = ? AND `read_at` IS NULL", { profile.id })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:report", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:report", 10, 60) then return { success = false, error = "rate_limited" } end
    ...
    local videos = Bridge.Database.Query([[SELECT v.`profile_id`, v.`caption`, creator.`handle`, creator.`display_name`, media.`url`
        FROM `agent_phone_fliptok_videos` v
        JOIN `agent_phone_fliptok_profiles` creator ON creator.`id` = v.`profile_id`
        JOIN `agent_phone_media` media ON media.`id` = v.`media_id`
        WHERE v.`id` = ? AND v.`status` = 'published'
            AND (v.`profile_id` = ? OR v.`visibility` = 'public' OR
                (v.`visibility` = 'followers' AND EXISTS(SELECT 1 FROM `agent_phone_fliptok_follows` f
    ...
    if not webhook then
        Bridge.Debug("warn", "[agent_phone] FlipTok report webhook is not configured.", { always = true })
        return { success = false, error = "report_unavailable" }
    end
    local video = videos[1]
    local payload = {
        username = "Agent Phone · FlipTok Reports",
        ...
        embeds = {{
            ...
            footer = { text = "agent_phone FlipTok" },
        }},
    }
    PerformHttpRequest(webhook, function(status)
        ...
        if status_code < 200 or status_code >= 300 then
            Bridge.Debug("warn", "[agent_phone] FlipTok Discord report webhook returned HTTP %s.", status_code, { always = true })
        end
    end, "POST", json.encode(payload), { ["Content-Type"] = "application/json" })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:block", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if not AgentPhone.AllowOperation(source, "fliptok:block", 20, 60) then
        return { success = false, error = "rate_limited" }
    end
    local target_id = type(data) == "table" and tonumber(data.profileId) or nil
    ...
    if not Bridge.Database.Transaction({
        { query = "INSERT IGNORE INTO `agent_phone_fliptok_blocks` (`blocker_id`, `blocked_id`) VALUES (?, ?)", params = { profile.id, target_id } },
        { query = "DELETE FROM `agent_phone_fliptok_follows` WHERE (`follower_id` = ? AND `following_id` = ?) OR (`follower_id` = ? AND `following_id` = ?)", params = { profile.id, target_id, target_id, profile.id } },
    }) then return { success = false, error = "request_failed" } end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:fliptok:delete", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if type(data) ~= "table" or type(data.id) ~= "string" then return { success = false, error = "video_not_found" } end
    local result = Bridge.Database.Query("UPDATE `agent_phone_fliptok_videos` SET `status` = 'removed' WHERE `id` = ? AND `profile_id` = ?", { data.id, profile.id })
...
local function run_verify_command(source, arguments)
    local command_locale = AgentPhoneLocales.Resolve(Config.Bridge.Locale).FlipTokCommand
    ...
    local function send_command_feedback(message, notification_type)
        ...
        TriggerClientEvent("agent_phone:fliptok:command-feedback", source, {
            message = message,
    ...
    if source ~= 0 and not Bridge.Framework.HasPermission(source, "fliptokverify") then
        ...
        Bridge.Debug(
            "warn",
            "[agent_phone] Player %d attempted to use the FlipTok verification command without an admin group.",
            source
        )
        return
    end
    ...
    local rows = Bridge.Database.Query("SELECT `id`, `verified` FROM `agent_phone_fliptok_profiles` WHERE `handle` = ? LIMIT 1", { handle })
    if not rows[1] then
    ...
    Bridge.Database.Query("UPDATE `agent_phone_fliptok_profiles` SET `verified` = ? WHERE `id` = ?", { verified and 1 or 0, rows[1].id })
    if verified then
        Bridge.Database.Query("INSERT INTO `agent_phone_fliptok_notifications` (`id`, `recipient_id`, `actor_id`, `kind`) VALUES (?, ?, ?, 'verified')", {
            new_id(), rows[1].id, rows[1].id,
        })
        notify_profile(tonumber(rows[1].id), tonumber(rows[1].id), "verified")
    end
    TriggerClientEvent("agent_phone:fliptok:verification-changed", -1, {
...
local function refresh_verify_command()
    ...
    if type(command_name) ~= "string" or command_name == "" then
        error("[agent_phone] Config.FlipTok.VerifyCommand must be a non-empty command name.")
    end
...
AddEventHandler("agent_phone:configurator:serverUpdated", refresh_verify_command)
end)
