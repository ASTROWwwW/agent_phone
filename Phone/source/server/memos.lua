...
local function device_owner(source, imei)
    if not AgentPhoneImei.IsValid(imei) then
        return nil, { success = false, error = "invalid_request" }
    end
    if not AgentPhone.FindDeviceSlots(source, imei)[1] then
...
local function memo_dto(row)
    ...
    if type(waveform) ~= "table" then
        error(("[agent_phone] Voice memo %s has an invalid waveform payload."):format(tostring(row.id)))
    end
    return {
        id = row.id,
        title = row.title,
...
local function list_memos(owner)
    ...
    local rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_voice_memos` memo
        INNER JOIN `agent_phone_media` media ON media.`id` = memo.`media_id`
...
function AgentPhoneMemos.List(account_id, imei)
...
local function upload_result(source, correlation_id, success, error_code, memo)
    TriggerClientEvent("agent_phone:memos:upload-result", source, {
...
local function discard_verified_upload(remote_id)
    local deleted, delete_error = AgentPhoneMedia.DeleteRemoteFile(remote_id)
    if not deleted then
        Bridge.Debug(
            "warn",
...
Bridge.Callbacks.Register("agent_phone:memos:list", function(source)
    if not AgentPhone.AllowOperation(source, "memos_list", Config.Memos.ListsPerMinute, 60) then
...
Bridge.Callbacks.Register("agent_phone:memos:update", function(source, data)
    if not AgentPhone.AllowOperation(source, "memos_write", Config.Memos.WritesPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local owner, error_response = session_owner(source)
    if not owner then
    ...
    local result = Bridge.Database.Query(([[
        UPDATE `agent_phone_voice_memos` memo
        INNER JOIN `agent_phone_media` media ON media.`id` = memo.`media_id`
...
Bridge.Callbacks.Register("agent_phone:memos:delete", function(source, data)
    if not AgentPhone.AllowOperation(source, "memos_write", Config.Memos.WritesPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local owner, error_response = session_owner(source)
    ...
    if (tonumber(references[1] and references[1].count) or 0) <= 1 then
        local deleted, delete_error = AgentPhoneMedia.DeleteRemoteFile(memo.remote_id)
        if not deleted then
    ...
    local result = Bridge.Database.Query(([[
        DELETE media FROM `agent_phone_media` media
        INNER JOIN `agent_phone_voice_memos` memo ON memo.`media_id` = media.`id`
        WHERE memo.`id` = ? AND %s AND media.`media_type` = 'audio'
    ]]):format(condition), params)
    pending_deletes[memo.remote_id] = nil
    if affected_rows(result) ~= 1 then
        error(("[agent_phone] Voice memo %s disappeared during deletion."):format(data.id))
    end
    return { success = true, data = { id = data.id } }
end)

RegisterNetEvent("agent_phone:memos:request-upload", function(data)
    local src = source
    if not AgentPhone.AllowOperation(src, "memos_upload", Config.Memos.UploadsPerMinute, 60) then
    ...
    local presigned_url, presigned_error = AgentPhoneMedia.RequestPresignedUrl()
    if not presigned_url then
        Bridge.Debug(
            "error",
            "[agent_phone] Voice memo upload could not start for source %s: %s.",
            tostring(src),
    ...
    TriggerClientEvent("agent_phone:memos:upload-ready", src, {
...
RegisterNetEvent("agent_phone:memos:complete-upload", function(data)
    local src = source
    ...
    if not owner or owner.imei ~= state.owner.imei or owner.account_id ~= state.owner.account_id then
        pending_uploads[request_id] = nil
        local rejected, _, trusted_remote = AgentPhoneMedia.VerifyRemoteUpload(
            state,
            data.remoteId,
            data.url
        )
    ...
    local verified, verify_error, trusted_remote = AgentPhoneMedia.VerifyRemoteUpload(
        state,
        data.remoteId,
        data.url
    )
    ...
    local transaction = {
        ...
        {
            query = [[
                INSERT INTO `agent_phone_voice_memos`
...
RegisterNetEvent("agent_phone:memos:cancel-upload", function(data)
    local src = source
    local request_id = type(data) == "table" and data.requestId or nil
    local state = type(request_id) == "string" and pending_uploads[request_id] or nil
...
RegisterNetEvent("agent_phone:memos:fail-upload", function(data)
    local src = source
    local request_id = type(data) == "table" and data.requestId or nil
    local state = type(request_id) == "string" and pending_uploads[request_id] or nil
    if not state or state.source ~= src or state.completing then
