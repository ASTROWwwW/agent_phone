...
local function normalize_target(target)
    ...
    if target.kind == "number" then
        local phone_number = AgentPhoneSimNumber.Normalize(
            target.value,
    ...
    if target.kind == "device" then
        if not AgentPhoneImei.IsValid(target.value) then
            return nil, "invalid_target"
        end
...
Bridge.Database.AfterMigration("agent_phone", function()
    if type(AgentPhoneDeviceDirectory) ~= "table"
        or type(AgentPhoneDeviceDirectory.GetOnlineBySource) ~= "function"
        or type(AgentPhoneDeviceDirectory.GetOnlineByPhoneNumber) ~= "function"
        or type(AgentPhoneDeviceDirectory.GetOnlineByImei) ~= "function"
    then
        error("[agent_phone] Notification router initialized before the phone device directory.")
    end
    ...
    local function add_candidate(candidates, identity, expected_phone_number)
        ...
        if not AgentPhoneImei.IsValid(identity.imei) then
            return
        end
    ...
    local function resolve_candidates(target)
        local candidates = {}
        if target.kind == "source" then
            add_candidate(candidates, AgentPhoneDeviceDirectory.GetOnlineBySource(target.value))
            return candidates
        end

        if target.kind == "number" then
            local identity = AgentPhoneDeviceDirectory.GetOnlineByPhoneNumber(target.value)
            add_candidate(candidates, identity, target.value)
            return candidates
        end

        if target.kind == "device" then
            add_candidate(candidates, AgentPhoneDeviceDirectory.GetOnlineByImei(target.value))
            return candidates
        end
        ...
        for _, player in ipairs(Bridge.Framework.GetPlayers()) do
            ...
            if player_source and not candidates[player_source] then
                add_candidate(
                    candidates,
                    AgentPhoneDeviceDirectory.GetOnlineBySource(player_source)
                )
            end
        end
        return candidates
    ...
    local function load_device_context(imei)
        ...
        if not row or row.imei ~= imei or type(row.device_name) ~= "string" then
            Bridge.Debug(
                "error",
                "[agent_phone] Notification delivery could not load device %s.",
                tostring(imei)
            )
            return nil
        end
    ...
    function AgentPhoneNotifications.Send(target, notification)
        ...
        for _, candidate in pairs(candidates) do
            local identity = AgentPhoneDeviceDirectory.GetOnlineBySource(candidate.source)
            if identity_still_matches(candidate, identity) then
                ...
                local current_identity = device
                    and AgentPhoneDeviceDirectory.GetOnlineBySource(candidate.source)
                    or nil
                if device and identity_still_matches(candidate, current_identity) then
                    TriggerClientEvent(
                        "agent_phone:notifications:show",
                        candidate.source,
                        {
    ...
    Bridge.Callbacks.Register("agent_phone:notifications:self", function(source, notification)
        local result, notification_error = AgentPhoneNotifications.Send(
