...
local function stable_uuid(seed)
    ...
    if type(id) ~= "string" or #id ~= 36 then
        error("[agent_phone] Test data could not generate a stable UUID.")
    end
    return id
end
...
local function seed_hash(seed)
    ...
    if type(value) ~= "string" or #value ~= 12 then
        error("[agent_phone] Test data could not generate a stable account suffix.")
    end
    return value
end
...
local function database_uuid()
    ...
    if type(id) ~= "string" then
        error("[agent_phone] Test data could not generate a database UUID.")
    end
    return id
end
...
local function ensure_account(email)
    Bridge.Database.Query(
        "INSERT IGNORE INTO `agent_phone_accounts` (`email`, `password`) VALUES (?, ?)",
        { email, "agent-phone-test" }
    )
    local rows = Bridge.Database.Query(
        "SELECT `id`, `email` FROM `agent_phone_accounts` WHERE `email` = ? LIMIT 1",
        { email }
    )
    if not rows[1] then
        error(("[agent_phone] Test data account '%s' could not be loaded."):format(email))
    end
...
local function ensure_media(account_id, remote_id, url, media_type)
    local rows = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_media`
        WHERE `account_id` = ? AND `remote_id` = ?
        ORDER BY `id` LIMIT 1
    ]], { account_id, remote_id })
    if rows[1] then
        Bridge.Database.Query(
            "UPDATE `agent_phone_media` SET `url` = ?, `media_type` = ? WHERE `id` = ?",
            { url, media_type, rows[1].id }
        )
        return tonumber(rows[1].id)
    end
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_media` (`account_id`, `url`, `remote_id`, `media_type`)
        VALUES (?, ?, ?, ?)
    ...
    if not rows[1] then
        error("[agent_phone] Test media could not be created.")
    end
    return tonumber(rows[1].id)
end
...
local function reserve_sim(owner_identifier, firstname, lastname)
    ...
    local sim_id
    local number = AgentPhoneSimNumber.Reserve(database_uuid, function(candidate)
        if AgentPhoneCompanies.IsServiceNumber(candidate) then
            return false
        end
        sim_id = database_uuid()
        local result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_sims`
    ...
    if not number then
        error("[agent_phone] Test data could not reserve a SIM number.")
    end
...
local function restore_sim_attachment(sim_id, current_imei, previous_imei)
    local statements = {
        {
            query = "UPDATE `agent_phone_devices` SET `sim_id` = NULL WHERE `imei` = ? AND `sim_id` = ?",
            params = { current_imei, sim_id },
        },
    }
    if previous_imei then
        statements[#statements + 1] = {
            query = "UPDATE `agent_phone_devices` SET `sim_id` = ? WHERE `imei` = ? AND `sim_id` IS NULL",
    ...
    local rows = Bridge.Database.Query(
        "SELECT `imei` FROM `agent_phone_devices` WHERE `sim_id` = ? LIMIT 1",
        { sim_id }
    )
    if previous_imei then
...
local function move_sim_to_device(sim_id, imei)
    ...
    local moved = Bridge.Database.Transaction({
        {
            query = "UPDATE `agent_phone_devices` SET `sim_id` = NULL WHERE `sim_id` = ? AND `imei` <> ?",
            params = { sim_id, imei },
        },
        {
            query = "UPDATE `agent_phone_devices` SET `sim_id` = ? WHERE `imei` = ? AND `sim_id` IS NULL",
    ...
    if not moved then
        error("[agent_phone] Test data could not move the player's SIM to the selected phone.")
    end

    rows = Bridge.Database.Query(
        "SELECT `sim_id` FROM `agent_phone_devices` WHERE `imei` = ? LIMIT 1",
        { imei }
    )
    if not rows[1] or rows[1].sim_id ~= sim_id then
        if not restore_sim_attachment(sim_id, imei, previous_imei) then
            error("[agent_phone] Test data could not verify the SIM move or restore its previous device.")
        end
        error("[agent_phone] Test data could not verify the SIM move.")
    end
...
local function ensure_bot(label, email_local, imei, firstname, lastname)
    ...
    local sim = reserve_sim("agent_phone:testbot:" .. label, firstname, lastname)
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_devices` (`imei`, `account_id`, `sim_id`, `device_name`)
        VALUES (?, ?, ?, ?)
...
local function ensure_numeric_profile(table_name, account_id)
    ...
    if not rows[1] then
        error(("[agent_phone] Test profile in '%s' could not be loaded."):format(table_name))
    end
    return tonumber(rows[1].id)
end
...
local function seed_core(context)
    ...
    local games = {
        ...
        ["agent-flappy"] = { design = "neon", highScore = 18, soundEnabled = true },
        ["neon-drop"] = { bestLines = 14, bestScore = 12600, soundEnabled = true },
    }
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_device_data` (`device_imei`, `namespace`, `payload`)
        VALUES (?, 'alarms', ?)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_contacts`
    ...
    Bridge.Database.Query("DELETE FROM `agent_phone_call_entries` WHERE `call_id` = ? AND `account_id` = ?", { call_id, account_id })
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_call_entries`
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_notes` (`id`, `account_id`, `title`, `body`, `pinned`)
        VALUES (?, ?, 'Phone Test-Checkliste', 'Kontakte\nNachrichten\nAnrufe\nSocial Apps\nMarktplatz\nFirmen', 1)
        ON DUPLICATE KEY UPDATE `title` = VALUES(`title`), `body` = VALUES(`body`), `pinned` = 1
    ]], { note_one, account_id })
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_notes` (`id`, `account_id`, `title`, `body`, `pinned`)
        VALUES (?, ?, 'Ideen für später', 'DarkChat testen und eine AgentRide-Fahrt bewerten.', 0)
    ...
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_mail_entries` (`message_id`, `account_id`, `folder`)
        VALUES (?, ?, 'inbox')
    ]], { mail_id, account_id })
    ...
    Bridge.Database.Query(
        "DELETE FROM `agent_phone_bank_transactions` WHERE `owner_identifier` = ? AND `reference` LIKE 'agent-phone-test:%'",
        { context.identifier }
    )
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_bank_transactions` (`owner_identifier`, `kind`, `amount`, `label`, `reference`, `created_at`)
        VALUES (?, 'deposit', 2500, 'Test paycheck', 'agent-phone-test:paycheck', DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 DAY)),
               (?, 'withdrawal', 85, 'Los Santos Customs', 'agent-phone-test:repair', DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 3 HOUR)),
               (?, 'transfer_in', 420, 'Alex Rivera', 'agent-phone-test:transfer', DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 30 MINUTE))
    ]], { context.identifier, context.identifier, context.identifier })

    local invoice_id = stable_uuid(context.key .. ":invoice:repair")
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_billing_invoices`
            (`id`, `recipient_identifier`, `issuer_identifier`, `issuer_account`, `issuer_label`,
             `title`, `description`, `amount`, `currency`, `status`, `due_at`)
        VALUES (?, ?, 'agent_phone:testbot:mechanic', 'society_mechanic', 'Los Santos Customs',
    ...
    Bridge.Database.Query(
        "DELETE FROM `agent_phone_billing_events` WHERE `invoice_id` = ? AND `note` = 'Generated by the phone test data command.'",
        { invoice_id }
    )
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_billing_events` (`invoice_id`, `event`, `actor_identifier`, `note`)
        VALUES (?, 'created', 'agent_phone:testbot:mechanic', 'Generated by the phone test data command.')
    ]], { invoice_id })
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_map_markers` (`id`, `device_imei`, `label`, `color`, `position_x`, `position_y`, `position_z`)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_music_youtube_songs` (`id`, `account_id`, `video_id`, `title`, `artist`)
        VALUES (?, ?, 'dQw4w9WgXcQ', 'Never Gonna Give You Up', 'Rick Astley')
        ON DUPLICATE KEY UPDATE `title` = VALUES(`title`), `artist` = VALUES(`artist`)
    ]], { song_id, account_id })
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_music_playlists` (`id`, `account_id`, `name`)
        VALUES (?, ?, 'Test Drive Mix')
        ON DUPLICATE KEY UPDATE `name` = VALUES(`name`)
    ]], { playlist_id, account_id })
    Bridge.Database.Query("DELETE FROM `agent_phone_music_playlist_items` WHERE `playlist_id` = ?", { playlist_id })
    Bridge.Database.Query([[
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_easyshare_transfers`
...
local function seed_marketplace(context)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_marketplace_profiles` (`account_id`, `display_name`, `bio`, `avatar_media_id`)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_marketplace_listings`
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_marketplace_images` (`listing_id`, `media_id`, `gradient`, `sort_order`)
        VALUES (?, ?, ?, 1)
    ...
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_marketplace_favorites` (`account_id`, `listing_id`) VALUES (?, ?)
    ]], { account_id, bot_listing })
    local inquiry_id = stable_uuid(context.key .. ":market:inquiry")
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_marketplace_inquiries`
    ...
    Bridge.Database.Query("DELETE FROM `agent_phone_marketplace_messages` WHERE `inquiry_id` = ?", { inquiry_id })
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_marketplace_messages` (`inquiry_id`, `sender_account_id`, `body`, `read_at`)
...
local function seed_social_apps(context)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_picstagram_profiles`
    ...
    local pic_user = ensure_string_profile("agent_phone_picstagram_profiles", account_id)
    local pic_bot_seed = stable_uuid("agent_phone:testbot:pic:alex")
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_picstagram_profiles`
    ...
    local pic_bot = ensure_string_profile("agent_phone_picstagram_profiles", bot_id)
    Bridge.Database.Query([[
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_picstagram_posts` (`id`, `profile_id`, `caption`, `location`)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_picstagram_post_media` (`post_id`, `media_id`, `position`)
        VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE `media_id` = VALUES(`media_id`)
    ]], { pic_post, context.media.bot_city })
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_picstagram_follows` (`follower_id`, `following_id`, `status`)
        VALUES (?, ?, 'accepted')
    ]], { pic_user, pic_bot })
    Bridge.Database.Query([[
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_picstagram_stories` (`id`, `profile_id`, `media_id`, `body`, `expires_at`)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_picstagram_activities` (`id`, `recipient_id`, `actor_id`, `post_id`, `kind`)
        VALUES (?, ?, ?, ?, 'like')
        ON DUPLICATE KEY UPDATE `read_at` = NULL
    ]], { pic_activity, pic_user, pic_bot, pic_post })

    Bridge.Database.Query([[
        INSERT INTO `agent_phone_fliptok_profiles` (`account_id`, `handle`, `display_name`, `bio`)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_fliptok_profiles`
    ...
    local flip_user = ensure_numeric_profile("agent_phone_fliptok_profiles", account_id)
    local flip_bot = ensure_numeric_profile("agent_phone_fliptok_profiles", bot_two_id)
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_fliptok_sessions` (`device_imei`, `profile_id`)
        VALUES (?, ?) ON DUPLICATE KEY UPDATE `profile_id` = VALUES(`profile_id`)
    ]], { context.imei, flip_user })
    local flip_video = stable_uuid(context.key .. ":flip:video")
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_fliptok_videos`
    ...
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_fliptok_follows` (`follower_id`, `following_id`) VALUES (?, ?)
    ]], { flip_user, flip_bot })
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_fliptok_reactions` (`video_id`, `profile_id`, `kind`)
        VALUES (?, ?, 'like'), (?, ?, 'save')
    ]], { flip_video, flip_user, flip_video, flip_user })
    local flip_comment = stable_uuid(context.key .. ":flip:comment")
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_fliptok_comments` (`id`, `video_id`, `profile_id`, `body`)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_fliptok_notifications` (`id`, `recipient_id`, `actor_id`, `video_id`, `kind`)
        VALUES (?, ?, ?, ?, 'follow') ON DUPLICATE KEY UPDATE `read_at` = NULL
    ]], { flip_notification, flip_user, flip_bot, flip_video })

    Bridge.Database.Query([[
        INSERT INTO `agent_phone_feather_profiles` (`account_id`, `handle`, `display_name`, `bio`, `avatar_media_id`)
        VALUES (?, ?, ?, 'Testing Feather in game.', ?)
        ON DUPLICATE KEY UPDATE `handle` = VALUES(`handle`), `display_name` = VALUES(`display_name`),
            `bio` = VALUES(`bio`), `avatar_media_id` = VALUES(`avatar_media_id`)
    ]], { account_id, "tester" .. suffix, context.player_name, context.media.user_portrait })
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_feather_profiles`
            (`account_id`, `handle`, `display_name`, `bio`, `avatar_media_id`, `verified`)
        VALUES (?, 'alex_updates', 'Alex Rivera', 'Los Santos live updates.', ?, 1)
        ON DUPLICATE KEY UPDATE `display_name` = VALUES(`display_name`), `bio` = VALUES(`bio`),
            `avatar_media_id` = VALUES(`avatar_media_id`), `verified` = 1
    ]], { bot_id, context.media.bot_portrait })
    local feather_user = ensure_numeric_profile("agent_phone_feather_profiles", account_id)
    local feather_bot = ensure_numeric_profile("agent_phone_feather_profiles", bot_id)
    local feather_post = stable_uuid(context.key .. ":feather:post")
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_feather_posts` (`id`, `profile_id`, `body`)
    ...
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_feather_hashtags` (`post_id`, `tag`)
        VALUES (?, 'lossantos'), (?, 'phoneqa')
    ]], { feather_post, feather_post })
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_feather_follows` (`follower_id`, `following_id`) VALUES (?, ?)
    ]], { feather_user, feather_bot })
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_feather_reactions` (`post_id`, `profile_id`, `kind`)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_feather_notifications` (`id`, `recipient_id`, `actor_id`, `post_id`, `kind`)
        VALUES (?, ?, ?, ?, 'follow') ON DUPLICATE KEY UPDATE `read_at` = NULL
    ]], { feather_notification, feather_user, feather_bot, feather_post })

    Bridge.Database.Query([[
        INSERT INTO `agent_phone_flare_profiles`
    ...
    local flare_user = ensure_numeric_profile("agent_phone_flare_profiles", account_id)
    local flare_bot = ensure_numeric_profile("agent_phone_flare_profiles", bot_two_id)
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_flare_profile_photos` (`profile_id`, `media_id`, `sort_order`)
        VALUES (?, ?, 1), (?, ?, 1)
    ...
    local proposed_match_id = stable_uuid(
        ("agent_phone:testdata:flare:match:%s:%s"):format(account_a, account_b)
    )
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_flare_matches` (`id`, `account_a_id`, `account_b_id`)
    ...
    local match_rows = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_flare_matches`
    ...
    if type(match_id) ~= "string" then
        error("[agent_phone] Test data Flare match could not be loaded.")
    end
    local flare_body = "Hey! Bereit für einen vollständigen App-Test?"
    local message_rows = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_flare_messages`
    ...
    local flare_message = message_rows[1] and message_rows[1].id
        or stable_uuid("agent_phone:testdata:flare:message:" .. match_id)
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_flare_messages` (`id`, `match_id`, `sender_account_id`, `body`, `read_at`)
        VALUES (?, ?, ?, ?, NULL)
        ON DUPLICATE KEY UPDATE `body` = VALUES(`body`), `read_at` = NULL
...
local function seed_private_and_services(context)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_darkchat_profiles`
    ...
    local dark_user = ensure_numeric_profile("agent_phone_darkchat_profiles", account_id)
    local dark_bot = ensure_numeric_profile("agent_phone_darkchat_profiles", bot_id)
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_darkchat_contacts` (`profile_id`, `contact_profile_id`, `alias_override`)
        VALUES (?, ?, 'Ghost')
    ]], { dark_user, dark_bot })
    local conversation_id = stable_uuid(context.key .. ":dark:conversation")
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_darkchat_conversations` (`id`, `disappearing_seconds`)
        VALUES (?, 0) ON DUPLICATE KEY UPDATE `disappearing_seconds` = 0
    ]], { conversation_id })
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_darkchat_members` (`conversation_id`, `profile_id`, `last_read_at`)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_darkchat_messages` (`id`, `conversation_id`, `sender_profile_id`, `body`)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_darkchat_messages`
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_agentride_profiles` (`id`, `owner_identifier`)
        VALUES (?, ?) ON DUPLICATE KEY UPDATE `owner_identifier` = VALUES(`owner_identifier`)
    ]], { ride_user_seed, context.identifier })
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_agentride_profiles` (`id`, `owner_identifier`)
        VALUES (?, 'agent_phone:testbot:alex') ON DUPLICATE KEY UPDATE `owner_identifier` = VALUES(`owner_identifier`)
    ]], { ride_bot_seed })
    local ride_user_rows = Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_agentride_profiles` WHERE `owner_identifier` = ? LIMIT 1",
        { context.identifier }
    )
    local ride_bot_rows = Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_agentride_profiles` WHERE `owner_identifier` = 'agent_phone:testbot:alex' LIMIT 1",
        {}
    )
    ...
    if type(ride_user) ~= "string" or type(ride_bot) ~= "string" then
        error("[agent_phone] Test AgentRide profiles could not be loaded.")
    end
    local ride_id = stable_uuid(context.key .. ":agentride:completed")
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_agentride_rides`
    ...
    local company_rows = Bridge.Database.Query(
        "SELECT `company_id` FROM `agent_phone_company_profiles` WHERE `company_id` = 'mechanic' LIMIT 1",
        {}
    )
    if company_rows[1] then
        ...
        Bridge.Database.Query([[
            ...
            VALUES (?, ?, 'company', 'agent_phone:testbot:mechanic',
        ...
        Bridge.Database.Query([[
            ...
            VALUES (?, ?, 'status', 'company', 'agent_phone:testbot:mechanic', 'in_progress', 'waiting_customer',
...
local function seed_for_source(source)
    ...
    if not phone_slot then
        error(("[agent_phone] Test data source %s has no phone item."):format(source))
    end
    local imei, device_error = AgentPhone.EnsureDevice(source, phone_slot)
    if not imei then
        error(("[agent_phone] Test data could not resolve the phone device: %s"):format(tostring(device_error)))
    end
    local identifier = Bridge.Framework.GetIdentifier(source)
    if type(identifier) ~= "string" or identifier == "" then
        error("[agent_phone] Test data could not resolve the player identifier.")
    end
    ...
    local device = AgentPhone.LoadDevice(imei)
    local account
    if device and device.account_id then
        local rows = Bridge.Database.Query(
            "SELECT `id`, `email` FROM `agent_phone_accounts` WHERE `id` = ? LIMIT 1",
            { device.account_id }
        )
        account = rows[1]
        if not account then
            error("[agent_phone] Test data found a device with a missing Agent Cloud account.")
        end
        account.id = tonumber(account.id)
    else
        account = ensure_account("tester_" .. seed_hash(identifier) .. "@" .. Config.Mail.Domain)
        Bridge.Database.Query("UPDATE `agent_phone_devices` SET `account_id` = ? WHERE `imei` = ?", { account.id, imei })
    end

    device = AgentPhone.LoadDevice(imei)
    local sim
    if device and device.sim_id then
        local rows = Bridge.Database.Query("SELECT * FROM `agent_phone_sims` WHERE `id` = ? LIMIT 1", { device.sim_id })
        sim = rows[1]
    else
        ...
        metadata.formatted_number = AgentPhoneSimNumber.Format(
            sim.phone_number,
        ...
        if not Bridge.Inventory.SetSlotMetadata(source, phone_slot.slot, metadata) then
            if not restore_sim_attachment(sim.id, imei, previous_imei) then
                error(
                    "[agent_phone] Test data could not update the phone item's SIM metadata or restore its previous device."
                )
            end
            error("[agent_phone] Test data could not update the phone item's SIM metadata.")
        end
    end
...
local function run_test_data_command(source)
    ...
    if source <= 0 then
        Bridge.Debug("warn", "[agent_phone] The test data command must be run by an in-game player.")
        return
    end
    if Config.TestData.AdminOnly and not Bridge.Framework.HasPermission(source, "phonetestdata") then
        Bridge.Debug("warn", "[agent_phone] Source %s attempted to run the restricted test data command.", tostring(source))
        TriggerClientEvent("agent_phone:testdata:feedback", source, false)
        return
    end
    local now = os.time()
    if seed_attempts[source] and now - seed_attempts[source] < 30 then
        Bridge.Debug("warn", "[agent_phone] Source %s repeated the test data command too quickly.", tostring(source))
        TriggerClientEvent("agent_phone:testdata:feedback", source, false)
        return
    end
    ...
    if not success then
        Bridge.Debug("error", "[agent_phone] Test data seeding failed for source %s: %s", tostring(source), tostring(result))
        TriggerClientEvent("agent_phone:testdata:feedback", source, false)
        return
    end
    Bridge.Debug("info", "[agent_phone] Test data seeded for source %s.", tostring(source), { always = true })
    TriggerClientEvent("agent_phone:testdata:feedback", source, true, result)
end
...
local function refresh_test_data_command()
    ...
    if type(active_test_data_command) ~= "string" or active_test_data_command == "" then
        error("[agent_phone] Config.TestData.Command must be a non-empty command name.")
    end
...
AddEventHandler("agent_phone:configurator:serverUpdated", refresh_test_data_command)
