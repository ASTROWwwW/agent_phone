...
local function register_policy(policy)
    ...
    if existing then
        Bridge.Debug(
            "warn",
            "[agent_phone] Rejected duplicate custom app policy '%s' from '%s'; it is already owned by '%s'.",
            app_id,
...
local function has_custom_app_permission(app_id, permission)
    ...
    if not valid_id or type(permission) ~= "string" or not AgentPhoneApps.AllowedPermissions[permission] then
        return false
    end
...
local function send_custom_app_message(player_source, app_id, payload)
    ...
    TriggerClientEvent(
        "agent_phone:custom-app:message",
        player_source,
        owner_resource,
        app_id,
        normalized_payload
...
AgentPhoneApps.GetPolicy = get_custom_app_policy
AgentPhoneApps.HasPermission = has_custom_app_permission
AgentPhoneApps.ServerPublicApi = {
...
local function refresh_bundled_policies()
    ...
    if Config.CustomApps.Enabled and Config.CustomApps.BundledApps then
        local bundled = AgentPhoneApps.GetBundledManifests()
