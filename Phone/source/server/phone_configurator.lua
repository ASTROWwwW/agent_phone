AgentPhoneConfigurator = AgentPhoneConfigurator or {}

local TABLE_NAME = "agent_phone_configurator"
...
local function copy_value(value, active)
    ...
    if active[value] then
        error("[agent_phone] Phone configurator cannot copy a cyclic table.")
    end
    active[value] = true
...
local function serialize_value(value, active)
    ...
    if value_type == "number" then
        if value ~= value or value == math.huge or value == -math.huge then
            error("[agent_phone] Phone configurator cannot serialize a non-finite number.")
        end
        return value
    end
    if value_type == "vector2" then
        return { __agentType = "vector2", x = value.x, y = value.y }
    end
    if value_type == "vector3" then
        return { __agentType = "vector3", x = value.x, y = value.y, z = value.z }
    end
    if value_type == "vector4" then
        return { __agentType = "vector4", x = value.x, y = value.y, z = value.z, w = value.w }
    end
    if value_type ~= "table" then
        error(("[agent_phone] Phone configurator cannot serialize value type '%s'."):format(value_type))
    end

    active = active or {}
    if active[value] then
        error("[agent_phone] Phone configurator cannot serialize a cyclic table.")
    end
    active[value] = true
    ...
    if has_numeric_map_key then
        ...
        for key, child in pairs(value) do
            if type(key) ~= "string" and type(key) ~= "number" then
                error("[agent_phone] Phone configurator only supports string and numeric table keys.")
            end
            entries[#entries + 1] = {
                key = key,
                keyType = type(key),
        ...
        active[value] = nil
        return { __agentType = "map", entries = entries }
    end
...
local function upgrade_legacy_map(defaults, saved)
    ...
    for key, child in pairs(saved) do
        ...
        if normalized_key == nil then
            error("[agent_phone] Phone configurator could not migrate a numeric configuration key.")
        end
        entries[#entries + 1] = {
            key = normalized_key,
            keyType = key_type,
...
local function merge_values(defaults, saved, path, excluded_paths)
    ...
    if defaults.__agentType == "map" and not saved.__agentType then
        saved = upgrade_legacy_map(defaults, saved)
    end
    if defaults.__agentType == "map" and saved.__agentType == "map" then
        local saved_entries = {}
    ...
    if defaults.__agentType or saved.__agentType then
        return copy_value(saved)
    end
...
local function decode_payload(encoded, scope)
    if type(encoded) ~= "string" or encoded == "" then
        error(("[agent_phone] Phone configurator has an empty %s SQL payload."):format(scope))
    end

    local success, decoded = pcall(json.decode, encoded)
    if not success or type(decoded) ~= "table" then
        error(("[agent_phone] Phone configurator failed to decode the %s SQL payload."):format(scope))
    end
    return decoded
end
...
local function encode_payload(payload, scope)
    ...
    if not success or type(encoded) ~= "string" then
        error(("[agent_phone] Phone configurator failed to encode the %s SQL payload."):format(scope))
    end
    if #encoded > MAX_PAYLOAD_BYTES then
        error(("[agent_phone] Phone configurator %s payload exceeds %s bytes."):format(scope, MAX_PAYLOAD_BYTES))
    end
    return encoded
end
...
local function restore_redacted_values(value, current, path)
    ...
    if value.__agentType == "map" then
        ...
        if type(current) == "table" and current.__agentType == "map" then
...
local function company_definition_entry_default(company_id, configuration)
    ...
    return {
        ...
        Icon = "building",
        Job = company_id or "",
        Location = { __agentType = "vector3", x = 0.0, y = 0.0, z = 0.0 },
        LocationLabel = "",
...
local function build_structure(value, scope, path)
    ...
    if value.__agentType == "vector2" or value.__agentType == "vector3" or value.__agentType == "vector4" then
        return {
            kind = "vector",
            vectorType = value.__agentType,
        }
    end
    if value.__agentType == "map" then
        local entries = {}
...
local function build_sections(scope, payload, defaults, sections, field_index)
    ...
    for _, key in ipairs(keys) do
        ...
        elseif type(value) == "table" and not value.__agentType and not is_sequence(value) and next(value) ~= nil then
            local fields = {}
...
local function validate_structured_value(value, depth, state)
    ...
    local agent_type = value.__agentType
    if agent_type then
        if agent_type == "vector2" or agent_type == "vector3" or agent_type == "vector4" then
            local axes = agent_type == "vector2" and { "x", "y" }
                or agent_type == "vector3" and { "x", "y", "z" }
                or { "x", "y", "z", "w" }
        ...
        if agent_type ~= "map" or type(value.entries) ~= "table" then
            return false
        end
...
local function validate_locked_structure(structure, value)
    ...
    if structure.kind == "vector" then
        return type(value) == "table" and value.__agentType == structure.vectorType
    end
    if structure.kind == "list" then
        if type(value) ~= "table" or value.__agentType or (next(value) and not is_sequence(value)) then
            return false
        end
    ...
    if structure.kind == "table" then
        if type(value) ~= "table" or value.__agentType or is_sequence(value) then
            return false
        end
...
function AgentPhoneConfigurator.Broadcast(target)
    TriggerClientEvent("agent_phone:configurator:sync", target or -1, {
...
local function read_stored_row()
    ...
    local row = rows[1]
    if not row then
        error("[agent_phone] Phone configurator could not load its SQL row.")
    end
    return row
end
...
local function migrate_blank_company_definitions()
    local migration_name = "agent-phone:configurator:company-definition-defaults:v1"
    ...
    TriggerEvent("agent_phone:configurator:serverUpdated", revision)
    AgentPhoneConfigurator.Broadcast(-1)
    Bridge.Debug(
        "info",
        "[agent_phone] Migrated blank Phone Configurator company definitions: %s",
...
local function migrate_police_request_defaults()
    local migration_name = "agent-phone:configurator:police-requests:v1"
    local completed = Bridge.Database.Query(
        "SELECT 1 FROM `agent_phone_migrations` WHERE `name` = ? LIMIT 1",
        { migration_name }
    )
    ...
    Bridge.Debug(
        "info",
        "[agent_phone] Migrated Phone Configurator police request defaults.",
        { always = true }
    )
end

local function migrate_police_service_line_messaging()
    local migration_name = "agent-phone:configurator:service-line-messaging:v2"
    local completed = Bridge.Database.Query(
        "SELECT 1 FROM `agent_phone_migrations` WHERE `name` = ? LIMIT 1",
        { migration_name }
    )
    ...
    Bridge.Debug(
        "info",
        "[agent_phone] Enabled Phone Configurator police service-line messaging.",
        { always = true }
    )
end
...
Bridge.Database.Migrate("agent_phone_configurator", { AgentPhoneConfiguratorSchema })
...
apply_stored_row(read_stored_row())
apply_runtime_configuration()
Bridge.Database.AfterMigration("agent_phone", migrate_blank_company_definitions)
Bridge.Database.AfterMigration("agent_phone", migrate_police_request_defaults)
Bridge.Database.AfterMigration("agent_phone", migrate_police_service_line_messaging)

function AgentPhoneConfigurator.GetAdminData()
    local data = build_admin_data()
    return data
end

function AgentPhoneConfigurator.Save(expected_revision, changes, actor_identifier, actor_name)
    if not configurator_enabled then
        return { success = false, error = "configurator_disabled" }
    end
    if tonumber(expected_revision) ~= revision then
        return { success = false, error = "revision_conflict", data = AgentPhoneConfigurator.GetAdminData() }
    end
    ...
    local companies_valid, validation_error = AgentPhoneCompanies.ValidateConfiguration(candidate_config)
    if not companies_valid then
        Bridge.Debug(
            "warn",
            "[agent_phone] Rejected invalid Phone Configurator Companies configuration: %s",
            validation_error,
            { always = true }
        )
    ...
    then
        error("[agent_phone] Phone configurator SQL verification failed after saving config and media payloads.")
    end
    ...
    return { success = true, data = AgentPhoneConfigurator.GetAdminData() }
end

Bridge.Callbacks.Register("agent_phone:configurator:runtime", function()
    return {
        success = true,
        data = {
...
Bridge.Debug(
    "info",
    "[agent_phone] Phone configurator initialized enabled=%s revision=%s.",
