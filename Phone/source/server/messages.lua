...
local function resolve_message_recipient(value)
    local service_line = AgentPhoneCompanies.GetServiceLine(value)
    if service_line then
        return service_line.number, service_line
    end
    return AgentPhoneSimNumber.Normalize(value, Config.Sim.NumberLength, Config.Sim.NumberPrefix), nil
end
...
local function shared_contact(device, contact_id)
    ...
    local rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_contacts` contact
        LEFT JOIN `agent_phone_media` media ON media.`id` = contact.`avatar_media_id`
    ...
    local number = AgentPhoneSimNumber.Normalize(
    ...
    then
        error(("[agent_phone] Contact %s cannot be shared because its stored data is invalid."):format(contact_id))
    end
    local snapshot = {
...
local function format_message(row)
    ...
    if row.message_type == "voice" then
        ...
        if type(waveform) ~= "table" then
            error(("[agent_phone] Voice message %s has an invalid waveform payload."):format(tostring(row.id)))
        end
    ...
    elseif row.message_type == "contact" then
        ...
        then
            error(("[agent_phone] Message %s has an invalid contact payload."):format(tostring(row.id)))
        end
        row.contact = contact
        row.media_payload = nil
    ...
    elseif row.message_type == "share" then
        ...
        then
            error(("[agent_phone] Message %s has an invalid share payload."):format(tostring(row.id)))
        end
        row.share = share
        row.media_payload = nil
        row.media_duration_ms = nil
    ...
    elseif attachment_assets[row.message_type] then
        if not valid_stored_attachment(row.message_type, row.media_payload) then
            error(("[agent_phone] Message %s has an invalid attachment asset."):format(tostring(row.id)))
        end
...
local function validate_attachment(source, device, message_type, data)
    ...
    if not built_in_asset then
        ...
        local rows = Bridge.Database.Query(([[
            SELECT `url`, `media_type`, `mime_type` FROM `agent_phone_media`
            WHERE `id` = ? AND %s
            LIMIT 1
        ]]):format(condition), params)
        local media = rows[1]
        ...
        then
            Bridge.Debug("warn", ("[agent_phone] Rejected unowned SMS media from source %s."):format(tostring(source)))
            return nil
        end
        payload = media.url
...
local function notify_sim(sim_id, event_name, data)
    ...
    for _, device in ipairs(devices) do
        for _, player_source in ipairs(Bridge.Framework.GetPlayers()) do
            ...
            if AgentPhone.FindDeviceSlots(source, device.imei)[1] then
                local payload = {}
...
Bridge.Callbacks.Register("agent_phone:messages:conversations", function(source)
...
Bridge.Callbacks.Register("agent_phone:messages:thread", function(source, data)
    ...
    Bridge.Database.Query([[
        UPDATE `agent_phone_sms_messages` SET `read_at` = CURRENT_TIMESTAMP
        WHERE `recipient_sim_id` = ? AND `sender_number` = ? AND `read_at` IS NULL
    ]], { device.sim_id, number })
    return { success = true, data = rows }
end)

Bridge.Callbacks.Register("agent_phone:messages:delete", function(source, data)
    if not AgentPhone.AllowOperation(source, "message_delete", 10, 60) then
    ...
    Bridge.Database.Query(([[
        DELETE FROM `agent_phone_sms_messages`
...
Bridge.Callbacks.Register("agent_phone:messages:media", function(source, data)
    if not AgentPhone.AllowOperation(source, "message_media", Config.Messages.MediaLoadsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
...
Bridge.Callbacks.Register("agent_phone:messages:send", function(source, data)
    if not AgentPhone.AllowOperation(source, "message_send", Config.Messages.SendsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    if type(data) ~= "table" then
        return { success = false, error = "invalid_request" }
    ...
    elseif message_type == "share" then
        local share_error
        local encoded
        share, share_error, encoded = AgentPhoneEasyShare.SanitizeChatPayload(source, data.sharePayload)
        if not share then
    ...
    if service_line then
        ...
        local id = uuid()
        local routed = AgentPhoneCompanies.RouteServiceLineMessage(source, {
            id = id,
            body = body,
        ...
        local rows = Bridge.Database.Query([[
            ...
            FROM `agent_phone_sms_messages` WHERE `id` = ? LIMIT 1
        ]], { id })
        if not rows[1] then
            Bridge.Debug(
                "error",
                "[agent_phone] Routed service-line SMS %s could not be read back.",
                tostring(id),
                { always = true }
            )
        ...
        TriggerClientEvent("agent_phone:messages:changed", source, { phoneNumber = number })
        return { success = true, data = message }
    end
    local recipients = Bridge.Database.Query([[
        SELECT s.`id`, s.`phone_number`
        FROM `agent_phone_sims` s
        LEFT JOIN `agent_phone_devices` d ON d.`sim_id` = s.`id`
    ...
    local message = format_message(rows[1])
    TriggerClientEvent("agent_phone:messages:changed", source, { phoneNumber = number })
    notify_sim(recipient.id, "agent_phone:messages:new", {
        message = message,
