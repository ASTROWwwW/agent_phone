...
local function require_profile(source)
    ...
    local rows = Bridge.Database.Query(
        "SELECT * FROM `agent_phone_feather_profiles` WHERE `account_id` = ? LIMIT 1",
        { account.id }
    )
...
local function are_blocked(first_id, second_id)
    return Bridge.Database.Query([[SELECT `id` FROM `agent_phone_feather_blocks` WHERE
...
local function load_profile(profile_id, viewer_id)
    local rows = Bridge.Database.Query([[
        SELECT p.`id`, p.`handle`, p.`display_name`, p.`bio`, p.`verified`, m.`url` AS `avatar_url`,
            EXISTS(SELECT 1 FROM `agent_phone_feather_follows` f WHERE f.`follower_id` = ? AND f.`following_id` = p.`id`) AS `is_following`,
            (SELECT COUNT(*) FROM `agent_phone_feather_follows` f WHERE f.`following_id` = p.`id`) AS `followers`,
            (SELECT COUNT(*) FROM `agent_phone_feather_follows` f WHERE f.`follower_id` = p.`id`) AS `following`,
            (SELECT COUNT(*) FROM `agent_phone_feather_posts` fp WHERE fp.`profile_id` = p.`id` AND fp.`status` = 'published') AS `post_count`
        FROM `agent_phone_feather_profiles` p
        LEFT JOIN `agent_phone_media` m ON m.`id` = p.`avatar_media_id`
        WHERE p.`id` = ? LIMIT 1
...
local function hydrate_posts(rows, viewer_id)
    ...
    local media_rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_feather_post_media` pm
...
local function list_posts(viewer_id, condition, values, ranking, offset)
    ...
    local rows = Bridge.Database.Query(([[
        SELECT fp.`id`, fp.`profile_id`, fp.`body`, fp.`reply_to_id`, fp.`quote_id`,
            ...
            EXISTS(SELECT 1 FROM `agent_phone_feather_follows` f WHERE f.`follower_id` = ? AND f.`following_id` = fp.`profile_id`) AS `is_following`,
            EXISTS(SELECT 1 FROM `agent_phone_feather_reactions` r WHERE r.`profile_id` = ? AND r.`post_id` = fp.`id` AND r.`kind` = 'like') AS `is_liked`,
            EXISTS(SELECT 1 FROM `agent_phone_feather_reactions` r WHERE r.`profile_id` = ? AND r.`post_id` = fp.`id` AND r.`kind` = 'bookmark') AS `is_bookmarked`,
            (SELECT COUNT(*) FROM `agent_phone_feather_reactions` r WHERE r.`post_id` = fp.`id` AND r.`kind` = 'like') AS `like_count`,
            (SELECT COUNT(*) FROM `agent_phone_feather_posts` reply WHERE reply.`reply_to_id` = fp.`id` AND reply.`status` = 'published') AS `reply_count`
        FROM `agent_phone_feather_posts` fp
        JOIN `agent_phone_feather_profiles` p ON p.`id` = fp.`profile_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = p.`avatar_media_id`
        WHERE fp.`status` = 'published' AND %s
            AND NOT EXISTS(SELECT 1 FROM `agent_phone_feather_blocks` b WHERE
...
local function notify_profile(recipient_id, actor, kind, post_id)
    ...
    local recipients = Bridge.Database.Query(
        "SELECT `account_id` FROM `agent_phone_feather_profiles` WHERE `id` = ? LIMIT 1",
        { recipient_id }
    )
    if not recipients[1] then return end
    Bridge.Database.Query([[INSERT INTO `agent_phone_feather_notifications`
        (`id`, `recipient_id`, `actor_id`, `post_id`, `kind`) VALUES (?, ?, ?, ?, ?)]], {
        new_id(), recipient_id, actor.id, post_id, kind,
    })
    AgentPhone.NotifyAccountDevices(tonumber(recipients[1].account_id), "agent_phone:feather:new", {
...
local function suggestions(profile_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_feather_profiles` p
        LEFT JOIN `agent_phone_media` m ON m.`id` = p.`avatar_media_id`
        WHERE p.`id` <> ? AND NOT EXISTS(SELECT 1 FROM `agent_phone_feather_follows` f WHERE f.`follower_id` = ? AND f.`following_id` = p.`id`)
            AND NOT EXISTS(SELECT 1 FROM `agent_phone_feather_blocks` b WHERE
...
local function list_connections(profile_id, viewer_id, mode)
    ...
    local rows = Bridge.Database.Query(([[
        SELECT p.`id`, p.`handle`, p.`display_name`, p.`bio`, p.`verified`, avatar.`url` AS `avatar_url`,
            EXISTS(SELECT 1 FROM `agent_phone_feather_follows` viewer_follow WHERE viewer_follow.`follower_id` = ? AND viewer_follow.`following_id` = p.`id`) AS `is_following`,
            (SELECT COUNT(*) FROM `agent_phone_feather_follows` f WHERE f.`following_id` = p.`id`) AS `followers`,
            (SELECT COUNT(*) FROM `agent_phone_feather_follows` f WHERE f.`follower_id` = p.`id`) AS `following`,
            (SELECT COUNT(*) FROM `agent_phone_feather_posts` fp WHERE fp.`profile_id` = p.`id` AND fp.`status` = 'published') AS `post_count`
        FROM `agent_phone_feather_follows` connection
        JOIN `agent_phone_feather_profiles` p ON %s
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = p.`avatar_media_id`
        WHERE %s AND NOT EXISTS(SELECT 1 FROM `agent_phone_feather_blocks` b WHERE
            (b.`blocker_id` = ? AND b.`blocked_id` = p.`id`) OR
            (b.`blocked_id` = ? AND b.`blocker_id` = p.`id`))
...
local function trending_hashtags()
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_feather_hashtags` h
        JOIN `agent_phone_feather_posts` fp ON fp.`id` = h.`post_id` AND fp.`status` = 'published'
        GROUP BY h.`tag`
...
local function backfill_hashtags()
    local posts = Bridge.Database.Query([[
        SELECT fp.`id`, fp.`body` FROM `agent_phone_feather_posts` fp
    ...
    for _, post in ipairs(posts) do
        for _, tag in ipairs(extract_hashtags(post.body)) do
            statements[#statements + 1] = {
                query = "INSERT IGNORE INTO `agent_phone_feather_hashtags` (`post_id`, `tag`) VALUES (?, ?)",
                params = { post.id, tag },
            }
            if #statements == 500 then
                if not Bridge.Database.Transaction(statements) then
                    error("[agent_phone] Failed to backfill Feather hashtags.")
                end
                statements = {}
            end
        end
...
Bridge.Callbacks.Register("agent_phone:feather:bootstrap", function(source)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then return error_response end
    local rows = Bridge.Database.Query("SELECT `id` FROM `agent_phone_feather_profiles` WHERE `account_id` = ? LIMIT 1", { account.id })
    if not rows[1] then
...
Bridge.Callbacks.Register("agent_phone:feather:create-profile", function(source, data)
    if not AgentPhone.AllowOperation(source, "feather:create-profile", 5, 60) then
    ...
    if data.avatarId then
        local media, media_error = AgentPhoneMedia.ResolveOwnedMedia(source, data.avatarId, "photo")
        if not media then return { success = false, error = media_error } end
        avatar_id = math.floor(tonumber(data.avatarId))
    end
    local existing = Bridge.Database.Query("SELECT `id` FROM `agent_phone_feather_profiles` WHERE `account_id` = ? LIMIT 1", { account.id })
    if existing[1] then return { success = false, error = "already_registered" } end
    local duplicate = Bridge.Database.Query("SELECT `id` FROM `agent_phone_feather_profiles` WHERE `handle` = ? LIMIT 1", { handle })
    if duplicate[1] then return { success = false, error = "handle_taken" } end
    Bridge.Database.Query([[INSERT INTO `agent_phone_feather_profiles`
...
Bridge.Callbacks.Register("agent_phone:feather:update-profile", function(source, data)
    ...
    Bridge.Database.Query("UPDATE `agent_phone_feather_profiles` SET `display_name` = ?, `bio` = ?, `avatar_media_id` = ? WHERE `id` = ?", {
        display_name, bio, avatar_id, profile.id,
    })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:feather:feed", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    data = type(data) == "table" and data or {}
    ...
    local values = {}
    local ranking = "(SELECT COUNT(*) FROM `agent_phone_feather_reactions` rank_reactions WHERE rank_reactions.`post_id` = fp.`id`) DESC, fp.`created_at` DESC"
    if data.mode == "following" then
        condition = "fp.`reply_to_id` IS NULL AND (fp.`profile_id` = ? OR EXISTS(SELECT 1 FROM `agent_phone_feather_follows` follow_filter WHERE follow_filter.`follower_id` = ? AND follow_filter.`following_id` = fp.`profile_id`))"
...
Bridge.Callbacks.Register("agent_phone:feather:explore", function(source, data)
    if not AgentPhone.AllowOperation(source, "feather:explore", Config.Feather.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
...
Bridge.Callbacks.Register("agent_phone:feather:network", function(source, data)
    if not AgentPhone.AllowOperation(source, "feather:network", Config.Feather.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
...
Bridge.Callbacks.Register("agent_phone:feather:create-post", function(source, data)
    if not AgentPhone.AllowOperation(source, "feather:create-post", Config.Feather.PostsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    ...
    local target = nil
    if target_id then
        local targets = Bridge.Database.Query([[SELECT fp.`id`, fp.`profile_id` FROM `agent_phone_feather_posts` fp
    ...
    for _, value in ipairs(media_ids) do
        ...
        local media, media_error = AgentPhoneMedia.ResolveOwnedMedia(source, id, "photo")
        if not media then return { success = false, error = media_error } end
        seen[id] = true
    ...
    local statements = {{
        query = "INSERT INTO `agent_phone_feather_posts` (`id`, `profile_id`, `body`, `reply_to_id`, `quote_id`) VALUES (?, ?, ?, ?, ?)",
        params = { post_id, profile.id, body, reply_to_id, quote_id },
    }}
    for index, media_id in ipairs(normalized_media) do
        statements[#statements + 1] = {
            query = "INSERT INTO `agent_phone_feather_post_media` (`post_id`, `media_id`, `sort_order`) VALUES (?, ?, ?)",
            params = { post_id, media_id, index - 1 },
        }
    end
    for _, tag in ipairs(extract_hashtags(body)) do
        statements[#statements + 1] = {
            query = "INSERT INTO `agent_phone_feather_hashtags` (`post_id`, `tag`) VALUES (?, ?)",
            params = { post_id, tag },
        }
    end
...
Bridge.Callbacks.Register("agent_phone:feather:react", function(source, data)
    if not AgentPhone.AllowOperation(source, "feather:react", Config.Feather.ActionsPerMinute, 60) then return { success = false, error = "rate_limited" } end
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    data = type(data) == "table" and data or {}
    ...
    local posts = Bridge.Database.Query("SELECT `profile_id` FROM `agent_phone_feather_posts` WHERE `id` = ? AND `status` = 'published' LIMIT 1", { data.id })
    ...
    if data.active then
        Bridge.Database.Query("INSERT IGNORE INTO `agent_phone_feather_reactions` (`post_id`, `profile_id`, `kind`) VALUES (?, ?, ?)", { data.id, profile.id, kind })
        if kind ~= "bookmark" then notify_profile(tonumber(posts[1].profile_id), profile, kind, data.id) end
    else
        Bridge.Database.Query("DELETE FROM `agent_phone_feather_reactions` WHERE `post_id` = ? AND `profile_id` = ? AND `kind` = ?", { data.id, profile.id, kind })
    end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:feather:follow", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    data = type(data) == "table" and data or {}
    ...
    local targets = Bridge.Database.Query("SELECT `id` FROM `agent_phone_feather_profiles` WHERE `id` = ? LIMIT 1", { target_id })
    ...
    if data.active then
        Bridge.Database.Query("INSERT IGNORE INTO `agent_phone_feather_follows` (`follower_id`, `following_id`) VALUES (?, ?)", { profile.id, target_id })
        notify_profile(target_id, profile, "follow", nil)
    else
        Bridge.Database.Query("DELETE FROM `agent_phone_feather_follows` WHERE `follower_id` = ? AND `following_id` = ?", { profile.id, target_id })
    end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:feather:connections", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    data = type(data) == "table" and data or {}
...
Bridge.Callbacks.Register("agent_phone:feather:remove-connection", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    data = type(data) == "table" and data or {}
    local target_id = math.floor(tonumber(data.profileId) or 0)
    ...
    if mode == "followers" then
        Bridge.Database.Query("DELETE FROM `agent_phone_feather_follows` WHERE `follower_id` = ? AND `following_id` = ?", { target_id, profile.id })
    else
        Bridge.Database.Query("DELETE FROM `agent_phone_feather_follows` WHERE `follower_id` = ? AND `following_id` = ?", { profile.id, target_id })
    end
    return { success = true }
...
Bridge.Callbacks.Register("agent_phone:feather:profile", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    data = type(data) == "table" and data or {}
    ...
    if data.handle then
        local handle = normalize_handle(data.handle)
        local rows = handle and Bridge.Database.Query("SELECT `id` FROM `agent_phone_feather_profiles` WHERE `handle` = ? LIMIT 1", { handle }) or {}
...
Bridge.Callbacks.Register("agent_phone:feather:bookmarks", function(source)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    local bookmarked = list_posts(
        tonumber(profile.id),
        "EXISTS(SELECT 1 FROM `agent_phone_feather_reactions` saved WHERE saved.`profile_id` = ? AND saved.`post_id` = fp.`id` AND saved.`kind` = 'bookmark')",
        { profile.id },
        "fp.`created_at` DESC",
        0
    )
...
Bridge.Callbacks.Register("agent_phone:feather:thread", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
...
Bridge.Callbacks.Register("agent_phone:feather:activities", function(source)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_feather_notifications` n
        JOIN `agent_phone_feather_profiles` actor ON actor.`id` = n.`actor_id`
...
Bridge.Callbacks.Register("agent_phone:feather:mark-activities", function(source)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    Bridge.Database.Query("UPDATE `agent_phone_feather_notifications` SET `read_at` = COALESCE(`read_at`, NOW()) WHERE `recipient_id` = ?", { profile.id })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:feather:delete", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    if type(data) ~= "table" or type(data.id) ~= "string" then return { success = false, error = "invalid_request" } end
    Bridge.Database.Query("UPDATE `agent_phone_feather_posts` SET `status` = 'removed' WHERE `id` = ? AND `profile_id` = ?", { data.id, profile.id })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:feather:block", function(source, data)
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    ...
    if not Bridge.Database.Transaction({
        { query = "INSERT IGNORE INTO `agent_phone_feather_blocks` (`blocker_id`, `blocked_id`) VALUES (?, ?)", params = { profile.id, target_id } },
        { query = "DELETE FROM `agent_phone_feather_follows` WHERE (`follower_id` = ? AND `following_id` = ?) OR (`follower_id` = ? AND `following_id` = ?)", params = { profile.id, target_id, target_id, profile.id } },
    }) then return { success = false, error = "request_failed" } end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:feather:report", function(source, data)
    if not AgentPhone.AllowOperation(source, "feather:report", Config.Feather.ReportsPerDay, 86400) then return { success = false, error = "rate_limited" } end
    local profile, error_response = require_profile(source)
    if not profile then return error_response end
    data = type(data) == "table" and data or {}
    ...
    local posts = Bridge.Database.Query("SELECT `id` FROM `agent_phone_feather_posts` WHERE `id` = ? AND `status` = 'published' LIMIT 1", { data.id })
    if not posts[1] then return { success = false, error = "post_not_found" } end
    Bridge.Database.Query([[INSERT IGNORE INTO `agent_phone_feather_reports`
