...
local function load_device_security(imei)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_device_security`
        WHERE `device_imei` = ?
        LIMIT 1
    ]], { imei })
    return rows[1]
...
function AgentPhoneSecurity.Load(imei)
    return load_device_security(imei)
end

function AgentPhoneSecurity.Status(imei, security, security_loaded)
...
Bridge.Callbacks.Register("agent_phone:security:unlock", function(source, data)
    if not AgentPhone.AllowOperation(source, "security_unlock", Config.Security.AttemptsPerMinute, 60) then
    ...
    if session.unlocked then
        return { success = true, data = { security = AgentPhoneSecurity.Status(session.imei) } }
    end
...
Bridge.Callbacks.Register("agent_phone:security:set-passcode", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:security:change-passcode", function(source, data)
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:security:disable-passcode", function(source, data)
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then
        return error_response
    end
