...
local function validate_request(source, data, operation)
    ...
    if type(data) ~= "table" or not AgentPhoneApps.HasPermission(data.appId, "device.storage") then
...
local function read_row(imei, app_id, data_key)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_custom_app_data`
...
Bridge.Callbacks.Register("agent_phone:custom-app:storage:get", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:custom-app:storage:set", function(source, data)
    ...
    return with_storage_write_lock(lock_key, function()
        local current_session, session_error = AgentPhone.RequireSession(source)
        if not current_session then
            return session_error
        end
        if current_session.imei ~= session.imei
            or not AgentPhoneApps.HasPermission(request.appId, "device.storage")
        then
        ...
        if expected_revision == 0 then
            local inserted = Bridge.Database.Query([[
                INSERT IGNORE INTO `agent_phone_custom_app_data`
        ...
        local updated = Bridge.Database.Query([[
            UPDATE `agent_phone_custom_app_data`
