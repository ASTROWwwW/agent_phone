...
Bridge.Callbacks.Register("agent_phone:device:development-open", function(source)
...
Bridge.Debug("debug", "[agent_phone] Server initialization started after database migration.", { always = true })

AgentPhone = {}

function AgentPhone.IsAppEnabled(app_id)
    return type(Config.Apps) ~= "table" or Config.Apps[app_id] ~= false
end

function AgentPhone.GetDisabledApps()
    local disabled = {}
...
local function reserve_imei()
    local imei = AgentPhoneImei.Reserve(function()
        local rows = Bridge.Database.Query("SELECT UUID() AS `id`", {})
        local uuid = rows[1] and rows[1].id
        if type(uuid) ~= "string" then
            error("[agent_phone] Database did not generate entropy for an IMEI.")
        end
        return uuid
    end, function(candidate)
        local result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_devices` (`imei`, `device_name`)
            VALUES (?, ?)
...
local function remove_equipped_phone_number(source)
    ...
    if previous_number then
        TriggerEvent("agent_phone:server:phoneNumberChanged", source, nil)
    end
end
...
local function migrated_device_for_character(source)
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_migration_owners` mapping
        JOIN `agent_phone_character_devices` character_device
            ON character_device.`owner_identifier` = mapping.`owner_identifier`
            AND character_device.`device_imei` = mapping.`device_imei`
        JOIN `agent_phone_devices` device ON device.`imei` = mapping.`device_imei`
        WHERE mapping.`owner_identifier` = ?
        ORDER BY mapping.`created_at` DESC
        LIMIT 1
    ]], { identifier })
    local imei = rows[1] and rows[1].device_imei or nil
    if imei and not AgentPhoneImei.IsValid(imei) then
        Bridge.Debug(
            "error",
            "[agent_phone] Phone migration mapping contains invalid IMEI '%s' for '%s'.",
            tostring(imei),
            identifier
        )
        return nil
...
local function load_character_device(source, identifier)
    ...
    local rows = Bridge.Database.Query([[
        SELECT `device_imei`
        FROM `agent_phone_character_devices`
...
local function map_character_device(source, slot)
    ...
    if AgentPhoneImei.IsValid(legacy_imei) then
        ...
        local result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_character_devices` (`owner_identifier`, `device_imei`)
            VALUES (?, ?)
    ...
    Bridge.Database.Query([[
        ...
        LEFT JOIN `agent_phone_character_devices` c ON c.`device_imei` = d.`imei`
    ...
    Bridge.Debug(
        "error",
        "[agent_phone] Could not create a shared device mapping for source %s.",
        tostring(source)
    )
...
local function find_device_slots(source, imei)
    ...
    if #matches > 1 then
        Bridge.Debug(
            "warn",
            "[agent_phone] Source %s has %s items with duplicated IMEI %s.",
            tostring(source),
            tostring(#matches),
            imei
        )
...
local function resolve_used_slot(source, used_item)
    Bridge.Debug(
        "debug",
        "[agent_phone] Resolving used item for source %s: type=%s slot=%s id=%s name=%s.",
        tostring(source),
        type(used_item),
    ...
    if slot_id then
        ...
        Bridge.Debug(
            "debug",
            "[agent_phone] Inventory slot lookup for source %s slot %s returned name=%s amount=%s metadata_imei=%s.",
            tostring(source),
            tostring(slot_id),
        ...
        Bridge.Debug(
            "warn",
            "[agent_phone] Usable item callback reported invalid phone slot %s for source %s.",
            tostring(slot_id),
            tostring(source)
        )
    ...
    Bridge.Debug(
        "debug",
        "[agent_phone] Inventory fallback found %s phone slot candidates for source %s.",
        tostring(#slots),
        tostring(source),
        { always = true }
    )
    ...
    Bridge.Debug(
        "warn",
        "[agent_phone] Inventory '%s' returned no '%s' phone item for source %s. This is an item/configuration problem, not a missing SIM card.",
...
local function ensure_device(source, slot)
    ...
    Bridge.Debug(
        "debug",
        "[agent_phone] Ensuring device for source %s slot=%s amount=%s existing_imei=%s inventory=%s.",
        tostring(source),
        tostring(slot.slot),
        tostring(amount),
    ...
    if amount ~= 1 then
        Bridge.Debug("warn", "[agent_phone] Phone item in slot %s is stacked for source %s.", tostring(slot.slot), tostring(source))
    ...
    if imei and not AgentPhoneImei.IsValid(imei) then
        Bridge.Debug("warn", "[agent_phone] Phone item in slot %s has invalid IMEI metadata.", tostring(slot.slot))
    ...
    if not imei then
        ...
        if imei then
            ...
            if not metadata_written then
                Bridge.Debug(
                    "error",
                    "[agent_phone] Inventory '%s' could not adopt migrated phone metadata for source %s slot %s.",
        ...
        Bridge.Debug(
            "debug",
            "[agent_phone] Reserved IMEI %s; writing metadata to source %s slot %s.",
            imei,
            tostring(source),
            tostring(slot.slot),
            { always = true }
        ...
        Bridge.Debug(
            "debug",
            "[agent_phone] Metadata write result for source %s slot %s IMEI %s: %s.",
            tostring(source),
            tostring(slot.slot),
            imei,
        ...
        if not metadata_written then
            Bridge.Database.Query("DELETE FROM `agent_phone_devices` WHERE `imei` = ?", { imei })
            Bridge.Debug(
                "error",
                "[agent_phone] Inventory '%s' could not write phone metadata for source %s slot %s.",
                Bridge.Inventory.GetResourceName(),
                tostring(source),
                tostring(slot.slot)
            )
...
local function load_device(imei)
    local rows = Bridge.Database.Query([[
        ...
        LEFT JOIN `agent_phone_accounts` a ON a.`id` = d.`account_id`
        LEFT JOIN `agent_phone_sims` s ON s.`id` = d.`sim_id`
        WHERE d.`imei` = ?
        LIMIT 1
    ]], { imei })
    return rows[1]
...
local function bootstrap(source, security, security_loaded)
    ...
    return {
        token = session.token,
        disabledApps = AgentPhone.GetDisabledApps(),
        phoneNumberFormat = {
            length = Config.Sim.NumberLength,
            groups = Config.Sim.NumberGroups,
        },
        security = AgentPhoneSecurity.Status(device.imei, security, security_loaded),
        device = {
            imei = device.imei,
        ...
        account = device.account_id and {
            ...
            email = device.email,
            devices = AgentPhoneAccounts.List(device.account_id, device.imei),
        } or nil,
...
local function refresh_source(source)
    ...
    if payload then
        TriggerClientEvent("agent_phone:device:updated", source, payload)
    end
end

AgentPhone.EnsureDevice = ensure_device
AgentPhone.FindDeviceSlots = find_device_slots
AgentPhone.LoadDevice = load_device
AgentPhone.RefreshSource = refresh_source
...
local function resolve_equipped_phone_number(source)
    ...
    if Config.Phone.Unique ~= false then
        ...
        for _, slot in ipairs(slots) do
            ...
            if amount == 1 and AgentPhoneImei.IsValid(slot_imei) then
...
function AgentPhone.GetSourceFromNumber(phone_number)
    local normalized = AgentPhoneSimNumber.Normalize(
        phone_number,
...
function AgentPhone.FormatNumber(phone_number)
...
function AgentPhone.RequireDeviceSession(source)
    ...
    if not matches[1] then
        ...
        sessions[source] = nil
        remove_equipped_phone_number(source)
        TriggerClientEvent("agent_phone:device:invalidated", source)
...
function AgentPhone.RequireSession(source)
    local session, error_response = AgentPhone.RequireDeviceSession(source)
    if not session then
        return nil, error_response
    end
...
function AgentPhone.AllowOperation(source, operation, maximum, window_seconds)
    local now = os.time()
    ...
    if attempts.count >= maximum then
        Bridge.Debug(
            "warn",
            "[agent_phone] Rate limit '%s' exceeded by source %s.",
            operation,
            tostring(source)
        )
        return false
...
function AgentPhone.RequireAccount(source)
...
function AgentPhone.NotifyAccount(account_id, event_name, data)
...
function AgentPhone.NotifyAccountDevices(account_id, event_name, data)
    local rows = Bridge.Database.Query([[
...
function AgentPhone.RefreshAccount(account_id)
    for source in pairs(sessions) do
        local account = AgentPhone.RequireAccount(source)
        if account and account.id == tonumber(account_id) then
...
function AgentPhone.RefreshDevice(imei)
...
local function perform_phone_open(source, used_item)
    ...
    Bridge.Debug(
        "debug",
        "[agent_phone] Usable phone callback invoked for source %s.",
        tostring(source),
        { always = true }
    )
    ...
    if not slot then
        Bridge.Debug(
            "debug",
            "[agent_phone] Phone open rejected for source %s: %s.",
            tostring(source),
            tostring(slot_error),
            { always = true }
        )
        TriggerClientEvent("agent_phone:device:error", source, slot_error)
    ...
    if not imei then
        Bridge.Debug(
            "debug",
            "[agent_phone] Phone open rejected for source %s slot %s: %s.",
            tostring(source),
            tostring(slot.slot),
            tostring(error_code),
            { always = true }
        )
        TriggerClientEvent("agent_phone:device:error", source, error_code)
        return false, error_code
    end
    local prepared, prepare_error = AgentPhoneSim.PrepareDevice(source, slot, imei)
    if not prepared then
        Bridge.Debug(
            "error",
            "[agent_phone] Phone preparation failed for source %s IMEI %s: %s.",
            tostring(source),
            imei,
    ...
    if not payload then
        ...
        Bridge.Debug(
            "error",
            "[agent_phone] Phone bootstrap failed for source %s slot %s IMEI %s: %s.",
            tostring(source),
            tostring(slot.slot),
            imei,
            tostring(error_code)
    ...
    Bridge.Debug(
        "debug",
        "[agent_phone] Triggering client open for source %s slot %s IMEI %s account_linked=%s after %sms.",
        tostring(source),
        tostring(slot.slot),
        imei,
    ...
    TriggerClientEvent("agent_phone:device:open", source, payload)
    return true
end

local function open_phone(source, used_item)
    if phone_open_in_progress[source] then
        TriggerClientEvent("agent_phone:device:error", source, "operation_in_progress")
    ...
    if not AgentPhone.AllowOperation(source, "phone_open", request_limit, 60) then
        TriggerClientEvent("agent_phone:device:error", source, "rate_limited")
    ...
    if not completed then
        Bridge.Debug(
            "error",
            "[agent_phone] Phone open failed unexpectedly for source %s: %s",
            tostring(source),
            tostring(success),
            { always = true }
        )
        TriggerClientEvent("agent_phone:device:error", source, "request_failed")
...
local function register_configured_phone_item()
    ...
    Bridge.Debug(
        "debug",
        "[agent_phone] Registering usable item '%s' through inventory '%s'.",
        item_name,
        tostring(Bridge.Inventory.GetResourceName()),
        { always = true }
    )
    if not Bridge.Inventory.RegisterUsableItem(item_name, function(...)
        ...
        error(("[agent_phone] Inventory '%s' did not register phone item '%s' as usable."):format(
            tostring(Bridge.Inventory.GetResourceName()),
            tostring(item_name)
        ))
    end
...
Bridge.Callbacks.Register("agent_phone:device:close", function(source)
    sessions[source] = nil
    return { success = true }
end)
...
Bridge.Callbacks.Register("agent_phone:device:equipped-number", function(source)
    return {
        success = true,
        data = { phoneNumber = AgentPhone.GetEquippedPhoneNumber(source) },
    }
end)

Bridge.Callbacks.Register("agent_phone:device:notification-open", function(source, data)
    if not AgentPhone.AllowOperation(source, "notification_open", 10, 60)
        or type(data) ~= "table" or type(data.imei) ~= "string"
    then
        return { success = false, error = "invalid_request" }
    end
    if not AgentPhone.OpenDeviceForCall(source, data.imei) then
