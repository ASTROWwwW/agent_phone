...
local function load_profile(source)
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_radio_profiles` WHERE `identifier` = ? LIMIT 1
    ]], { identifier })
    local row = rows[1]
    if row then
...
local function broadcast_frequency(frequency)
    ...
    for _, member in ipairs(members) do
        TriggerClientEvent("agent_phone:radio:members", member.id, {
            frequency = frequency,
            members = members,
        })
    end
...
local function notify_frequency(frequency, excluded_source, player_name, joined)
    ...
    for player_source, channel in pairs(channels) do
        if player_source ~= excluded_source
            ...
            TriggerClientEvent("agent_phone:radio:notification", player_source, {
                joined = joined,
...
Bridge.Callbacks.Register("agent_phone:radio:get", function(source)
...
Bridge.Callbacks.Register("agent_phone:radio:connect", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:radio:disconnect", function(source)
...
Bridge.Callbacks.Register("agent_phone:radio:set-speaker", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:radio:save-settings", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:radio:save-badge", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:radio:save-display-name", function(source, data)
