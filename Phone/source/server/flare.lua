...
local function decode_interests(value)
    ...
    if type(decoded) ~= "table" then
        error("[agent_phone] Invalid Flare interests JSON.")
    end
    local result = {}
...
local function load_profile(account_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_flare_profiles`
...
local function list_suggestions(account_id, profile)
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_flare_profiles` mine
        JOIN `agent_phone_flare_profiles` target ON target.`account_id` <> mine.`account_id`
        LEFT JOIN `agent_phone_flare_swipes` swipe
            ON swipe.`swiper_account_id` = mine.`account_id`
            AND swipe.`target_account_id` = target.`account_id`
        LEFT JOIN `agent_phone_flare_matches` matched
        ...
        WHERE mine.`account_id` = ?
            ...
            AND EXISTS (
                SELECT 1
                FROM `agent_phone_flare_profile_photos` target_photo
                JOIN `agent_phone_media` target_media
...
local function list_likes(account_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_flare_swipes` incoming
        JOIN `agent_phone_flare_profiles` profile
            ON profile.`account_id` = incoming.`swiper_account_id`
        LEFT JOIN `agent_phone_flare_swipes` response
...
local function list_matches(account_id)
    local rows = Bridge.Database.Query([[
        SELECT match_row.`id`, match_row.`created_at`,
            ...
            (SELECT COUNT(*) FROM `agent_phone_flare_messages` unread
                WHERE unread.`match_id` = match_row.`id`
                    AND unread.`sender_account_id` <> ?
                    AND unread.`read_at` IS NULL) AS `unread`
        FROM `agent_phone_flare_matches` match_row
        JOIN `agent_phone_flare_profiles` profile ON profile.`account_id` =
            IF(match_row.`account_a_id` = ?, match_row.`account_b_id`, match_row.`account_a_id`)
        LEFT JOIN `agent_phone_flare_messages` latest ON latest.`id` = (
            SELECT message.`id` FROM `agent_phone_flare_messages` message
...
local function validate_profile(source, data)
    ...
    for _, value in ipairs(data.photoMediaIds) do
        ...
        if not media_id or media_id < 1 or media_id ~= math.floor(media_id)
            ...
            or not AgentPhoneMedia.ResolveOwnedMedia(source, media_id, "photo")
        then
            return nil, "invalid_profile_photos"
        end
...
local function load_match(account_id, match_id)
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_flare_matches`
...
local function validate_message(source, data)
    ...
    if message_type == "share" then
        local share, share_error, encoded = AgentPhoneEasyShare.SanitizeChatPayload(source, data.sharePayload)
        if not share then
    ...
    if message_type == "gif" then
        ...
        media_url = AgentPhoneMedia.ResolveOwnedMedia(source, media_id, expected_type)
        if type(media_url) ~= "string" or #media_url > Config.Media.UrlMaxLength
            or not media_url:match("^https://")
        then
            Bridge.Debug("warn", ("[agent_phone] Rejected unowned Flare media from source %s."):format(tostring(source)))
            return nil, "invalid_attachment"
        end
    end
...
local function message_payload(row, account_id)
    ...
    if row.message_type == "share" and type(share_payload) ~= "table" then
        error(("[agent_phone] Flare message %s has an invalid share payload."):format(tostring(row.id)))
    end
    return {
        id = row.id,
...
Bridge.Callbacks.Register("agent_phone:flare:bootstrap", function(source)
...
Bridge.Callbacks.Register("agent_phone:flare:delete-profile", function(source)
    if not AgentPhone.AllowOperation(source, "flare_profile_delete", 3, 60) then
    ...
    if not Bridge.Database.Transaction({
        {
            query = [[
                DELETE FROM `agent_phone_flare_matches`
        ...
        {
            query = [[
                DELETE FROM `agent_phone_flare_swipes`
                WHERE `swiper_account_id` = ? OR `target_account_id` = ?
            ]],
            params = { account.id, account.id },
        },
        {
            query = "DELETE FROM `agent_phone_flare_profiles` WHERE `account_id` = ?",
            params = { account.id },
        },
    }) then
...
Bridge.Callbacks.Register("agent_phone:flare:save-profile", function(source, data)
    if not AgentPhone.AllowOperation(source, "flare_profile", 12, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
    ...
    if profile.replace_photos then
        statements[#statements + 1] = {
            query = [[
                DELETE photo FROM `agent_phone_flare_profile_photos` photo
                JOIN `agent_phone_flare_profiles` profile ON profile.`id` = photo.`profile_id`
        ...
        for index, media_id in ipairs(profile.photo_media_ids) do
            statements[#statements + 1] = {
                query = [[
                    INSERT INTO `agent_phone_flare_profile_photos`
                        (`profile_id`, `media_id`, `sort_order`)
                    SELECT `id`, ?, ? FROM `agent_phone_flare_profiles` WHERE `account_id` = ?
                ]],
...
Bridge.Callbacks.Register("agent_phone:flare:set-discovery", function(source, data)
    if not AgentPhone.AllowOperation(source, "flare_discovery", 20, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
    ...
    Bridge.Database.Query([[
        UPDATE `agent_phone_flare_profiles` SET `discoverable` = ? WHERE `account_id` = ?
    ]], { data.enabled and 1 or 0, account.id })
    return { success = true, data = bootstrap(account.id) }
end)

Bridge.Callbacks.Register("agent_phone:flare:swipe", function(source, data)
    if not AgentPhone.AllowOperation(source, "flare_swipe", 120, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
    ...
    local targets = Bridge.Database.Query([[
        ...
        FROM `agent_phone_flare_profiles` mine
        JOIN `agent_phone_flare_profiles` target ON target.`id` = ?
        WHERE mine.`account_id` = ?
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_flare_swipes` (`swiper_account_id`, `target_account_id`, `choice`)
        VALUES (?, ?, ?)
    ...
    local reciprocal = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_flare_swipes`
    ...
    local insert_result = Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_flare_matches` (`id`, `account_a_id`, `account_b_id`)
        VALUES (?, ?, ?)
    ...
    if created_match and match_created then
        AgentPhone.NotifyAccountDevices(target_account_id, "agent_phone:flare:match", {
...
Bridge.Callbacks.Register("agent_phone:flare:rewind", function(source)
    if not AgentPhone.AllowOperation(source, "flare_rewind", 20, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
    ...
    Bridge.Database.Query([[
        DELETE FROM `agent_phone_flare_swipes` WHERE `id` = ? AND `swiper_account_id` = ?
    ]], { swipe.id, account.id })
    return { success = true, data = bootstrap(account.id) }
end)

Bridge.Callbacks.Register("agent_phone:flare:unmatch", function(source, data)
    if not AgentPhone.AllowOperation(source, "flare_unmatch", 20, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
...
Bridge.Callbacks.Register("agent_phone:flare:thread", function(source, data)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
        return error_response
    end
    ...
    Bridge.Database.Query([[
        UPDATE `agent_phone_flare_messages` SET `read_at` = CURRENT_TIMESTAMP
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_flare_messages`
        WHERE `match_id` = ?
...
Bridge.Callbacks.Register("agent_phone:flare:send", function(source, data)
    if not AgentPhone.AllowOperation(source, "flare_message", 60, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
    ...
    local own_profile = load_profile(account.id)
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_flare_messages`
    ...
    AgentPhone.NotifyAccountDevices(recipient_account_id, "agent_phone:flare:message", {
        matchId = match.id,
        body = message.body,
