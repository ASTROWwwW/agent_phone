...
local function profile_for(source, identifier)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_health_profiles`
...
Bridge.Callbacks.Register("agent_phone:health:overview", function(source)
    if not AgentPhone.AllowOperation(source, "health_overview", 60, 60) then
...
Bridge.Callbacks.Register("agent_phone:health:save-profile", function(source, data)
    if not AgentPhone.AllowOperation(source, "health_profile_save", 12, 60) then
        return { success = false, error = "rate_limited" }
    end
    local identifier, error_response = character_identifier(source, true)
    if not identifier then
    ...
    if emergency_phone ~= "" then
        emergency_phone = AgentPhoneSimNumber.Normalize(
            emergency_phone,
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_health_profiles`
...
RegisterNetEvent("agent_phone:health:record-activity", function(data)
    ...
    then
        Bridge.Debug("warn", "[agent_phone] Rejected malformed health activity from source %s.", tostring(player_source))
        return
    end
    if not AgentPhone.AllowOperation(player_source, "health_activity", Config.Health.ReportsPerMinute, 60) then
        return
    end
    ...
    then
        Bridge.Debug("warn", "[agent_phone] Rejected implausible health activity from source %s.", tostring(player_source))
        return
    end
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_health_daily`
    ...
    TriggerClientEvent("agent_phone:health:changed", player_source)
end)
