...
local function validate_number_configuration()
    local valid, validation_error = AgentPhoneSimNumber.ValidateConfiguration(
        Config.Sim.NumberLength,
        Config.Sim.NumberPrefix
    )
    if not valid then
        error(("[agent_phone] Invalid SIM number configuration: %s."):format(validation_error))
    end
end
...
local function reserve_sim(sim_type, is_virtual)
    local sim_id
    local number = AgentPhoneSimNumber.Reserve(uuid, function(candidate)
        if AgentPhoneCompanies.IsServiceNumber(candidate) then
            return false
        end
        sim_id = uuid()
        local result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_sims` (`id`, `phone_number`, `sim_type`, `is_virtual`)
            VALUES (?, ?, ?, ?)
...
local function load_sim(sim_id)
    local rows = Bridge.Database.Query("SELECT * FROM `agent_phone_sims` WHERE `id` = ? LIMIT 1", { sim_id })
    return rows[1]
end
...
local function sim_metadata(sim)
    local metadata = {
        ...
        formatted_number = AgentPhoneSimNumber.Format(sim.phone_number, Config.Sim.NumberGroups, Config.Sim.NumberLength, Config.Sim.NumberPrefix),
...
local function set_phone_sim_metadata(source, phone_slot, sim)
    ...
    metadata.formatted_number = sim and AgentPhoneSimNumber.Format(
        sim.phone_number,
...
local function prepare_device(source, phone_slot, imei)
    ...
    if affected_rows(result) ~= 1 then
        Bridge.Database.Query("DELETE FROM `agent_phone_sims` WHERE `id` = ?", { automatic_sim.id })
    ...
    if not set_phone_sim_metadata(source, phone_slot, automatic_sim) then
        Bridge.Database.Query(
            "UPDATE `agent_phone_devices` SET `sim_id` = NULL WHERE `imei` = ? AND `sim_id` = ?",
            { imei, automatic_sim.id }
        )
        Bridge.Database.Query("DELETE FROM `agent_phone_sims` WHERE `id` = ?", { automatic_sim.id })
        return false, "metadata_unsupported"
    end
    TriggerEvent("agent_phone:server:phoneNumberGenerated", source, automatic_sim.phone_number)
    return true
end

AgentPhoneSim.PrepareDevice = prepare_device

function AgentPhoneSim.ChangeNumber(source, imei, sim_id, value)
    local number = AgentPhoneSimNumber.Normalize(value, Config.Sim.NumberLength, Config.Sim.NumberPrefix)
    ...
    local existing = Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_sims` WHERE `phone_number` = ? AND `id` <> ? LIMIT 1",
        { number, sim_id }
    )
    if existing[1] then
    ...
    local result = Bridge.Database.Query([[
        UPDATE IGNORE `agent_phone_sims`
        SET `phone_number` = ?
...
local function ensure_sim(source, slot, sim_type)
    ...
    if sim and AgentPhoneCompanies.IsServiceNumber(sim.phone_number) then
        Bridge.Debug(
            "error",
            "[agent_phone] SIM %s uses reserved company service number %s.",
            tostring(sim.id),
    ...
    if not sim then
        ...
        if not Bridge.Inventory.SetSlotMetadata(source, slot.slot, sim_metadata(sim)) then
            Bridge.Database.Query("DELETE FROM `agent_phone_sims` WHERE `id` = ?", { sim.id })
...
local function list_phone_choices(source)
    ...
    local choice_error
    for _, slot in ipairs(Bridge.Inventory.GetSlotsWithItem(source, Config.Phone.Item)) do
        local imei, error_code = AgentPhone.EnsureDevice(source, slot)
...
local function insert_sim(source, phone_imei, confirmed)
    ...
    if AgentPhoneCompanies.IsServiceNumber(pending.sim.phone_number) then
        pending_insertions[source] = nil
        Bridge.Debug(
            "error",
            "[agent_phone] Refused to insert SIM %s with reserved company service number %s.",
    ...
    local phone_matches = AgentPhone.FindDeviceSlots(source, phone_imei)
    ...
    local device = AgentPhone.LoadDevice(phone_imei)
    ...
    local transaction = {
        {
            query = "UPDATE `agent_phone_devices` SET `sim_id` = ? WHERE `imei` = ?",
    ...
    if old_sim then
        ...
        AgentPhoneCalls.EndForSim(old_sim.id, "sim_removed")
    end
    AgentPhone.RefreshDevice(phone_imei)
    TriggerClientEvent("agent_phone:sim:picker-close", source)
    return { success = true }
end
...
local function use_sim(source, used_item)
    ...
    if not sim_type then
        Bridge.Debug("warn", "[agent_phone] Usable SIM callback received an invalid item for source %s.", tostring(source))
        return false
    end
    if operation_locks[source] then
    ...
    if not slot then
        operation_locks[source] = nil
        TriggerClientEvent("agent_phone:device:error", source, "sim_slot_missing")
        return false
    end
    ...
    if #choices == 0 then
        operation_locks[source] = nil
        TriggerClientEvent("agent_phone:device:error", source, choice_error or "phone_required")
        return false
    end
    ...
    TriggerClientEvent("agent_phone:sim:picker", source, {
        choices = choices,
...
Bridge.Callbacks.Register("agent_phone:sim:insert", function(source, data)
    if Config.Sim.Enabled == false then
        return { success = false, error = "disabled" }
    end
...
Bridge.Callbacks.Register("agent_phone:sim:picker-close", function(source)
    if operation_locks[source] then
        return { success = false, error = "operation_in_progress" }
    end
    pending_insertions[source] = nil
...
Bridge.Callbacks.Register("agent_phone:sim:eject", function(source)
    if Config.Sim.Enabled == false then
        return { success = false, error = "disabled" }
    end
    if operation_locks[source] then
    ...
    if affected_rows(Bridge.Database.Query("UPDATE `agent_phone_devices` SET `sim_id` = NULL WHERE `imei` = ? AND `sim_id` = ?", {
        session.imei,
        sim.id,
    })) ~= 1 then
    ...
    operation_locks[source] = nil
    AgentPhoneCompanies.ClearCallAvailability(source)
    AgentPhoneCalls.EndForSim(sim.id, "sim_removed")
    AgentPhone.RefreshDevice(session.imei)
    return { success = true }
end)
...
if Config.Sim.Enabled ~= false then
    Bridge.Database.Query([[
        UPDATE `agent_phone_devices` d
        INNER JOIN `agent_phone_sims` s ON s.`id` = d.`sim_id`
        SET d.`sim_id` = NULL
