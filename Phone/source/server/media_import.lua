...
function AgentPhoneMediaImport.ResponseHeader(headers, name)
    ...
    for index, definition in ipairs(config.Websites or {}) do
        ...
        if website then
            if websites[website.Id] then
                error(("[agent_phone] Duplicate media import website id '%s'."):format(website.Id))
            end
            ...
            if website_error == "missing_api_key" then
                Bridge.Debug(
                    "warn",
                    "[agent_phone] Media import source '%s' at index %s is disabled because Config.Media.FiveManage.ApiKey is empty. Add a FiveManage V3 token with Media access in the Phone Configurator and save it.",
                ...
                Bridge.Debug(
                    "warn",
                    "[agent_phone] Media import source '%s' at index %s is disabled because its configuration is invalid: %s.",
                    tostring(source_name or "unknown"),
                    tostring(index),
...
local function store_import(owner, media)
    ...
    if owner.account_id then
        result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_media`
...
function AgentPhoneMediaImport.RegisterAdapter(name, adapter)
...
function AgentPhoneMediaImport.HttpRequest(url, headers, timeout_ms, method)
    ...
    PerformHttpRequest(url, function(status, response_body, response_headers, error_data)
        ...
        if response_status == 0 then
            Bridge.Debug(
                "warn",
                "[agent_phone] Media import HTTP %s request to '%s' failed: %s.",
                request_method,
                request_host,
        ...
        elseif response_status >= 400 then
            Bridge.Debug(
                "debug",
                "[agent_phone] Media import HTTP %s request to '%s' returned status %s.",
                request_method,
                request_host,
    ...
    SetTimeout(timeout_ms, function()
        ...
        Bridge.Debug(
            "warn",
            "[agent_phone] Media import HTTP %s request to '%s' timed out after %s ms.",
            request_method,
            request_host,
            tostring(timeout_ms),
            { always = true }
...
function AgentPhoneMediaImport.ResolveUrl(source_id, url)
    ...
    local response = AgentPhoneMediaImport.HttpRequest(
        trimmed_url,
        {},
    ...
    local content_type = AgentPhoneMediaImport.ResponseHeader(response.headers, "content-type")
    ...
    local content_length = tonumber(AgentPhoneMediaImport.ResponseHeader(response.headers, "content-length"))
    ...
    local external_id = ("url:%08x%08x"):format(
        ...
        joaat("agent_phone:" .. trimmed_url) & 0xffffffff
    )
...
function AgentPhoneMediaImport.UrlEncode(value)
    return tostring(value):gsub("\n", "\r\n"):gsub("([^%w%-_%.~])", function(character)
        return ("%%%02X"):format(character:byte())
    end)
end

function AgentPhoneMediaImport.Resolve(source_id, external_id)
...
function AgentPhoneMediaImport.Initialize()
    assert(not initialized, "Media import was initialized more than once")
    build_registry()
    initialized = true

    Bridge.Callbacks.Register("agent_phone:media:import:sources", function(source)
    ...
    Bridge.Callbacks.Register("agent_phone:media:import:list", function(source, data)
        local owner, error_response = session_owner(source)
        if not owner then
            return error_response
        end
        ...
        if type(result) ~= "table" or type(result.items) ~= "table" then
            Bridge.Debug(
                "warn",
                "[agent_phone] Import source '%s' returned an invalid list response.",
                website.Id
            )
        ...
        for _, item in ipairs(result.items) do
            ...
            if normalized then
                ...
                Bridge.Debug(
                    "warn",
                    "[agent_phone] Rejected media '%s' from import source '%s': %s.",
    ...
    Bridge.Callbacks.Register("agent_phone:media:import:commit", function(source, data)
        local owner, error_response = session_owner(source)
        if not owner then
            return error_response
        end
    ...
    Bridge.Callbacks.Register("agent_phone:media:import:url", function(source, data)
        local owner, error_response = session_owner(source)
        if not owner then
            return error_response
        end
        ...
        local normalized, resolve_error = AgentPhoneMediaImport.ResolveUrl(website.Id, data.url)
        if not normalized then
            Bridge.Debug(
                "warn",
                "[agent_phone] Media URL import failed for player %s, source '%s', host '%s': %s.",
                tostring(source),
                website.Id,
        ...
        Bridge.Debug(
            "debug",
            "[agent_phone] Media URL import resolved for player %s via source '%s' as %s '%s' (%s bytes).",
            tostring(source),
            website.Id,
