...
local function refresh_runtime_configuration()
    if Config.AgentRide.DistanceUnit ~= "kilometer" and Config.AgentRide.DistanceUnit ~= "mile" then
        error(("[agent_phone] Invalid AgentRide distance unit '%s'."):format(tostring(Config.AgentRide.DistanceUnit)))
    end

    local next_services = {}
    for index = 1, #Config.AgentRide.Services do
        local service = Config.AgentRide.Services[index]
        if service.Id ~= "taxi" and service.Id ~= "comfort" and service.Id ~= "xl" and service.Id ~= "premium" then
            error(("[agent_phone] Invalid AgentRide service class '%s'."):format(tostring(service.Id)))
        end
        if next_services[service.Id] then
            error(("[agent_phone] Duplicate AgentRide service class '%s'."):format(service.Id))
        end
...
local ride_select = [[
    ...
    FROM `agent_phone_agentride_rides` r
    INNER JOIN `agent_phone_agentride_profiles` passenger
        ON passenger.`id` = r.`passenger_profile_id`
    LEFT JOIN `agent_phone_media` passenger_avatar
        ON passenger_avatar.`id` = passenger.`avatar_media_id`
    LEFT JOIN `agent_phone_agentride_profiles` driver
        ON driver.`id` = r.`driver_profile_id`
    LEFT JOIN `agent_phone_media` driver_avatar
...
local function quick_locations()
    local result = {}
    for index = 1, #Config.AgentRide.QuickLocations do
        local location = Config.AgentRide.QuickLocations[index]
        result[index] = {
            coords = {
                x = location.Coords.x,
                y = location.Coords.y,
...
local function require_profile(source)
    ...
    if type(identifier) ~= "string" or identifier == "" or #identifier > 80 then
        return nil, { success = false, error = "agentride_unavailable" }
    end
    Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_agentride_profiles` (`id`, `owner_identifier`)
        SELECT UUID(), ?
    ]], { identifier })
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_agentride_profiles` profile
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = profile.`avatar_media_id`
        WHERE profile.`owner_identifier` = ?
        LIMIT 1
    ]], { identifier })
    if not rows[1] then
        error(("[agent_phone] Failed to load AgentRide profile for source %s."):format(tostring(source)))
    end
    return rows[1]
end
...
local function driver_eligible(source)
    ...
    local minimum_grade = Config.AgentRide.DriverJobs[job.name]
...
local function profile_snapshot(source, profile)
    local rows = Bridge.Database.Query([[
        SELECT
            (SELECT COUNT(*)
                FROM `agent_phone_agentride_rides`
    ...
    return {
        ...
        currency = Config.AgentRide.Currency,
        defaultPaymentMethod = Config.AgentRide.PaymentAccount,
    ...
    local metrics_rows = Bridge.Database.Query(([[
        SELECT profile.`id`,
            (SELECT AVG(ride.`passenger_rating`)
                FROM `agent_phone_agentride_rides` ride
...
local function phone_number_for_source(source)
    local session = source and AgentPhone.RequireSession(source) or nil
    if not session then
        return nil
    end
...
local function available_requests(profile_id)
    ...
    ]], { profile_id }, Config.AgentRide.HistoryLimit)
...
local function ride_history(profile_id)
    ...
    ]], { profile_id, profile_id }, Config.AgentRide.HistoryLimit)
    return ride_dtos(rows)
end
...
local function process_pending_refund(row)
    ...
    if row.refund_status == "completed" then
        ...
        local delete_success, delete_result = pcall(
            Bridge.Database.Query,
            [[
                DELETE FROM `agent_phone_agentride_rides`
        ...
        if not delete_success then
            Bridge.Debug(
                "error",
                "[agent_phone] Failed to remove refunded payment-pending AgentRide '%s': %s",
                row.id,
    ...
    local claim_success, claim_result = pcall(
        Bridge.Database.Query,
        [[
            UPDATE `agent_phone_agentride_rides`
    ...
    if not claim_success then
        Bridge.Debug(
            "error",
            "[agent_phone] Failed to claim pending refund for AgentRide '%s': %s",
            row.id,
    ...
    if price > 0 then
        money_success, money_result = pcall(
            ...
            passenger_source,
            Config.AgentRide.PaymentAccount,
            price
        )
    end
    if not money_success then
        Bridge.Debug(
            "error",
            "[agent_phone] Framework refund crashed for AgentRide '%s'; processing state prevents an automatic duplicate refund and requires manual reconciliation: %s",
            row.id,
    ...
    if not money_result then
        ...
        if not reset_success or affected_rows(reset_result) ~= 1 then
            Bridge.Debug(
                "error",
                "[agent_phone] Refund failed and its claim could not be reset for AgentRide '%s'; manual reconciliation is required.",
                row.id
            )
        end
        Bridge.Debug(
            "error",
            "[agent_phone] Framework refund failed for AgentRide '%s': %s",
            row.id,
            tostring(money_result)
        )
        return false
    ...
    if not finalize_success or affected_rows(finalize_result) ~= 1 then
        Bridge.Debug(
            "error",
            "[agent_phone] Refund was paid but could not be finalized for AgentRide '%s'; processing state prevents an automatic duplicate refund.",
            row.id
        )
        return false
    end
    if passenger_source then
        TriggerClientEvent("agent_phone:banking:changed", passenger_source)
    end
    if row.status == "payment_pending" then
        local delete_success, delete_result = pcall(
            Bridge.Database.Query,
        ...
        if not delete_success or affected_rows(delete_result) ~= 1 then
            Bridge.Debug(
                "error",
                "[agent_phone] Refunded payment-pending AgentRide '%s' could not be removed; recovery will retry cleanup.",
                row.id
            )
            return false
        end
...
local function transition_tip_status(ride_id, expected_status, next_status)
    ...
    if not update_success or affected_rows(update_result) ~= 1 then
        Bridge.Debug(
            "error",
            "[agent_phone] Failed to move AgentRide '%s' tip state from '%s' to '%s'.",
            ride_id,
            expected_status,
            next_status
        )
...
local function push_update(source, fields)
    ...
    TriggerClientEvent("agent_phone:agentride:changed", source, state_snapshot(source, profile, fields))
end
...
local function transition_driver_ride(source, data, expected_statuses, next_status, timestamp_column, radius, location_kind)
    if not AgentPhone.AllowOperation(source, "agentride_action", Config.AgentRide.ActionsPerMinute, 60) then
...
Bridge.Callbacks.Register("agent_phone:agentride:bootstrap", function(source)
    if not AgentPhone.AllowOperation(source, "agentride_read", Config.AgentRide.ReadsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    if not profile then
...
Bridge.Callbacks.Register("agent_phone:agentride:update-profile", function(source, data)
    if not AgentPhone.AllowOperation(source, "agentride_action", Config.AgentRide.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    if not name_length
        or name_length < Config.AgentRide.ProfileNameMinLength
        or name_length > Config.AgentRide.ProfileNameMaxLength
    then
    ...
    local result, lock_error = run_locked("profile:" .. profile.id, function()
        local update = Bridge.Database.Query([[
            UPDATE `agent_phone_agentride_profiles`
...
Bridge.Callbacks.Register("agent_phone:agentride:history", function(source)
    if not AgentPhone.AllowOperation(source, "agentride_read", Config.AgentRide.ReadsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
...
Bridge.Callbacks.Register("agent_phone:agentride:quote", function(source, data)
    if not AgentPhone.AllowOperation(source, "agentride_quote", Config.AgentRide.QuotesPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    if not profile then
    ...
    if not is_near(source, pickup, Config.AgentRide.PickupSelectionRadius) then
    ...
    if custom_fare ~= nil then
        ...
        if not Config.AgentRide.CustomFare.Enabled
    ...
    local distance_meters = math.floor((direct_distance * Config.AgentRide.RouteDistanceMultiplier) + 0.5)
    if distance_meters < Config.AgentRide.MinimumDistanceMeters
        or distance_meters > Config.AgentRide.MaximumDistanceMeters
    then
        return { success = false, error = "invalid_route" }
    end
    local duration_seconds = math.max(
        60,
        math.floor((distance_meters / Config.AgentRide.AverageSpeedMetersPerSecond) + 0.5)
    )
    local now = os.time()
    local expires_at = now + Config.AgentRide.QuoteLifetimeSeconds
    local quote_group = generate_uuid()
    local options = {}
    local distance_unit = Config.AgentRide.DistanceUnit
    ...
    for index = 1, #Config.AgentRide.Services do
        ...
        local minimum_custom_price = math.floor(math.max(
            Config.AgentRide.CustomFare.MinimumPrice,
            service.MinimumFare,
            calculated_price * Config.AgentRide.CustomFare.MinimumCalculatedMultiplier
        ) + 0.5)
        local maximum_custom_price = math.floor(math.min(
            Config.AgentRide.CustomFare.MaximumPrice,
            calculated_price * Config.AgentRide.CustomFare.MaximumCalculatedMultiplier
        ) + 0.5)
...
Bridge.Callbacks.Register("agent_phone:agentride:request", function(source, data)
    if not AgentPhone.AllowOperation(source, "agentride_action", Config.AgentRide.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    if not is_near(source, quote.pickup, Config.AgentRide.PickupSelectionRadius) then
        return { success = false, error = "pickup_too_far" }
    end
    local result, lock_error = run_locked("profile:" .. profile.id, function()
        ...
        local payout = math.floor((price * Config.AgentRide.DriverPayoutPercent / 100) + 0.5)
        Bridge.Database.Query([[
        ...
        if not charge_success then
            ...
            Bridge.Debug(
                "error",
                "[agent_phone] Fare debit crashed for AgentRide '%s'; payment state is ambiguous and requires manual reconciliation: %s",
                ride_id,
        ...
        if not charge_result then
            local delete_success, delete_result = pcall(
                Bridge.Database.Query,
                "DELETE FROM `agent_phone_agentride_rides` WHERE `id` = ? AND `status` = 'payment_pending'",
                { ride_id }
            )
            if not delete_success or affected_rows(delete_result) ~= 1 then
                Bridge.Debug(
                    "error",
                    "[agent_phone] Failed to remove uncharged payment-pending AgentRide '%s'.",
                    ride_id
                )
            end
        ...
        if price > 0 then
            ...
            if not capture_success then
                Bridge.Debug(
                    "error",
                    "[agent_phone] Fare was charged but its recovery marker failed for AgentRide '%s': %s. Manual reconciliation is required.",
                    ride_id,
            ...
            if affected_rows(capture_result) ~= 1 then
                ...
                Bridge.Debug(
                    "error",
                    "[agent_phone] Fare recovery failed before activating AgentRide '%s'. Manual reconciliation is required.",
                    ride_id
                )
                return { success = false, error = "refund_failed" }
            end
        ...
        if not update_success or affected_rows(update) ~= 1 then
            ...
            elseif price > 0 then
                ...
                if not delete_success or affected_rows(delete_result) ~= 1 then
                    Bridge.Debug(
                        "error",
                        "[agent_phone] Failed to clean payment-pending AgentRide '%s' after request activation failed.",
                        ride_id
                    )
                end
                if not update_success then
                    Bridge.Debug(
                        "error",
                        "[agent_phone] Failed to activate AgentRide '%s': %s",
                        ride_id,
                        tostring(update)
                    )
                end
...
Bridge.Callbacks.Register("agent_phone:agentride:set-driver-status", function(source, data)
    if not AgentPhone.AllowOperation(source, "agentride_action", Config.AgentRide.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
...
Bridge.Callbacks.Register("agent_phone:agentride:accept", function(source, data)
    if not AgentPhone.AllowOperation(source, "agentride_action", Config.AgentRide.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
...
Bridge.Callbacks.Register("agent_phone:agentride:arrive", function(source, data)
    return transition_driver_ride(
        ...
        "arrived",
        "arrived_at",
        Config.AgentRide.ArrivalRadius,
        "pickup"
    )
end)

Bridge.Callbacks.Register("agent_phone:agentride:start", function(source, data)
    return transition_driver_ride(
        source,
        data,
        { arrived = true },
...
Bridge.Callbacks.Register("agent_phone:agentride:complete", function(source, data)
    if not AgentPhone.AllowOperation(source, "agentride_action", Config.AgentRide.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    local result, lock_error = run_locked("ride:" .. ride_id, function()
        ...
        if not is_near(source, destination, Config.AgentRide.CompletionRadius) then
            return { success = false, error = "destination_too_far" }
        end
        local passenger_source = source_for_identifier(row.passenger_identifier)
        if not passenger_source or not is_near(passenger_source, destination, Config.AgentRide.CompletionRadius) then
            return { success = false, error = "passenger_too_far" }
        end
        ...
        if not payout_success then
            Bridge.Debug(
                "error",
                "[agent_phone] Driver payout crashed for AgentRide '%s'; completing state prevents an ambiguous retry and requires manual reconciliation: %s",
                ride_id,
        ...
        if not payout_result then
            ...
            if not reset_success or affected_rows(reset_result) ~= 1 then
                Bridge.Debug(
                    "error",
                    "[agent_phone] Driver payout failed and AgentRide '%s' could not be restored to in-progress.",
                    ride_id
                )
            end
            return { success = false, error = "payout_failed" }
        ...
        if affected_rows(completed) ~= 1 then
            ...
            if compensation_success and compensation_result then
                ...
                if reset_success and affected_rows(reset_result) == 1 then
                    ...
                    Bridge.Debug(
                        "error",
                        "[agent_phone] Compensated driver payout after AgentRide '%s' completion could not be finalized.",
                        ride_id
                    )
                    return { success = false, error = "payout_failed" }
                end
                Bridge.Debug(
                    "error",
                    "[agent_phone] Driver payout was compensated but AgentRide '%s' could not be restored; manual reconciliation is required.",
                    ride_id
                )
                return { success = false, error = "payout_failed" }
            end
            Bridge.Debug(
                "error",
                "[agent_phone] Failed to compensate payout for unfinalized AgentRide '%s'; completing state prevents an automatic duplicate payout: %s",
                ride_id,
...
Bridge.Callbacks.Register("agent_phone:agentride:cancel", function(source, data)
    if not AgentPhone.AllowOperation(source, "agentride_action", Config.AgentRide.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    local result, lock_error = run_locked("ride:" .. ride_id, function()
        ...
        if not process_pending_refund(updated_row) then
            Bridge.Debug(
                "info",
                "[agent_phone] Refund for cancelled AgentRide '%s' remains queued for server recovery.",
                ride_id
            )
        end
...
Bridge.Callbacks.Register("agent_phone:agentride:rate", function(source, data)
    if not AgentPhone.AllowOperation(source, "agentride_action", Config.AgentRide.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    if not tip or tip ~= math.floor(tip) or tip < 0 or tip > Config.AgentRide.MaximumTip then
        return { success = false, error = "invalid_tip" }
    end
    if not comment_length or comment_length > Config.AgentRide.RatingCommentMaxLength then
    ...
    local result, lock_error = run_locked("ride:" .. ride_id, function()
        ...
        if tip > 0 then
            ...
            if not debit_success then
                ...
                Bridge.Debug(
                    "error",
                    "[agent_phone] Passenger tip debit crashed for AgentRide '%s'; manual reconciliation is required: %s",
                    ride_id,
            ...
            if not credit_success then
                ...
                Bridge.Debug(
                    "error",
                    "[agent_phone] Driver tip credit crashed for AgentRide '%s'; retry is blocked for manual reconciliation: %s",
                    ride_id,
            ...
            if not credit_result then
                ...
                if refund_success and refund_result then
                    ...
                    Bridge.Debug(
                        "error",
                        "[agent_phone] Passenger refund after rejected driver tip credit is ambiguous or failed for AgentRide '%s'; retry is blocked for manual reconciliation: %s",
                        ride_id,
        ...
        if not update_success then
            Bridge.Debug(
                "error",
                "[agent_phone] Rating query crashed after tip transfer for AgentRide '%s'; processing state blocks an ambiguous retry and requires manual reconciliation: %s",
                ride_id,
                tostring(update)
            )
            return { success = false, error = "tip_failed" }
        ...
        if affected_rows(update) ~= 1 then
            ...
            if not driver_debit_success or not driver_debit_result then
                ...
                Bridge.Debug(
                    "error",
                    "[agent_phone] Rating persistence failed and driver tip debit was ambiguous or rejected for AgentRide '%s'; passenger refund was not attempted and manual reconciliation is required: %s",
                    ride_id,
                    tostring(driver_debit_result)
                )
                return { success = false, error = "tip_failed" }
            end
            TriggerClientEvent("agent_phone:banking:changed", driver_source)
            ...
            if passenger_refund_success and passenger_refund_result then
                ...
                Bridge.Debug(
                    "error",
                    "[agent_phone] Compensated rating persistence failure for AgentRide '%s'.",
                    ride_id
                )
            else
                transition_tip_status(ride_id, "processing", "failed")
                Bridge.Debug(
                    "error",
                    "[agent_phone] Driver tip was removed but passenger refund was ambiguous or rejected for AgentRide '%s'; manual reconciliation is required: %s",
                    ride_id,
    ...
    while true do
        local recovery_success, recovery_rows = pcall(
            ...
            (([[
                ...
                LIMIT 50
            ]]):format(math.floor(Config.AgentRide.PaymentPendingRecoverySeconds)))
        )
        ...
        Wait(math.floor(Config.AgentRide.RecoveryIntervalSeconds * 1000))
    end
end)
