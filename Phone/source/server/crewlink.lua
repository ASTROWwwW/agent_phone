...
if password_pepper == "" then
    Bridge.Debug(
        "warn",
        "[agent_phone] Config.Server.CrewLinkPasswordPepper is empty. CrewLink passwords still work, but their hashes lack the required server-side secret. Set a stable random value in config/config.lua before production; changing it later invalidates existing CrewLink passwords.",
        { always = true }
    )
end
...
local function new_id()
    ...
    if not rows[1] or type(rows[1].id) ~= "string" then
        error("[agent_phone] Database did not generate a CrewLink id.")
    end
    return rows[1].id
end
...
local function new_invite_code()
    for _ = 1, 5 do
        ...
        if type(code) == "string" then
            local existing = Bridge.Database.Query(
                "SELECT 1 FROM `agent_phone_crewlink_groups` WHERE `invite_code` = ? LIMIT 1",
                { code }
            )
    ...
    error("[agent_phone] Could not generate a unique CrewLink invite code.")
end
...
local function profile_for_session(source)
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_crewlink_sessions` crew_session
        JOIN `agent_phone_crewlink_profiles` p ON p.`id` = crew_session.`profile_id`
...
local function membership(profile_id, group_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_crewlink_memberships` m
...
local function group_count(group_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_crewlink_memberships`
        WHERE `group_id` = ?
    ]], { group_id })
...
local function member_dtos(group_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_crewlink_memberships` m
        JOIN `agent_phone_crewlink_profiles` p ON p.`id` = m.`profile_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = p.`avatar_media_id`
        WHERE m.`group_id` = ?
...
local function list_groups(profile)
    local rows = Bridge.Database.Query([[
        SELECT g.`id`, g.`name`, g.`colour`, g.`invite_code`, g.`owner_profile_id`,
            ...
            (SELECT COUNT(*) FROM `agent_phone_crewlink_memberships` gm
...
local function active_pings(group_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_crewlink_pings` pg
        LEFT JOIN `agent_phone_crewlink_profiles` cp ON cp.`id` = pg.`creator_profile_id`
...
local function pending_invitations(profile_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_crewlink_invitations` i
        JOIN `agent_phone_crewlink_groups` g ON g.`id` = i.`group_id`
        JOIN `agent_phone_crewlink_profiles` p ON p.`id` = i.`inviter_profile_id`
...
local function live_sources_by_account()
    ...
    for _, player_source in ipairs(Bridge.Framework.GetPlayers()) do
        ...
        local account = AgentPhone.RequireAccount(source)
...
local function profile_account_id(profile_id)
    local rows = Bridge.Database.Query(
        "SELECT `account_id` FROM `agent_phone_crewlink_profiles` WHERE `id` = ? LIMIT 1",
        { profile_id }
    )
...
local function notify_group(group_id, kind, actor, extra)
    for _, account_id in ipairs(group_account_ids(group_id)) do
        ...
        AgentPhone.NotifyAccountDevices(account_id, "agent_phone:crewlink:notification", payload)
    end
end

local function refresh_group(group_id)
    for _, account_id in ipairs(group_account_ids(group_id)) do
        AgentPhone.NotifyAccount(account_id, "agent_phone:crewlink:changed", { groupId = group_id })
    end
end
...
local function allow(source, operation)
    return AgentPhone.AllowOperation(
        source,
        "crewlink:" .. operation,
...
Bridge.Callbacks.Register("agent_phone:crewlink:bootstrap", function(source)
    if not AgentPhone.AllowOperation(
        source,
        "crewlink:bootstrap",
...
Bridge.Callbacks.Register("agent_phone:crewlink:register", function(source, data)
    if not AgentPhone.AllowOperation(source, "crewlink:register", 5, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    if avatar_media_id > 0 and not AgentPhoneMedia.ResolveOwnedMedia(source, avatar_media_id, "photo") then
        return { success = false, error = "invalid_profile_image" }
    end
    local existing = Bridge.Database.Query([[
        SELECT p.`id`, c.`profile_id` AS `credential_profile_id`
        FROM `agent_phone_crewlink_profiles` p
        LEFT JOIN `agent_phone_crewlink_credentials` c ON c.`profile_id` = p.`id`
    ...
    local duplicate = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_crewlink_profiles`
    ...
    queries[#queries + 1] = {
        query = [[INSERT INTO `agent_phone_crewlink_sessions` (`device_imei`, `profile_id`)
...
Bridge.Callbacks.Register("agent_phone:crewlink:login", function(source, data)
    if not AgentPhone.AllowOperation(source, "crewlink:login", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then
    ...
    local profiles = Bridge.Database.Query([[
        SELECT p.`id` FROM `agent_phone_crewlink_profiles` p
        JOIN `agent_phone_crewlink_credentials` c ON c.`profile_id` = p.`id`
        WHERE p.`account_id` = ?
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_crewlink_sessions` (`device_imei`, `profile_id`)
        VALUES (?, ?) ON DUPLICATE KEY UPDATE `profile_id` = VALUES(`profile_id`),
            `updated_at` = CURRENT_TIMESTAMP
...
Bridge.Callbacks.Register("agent_phone:crewlink:logout", function(source)
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then
        return error_response
    end
    Bridge.Database.Query(
        "DELETE FROM `agent_phone_crewlink_sessions` WHERE `device_imei` = ?",
        { session.imei }
    )
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:crewlink:update-profile", function(source, data)
    ...
    if data.avatarMediaId ~= nil then
        ...
        if submitted_avatar_id > 0
            and not AgentPhoneMedia.ResolveOwnedMedia(source, submitted_avatar_id, "photo")
        then
            return { success = false, error = "invalid_profile_image" }
        end
    ...
    local result = Bridge.Database.Query([[
        UPDATE IGNORE `agent_phone_crewlink_profiles`
...
Bridge.Callbacks.Register("agent_phone:crewlink:create-group", function(source, data)
    ...
    local success = Bridge.Database.Transaction({
        {
            query = [[INSERT INTO `agent_phone_crewlink_groups`
                (`id`, `name`, `colour`, `owner_profile_id`, `invite_code`)
                VALUES (?, ?, ?, ?, ?)]],
            params = { group_id, name, colour, profile.id, new_invite_code() },
        },
        {
            query = [[INSERT INTO `agent_phone_crewlink_memberships`
                (`group_id`, `profile_id`, `role`) VALUES (?, ?, 'owner')]],
            params = { group_id, profile.id },
        },
        {
            query = "UPDATE `agent_phone_crewlink_profiles` SET `active_group_id` = ? WHERE `id` = ?",
            params = { group_id, profile.id },
        },
    })
    if not success then
    ...
    TriggerEvent("agent_phone:crewlink:memberJoined", group_id, profile.id)
    TriggerEvent("agent_phone:crewlink:activeChanged", profile.id, group_id)
    local updated = require_profile(source)
    return { success = true, data = bootstrap(updated) }
end)

Bridge.Callbacks.Register("agent_phone:crewlink:update-group", function(source, data)
    if not allow(source, "group") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    Bridge.Database.Query([[
        UPDATE `agent_phone_crewlink_groups`
...
Bridge.Callbacks.Register("agent_phone:crewlink:delete-group", function(source, data)
    if not allow(source, "group") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    local success = Bridge.Database.Transaction({
        {
            query = "UPDATE `agent_phone_crewlink_profiles` SET `active_group_id` = NULL WHERE `active_group_id` = ?",
            params = { data.groupId },
        },
        {
            query = "DELETE FROM `agent_phone_crewlink_groups` WHERE `id` = ? AND `owner_profile_id` = ?",
    ...
    for _, account_id in ipairs(affected_accounts) do
        AgentPhone.NotifyAccount(account_id, "agent_phone:crewlink:changed", { groupId = data.groupId })
    end
    TriggerEvent("agent_phone:crewlink:memberLeft", data.groupId, profile.id)
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:crewlink:set-active", function(source, data)
    if not allow(source, "group") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    Bridge.Database.Query(
        "UPDATE `agent_phone_crewlink_profiles` SET `active_group_id` = ? WHERE `id` = ?",
        { data.groupId, profile.id }
    )
    TriggerEvent("agent_phone:crewlink:activeChanged", profile.id, data.groupId)
    local updated = require_profile(source)
    return { success = true, data = bootstrap(updated) }
end)

Bridge.Callbacks.Register("agent_phone:crewlink:join-code", function(source, data)
    ...
    local rows = Bridge.Database.Query([[
        SELECT `id`, `name` FROM `agent_phone_crewlink_groups`
    ...
    TriggerEvent("agent_phone:crewlink:memberJoined", group.id, profile.id)
    local updated = require_profile(source)
    return { success = true, data = bootstrap(updated) }
end)

Bridge.Callbacks.Register("agent_phone:crewlink:rotate-code", function(source, data)
    if not allow(source, "invite") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    Bridge.Database.Query("UPDATE `agent_phone_crewlink_groups` SET `invite_code` = ? WHERE `id` = ?", {
        code,
        data.groupId,
    })
    refresh_group(data.groupId)
...
Bridge.Callbacks.Register("agent_phone:crewlink:nearby", function(source)
    ...
    for _, target_value in ipairs(Bridge.Framework.GetPlayers()) do
        ...
        if target ~= source and GetPlayerRoutingBucket(target) == player_bucket then
            ...
            if distance <= Config.CrewLink.NearbyInviteDistance then
                local target_account = AgentPhone.RequireAccount(target)
                if target_account then
                    local rows = Bridge.Database.Query([[
                        SELECT `id`, `username` FROM `agent_phone_crewlink_profiles`
...
Bridge.Callbacks.Register("agent_phone:crewlink:invite-nearby", function(source, data)
    if not allow(source, "invite") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_crewlink_invitations`
    ...
    AgentPhone.NotifyAccountDevices(target_account.id, "agent_phone:crewlink:notification", {
        kind = "invite",
        actor = profile.username,
        groupName = member.group_name,
    })
    TriggerClientEvent("agent_phone:crewlink:changed", target, { groupId = profile.active_group_id })
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:crewlink:respond-invite", function(source, data)
    if not allow(source, "invite") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    if not accepted then
        Bridge.Database.Query(
            "UPDATE `agent_phone_crewlink_invitations` SET `status` = 'declined' WHERE `id` = ?",
            { invitation.id }
        )
        return { success = true }
    end
    ...
    local success = Bridge.Database.Transaction({
        ...
        {
            query = "UPDATE `agent_phone_crewlink_invitations` SET `status` = 'accepted' WHERE `id` = ? AND `status` = 'pending'",
    ...
    TriggerEvent("agent_phone:crewlink:memberJoined", invitation.group_id, profile.id)
    local updated = require_profile(source)
    return { success = true, data = bootstrap(updated) }
end)

Bridge.Callbacks.Register("agent_phone:crewlink:update-member", function(source, data)
    if not allow(source, "group") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    Bridge.Database.Query([[
        UPDATE `agent_phone_crewlink_memberships` SET `role` = ?
    ...
    if target_account_id then
        AgentPhone.NotifyAccountDevices(target_account_id, "agent_phone:crewlink:notification", {
            kind = "role",
            actor = profile.username,
...
Bridge.Callbacks.Register("agent_phone:crewlink:transfer-owner", function(source, data)
    if not allow(source, "group") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    local success = Bridge.Database.Transaction({
        {
            query = "UPDATE `agent_phone_crewlink_groups` SET `owner_profile_id` = ? WHERE `id` = ? AND `owner_profile_id` = ?",
            params = { data.profileId, data.groupId, profile.id },
        },
        {
            query = "UPDATE `agent_phone_crewlink_memberships` SET `role` = 'coordinator' WHERE `group_id` = ? AND `profile_id` = ?",
            params = { data.groupId, profile.id },
        },
        {
            query = "UPDATE `agent_phone_crewlink_memberships` SET `role` = 'owner' WHERE `group_id` = ? AND `profile_id` = ?",
...
Bridge.Callbacks.Register("agent_phone:crewlink:remove-member", function(source, data)
    if not allow(source, "group") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    local success = Bridge.Database.Transaction({
        {
            query = "DELETE FROM `agent_phone_crewlink_memberships` WHERE `group_id` = ? AND `profile_id` = ?",
            params = { data.groupId, data.profileId },
        },
        {
            query = "UPDATE `agent_phone_crewlink_profiles` SET `active_group_id` = NULL WHERE `id` = ? AND `active_group_id` = ?",
    ...
    if target_account_id then
        AgentPhone.NotifyAccount(target_account_id, "agent_phone:crewlink:changed", { groupId = data.groupId })
        AgentPhone.NotifyAccountDevices(target_account_id, "agent_phone:crewlink:notification", {
            kind = "removed",
            actor = profile.username,
            groupName = target.group_name,
    ...
    TriggerEvent("agent_phone:crewlink:memberLeft", data.groupId, data.profileId)
    refresh_group(data.groupId)
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:crewlink:leave", function(source, data)
    if not allow(source, "group") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
...
local function create_ping(group_id, creator_profile_id, source_resource, data)
    ...
    local count_rows = Bridge.Database.Query([[
        SELECT COUNT(*) AS `count` FROM `agent_phone_crewlink_pings`
    ...
    TriggerEvent("agent_phone:crewlink:pingCreated", group_id, ping)
    return ping
end

Bridge.Callbacks.Register("agent_phone:crewlink:create-ping", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:crewlink:remove-ping", function(source, data)
    if not allow(source, "ping") then
        return { success = false, error = "rate_limited" }
    end
    local profile, error_response = require_profile(source)
    ...
    local rows = Bridge.Database.Query([[
        SELECT `creator_profile_id` FROM `agent_phone_crewlink_pings`
    ...
    Bridge.Database.Query("DELETE FROM `agent_phone_crewlink_pings` WHERE `id` = ? AND `group_id` = ?", {
        data.pingId,
        group_id,
    })
    TriggerEvent("agent_phone:crewlink:pingRemoved", group_id, data.pingId)
    refresh_group(group_id)
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:crewlink:live", function(source)
    if not AgentPhone.AllowOperation(
        source,
        "crewlink:live",
        Config.CrewLink.LiveRequestsPerMinute,
...
exports("CreateCrewLinkPing", function(group_id, data)
    ...
    local groups = Bridge.Database.Query(
        "SELECT `id`, `name` FROM `agent_phone_crewlink_groups` WHERE `id` = ? LIMIT 1",
        { group_id }
    )
...
exports("RemoveCrewLinkPing", function(group_id, ping_id)
    ...
    local result = Bridge.Database.Query([[
        DELETE FROM `agent_phone_crewlink_pings`
        WHERE `id` = ? AND `group_id` = ? AND `source_resource` = ?
    ]], { ping_id, group_id, invoking_resource })
    if affected_rows(result) ~= 1 then
        return false
    end
    TriggerEvent("agent_phone:crewlink:pingRemoved", group_id, ping_id)
    refresh_group(group_id)
    return true
end)
end)
