...
local function storage_config()
    ...
    if not supported_systems[system] then
        error(("[agent_phone] Unsupported garage system '%s'."):format(system))
    end
...
local function refund_valet(source, order)
    ...
    if not Bridge.Framework.AddMoney(source, Config.Garage.Valet.Account, order.cost) then
        Bridge.Debug("error", "[agent_phone] Failed to refund valet order '%s' for player %s.", tostring(order.id), tostring(source))
        return
    end
...
Bridge.Callbacks.Register("agent_phone:garage:valet-request", function(source, data)
    local valet = Config.Garage.Valet
    if not valet.Enabled then
        return { success = false, error = "valet_disabled" }
    end
    if not AgentPhone.AllowOperation(source, "garage_valet", valet.RequestsPerMinute, 60) then
...
Bridge.Callbacks.Register("agent_phone:garage:valet-cancel", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:garage:valet-complete", function(source, data)
    local order = active_valets[source]
    if not order or type(data) ~= "table" or data.orderId ~= order.id then
        return { success = false, error = "valet_not_found" }
    end
    ...
    while true do
        ...
        for source, order in pairs(active_valets) do
            if order.expires_at <= now then
                cancel_valet(source, order)
                TriggerClientEvent("agent_phone:garage:valet-aborted", source, "valet_timeout")
            end
        end
    end
end)

Bridge.Callbacks.Register("agent_phone:garage:vehicles", function(source)
    if not AgentPhone.AllowOperation(source, "garage_vehicles", Config.Garage.RequestsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then
