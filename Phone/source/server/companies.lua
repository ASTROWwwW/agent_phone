...
local function validate_configuration(configuration)
    ...
    if type(company_config) ~= "table" or type(company_config.Definitions) ~= "table" then
        return nil, "[agent_phone] Config.Companies.Definitions must be configured."
    end
    if type(company_config.Enabled) ~= "boolean" then
        return nil, "[agent_phone] Config.Companies.Enabled must be a boolean."
    end
    for _, field in ipairs({
        ...
        if not valid_integer(company_config[field[1]], 1, field[2]) then
            return nil, ("[agent_phone] Config.Companies.%s is outside its supported range."):format(field[1])
        end
    end
    ...
    then
        return nil, "[agent_phone] Config.Companies.CallRouting is invalid."
    end
    ...
    if type(company_config.Statuses) ~= "table" then
        return nil, "[agent_phone] Config.Companies.Statuses must be configured."
    end
    ...
    if type(company_config.AvailabilityStatuses) ~= "table" then
        return nil, "[agent_phone] Config.Companies.AvailabilityStatuses must be configured."
    end
    ...
    if not valid_array(company_config.Categories, 100) then
        return nil, "[agent_phone] Config.Companies.Categories must be a bounded array."
    end
    local category_ids = {}
    ...
    for company_id, definition in pairs(company_config.Definitions) do
        ...
        then
            return nil, ("[agent_phone] Company definition '%s' is invalid."):format(tostring(company_id))
        end
        ...
        then
            return nil, ("[agent_phone] Company definition '%s' has invalid public profile defaults."):format(company_id)
        end
        ...
        if definition.Location ~= nil then
            ...
            if location_type ~= "table" and location_type ~= "vector3" then
                return nil, ("[agent_phone] Company definition '%s' has invalid location coordinates."):format(company_id)
            end
        ...
        if validated_definitions_by_job[definition.Job] then
            return nil, ("[agent_phone] Framework job '%s' is assigned to more than one company."):format(definition.Job)
        end
        ...
        for _, service in ipairs(default_services) do
            ...
            if configured_service_ids[service.Id] then
                return nil, ("[agent_phone] Default company service ID '%s' is configured more than once."):format(service.Id)
            end
...
function AgentPhoneCompanies.ValidateConfiguration(configuration)
...
local function seed_companies()
    if #definition_ids > 0 then
        ...
        local collisions = Bridge.Database.Query(([[
            SELECT `phone_number` FROM `agent_phone_sims`
            WHERE `phone_number` IN (%s)
            LIMIT 1
        ]]):format(table.concat(placeholders, ", ")), numbers)
        if collisions[1] then
            error(("[agent_phone] Company service number '%s' collides with an existing SIM."):format(
    ...
    for _, company_id in ipairs(definition_ids) do
        ...
        if location then
            Bridge.Database.Query([[
                INSERT IGNORE INTO `agent_phone_company_profiles`
        ...
        for index, service in ipairs(definition.Services or {}) do
            Bridge.Database.Query([[
                INSERT IGNORE INTO `agent_phone_company_services`
            ...
            local seeded = Bridge.Database.Query(
                "SELECT `company_id` FROM `agent_phone_company_services` WHERE `id` = ? LIMIT 1",
                { service.Id }
            )
            if not seeded[1] or seeded[1].company_id ~= company_id then
                error(("[agent_phone] Default service ID '%s' collides with another company."):format(service.Id))
            end
        end
    end
...
local function migrate_requestable_emergency_companies()
    local migration_name = "agent-phone:companies:requestable-emergency:v1"
    local completed = Bridge.Database.Query(
        "SELECT 1 FROM `agent_phone_migrations` WHERE `name` = ? LIMIT 1",
        { migration_name }
    )
    if completed[1] then
        return
    ...
    for _, company_id in ipairs(definition_ids) do
        ...
        if definition.Emergency and definition.AcceptsRequests then
            statements[#statements + 1] = {
                query = [[
                    UPDATE `agent_phone_company_profiles`
    ...
    statements[#statements + 1] = {
        query = [[
            INSERT IGNORE INTO `agent_phone_migrations` (`name`, `source`, `stats`)
            VALUES (?, ?, ?)
        ]],
        params = {
            migration_name,
...
local function tombstone_removed_companies()
    local profiles = Bridge.Database.Query([[
        ...
        FROM `agent_phone_company_profiles` profile
        INNER JOIN `agent_phone_company_requests` request ON request.`company_id` = profile.`company_id`
    ...
    for _, profile in ipairs(profiles) do
        if not definitions[profile.company_id] then
            ...
            if not Bridge.Database.Transaction({
                {
                    query = [[
                        UPDATE `agent_phone_company_requests`
...
function AgentPhoneCompanies.GetServiceLine(number)
    local normalized = AgentPhoneSimNumber.NormalizeService(number, Config.Sim.NumberLength)
    return normalized and service_line_payload(service_lines_by_number[normalized]) or nil
end

function AgentPhoneCompanies.GetServiceLineForCompany(company_id)
    return type(company_id) == "string" and service_line_payload(company_id) or nil
end

function AgentPhoneCompanies.IsServiceNumber(number)
    local normalized = AgentPhoneSimNumber.NormalizeService(number, Config.Sim.NumberLength)
    return normalized ~= nil and service_lines_by_number[normalized] ~= nil
end

function AgentPhoneCompanies.IsSystemContactNumber(number)
...
function AgentPhoneCompanies.GetSystemContacts()
    local contacts = {}
    if not Config.Companies.Enabled then
        return contacts
    end
...
function AgentPhoneCompanies.ClearCallAvailability(source)
...
local function current_device(source, registered_required)
    ...
    local device = AgentPhone.LoadDevice(session.imei)
    if not device then
...
function AgentPhoneCompanies.CanAnswerCompanyCall(source, company_id, imei, sim_id)
...
function AgentPhoneCompanies.CanPlaceCompanyCall(source, company_id)
    source = tonumber(source)
...
function AgentPhoneCompanies.GetCallTargets(company_id)
    local targets = {}
    local definition = definitions[company_id]
    ...
    for source, readiness in pairs(call_availability) do
        ...
        local device = member and AgentPhone.LoadDevice(readiness.imei) or nil
        local device_slots = device and AgentPhone.FindDeviceSlots(source, readiness.imei) or {}
...
local function profile_row(company_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_company_profiles` p
        LEFT JOIN `agent_phone_media` logo ON logo.`id` = p.`logo_media_id`
        LEFT JOIN `agent_phone_media` cover ON cover.`id` = p.`cover_media_id`
        WHERE p.`company_id` = ?
        LIMIT 1
    ]], { company_id })
    return rows[1]
...
local function company_services(company_id, include_inactive)
    ...
    local rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_company_services`
        WHERE `company_id` = ?%s
...
local function company_hours(company_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_company_hours`
        WHERE `company_id` = ?
        ORDER BY `weekday`
    ]], { company_id })
    local hours = {}
...
local function current_announcement(company_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_company_announcements`
...
local function allow_read(source, operation)
    ...
    if not AgentPhone.AllowOperation(source, "companies_" .. operation:lower(), limit, 60) then
...
local function cleanup_retained_data()
    ...
    local success = Bridge.Database.Transaction({
        {
            query = [[
                DELETE FROM `agent_phone_company_requests`
        ...
        {
            query = [[
                DELETE FROM `agent_phone_company_audit`
        ...
        {
            query = [[
                DELETE FROM `agent_phone_company_announcements`
...
Bridge.Callbacks.Register("agent_phone:companies:list", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:companies:get", function(source, data)
...
local function request_row(request_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_company_requests` r
        LEFT JOIN `agent_phone_company_services` service ON service.`id` = r.`service_id`
        LEFT JOIN `agent_phone_company_profiles` profile ON profile.`company_id` = r.`company_id`
        LEFT JOIN `agent_phone_media` logo ON logo.`id` = profile.`logo_media_id`
        WHERE r.`id` = ?
        LIMIT 1
    ]], { request_id })
    return rows[1]
...
local function request_detail(access)
    ...
    if access.audience == "customer" then
        Bridge.Database.Query(
            "UPDATE `agent_phone_company_requests` SET `customer_unread` = 0, `updated_at` = `updated_at` WHERE `id` = ?",
            { row.id }
        )
        row.customer_unread = 0
    else
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_company_request_reads`
    ...
    local messages = Bridge.Database.Query([[
        SELECT `id`, `sender_type`, `sender_identifier`, `sender_sim_id`, `body`,
            UNIX_TIMESTAMP(`created_at`) AS `created_at_unix`
        FROM `agent_phone_company_request_messages`
        WHERE `request_id` = ?
    ...
    local events = Bridge.Database.Query([[
        ...
        FROM `agent_phone_company_request_events`
        WHERE `request_id` = ?
        ORDER BY `created_at`, `id`
    ]], { row.id })
    ...
    local media_rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_company_request_media` request_media
        INNER JOIN `agent_phone_media` media ON media.`id` = request_media.`media_id`
...
local company_request_list_select = [[
    ...
    LEFT JOIN `agent_phone_company_profiles` profile ON profile.`company_id` = r.`company_id`
    LEFT JOIN `agent_phone_media` logo ON logo.`id` = profile.`logo_media_id`
    LEFT JOIN `agent_phone_company_request_reads` company_read
...
local function work_context(source)
    ...
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_company_requests` request
        LEFT JOIN `agent_phone_company_request_reads` company_read
...
local function notify_sim(sim_id, event_name, payload)
    local devices = Bridge.Database.Query([[
        ...
        FROM `agent_phone_devices` d
        LEFT JOIN `agent_phone_device_data` settings
    ...
    for _, device in ipairs(devices) do
        for _, player_source in ipairs(Bridge.Framework.GetPlayers()) do
            ...
            if AgentPhone.FindDeviceSlots(target, device.imei)[1] then
                local event_payload = {}
...
local function notify_source(source, event_name, payload)
    local session = AgentPhone.RequireSession(source)
    if not session then
        return
    end
    local rows = Bridge.Database.Query([[
...
local function emit_public_change(company_id)
    ...
    notify_company(company_id, "agent_phone:companies:changed", {
        area = "work",
        companyId = company_id,
    })
    ...
    for _, player_source in ipairs(Bridge.Framework.GetPlayers()) do
        notify_source(tonumber(player_source) or player_source, "agent_phone:companies:changed", {
            area = "directory",
            companyId = company_id,
        })
    end
...
local function emit_request_change(row, customer, company, excluded_source)
    ...
    if customer then
        notify_sim(row.customer_sim_id, "agent_phone:companies:changed", payload)
    end
    if company then
        payload = {
            area = "work",
        ...
        notify_company(row.company_id, "agent_phone:companies:changed", payload, excluded_source)
    end
end

Bridge.Callbacks.Register("agent_phone:companies:my-requests", function(source, data)
    local session, error_response = allow_read(source, "Read")
    if not session then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:companies:get-request", function(source, data)
    local session, error_response = allow_read(source, "Read")
    if not session then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:companies:work-context", function(source)
    local session, error_response = allow_read(source, "Read")
    if not session then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:companies:work-queue", function(source, data)
    local session, error_response = allow_read(source, "Read")
    if not session then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:companies:list-members", function(source)
    local session, error_response = allow_read(source, "Read")
    if not session then
        return error_response
    end
...
local function allow_mutation(source, operation, limit_name)
    ...
    if not AgentPhone.AllowOperation(source, "companies_" .. operation, limit, 60) then
...
function AgentPhoneCompanies.RouteServiceLineMessage(source, data)
    ...
    local service_line = AgentPhoneCompanies.GetServiceLine(data.phoneNumber)
    ...
    local statements = {
        {
            query = "UPDATE `agent_phone_sims` SET `updated_at` = `updated_at` WHERE `id` = ?",
            params = { device.sim_id },
        },
        {
            query = [[
                INSERT INTO `agent_phone_company_requests`
                ...
                FROM `agent_phone_sims` sim
                INNER JOIN `agent_phone_company_profiles` profile ON profile.`company_id` = ?
                WHERE sim.`id` = ? AND NOT EXISTS (
                    SELECT 1 FROM `agent_phone_company_requests` existing
        ...
        },
        {
            query = [[
                ...
                FROM `agent_phone_company_requests` WHERE `id` = ?
            ]],
        ...
        {
            query = [[
                INSERT INTO `agent_phone_company_request_messages`
        ...
        {
            query = [[
                INSERT INTO `agent_phone_sms_messages`
                    (`id`, `sender_sim_id`, `recipient_sim_id`, `sender_number`, `recipient_number`,
                        `message_type`, `body`)
                SELECT ?, sim.`id`, NULL, sim.`phone_number`, ?, 'text', ?
                FROM `agent_phone_sims` sim
                INNER JOIN `agent_phone_company_request_messages` message ON message.`id` = ?
                WHERE sim.`id` = ?
            ]],
            params = {
                data.id,
    ...
    local routed = Bridge.Database.Query([[
        ...
        FROM `agent_phone_sms_messages` sms
        INNER JOIN `agent_phone_company_request_messages` message ON message.`id` = sms.`id`
        INNER JOIN `agent_phone_company_requests` request ON request.`id` = message.`request_id`
        LEFT JOIN `agent_phone_company_request_events` created
    ...
    if not routed[1] then
        Bridge.Debug(
            "error",
            "[agent_phone] Service-line message %s committed without a complete route.",
            tostring(data.id),
            { always = true }
        )
        return { success = false, error = "request_failed" }
    ...
    if routed[1].created_event_id then
        notify_company(row.company_id, "agent_phone:companies:notification", notification_payload(
            "newRequest",
            "work",
            row
        ), source)
    elseif row.assigned_identifier then
        notify_identifier(
            ...
            row.company_id,
            "agent_phone:companies:notification",
...
Bridge.Callbacks.Register("agent_phone:companies:create-request", function(source, data)
    ...
    local profiles = Bridge.Database.Query(
        "SELECT `accepts_requests` FROM `agent_phone_company_profiles` WHERE `company_id` = ? LIMIT 1",
        { company_id }
    )
    ...
    local services = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_company_services`
    ...
    for _, media_id in ipairs(media_ids) do
        ...
        if not normalized or media_seen[normalized]
            or not AgentPhoneMedia.ResolveOwnedMedia(source, normalized, "photo")
        then
            return { success = false, error = "invalid_media" }
        end
    ...
    local statements = {
        ...
        },
        {
            query = [[
                ...
                FROM `agent_phone_sims` sim
                INNER JOIN `agent_phone_company_profiles` profile ON profile.`company_id` = ?
                INNER JOIN `agent_phone_company_services` service
                ...
                WHERE sim.`id` = ? AND profile.`accepts_requests` = 1
                    ...
                    SELECT COUNT(*) FROM `agent_phone_company_requests`
                    WHERE `customer_sim_id` = ? AND `status` NOT IN ('completed', 'cancelled')
                ) < ?
            ]],
            params = {
    ...
    for index, media_id in ipairs(normalized_media) do
        statements[#statements + 1] = {
            query = [[
                INSERT INTO `agent_phone_company_request_media` (`request_id`, `media_id`, `sort_order`)
                SELECT `id`, ?, ? FROM `agent_phone_company_requests` WHERE `id` = ?
            ]],
    ...
    if not row then
        local available = Bridge.Database.Query([[
            SELECT service.`id`
            FROM `agent_phone_company_profiles` profile
            INNER JOIN `agent_phone_company_services` service ON service.`company_id` = profile.`company_id`
    ...
    emit_request_change(row, true, true, source)
    notify_company(company_id, "agent_phone:companies:notification", notification_payload(
        "newRequest",
        "work",
        row
    ), source)
...
Bridge.Callbacks.Register("agent_phone:companies:cancel-request", function(source, data)
    ...
    local committed = Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_company_request_events` WHERE `id` = ? LIMIT 1",
        { event_id }
    )
    ...
    if actor_type == "customer" then
        ...
        notify_sim(row.customer_sim_id, "agent_phone:companies:notification", notification_payload(
            "requestUpdated",
            "customer",
            row
        ))
...
Bridge.Callbacks.Register("agent_phone:companies:send-message", function(source, data)
    ...
    local service_line = access.row.channel == "service_line"
        and AgentPhoneCompanies.GetServiceLineForCompany(access.row.company_id) or nil
    ...
    if access.audience == "customer" then
        statements[#statements + 1] = {
            query = [[
                INSERT INTO `agent_phone_company_request_messages`
                    (`id`, `request_id`, `sender_type`, `sender_sim_id`, `body`)
                SELECT ?, `id`, 'customer', ?, ? FROM `agent_phone_company_requests`
                WHERE `id` = ? AND `revision` = ? AND `mutation_token` = ?
            ]],
        ...
        statements[#statements + 1] = {
            query = [[
                ...
                SELECT ?, `id`, 'company', ?, ? FROM `agent_phone_company_requests`
                WHERE `id` = ? AND `revision` = ? AND `mutation_token` = ?
            ]],
        ...
        if access.audience == "customer" then
            statements[#statements + 1] = {
                query = [[
                    ...
                    FROM `agent_phone_company_request_messages` message
                    INNER JOIN `agent_phone_company_requests` request ON request.`id` = message.`request_id`
                    INNER JOIN `agent_phone_sims` sim ON sim.`id` = request.`customer_sim_id`
    ...
    if new_status ~= access.row.status then
        statements[#statements + 1] = {
            query = [[
                INSERT INTO `agent_phone_company_request_events`
                    (`id`, `request_id`, `event_type`, `actor_type`, `from_status`, `to_status`)
                SELECT ?, `id`, 'status', 'customer', ?, ? FROM `agent_phone_company_requests`
                WHERE `id` = ? AND `revision` = ? AND `mutation_token` = ?
            ]],
    ...
    local inserted = Bridge.Database.Query([[
        ...
        FROM `agent_phone_company_request_messages` message
        LEFT JOIN `agent_phone_sms_messages` sms ON sms.`id` = message.`id`
    ...
    if service_line and not inserted[1].sms_id then
        Bridge.Debug(
            "error",
            "[agent_phone] Company request message %s committed without its service-line SMS.",
            tostring(message_id),
            { always = true }
        )
        return { success = false, error = "request_failed" }
    ...
    if access.audience == "customer" then
        if service_line then
            TriggerClientEvent("agent_phone:messages:changed", source, {
    ...
    elseif service_line then
        notify_sim(row.customer_sim_id, "agent_phone:messages:new", {
            phoneNumber = service_line.number,
...
Bridge.Callbacks.Register("agent_phone:companies:claim-request", function(source, data)
    ...
    if not committed[1] then
        local current = Bridge.Database.Query([[
            SELECT `id` FROM `agent_phone_company_requests`
...
Bridge.Callbacks.Register("agent_phone:companies:assign-request", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:companies:update-request-status", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:companies:call-customer", function(source, data)
    ...
    if not AgentPhoneCompanies.CanPlaceCompanyCall(source, access.row.company_id) then
        return { success = false, error = "not_authorized" }
    end
    local rows = Bridge.Database.Query([[
    ...
    local result = AgentPhoneCalls.StartCompanyCall(source, access.row.company_id, rows[1].phone_number)
...
Bridge.Callbacks.Register("agent_phone:companies:dial-service-line", function(source, data)
    ...
    return AgentPhoneCalls.StartCompanyCall(source, member.company_id, phone_number)
end)
...
local function audit_statement(company_id, identifier, action, target_type, target_id, mutation_token, audit_id)
    return {
        query = [[
            INSERT INTO `agent_phone_company_audit`
                (`id`, `company_id`, `actor_identifier`, `action`, `target_type`, `target_id`, `metadata`)
            SELECT ?, `company_id`, ?, ?, ?, ?, ?
            FROM `agent_phone_company_profiles`
            WHERE `company_id` = ? AND `mutation_token` = ?
        ]],
        params = {
            audit_id,
...
local function audit_committed(audit_id)
    local rows = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_company_audit`
        WHERE `id` = ?
        LIMIT 1
    ]], { audit_id })
    return rows[1] ~= nil
...
Bridge.Callbacks.Register("agent_phone:companies:update-availability", function(source, data)
...
local function media_id_for_profile(source, value)
    ...
    if not media_id or not AgentPhoneMedia.ResolveOwnedMedia(source, media_id, "photo") then
        return nil
    end
    return media_id
end

Bridge.Callbacks.Register("agent_phone:companies:update-profile", function(source, data)
    ...
    if data.phoneNumber ~= nil then
        local number = AgentPhoneSimNumber.NormalizeService(data.phoneNumber, Config.Sim.NumberLength)
    ...
    if not Bridge.Database.Transaction({
        {
            query = ("UPDATE `agent_phone_company_profiles` SET %s WHERE `company_id` = ? AND `revision` = ?")
...
Bridge.Callbacks.Register("agent_phone:companies:update-hours", function(source, data)
    ...
    local statements = {
        ...
        {
            query = [[
                DELETE hours FROM `agent_phone_company_hours` hours
                INNER JOIN `agent_phone_company_profiles` profile ON profile.`company_id` = hours.`company_id`
    ...
    for _, entry in ipairs(hours) do
        ...
        if entry.isClosed then
            query = [[
                INSERT INTO `agent_phone_company_hours` (`company_id`, `weekday`, `is_closed`)
                SELECT `company_id`, ?, 1 FROM `agent_phone_company_profiles`
                WHERE `company_id` = ? AND `mutation_token` = ?
            ]]
            params = { entry.day, member.company_id, mutation_token }
        else
            query = [[
                INSERT INTO `agent_phone_company_hours`
                    (`company_id`, `weekday`, `is_closed`, `opens_at`, `closes_at`)
                SELECT `company_id`, ?, 0, ?, ? FROM `agent_phone_company_profiles`
                WHERE `company_id` = ? AND `mutation_token` = ?
            ]]
...
Bridge.Callbacks.Register("agent_phone:companies:update-services", function(source, data)
    ...
    local existing_rows = Bridge.Database.Query(
        "SELECT `id` FROM `agent_phone_company_services` WHERE `company_id` = ? AND `archived` = 0",
        { member.company_id }
    )
    local existing = {}
    ...
    local statements = {
        ...
        {
            query = [[
                UPDATE `agent_phone_company_services` service
                INNER JOIN `agent_phone_company_profiles` profile ON profile.`company_id` = service.`company_id`
    ...
    for _, service in ipairs(normalized) do
        statements[#statements + 1] = {
            query = [[
                INSERT INTO `agent_phone_company_services`
                    (`id`, `company_id`, `title`, `description`, `price_text`, `requests_enabled`, `active`, `sort_order`)
                SELECT ?, `company_id`, ?, ?, ?, ?, ?, ? FROM `agent_phone_company_profiles`
                WHERE `company_id` = ? AND `mutation_token` = ?
                ON DUPLICATE KEY UPDATE
...
Bridge.Callbacks.Register("agent_phone:companies:publish-announcement", function(source, data)
    ...
    local statements = {
        ...
        {
            query = [[
                UPDATE `agent_phone_company_announcements` announcement
                INNER JOIN `agent_phone_company_profiles` profile ON profile.`company_id` = announcement.`company_id`
    ...
    if body ~= "" then
        ...
        if expiry then
            insert_query = [[
                INSERT INTO `agent_phone_company_announcements`
                    (`id`, `company_id`, `title`, `body`, `created_by`, `expires_at`)
                SELECT ?, `company_id`, '', ?, ?, FROM_UNIXTIME(?) FROM `agent_phone_company_profiles`
                WHERE `company_id` = ? AND `mutation_token` = ?
            ]]
...
Bridge.Callbacks.Register("agent_phone:companies:set-call-availability", function(source, data)
    ...
    if not data.available then
        AgentPhoneCompanies.ClearCallAvailability(source)
