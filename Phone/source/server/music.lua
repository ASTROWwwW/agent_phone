...
local function discover_music_assets()
    ...
    local function walk(mount_path, relative_directory, depth)
        ...
        for name in directory:lines() do
            ...
            if safe_music_asset_name(name) then
                ...
                Bridge.Debug(
                    "warn",
                    "[agent_phone] Skipped unsafe music path entry '%s'.",
                    tostring(name),
                    { always = true }
                )
            end
    ...
    if failure_reason then
        Bridge.Debug(
            "error",
            "[agent_phone] Music asset discovery failed; no server tracks were loaded: %s",
            failure_reason,
            { always = true }
        )
        return nil
...
local function normalize_server_tracks()
    ...
    for index, configured in ipairs(Config.Music.Tracks or {}) do
        ...
        then
            Bridge.Debug(
                "error",
                "[agent_phone] Ignored invalid Config.Music.Tracks entry at index %s. Only Id, Title, and Artist are required.",
                tostring(index),
                { always = true }
            )
        elseif configured_track_ids[id:lower()] then
            Bridge.Debug(
                "error",
                "[agent_phone] Ignored duplicate Config.Music.Tracks Id '%s' at index %s. IDs are case-insensitive.",
                id,
                tostring(index),
                { always = true }
            )
            ...
            if file_error == "missing" then
                Bridge.Debug(
                    "error",
                    "[agent_phone] Ignored music track '%s': no %s.ogg or %s.mp3 was found under config/music.",
                    id,
                    id,
                    id,
                    { always = true }
            ...
            elseif file_error == "ambiguous" then
                Bridge.Debug(
                    "error",
                    "[agent_phone] Ignored music track '%s': multiple equally preferred audio files were found: %s",
                    id,
                ...
                if artwork_error == "ambiguous" then
                    Bridge.Debug(
                        "warn",
                        "[agent_phone] Music track '%s' was loaded without artwork because multiple equally preferred images were found: %s",
                        id,
...
AddEventHandler("agent_phone:configurator:serverUpdated", normalize_server_tracks)
...
local function list_youtube_tracks(account_id, imei)
    ...
    local rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_music_youtube_songs`
        WHERE %s
...
local function list_playlists(account_id, imei)
    ...
    local rows = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_music_playlists`
        WHERE %s
    ...
    local items = Bridge.Database.Query(([[
        ...
        FROM `agent_phone_music_playlist_items` AS `i`
        INNER JOIN `agent_phone_music_playlists` AS `p` ON `p`.`id` = `i`.`playlist_id`
        WHERE %s
...
local function owned_playlist(account_id, imei, playlist_id)
    ...
    local rows = Bridge.Database.Query(([[
        SELECT `id` FROM `agent_phone_music_playlists`
...
local function valid_personal_song(account_id, imei, song_id)
    ...
    append_params(params, owner_params)
    local rows = Bridge.Database.Query(([[
        SELECT `id` FROM `agent_phone_music_youtube_songs`
        WHERE `id` = ? AND %s LIMIT 1
    ]]):format(condition), params)
    return rows[1] ~= nil
end

Bridge.Callbacks.Register("agent_phone:music:bootstrap", function(source)
...
Bridge.Callbacks.Register("agent_phone:music:add-youtube", function(source, data)
    if not AgentPhone.AllowOperation(source, "music_write", Config.Music.ActionsPerMinute, 60) then
    ...
    local video_id = AgentPhoneYouTube.ParseId(payload.url)
    ...
    local count_rows = Bridge.Database.Query(([[
        SELECT COUNT(*) AS `count` FROM `agent_phone_music_youtube_songs` WHERE %s
    ...
    local metadata = (not custom_title or not custom_artist)
        and AgentPhoneYouTube.FetchMetadata(video_id)
        or nil
    ...
    if account_id then
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_music_youtube_songs`
...
Bridge.Callbacks.Register("agent_phone:music:remove-youtube", function(source, data)
    if not AgentPhone.AllowOperation(source, "music_write", Config.Music.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account_id, imei, error_response = session_owner(source)
    ...
    Bridge.Database.Query(([[
        DELETE `i` FROM `agent_phone_music_playlist_items` AS `i`
        INNER JOIN `agent_phone_music_playlists` AS `p` ON `p`.`id` = `i`.`playlist_id`
    ...
    Bridge.Database.Query(([[
        DELETE FROM `agent_phone_music_youtube_songs` WHERE `id` = ? AND %s
    ]]):format(song_condition), delete_params)
    return { success = true, data = bootstrap(account_id, imei) }
end)

Bridge.Callbacks.Register("agent_phone:music:create-playlist", function(source, data)
    if not AgentPhone.AllowOperation(source, "music_write", Config.Music.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account_id, imei, error_response = session_owner(source)
    ...
    local condition, owner_params = owner_condition(account_id, imei)
    local count_rows = Bridge.Database.Query(([[
        SELECT COUNT(*) AS `count` FROM `agent_phone_music_playlists` WHERE %s
    ]]):format(condition), owner_params)
    ...
    if account_id then
        Bridge.Database.Query([[
            INSERT INTO `agent_phone_music_playlists` (`id`, `account_id`, `device_imei`, `name`)
            VALUES (?, ?, NULL, ?)
        ]], { id, account_id, name })
    else
        Bridge.Database.Query([[
...
Bridge.Callbacks.Register("agent_phone:music:rename-playlist", function(source, data)
    if not AgentPhone.AllowOperation(source, "music_write", Config.Music.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account_id, imei, error_response = session_owner(source)
    ...
    append_params(params, owner_params)
    Bridge.Database.Query(([[
        UPDATE `agent_phone_music_playlists` SET `name` = ?
        WHERE `id` = ? AND %s
    ]]):format(condition), params)
    return { success = true, data = bootstrap(account_id, imei) }
end)

Bridge.Callbacks.Register("agent_phone:music:delete-playlist", function(source, data)
    if not AgentPhone.AllowOperation(source, "music_write", Config.Music.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account_id, imei, error_response = session_owner(source)
    ...
    local result = Bridge.Database.Query(([[
        DELETE FROM `agent_phone_music_playlists` WHERE `id` = ? AND %s
    ]]):format(condition), params)
...
Bridge.Callbacks.Register("agent_phone:music:add-to-playlist", function(source, data)
    if not AgentPhone.AllowOperation(source, "music_write", Config.Music.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account_id, imei, error_response = session_owner(source)
    ...
    local mutation_error = with_playlist_add_lock(playlist_id, function()
        local counts = Bridge.Database.Query([[
            ...
            FROM `agent_phone_music_playlist_items` WHERE `playlist_id` = ?
        ...
        local insert_result = Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_music_playlist_items`
        ...
        Bridge.Database.Query(
            "UPDATE `agent_phone_music_playlists` SET `updated_at` = CURRENT_TIMESTAMP WHERE `id` = ?",
            { playlist_id }
        )
        return nil
    end)
...
Bridge.Callbacks.Register("agent_phone:music:remove-from-playlist", function(source, data)
    if not AgentPhone.AllowOperation(source, "music_write", Config.Music.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local account_id, imei, error_response = session_owner(source)
    ...
    Bridge.Database.Query([[
        DELETE FROM `agent_phone_music_playlist_items`
