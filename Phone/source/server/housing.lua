...
function AgentPhoneHousing.ResolveShare(source, property_id)
    ...
    for _, property in ipairs(overview.properties) do
        if property.id == property_id then
            return {
                ...
                link = "agentphone://house/property/" .. property.id,
...
Bridge.Callbacks.Register("agent_phone:housing:overview", function(source)
...
Bridge.Callbacks.Register("agent_phone:housing:prepare", function(source, data)
    if not AgentPhone.AllowOperation(source, "housing_action", Config.Housing.ActionsPerMinute, 60) then
        return { success = false, error = "rate_limited" }
    end
    local session, error_response = AgentPhone.RequireSession(source)
    if not session then
