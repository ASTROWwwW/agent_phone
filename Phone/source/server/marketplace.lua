...
local function require_payload(source, operation, data)
    ...
    Bridge.Debug(
        "warn",
        "[agent_phone] Invalid marketplace payload for %s from source %s.",
        operation,
        tostring(source)
    )
    return nil
...
local function profile_dto(account)
    local rows = Bridge.Database.Query([[
        SELECT profile.`display_name`, profile.`bio`, profile.`avatar_media_id`,
            ...
            (SELECT COUNT(*) FROM `agent_phone_marketplace_listings` listing
                WHERE listing.`seller_account_id` = ?) AS `listing_count`
        FROM `agent_phone_accounts` account
        LEFT JOIN `agent_phone_marketplace_profiles` profile ON profile.`account_id` = account.`id`
...
local function require_profile(account_id)
    local rows = Bridge.Database.Query([[
        SELECT `account_id` FROM `agent_phone_marketplace_profiles`
...
Bridge.Callbacks.Register("agent_phone:marketplace:profile", function(source)
...
Bridge.Callbacks.Register("agent_phone:marketplace:auth", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    if not AgentPhone.AllowOperation(source, "marketplace:auth", 8, 60) then
    ...
    local credentials = Bridge.Database.Query([[
        SELECT `id` FROM `agent_phone_accounts`
    ...
    local result = Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_marketplace_profiles` (`account_id`, `display_name`, `bio`, `avatar_media_id`)
        VALUES (?, ?, '', NULLIF(?, 0))
...
Bridge.Callbacks.Register("agent_phone:marketplace:profile-save", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    if not AgentPhone.AllowOperation(source, "marketplace:profile-save", 12, 60) then
        return { success = false, error = "rate_limited" }
    end
...
local function expire_listings()
    Bridge.Database.Query([[
        UPDATE `agent_phone_marketplace_listings`
...
local function load_images(listing_id)
    return Bridge.Database.Query([[
        ...
        FROM `agent_phone_marketplace_images`
        WHERE `listing_id` = ?
        ORDER BY `sort_order`
    ]], { listing_id })
end
...
local function validate_images(source, images)
    ...
    for index, image in ipairs(images) do
        ...
        then
            Bridge.Debug(
                "warn",
                "[agent_phone] Rejected unowned marketplace image from source %s.",
                tostring(source)
            )
            return nil
        end
        local url = AgentPhoneMedia.ResolveOwnedMedia(source, normalized_id, "photo")
        if not url then
            Bridge.Debug(
                "warn",
                "[agent_phone] Rejected unowned marketplace image from source %s.",
...
local function validate_listing(source, account, data)
    ...
    local device = AgentPhone.LoadDevice(account.imei)
...
local function listing_summary_query(account_id, where_clause, order_clause, values, limit, offset)
    ...
    return Bridge.Database.Query(([[
        SELECT l.`id`, l.`title`, l.`category`, l.`item_condition`, l.`price_type`, l.`price`,
            ...
            (SELECT i.`gradient` FROM `agent_phone_marketplace_images` i
                WHERE i.`listing_id` = l.`id` ORDER BY i.`sort_order` LIMIT 1) AS `image`,
            EXISTS(SELECT 1 FROM `agent_phone_marketplace_favorites` f
                WHERE f.`listing_id` = l.`id` AND f.`account_id` = ?) AS `is_favorite`
        FROM `agent_phone_marketplace_listings` l
        JOIN `agent_phone_accounts` a ON a.`id` = l.`seller_account_id`
        LEFT JOIN `agent_phone_marketplace_profiles` profile ON profile.`account_id` = l.`seller_account_id`
        WHERE %s
        ORDER BY %s
        LIMIT ? OFFSET ?
...
local function marketplace_counts(account_id)
    local rows = Bridge.Database.Query([[
        SELECT
            ((SELECT COUNT(*) FROM `agent_phone_marketplace_messages` m
                JOIN `agent_phone_marketplace_inquiries` q ON q.`id` = m.`inquiry_id`
                WHERE (q.`seller_account_id` = ? OR q.`buyer_account_id` = ?)
                    AND m.`sender_account_id` <> ? AND m.`read_at` IS NULL)
            + (SELECT COUNT(*) FROM `agent_phone_marketplace_offers` o
                JOIN `agent_phone_marketplace_inquiries` q ON q.`id` = o.`inquiry_id`
                WHERE (q.`seller_account_id` = ? OR q.`buyer_account_id` = ?)
            ...
            (SELECT COUNT(*) FROM `agent_phone_marketplace_listings`
...
local function notify_changed(account_id)
    AgentPhone.NotifyAccount(account_id, "agent_phone:marketplace:changed", {
        counts = marketplace_counts(account_id),
    })
end

Bridge.Callbacks.Register("agent_phone:marketplace:list", function(source, data)
    ...
    if data.favorites == true then
        ...
        conditions[#conditions + 1] = [[EXISTS(SELECT 1 FROM `agent_phone_marketplace_favorites` favorite_filter
            WHERE favorite_filter.`listing_id` = l.`id` AND favorite_filter.`account_id` = ?)]]
        values[#values + 1] = account_id
    end
    if account_id then
        conditions[#conditions + 1] = [[NOT EXISTS(SELECT 1 FROM `agent_phone_marketplace_blocks` b
...
Bridge.Callbacks.Register("agent_phone:marketplace:get", function(source, data)
    local account_id, _, error_response = optional_account(source)
    if error_response then
        return error_response
    end
    ...
    local rows = Bridge.Database.Query([[
        SELECT l.*, COALESCE(profile.`display_name`, SUBSTRING_INDEX(a.`email`, '@', 1)) AS `seller_name`,
            ...
            (SELECT COUNT(*) FROM `agent_phone_marketplace_listings` own
        ...
        JOIN `agent_phone_accounts` a ON a.`id` = l.`seller_account_id`
        LEFT JOIN `agent_phone_marketplace_profiles` profile ON profile.`account_id` = l.`seller_account_id`
        LEFT JOIN `agent_phone_media` profile_avatar ON profile_avatar.`id` = profile.`avatar_media_id`
        WHERE l.`id` = ?
        LIMIT 1
    ]], { account_id or 0, id })
    ...
    if account_id then
        local blocks = Bridge.Database.Query([[
            SELECT 1 FROM `agent_phone_marketplace_blocks`
...
Bridge.Callbacks.Register("agent_phone:marketplace:list-own", function(source, data)
    local account, error_response = require_account(source)
    if not account then
        return error_response
    end
...
Bridge.Callbacks.Register("agent_phone:marketplace:create", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    local profile_error = require_profile(account.id)
    if profile_error then return profile_error end
    if not AgentPhone.AllowOperation(source, "marketplace:create", 5, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    for index, image in ipairs(listing.images) do
        statements[#statements + 1] = {
            query = [[INSERT INTO `agent_phone_marketplace_images`
...
Bridge.Callbacks.Register("agent_phone:marketplace:update", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    ...
    local current_rows = Bridge.Database.Query([[
        SELECT `revision` FROM `agent_phone_marketplace_listings`
    ...
    local statements = {
        ...
        { query = "DELETE FROM `agent_phone_marketplace_images` WHERE `listing_id` = ?", params = { id } },
    }
    for index, image in ipairs(listing.images) do
        statements[#statements + 1] = {
            query = [[INSERT INTO `agent_phone_marketplace_images`
    ...
    local rows = Bridge.Database.Query(
        "SELECT `revision` FROM `agent_phone_marketplace_listings` WHERE `id` = ? AND `seller_account_id` = ?",
        { id, account.id }
    )
...
Bridge.Callbacks.Register("agent_phone:marketplace:set-status", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    ...
    local listings = Bridge.Database.Query([[
        SELECT `status`, `reserved_account_id` FROM `agent_phone_marketplace_listings`
    ...
    if data.status == "reserved" then
        ...
        local inquiries = Bridge.Database.Query([[
            SELECT `buyer_account_id` FROM `agent_phone_marketplace_inquiries`
...
Bridge.Callbacks.Register("agent_phone:marketplace:favorite", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    ...
    if data.favorite then
        Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_marketplace_favorites` (`account_id`, `listing_id`)
            SELECT ?, `id` FROM `agent_phone_marketplace_listings`
            WHERE `id` = ? AND `status` IN ('active', 'reserved')
        ]], { account.id, data.id })
    else
        Bridge.Database.Query(
            "DELETE FROM `agent_phone_marketplace_favorites` WHERE `account_id` = ? AND `listing_id` = ?",
            { account.id, data.id }
        )
    end
    return { success = true }
...
Bridge.Callbacks.Register("agent_phone:marketplace:counts", function(source)
    local account, error_response = require_account(source)
    if not account then return error_response end
    return { success = true, data = marketplace_counts(account.id) }
end)

Bridge.Callbacks.Register("agent_phone:marketplace:list-inquiries", function(source)
    local account, error_response = require_account(source)
    if not account then return error_response end
    local rows = Bridge.Database.Query([[
        SELECT q.`id`, q.`listing_id`, q.`seller_account_id`, q.`buyer_account_id`, q.`updated_at`,
            l.`title`, l.`price`, l.`price_type`, l.`status`,
            (SELECT image.`gradient` FROM `agent_phone_marketplace_images` image
            ...
            (SELECT message.`body` FROM `agent_phone_marketplace_messages` message
                WHERE message.`inquiry_id` = q.`id` ORDER BY message.`id` DESC LIMIT 1) AS `last_message`,
            ((SELECT COUNT(*) FROM `agent_phone_marketplace_messages` unread
                WHERE unread.`inquiry_id` = q.`id` AND unread.`sender_account_id` <> ?
                    AND unread.`read_at` IS NULL)
            + (SELECT COUNT(*) FROM `agent_phone_marketplace_offers` unread_offer
        ...
        FROM `agent_phone_marketplace_inquiries` q
        JOIN `agent_phone_marketplace_listings` l ON l.`id` = q.`listing_id`
        JOIN `agent_phone_accounts` other_account ON other_account.`id` =
            CASE WHEN q.`seller_account_id` = ? THEN q.`buyer_account_id` ELSE q.`seller_account_id` END
        LEFT JOIN `agent_phone_marketplace_profiles` other_profile ON other_profile.`account_id` = other_account.`id`
        WHERE q.`seller_account_id` = ? OR q.`buyer_account_id` = ?
...
Bridge.Callbacks.Register("agent_phone:marketplace:get-inquiry", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    ...
    local inquiries = Bridge.Database.Query([[
        ...
        FROM `agent_phone_marketplace_inquiries` q
        JOIN `agent_phone_marketplace_listings` l ON l.`id` = q.`listing_id`
        JOIN `agent_phone_accounts` seller ON seller.`id` = q.`seller_account_id`
        JOIN `agent_phone_accounts` buyer ON buyer.`id` = q.`buyer_account_id`
        LEFT JOIN `agent_phone_marketplace_profiles` seller_profile ON seller_profile.`account_id` = seller.`id`
        LEFT JOIN `agent_phone_marketplace_profiles` buyer_profile ON buyer_profile.`account_id` = buyer.`id`
    ...
    Bridge.Database.Query([[
        UPDATE `agent_phone_marketplace_messages`
        SET `read_at` = CURRENT_TIMESTAMP
        WHERE `inquiry_id` = ? AND `sender_account_id` <> ? AND `read_at` IS NULL
    ]], { data.id, account.id })
    Bridge.Database.Query([[
        UPDATE `agent_phone_marketplace_offers`
        SET `read_at` = CURRENT_TIMESTAMP
    ...
    local messages = Bridge.Database.Query([[
        ...
        FROM `agent_phone_marketplace_messages`
        WHERE `inquiry_id` = ?
        ORDER BY `id` ASC
        LIMIT ?
    ...
    local offers = Bridge.Database.Query([[
        ...
        FROM `agent_phone_marketplace_offers`
        WHERE `inquiry_id` = ?
        ORDER BY `id` ASC
        LIMIT ?
...
Bridge.Callbacks.Register("agent_phone:marketplace:send-message", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    if not AgentPhone.AllowOperation(source, "marketplace:message", 20, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    if type(data.inquiryId) == "string" and #data.inquiryId == 36 then
        local rows = Bridge.Database.Query([[
            SELECT q.*, l.`status` FROM `agent_phone_marketplace_inquiries` q
            JOIN `agent_phone_marketplace_listings` l ON l.`id` = q.`listing_id`
            WHERE q.`id` = ? AND (q.`seller_account_id` = ? OR q.`buyer_account_id` = ?)
            LIMIT 1
    ...
    elseif type(data.listingId) == "string" and #data.listingId == 36 then
        local listings = Bridge.Database.Query([[
            SELECT `id`, `seller_account_id`, `status` FROM `agent_phone_marketplace_listings`
        ...
        if listing and tonumber(listing.seller_account_id) ~= account.id then
            ...
            local existing = Bridge.Database.Query([[
                SELECT * FROM `agent_phone_marketplace_inquiries`
    ...
    Bridge.Database.Query([[
        INSERT INTO `agent_phone_marketplace_messages` (`inquiry_id`, `sender_account_id`, `body`)
        VALUES (?, ?, ?)
    ]], { inquiry.id, account.id, body })
    Bridge.Database.Query(
        "UPDATE `agent_phone_marketplace_inquiries` SET `updated_at` = CURRENT_TIMESTAMP WHERE `id` = ?",
        { inquiry.id }
    )
    notify_changed(account.id)
    AgentPhone.NotifyAccountDevices(other_account_id, "agent_phone:marketplace:new-message", {
...
Bridge.Callbacks.Register("agent_phone:marketplace:make-offer", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    if not AgentPhone.AllowOperation(source, "marketplace:offer", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    local offer_result = Bridge.Database.Query([[
        INSERT INTO `agent_phone_marketplace_offers` (`inquiry_id`, `proposer_account_id`, `amount`)
        VALUES (?, ?, ?)
    ]], { inquiry.id, account.id, amount })
    local offer_id = insert_id(offer_result)
    if not offer_id then
        error("[agent_phone] Database did not return a marketplace offer id.")
    end

    local revision = tonumber(inquiry.offer_revision) or 0
    local update_result = Bridge.Database.Query([[
        UPDATE `agent_phone_marketplace_inquiries`
    ...
    if affected_rows(update_result) == 0 then
        Bridge.Database.Query(
            "UPDATE `agent_phone_marketplace_offers` SET `status` = 'countered' WHERE `id` = ?",
            { offer_id }
        )
...
Bridge.Callbacks.Register("agent_phone:marketplace:respond-offer", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    if not AgentPhone.AllowOperation(source, "marketplace:offer-response", 10, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    notify_changed(account.id)
    AgentPhone.NotifyAccountDevices(proposer_account_id, "agent_phone:marketplace:new-message", {
        action = data.action,
...
Bridge.Callbacks.Register("agent_phone:marketplace:report", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    if not AgentPhone.AllowOperation(source, "marketplace:report", 5, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    local result = Bridge.Database.Query([[
        INSERT IGNORE INTO `agent_phone_marketplace_reports`
...
Bridge.Callbacks.Register("agent_phone:marketplace:block", function(source, data)
    local account, error_response = require_account(source)
    if not account then return error_response end
    ...
    local listings = Bridge.Database.Query(
        "SELECT `seller_account_id` FROM `agent_phone_marketplace_listings` WHERE `id` = ? LIMIT 1",
        { data.listingId }
    )
    ...
    if data.blocked == false then
        Bridge.Database.Query([[
            DELETE FROM `agent_phone_marketplace_blocks`
            WHERE `blocker_account_id` = ? AND `blocked_account_id` = ?
        ]], { account.id, blocked_id })
    else
        Bridge.Database.Query([[
            INSERT IGNORE INTO `agent_phone_marketplace_blocks` (`blocker_account_id`, `blocked_account_id`)
            VALUES (?, ?)
        ]], { account.id, blocked_id })
    end
    return { success = true }
