...
local function new_id()
    ...
    if not rows[1] or type(rows[1].id) ~= "string" then
        error("[agent_phone] Database did not generate a Local Pages id.")
    end
    return rows[1].id
end
...
local function validate_images(source, images)
    ...
    for index, image in ipairs(images) do
        ...
        then
            Bridge.Debug("warn", "[agent_phone] Rejected unowned Local Pages image from source %s.", tostring(source))
            return nil
        end
...
local function hydrate_posts(rows)
    for _, post in ipairs(rows) do
        ...
        if not created_at then
            error("[agent_phone] Local Pages post has an invalid created_at timestamp.")
        end
...
local function list_posts(account_id, where_clause, values, limit, offset)
    ...
    return hydrate_posts(Bridge.Database.Query(([[
        SELECT p.`id`, p.`title`, p.`body`, p.`category`, p.`district`, p.`source_type`,
            ...
            EXISTS(SELECT 1 FROM `agent_phone_pages_reactions` r WHERE r.`post_id` = p.`id`
                AND r.`account_id` = ? AND r.`kind` = 'like') AS `is_liked`,
            EXISTS(SELECT 1 FROM `agent_phone_pages_reactions` r WHERE r.`post_id` = p.`id`
                AND r.`account_id` = ? AND r.`kind` = 'save') AS `is_saved`,
            (SELECT COUNT(*) FROM `agent_phone_pages_reactions` r
                WHERE r.`post_id` = p.`id` AND r.`kind` = 'like') AS `like_count`,
            (SELECT i.`gradient` FROM `agent_phone_pages_images` i
                WHERE i.`post_id` = p.`id` ORDER BY i.`sort_order` LIMIT 1) AS `image`,
            m.`price` AS `citymarkt_price`
        FROM `agent_phone_pages_posts` p
        JOIN `agent_phone_accounts` a ON a.`id` = p.`account_id`
        LEFT JOIN `agent_phone_pages_profiles` profile ON profile.`account_id` = p.`account_id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = profile.`avatar_media_id`
        LEFT JOIN `agent_phone_marketplace_listings` m ON m.`id` = p.`citymarkt_listing_id`
        WHERE %s
...
local function optional_account(source)
    ...
    local rows = Bridge.Database.Query("SELECT `account_id` FROM `agent_phone_devices` WHERE `imei` = ? LIMIT 1", { session.imei })
...
local function profile_dto(account)
    local rows = Bridge.Database.Query([[
        SELECT profile.`handle`, profile.`bio`, profile.`avatar_media_id`, avatar.`url` AS `avatar_url`,
            (SELECT COUNT(*) FROM `agent_phone_pages_posts` post
                WHERE post.`account_id` = ?) AS `post_count`
        FROM `agent_phone_accounts` account
        LEFT JOIN `agent_phone_pages_profiles` profile ON profile.`account_id` = account.`id`
        LEFT JOIN `agent_phone_media` avatar ON avatar.`id` = profile.`avatar_media_id`
        WHERE account.`id` = ?
        LIMIT 1
    ...
    local row = rows[1]
    if not row then
        error(("[agent_phone] Could not load Local Pages profile account %s."):format(account.id))
    end
    return {
...
local function require_profile(account_id)
    local rows = Bridge.Database.Query([[
        ...
        FROM `agent_phone_pages_profiles`
        WHERE `account_id` = ?
        LIMIT 1
    ]], { account_id })
    if not rows[1] then
...
Bridge.Callbacks.Register("agent_phone:pages:profile", function(source)
...
Bridge.Callbacks.Register("agent_phone:pages:profile-save", function(source, data)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then return error_response end
    if not AgentPhone.AllowOperation(source, "pages:profile-save", 12, 60) then
    ...
    if avatar_media_id > 0 and not AgentPhoneMedia.ResolveOwnedMedia(source, tostring(avatar_media_id), "photo") then
        Bridge.Debug("warn", "[agent_phone] Rejected unowned Local Pages profile image from source %s.", tostring(source))
    ...
    if existing[1] then
        Bridge.Database.Query([[
            UPDATE `agent_phone_pages_profiles`
...
Bridge.Callbacks.Register("agent_phone:pages:list", function(source, data)
    ...
    if data.saved == true then
        ...
        where[#where + 1] = "EXISTS(SELECT 1 FROM `agent_phone_pages_reactions` sr WHERE sr.`post_id` = p.`id` AND sr.`account_id` = ? AND sr.`kind` = 'save')"
...
Bridge.Callbacks.Register("agent_phone:pages:list-own", function(source)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then return error_response end
...
Bridge.Callbacks.Register("agent_phone:pages:get", function(source, data)
...
Bridge.Callbacks.Register("agent_phone:pages:create", function(source, data)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then return error_response end
    local _, profile_error = require_profile(account.id)
    if profile_error then return profile_error end
    if not AgentPhone.AllowOperation(source, "pages:create", 6, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    local id = new_id()
    local statements = {{
        query = [[INSERT INTO `agent_phone_pages_posts`
    ...
    for index, image in ipairs(images) do
        statements[#statements + 1] = {
            query = "INSERT INTO `agent_phone_pages_images` (`post_id`, `media_id`, `gradient`, `sort_order`) VALUES (?, ?, ?, ?)",
...
Bridge.Callbacks.Register("agent_phone:pages:share-citymarkt", function(source, data)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then return error_response end
    local _, profile_error = require_profile(account.id)
    if profile_error then return profile_error end
    if not AgentPhone.AllowOperation(source, "pages:share-citymarkt", 3, 60) then
        return { success = false, error = "rate_limited" }
    end
    ...
    local listings = Bridge.Database.Query([[
        ...
        FROM `agent_phone_marketplace_listings`
    ...
    local existing = Bridge.Database.Query("SELECT `id` FROM `agent_phone_pages_posts` WHERE `citymarkt_listing_id` = ? LIMIT 1", { listing.id })
    if existing[1] then return { success = false, error = "citymarkt_already_shared" } end
    local daily = Bridge.Database.Query([[
        SELECT COUNT(*) AS `count` FROM `agent_phone_pages_posts`
    ...
    local images = Bridge.Database.Query([[
        SELECT `media_id`, `gradient`, `sort_order` FROM `agent_phone_marketplace_images`
...
Bridge.Callbacks.Register("agent_phone:pages:react", function(source, data)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then return error_response end
    if not AgentPhone.AllowOperation(source, "pages:react", 30, 60) then
        return { success = false, error = "rate_limited" }
    end
    if type(data) ~= "table" or type(data.id) ~= "string" or (data.kind ~= "like" and data.kind ~= "save") or type(data.active) ~= "boolean" then
        return { success = false, error = "invalid_request" }
    end
    local posts = Bridge.Database.Query("SELECT `id` FROM `agent_phone_pages_posts` WHERE `id` = ? LIMIT 1", { data.id })
    if not posts[1] then return { success = false, error = "post_not_found" } end
    if data.active then
        Bridge.Database.Query("INSERT IGNORE INTO `agent_phone_pages_reactions` (`post_id`, `account_id`, `kind`) VALUES (?, ?, ?)", { data.id, account.id, data.kind })
    else
        Bridge.Database.Query("DELETE FROM `agent_phone_pages_reactions` WHERE `post_id` = ? AND `account_id` = ? AND `kind` = ?", { data.id, account.id, data.kind })
    end
    return { success = true }
end)

Bridge.Callbacks.Register("agent_phone:pages:delete", function(source, data)
    local account, error_response = AgentPhone.RequireAccount(source)
    if not account then return error_response end
    if type(data) ~= "table" or type(data.id) ~= "string" then
        return { success = false, error = "post_not_found" }
    end
    local result = Bridge.Database.Query("DELETE FROM `agent_phone_pages_posts` WHERE `id` = ? AND `account_id` = ?", { data.id, account.id })
