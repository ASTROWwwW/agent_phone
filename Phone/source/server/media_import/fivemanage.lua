local function api_key(website)
    return AgentPhoneMediaProviderConfig.FiveManageApiKey(website.ApiKey)
end
...
local function resolve_file(website, external_id)
    ...
    local response = AgentPhoneMediaImport.HttpRequest(
        ("%s/%s"):format(provider_url, AgentPhoneMediaImport.UrlEncode(external_id)),
...
local function probe_public_url(website, url)
    ...
    local response = AgentPhoneMediaImport.HttpRequest(url, {}, timeout, "HEAD")
    ...
    then
        Bridge.Debug(
            "debug",
            "[agent_phone] FiveManage HEAD probe did not provide usable metadata; trying a one-byte range request."
        )
        response = AgentPhoneMediaImport.HttpRequest(url, { ["Range"] = "bytes=0-0" }, timeout)
        content_type = AgentPhoneMediaImport.ResponseHeader(response.headers, "content-type")
        local content_range = AgentPhoneMediaImport.ResponseHeader(response.headers, "content-range")
        content_length = type(content_range) == "string" and tonumber(content_range:match("/(%d+)$"))
            or tonumber(AgentPhoneMediaImport.ResponseHeader(response.headers, "content-length"))
    end
    ...
    if response.status < 200 or response.status >= 300 then
        Bridge.Debug(
            "warn",
            "[agent_phone] FiveManage public URL probe returned HTTP %s.",
    ...
    local external_id = ("url:%08x%08x"):format(
        ...
        joaat("agent_phone:" .. url) & 0xffffffff
    )
    return {
...
AgentPhoneMediaImport.RegisterAdapter("fivemanage", {
    ...
    List = function(website, requested_type, page, limit)
        ...
        local url = ("%s?page=%s&limit=%s&type=%s&path=%s"):format(
            ...
            limit,
            provider_type,
            AgentPhoneMediaImport.UrlEncode(website.Path)
        )
        local response = AgentPhoneMediaImport.HttpRequest(
            url,
            { ["Authorization"] = api_key(website) },
    ...
    ResolveUrl = function(website, url)
        ...
        if external_id then
            ...
            if file then
                Bridge.Debug(
                    "debug",
                    "[agent_phone] FiveManage URL resolved through authenticated metadata for file '%s'.",
                    external_id
                )
                return file
            end
            ...
            Bridge.Debug(
                "debug",
                "[agent_phone] FiveManage metadata lookup for file '%s' failed with '%s'; probing the public URL.",
                external_id,
