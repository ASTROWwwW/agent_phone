...
AgentPhoneApps = AgentPhoneApps or {}
AgentPhoneApps.ProtocolVersion = 1
...
local function normalize_manifest(folder, definition)
    ...
    if definition.schemaVersion ~= AgentPhoneApps.ProtocolVersion then
    ...
    return {
        ...
        schemaVersion = AgentPhoneApps.ProtocolVersion,
...
function AgentPhoneApps.RegisterManifest(folder, definition)
    ...
    if not manifest then
        error(("[agent_phone] Invalid custom app manifest '%s': %s"):format(tostring(folder), error_code), 2)
    end

    if bundled_manifests_by_id[manifest.id] then
        error(("[agent_phone] Duplicate bundled custom app id '%s'."):format(manifest.id), 2)
    end
...
function AgentPhoneApps.GetBundledManifests()
    return bundled_manifests
end

function AgentPhoneApps.GetBundledManifest(app_id)
    return bundled_manifests_by_id[app_id]
end

AgentPhoneApps.AllowedCategories = ALLOWED_CATEGORIES
AgentPhoneApps.AllowedOrientations = ALLOWED_ORIENTATIONS
AgentPhoneApps.AllowedPermissions = ALLOWED_PERMISSIONS
AgentPhoneApps.ReservedAppIds = RESERVED_APP_IDS
AgentPhoneApps.ValidateAppId = validate_app_id
AgentPhoneApps.ValidateLocalizedText = validate_localized_text
AgentPhoneApps.ValidatePermissions = validate_permissions
