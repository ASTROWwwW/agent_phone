...
function AgentPhoneSimNumber.ValidateConfiguration(length, prefix)
...
function AgentPhoneSimNumber.Normalize(value, length, prefix)
    if type(value) ~= "string" and type(value) ~= "number" then
        return nil
    end
    if not AgentPhoneSimNumber.ValidateConfiguration(length, prefix) then
        return nil
    end
...
function AgentPhoneSimNumber.NormalizeService(value, maximum_length)
    if type(value) ~= "string" and type(value) ~= "number" then
        return nil
    end
    local number = tostring(value):gsub("%D", "")
...
function AgentPhoneSimNumber.FromEntropy(entropy, length, prefix)
...
function AgentPhoneSimNumber.Format(value, groups, length, prefix)
    local number = AgentPhoneSimNumber.Normalize(value, length, prefix)
    if not number then
...
function AgentPhoneSimNumber.Reserve(entropy, accept, length, prefix)
    for _ = 1, 20 do
        local candidate = AgentPhoneSimNumber.FromEntropy(entropy(), length, prefix)
