...
function AgentPhoneNavigation.Close(app_id)
    if not AgentPhoneClient.GetState().open then
        return false, "phone_closed"
    end
    if app_id ~= nil then
...
function AgentPhoneNavigation.GetState()
...
function AgentPhoneNavigation.IsDataLoaded()
    return data_loaded
end

function AgentPhoneNavigation.IsInstalled(app_id)
    local normalized_app_id = normalize_app_id(app_id)
    if not normalized_app_id then
        return false
    end
...
function AgentPhoneNavigation.GetCurrent(app_id)
    if not AgentPhoneClient.GetState().open then
        if app_id == nil then
            return nil
        end
...
function AgentPhoneNavigation.Reset()
...
AddEventHandler("agent_phone:client:phoneToggled", function(open)
    if not open then
