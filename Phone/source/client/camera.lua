...
local function update_camera_focus_claim()
    TriggerEvent("agent_phone:client:setCameraFocus", {
    ...
    CreateThread(function()
        while camera_state.active do
            ...
            if not camera_state.walkable then
                local passthrough_pressed = AgentPhoneFocus.IsHoldToLookPressed()
...
AddEventHandler("agent_phone:client:cameraFocusApplied", function(data)
    if type(data) ~= "table"
...
local function set_camera_active(active)
    ...
    TriggerEvent("agent_phone:client:cameraActiveChanged", active)
    if active then
        ...
        apply_camera_view()
        TriggerEvent("agent_phone:animation:camera", {
            active = true,
...
AgentPhoneCamera.DisableWalkable = disable_walkable_camera
AgentPhoneCamera.EnableWalkable = enable_walkable_camera
AgentPhoneCamera.GetState = get_camera_state
AgentPhoneCamera.SetFlashlight = set_flash_enabled
AgentPhoneCamera.SetSelfie = set_front_camera
AgentPhoneCamera.ToggleFrozen = toggle_camera_frozen
...
RegisterNUICallback("media:requestUpload", function(data, cb)
    ...
    Bridge.Debug(
        "debug",
        "[agent_phone][media-debug] NUI requested an upload (correlation=%s, type=%s).",
        tostring(data.correlationId),
        tostring(data.mediaType),
        { notice = true }
    )
    TriggerServerEvent("agent_phone:media:request-upload", data)
    cb({ success = true })
end)
...
RegisterNUICallback("media:completeUpload", function(data, cb)
    ...
    Bridge.Debug(
        "debug",
        "[agent_phone][media-debug] NUI completed the provider upload (correlation=%s, remote-id=%s, url=%s, original-url=%s).",
        tostring(data.correlationId),
    ...
    TriggerServerEvent("agent_phone:media:complete-upload", data)
    cb({ success = true })
end)
...
RegisterNUICallback("media:cancelUpload", function(data, cb)
    ...
    TriggerServerEvent("agent_phone:media:cancel-upload", data)
    cb({ success = true })
end)
...
RegisterNUICallback("media:failUpload", function(data, cb)
    ...
    Bridge.Debug(
        "error",
        "[agent_phone][media-debug] NUI reported an upload failure (correlation=%s, error=%s, stage=%s, status=%s, detail=%s).",
        tostring(data.correlationId),
        tostring(data.error),
    ...
    TriggerServerEvent("agent_phone:media:fail-upload", data)
    cb({ success = true })
end)
...
RegisterNUICallback("gallery:delete", function(data, cb)
    ...
    TriggerServerEvent("agent_phone:media:delete", data)
    cb({ success = true })
end)
...
RegisterNUICallback("gallery:delete-many", function(data, cb)
    ...
    TriggerServerEvent("agent_phone:media:delete-many", data)
    cb({ success = true })
end)
...
RegisterNUICallback("memos:requestUpload", function(data, cb)
    ...
    TriggerServerEvent("agent_phone:memos:request-upload", data)
    cb({ success = true })
end)
...
RegisterNUICallback("memos:completeUpload", function(data, cb)
    ...
    TriggerServerEvent("agent_phone:memos:complete-upload", data)
    cb({ success = true })
end)
...
RegisterNUICallback("memos:cancelUpload", function(data, cb)
    ...
    TriggerServerEvent("agent_phone:memos:cancel-upload", data)
    cb({ success = true })
end)
...
RegisterNUICallback("memos:failUpload", function(data, cb)
    ...
    TriggerServerEvent("agent_phone:memos:fail-upload", data)
    cb({ success = true })
end)

RegisterNetEvent("agent_phone:media:upload-ready", function(data)
    Bridge.Debug(
        "debug",
        "[agent_phone][media-debug] Client received upload-ready (correlation=%s, type=%s, presigned-url=%s).",
...
RegisterNetEvent("agent_phone:media:upload-result", function(data)
    Bridge.Debug(
        "debug",
        "[agent_phone][media-debug] Client received upload-result (correlation=%s, success=%s, error=%s).",
        tostring(type(data) == "table" and data.correlationId),
...
RegisterNetEvent("agent_phone:media:delete-result", function(data)
    SendNUIMessage({ type = "media:deleteResult", data = data })
end)

RegisterNetEvent("agent_phone:media:delete-many-result", function(data)
    SendNUIMessage({ type = "media:deleteManyResult", data = data })
end)

RegisterNetEvent("agent_phone:memos:upload-ready", function(data)
    SendNUIMessage({ type = "memos:uploadReady", data = data })
end)

RegisterNetEvent("agent_phone:memos:upload-result", function(data)
    SendNUIMessage({ type = "memos:uploadResult", data = data })
end)

AddEventHandler("agent_phone:nuiClosed", function()
    set_flash_enabled(false)
    set_camera_active(false)
end)
