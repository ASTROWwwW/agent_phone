...
local function load_model(model_hash)
    ...
    while not HasModelLoaded(model_hash) do
        if GetGameTimer() >= deadline then
            Bridge.Debug(
                "error",
                "[agent_phone] Timed out loading phone prop model '%s'.",
...
local function load_animation_dictionary(dictionary)
    ...
    while not HasAnimDictLoaded(dictionary) do
        if GetGameTimer() >= deadline then
            Bridge.Debug(
                "error",
                "[agent_phone] Timed out loading animation dictionary '%s'.",
                dictionary
            )
            return false
        end
...
local function delete_phone_prop()
    ...
    if DoesEntityExist(prop) then
        Bridge.Debug("error", "[agent_phone] Failed to delete the attached phone prop.")
        return false
    end
...
local function ensure_phone_prop(ped, revision)
    ...
    if not prop or prop == 0 or not DoesEntityExist(prop) then
        Bridge.Debug("error", "[agent_phone] Failed to create the phone prop.")
        return false
    end
...
AddEventHandler("agent_phone:animation:phone", function(open)
    if type(open) ~= "boolean" then
        Bridge.Debug("warn", "[agent_phone] Ignored invalid phone animation state.")
        return
    end
...
AddEventHandler("agent_phone:animation:call", function(data)
    if type(data) ~= "table" or type(data.state) ~= "string" or type(data.direction) ~= "string" then
        Bridge.Debug("warn", "[agent_phone] Ignored invalid call animation state.")
        return
    end
...
AddEventHandler("agent_phone:animation:camera", function(data)
    if type(data) ~= "table" or type(data.active) ~= "boolean" then
        Bridge.Debug("warn", "[agent_phone] Ignored invalid camera animation state.")
        return
    end
...
AddEventHandler("agent_phone:animation:reset", reset_animation_state)
