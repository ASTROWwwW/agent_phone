...
local function flush_activity()
    ...
    TriggerServerEvent("agent_phone:health:record-activity", {
        steps = pending_steps,
...
RegisterNUICallback("health:overview", function(data, cb)
    local result = Bridge.Callbacks.Trigger("agent_phone:health:overview", data or {})
    cb(result or { success = false, error = "request_failed" })
end)

RegisterNUICallback("health:save-profile", function(data, cb)
    local result = Bridge.Callbacks.Trigger("agent_phone:health:save-profile", data or {})
    cb(result or { success = false, error = "request_failed" })
end)

RegisterNetEvent("agent_phone:health:changed", function()
