...
local function server_prepare(action, data)
    ...
    return Bridge.Callbacks.Trigger("agent_phone:housing:prepare", request)
end

local function suspend_phone()
    TriggerEvent("agent_phone:client:setSuspended", true)
...
local function resume_phone()
    ...
    TriggerEvent("agent_phone:client:setSuspended", false)
...
local function run_camera(provider_name, camera_data)
    ...
    if not coords.x or not coords.y or not coords.z or not rotation.x or not rotation.z then
        Bridge.Debug("error", "[agent_phone] Rejected invalid housing camera data.")
        return
    end
    ...
    if not camera or camera == 0 then
        Bridge.Debug("error", "[agent_phone] Failed to create housing camera.")
        return
    end
    ...
    if close_phone_after_camera then
        TriggerEvent("agent_phone:client:forceClose")
    end
end
...
RegisterNUICallback("housing:overview", function(data, cb)
    ...
    local result = Bridge.Callbacks.Trigger("agent_phone:housing:overview", {})
