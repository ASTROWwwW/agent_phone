...
local function normalize_notification(notification)
    ...
    local valid_app_id = AgentPhoneApps.ValidateAppId(app_id)
    if not valid_app_id or not AgentPhoneApps.CanReceiveNotification(app_id) then
...
local function normalize_server_device(device)
    if type(device) ~= "table"
        or not AgentPhoneImei.IsValid(device.imei)
    then
...
function AgentPhoneNotifications.Show(notification)
...
function AgentPhoneNotifications.Send(notification)
    local normalized, error_code = normalize_notification(notification)
    if not normalized then
        return false, error_code
    end

    local result = Bridge.Callbacks.Trigger("agent_phone:notifications:self", normalized)
...
RegisterNetEvent("agent_phone:notifications:show", function(notification)
    if source ~= 65535 then
        Bridge.Debug("warn", "[agent_phone] Rejected a client-triggered notification delivery event.")
        return
    end
    ...
    if not normalized or not device then
        Bridge.Debug(
            "warn",
            "[agent_phone] Rejected an invalid server notification: %s.",
    ...
    show_normalized(normalized)
    TriggerEvent("agent_phone:client:pushNotification", {
...
local phone_locale = AgentPhoneLocales.Resolve(Config.Bridge.Locale)
local app_locales = phone_locale.Nui.Apps

AddEventHandler("agent_phone:configurator:updated", function()
    phone_locale = AgentPhoneLocales.Resolve(Config.Bridge.Locale)
    app_locales = phone_locale.Nui.Apps
end)

RegisterNetEvent("agent_phone:mail:new", function(data)
...
RegisterNetEvent("agent_phone:fliptok:command-feedback", function(data)
    Bridge.Framework.Notify("FlipTok", data.message, data.notificationType, 5000)
end)

RegisterNetEvent("agent_phone:fliptok:new", function(data)
...
RegisterNetEvent("agent_phone:picstagram:command-feedback", function(data)
    Bridge.Framework.Notify("Picstagram", data.message, data.notificationType, 5000)
end)

RegisterNetEvent("agent_phone:picstagram:new", function(data)
...
RegisterNetEvent("agent_phone:feather:new", function(data)
...
RegisterNetEvent("agent_phone:marketplace:new-message", function(data)
...
RegisterNetEvent("agent_phone:calendar:reminder", function(data)
...
RegisterNetEvent("agent_phone:flare:match", function(data)
...
RegisterNetEvent("agent_phone:flare:message", function(data)
    local locale = app_locales.flare
    data.title = locale.name
...
RegisterNetEvent("agent_phone:billing:new", function(data)
...
RegisterNetEvent("agent_phone:darkchat:new", function(data)
