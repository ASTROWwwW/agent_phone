...
local function request(name, data)
    local result = Bridge.Callbacks.Trigger("agent_phone:radio:" .. name, data or {})
...
AddEventHandler("agent_phone:client:radioProviderUpdated", function()
...
RegisterNetEvent("agent_phone:radio:members", function(data)
    if type(data) ~= "table" then
        return
    end
...
RegisterNetEvent("agent_phone:radio:notification", function(data)
    if not radio_settings.notifications or current_primary <= 0 then
        return
    end
    local locale = AgentPhoneLocales.Resolve(Config.Bridge.Locale).Nui.Apps.radio
    ...
    if result.success then
        ...
        if radio_settings.autoRejoin and tonumber(data.savedFrequency) and tonumber(data.savedFrequency) > 0 then
            ...
            if not joined.success then
                Bridge.Debug("warn", "[agent_phone] Radio auto-rejoin failed: %s", tostring(joined.error))
            end
        end
    end
