...
for index = 1, #server_callbacks do
    ...
    RegisterNUICallback(callback_name, function(data, cb)
        ...
        local result = Bridge.Callbacks.Trigger("agent_phone:" .. callback_name, data)
        if not result then
...
RegisterNetEvent("agent_phone:agentride:changed", function(data)
    normalize_state(data)
    SendNUIMessage({ type = "agentride:changed", data = data })
end)
