...
RegisterNetEvent("agent_phone:payphone:visual:start", function(data)
...
RegisterNetEvent("agent_phone:payphone:visual:stop", function(data)
...
local locale = AgentPhoneLocales.Resolve(Config.Bridge.Locale)
...
local function spawn_custom_payphones()
    ...
    if not configured_models[model_hash] then
        Bridge.Debug(
            "error",
            "[agent_phone] Config.Payphones.CustomProp must also be listed in Config.Payphones.Props.",
            { always = true }
        )
        return
    end
    if not load_model(model_hash) then
        Bridge.Debug("error", "[agent_phone] The configured custom payphone prop could not be loaded.", {
            always = true,
        })
        return
    end
    ...
    for index, coords in ipairs(locations) do
        ...
        if not heading then
            Bridge.Debug(
                "error",
                "[agent_phone] Ignored invalid Config.Payphones.CustomLocations entry %s; use vector4(x, y, z, heading).",
                index,
                { always = true }
            )
        else
            ...
            if entity == 0 or not DoesEntityExist(entity) then
                Bridge.Debug(
                    "error",
                    "[agent_phone] Failed to spawn custom payphone %s.",
                    index,
                    { always = true }
                )
            else
...
AddEventHandler("agent_phone:configurator:updated", function()
    locale = AgentPhoneLocales.Resolve(Config.Bridge.Locale)
    refresh_configured_models()
...
local function join_call_voice(channel)
    ...
    if not Bridge.Calls.Join(next_channel) then
        Bridge.Debug("error", "[agent_phone] The payphone call could not join the configured voice provider.", {
            always = true,
        })
        return false
    end
...
local function start_call_visuals()
    ...
    if not load_model(replacement_hash) or not load_animation(animation.Dictionary) then
        Bridge.Debug("error", "[agent_phone] The payphone animation assets could not be loaded.")
        visuals_starting = false
    ...
    if replacement == 0 or not DoesEntityExist(replacement) then
        Bridge.Debug("error", "[agent_phone] The animated payphone replacement prop could not be created.")
        visuals_starting = false
        SetModelAsNoLongerNeeded(replacement_hash)
        RemoveAnimDict(animation.Dictionary)
        return
    ...
    if replacement_network_id == 0 then
        Bridge.Debug("error", "[agent_phone] The animated payphone replacement prop is not networked.")
    ...
    if not network_animation_scene or network_animation_scene == -1 then
        Bridge.Debug("error", "[agent_phone] The payphone network synchronized scene could not be created.")
        network_animation_scene = nil
        stop_call_visuals()
        return
    end
    ...
    if not animation_scene or animation_scene == -1 then
        Bridge.Debug("error", "[agent_phone] The local handle for the payphone network scene was not created.")
        animation_scene = nil
        stop_call_visuals()
        return
    end
...
local function close_payphone()
    ...
    payphone_open = false
    if was_open then
        TriggerEvent("agent_phone:client:setPayphoneFocus", false)
...
local function open_payphone(booth)
    ...
    active_booth = booth
    payphone_open = true
    TriggerEvent("agent_phone:client:setPayphoneFocus", true)
    SendNUIMessage({
        type = "payphone:open",
...
RegisterNUICallback("payphone:dial", function(data, cb)
    ...
    local result = Bridge.Callbacks.Trigger("agent_phone:payphone:dial", payload)
...
RegisterNUICallback("payphone:hangup", function(data, cb)
    ...
    local result = Bridge.Callbacks.Trigger("agent_phone:payphone:hangup", { id = active_call_id })
    cb(type(result) == "table" and result or { success = false, error = "request_failed" })
end)
...
RegisterNUICallback("payphone:close", function(data, cb)
    ...
    if active_call_id then
        Bridge.Callbacks.Trigger("agent_phone:payphone:hangup", { id = active_call_id })
    end
    close_payphone()
    cb({ success = true })
end)

RegisterNetEvent("agent_phone:payphone:state", function(data)
...
AddEventHandler("agent_phone:client:nuiReady", function()
    ...
    while true do
        ...
        if call_id and booth then
            ...
            if IsControlJustReleased(0, 38) and not hangup_requested then
                hangup_requested = true
                Bridge.Callbacks.Trigger("agent_phone:payphone:hangup", { id = call_id })
            end
