...
local function refresh_focus_configuration()
    ...
    if enabled and (
        ...
        error("[agent_phone] Config.Phone.HoldToLook.Control must be a non-negative whole control index.")
    end
...
function AgentPhoneFocus.IsHoldToLookPressed()
...
function AgentPhoneFocus.ApplyFocusedControls()
...
function AgentPhoneFocus.ApplyGameInputControls(block_look)
...
function AgentPhoneFocus.Resolve(state)
...
function AgentPhoneFocus.Reapply()
    ...
    local focus = AgentPhoneFocus.Resolve(state)
    ...
    TriggerEvent("agent_phone:client:cameraFocusApplied", {
...
function AgentPhoneFocus.SetPhone(open, cursor_disabled)
...
function AgentPhoneFocus.SetAdminPanel(open)
...
function AgentPhoneFocus.SetCall(active)
    state.call_focus = active == true
    AgentPhoneFocus.Reapply()
end

function AgentPhoneFocus.SetSimPicker(active)
    state.sim_picker_open = active == true
    AgentPhoneFocus.Reapply()
end

function AgentPhoneFocus.SetTextInputFocused(active)
...
function AgentPhoneFocus.SetExternalGameInput(owner_resource, allow_game_input)
...
function AgentPhoneFocus.Reset()
    ...
    while true do
        if game_input or block_game then
            local look_passthrough = game_input
                ...
                and AgentPhoneFocus.IsHoldToLookPressed()
            if look_passthrough ~= state.look_passthrough then
                state.look_passthrough = look_passthrough
                AgentPhoneFocus.Reapply()
            end
            if block_game then
                AgentPhoneFocus.ApplyFocusedControls()
            else
                AgentPhoneFocus.ApplyGameInputControls(block_look)
            end
            Wait(0)
        else
            Wait(250)
...
AddEventHandler("agent_phone:client:setSuspended", function(suspended)
    state.activity_suspended = suspended == true
    AgentPhoneFocus.Reapply()
end)

AddEventHandler("agent_phone:client:setPayphoneFocus", function(focused)
    state.payphone_focus = focused == true
    AgentPhoneFocus.Reapply()
end)

AddEventHandler("agent_phone:client:setCameraFocus", function(data)
    if type(data) ~= "table" or type(data.active) ~= "boolean" or type(data.nuiFocused) ~= "boolean" then
        Bridge.Debug("error", "[agent_phone] Rejected invalid camera focus claim.")
        return
    end
