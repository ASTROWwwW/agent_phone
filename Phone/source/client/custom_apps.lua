...
local function debug_custom_app(area, message, ...)
    ...
    Bridge.Debug(
        "debug",
        ("[agent_phone][custom-apps][%s] %s"):format(area, formatted),
        { notice = true }
    )
end

AgentPhoneApps.Debug = debug_custom_app
...
local function normalize_external_definition_value(owner_resource, adapter_resource, definition)
    ...
    if definition.schemaVersion ~= nil and definition.schemaVersion ~= AgentPhoneApps.ProtocolVersion then
        return nil, "unsupported_schema_version"
    end
    local valid_id, id_error = AgentPhoneApps.ValidateAppId(definition.id)
    if not valid_id then
        return nil, id_error
    end
    if AgentPhoneApps.ReservedAppIds[definition.id] then
        return nil, "reserved_app_id"
    end

    local name, name_error = AgentPhoneApps.ValidateLocalizedText(
        definition.name,
        "invalid_name",
        64,
        true
    ...
    local description, description_error = AgentPhoneApps.ValidateLocalizedText(
    ...
    if not AgentPhoneApps.AllowedCategories[category] then
    ...
    local permissions, permissions_error = AgentPhoneApps.ValidatePermissions(definition.permissions)
    if not permissions then
        return nil, permissions_error
    end

    local orientation = definition.orientation or "portrait"
    if not AgentPhoneApps.AllowedOrientations[orientation] then
...
local function register_app(app)
    ...
    if existing then
        ...
        Bridge.Debug(
            "warn",
            "[agent_phone] Rejected duplicate custom app '%s' from '%s'; it is already owned by '%s'.",
            app_id,
            app.ownerResource,
            existing.ownerResource
        )
...
local function remove_registered_app(app_id, invoke_close_hook, synchronize)
    ...
    TriggerEvent("agent_phone:client:customAppRemoved", app.ownerResource, app_id)
    if synchronize then
        sync_catalog()
    end
    debug_custom_app(
...
local function verify_owned_app(owner_resource, app_id, adapter_resource)
    local valid_id, id_error = AgentPhoneApps.ValidateAppId(app_id)
    if not valid_id then
        return nil, id_error
    end
...
local function get_custom_app_capabilities()
    return {
        ...
        protocolVersion = AgentPhoneApps.ProtocolVersion,
    }
end
...
function AgentPhoneApps.CanReceiveNotification(app_id)
    local valid_id = AgentPhoneApps.ValidateAppId(app_id)
    if not valid_id then
        return false
    end
    return AgentPhoneApps.ReservedAppIds[app_id] == true or apps_by_id[app_id] ~= nil
end

AgentPhoneApps.SendCatalog = send_catalog
AgentPhoneApps.SetPhoneOpen = set_phone_open
AgentPhoneApps.RegisterClientHooks = register_bundled_client_hooks

local function refresh_bundled_apps()
    local manifests = Config.CustomApps.Enabled and Config.CustomApps.BundledApps
        and AgentPhoneApps.GetBundledManifests()
        or {}
    local desired_ids = {}
...
AddEventHandler("agent_phone:configurator:updated", refresh_bundled_apps)

AgentPhoneApps.CompatibilityCore = {
...
AgentPhoneApps.ClientPublicApi = {
...
RegisterNetEvent("agent_phone:custom-app:message", function(owner_resource, app_id, payload)
    if source ~= 65535 then
        Bridge.Debug("warn", "[agent_phone] Rejected a client-triggered custom app message.")
        return
    end
    ...
    if not success then
        Bridge.Debug(
            "warn",
            "[agent_phone] Server custom app message for %s could not be delivered: %s.",
            tostring(app_id),
...
RegisterNUICallback("custom-app:lifecycle", function(data, cb)
    ...
    local valid_id, id_error = AgentPhoneApps.ValidateAppId(data.appId)
    if not valid_id then
