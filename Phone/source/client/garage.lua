...
RegisterNUICallback("garage:vehicles", function(data, cb)
    ...
    local result = Bridge.Callbacks.Trigger("agent_phone:garage:vehicles", data)
...
local function fail_valet(error_code, server_cancel)
    ...
    if server_cancel then
        Bridge.Callbacks.Trigger("agent_phone:garage:valet-cancel", { orderId = order_id })
    end
...
local function run_valet_delivery(order)
    ...
    local completion = Bridge.Callbacks.Trigger(
        "agent_phone:garage:valet-complete",
        {
            orderId = current_valet.order_id,
...
RegisterNUICallback("garage:valet-request", function(data, cb)
    ...
    local result = Bridge.Callbacks.Trigger("agent_phone:garage:valet-request", data)
...
RegisterNUICallback("garage:valet-cancel", function(_, cb)
    ...
    local order_id = current_valet.order_id
    local result = Bridge.Callbacks.Trigger("agent_phone:garage:valet-cancel", { orderId = order_id })
...
RegisterNetEvent("agent_phone:garage:valet-aborted", function(error_code)
    if current_valet then
