...
function maskValue(value, path) {
  ...
  if (value?.__agentType === 'map' && Array.isArray(value.entries)) {
    return {
      __agentType: 'map',
      entries: value.entries.map((entry) => ({
        key: entry.key,
...
function companyDefinitionEntryDefault(definitions) {
  ...
  return {
    ...
    Icon: 'building',
    Job: '',
    Location: { __agentType: 'vector3', x: 0, y: 0, z: 0 },
    LocationLabel: '',
...
function buildStructure(value, scope, path) {
  ...
  if (/^vector[234]$/.test(value?.__agentType ?? '')) {
    return { kind: 'vector', vectorType: value.__agentType }
  }
