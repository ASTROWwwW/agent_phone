...
describe('server startup contracts', () => {
  it('registers the production phone-open callback before the database migration runs', () => {
    expect(
      phoneServer.indexOf(
        'Bridge.Callbacks.Register("agent_phone:device:open-request"',
      ),
    ).toBeLessThan(
      phoneServer.indexOf('Bridge.Database.AfterMigration("agent_phone"'),
    )
  })
  ...
  it('registers the development-open callback before the database migration runs', () => {
    ...
    expect(
      phoneServer.indexOf(
        'Bridge.Callbacks.Register("agent_phone:device:development-open"',
      ),
    ).toBeLessThan(
      phoneServer.indexOf('Bridge.Database.AfterMigration("agent_phone"'),
    )
