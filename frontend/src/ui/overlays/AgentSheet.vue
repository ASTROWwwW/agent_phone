...
const panelStyle = computed(() =>
  props.swipeToClose && dragOffset.value > 0
    ? { '--agent-sheet-drag-offset': `${dragOffset.value}px` }
    : undefined,
)
...
function startDrag(event: PointerEvent): void {
  ...
  const grabber =
    target instanceof Element
      ? target.closest('.agent-sheet__grabber')
      : undefined
  const extendedHandle =
    target instanceof Element
      ? target.closest('[data-agent-sheet-drag-handle]')
      : undefined
...
<template>
  <Transition
    name="agent-sheet-rise"
    :duration="{ enter: 400, leave: 400 }"
    appear
  >
    <div v-if="opened" ref="root" v-bind="$attrs" class="agent-sheet">
      <div
        v-if="backdrop"
        class="agent-overlay-backdrop"
        aria-hidden="true"
      ...
      <component
        :is="component"
        ref="panel"
        class="agent-sheet__panel"
        :class="{
          'agent-sheet__panel--dragging': isDragging,
          'agent-sheet__panel--settling': isSettling,
        }"
        :style="panelStyle"
        :role="effectiveRole"
        :aria-modal="
