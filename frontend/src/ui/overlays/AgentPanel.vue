...
<template>
  <Transition :name="`agent-panel-${side}`" appear>
    <div
      v-if="opened"
      ref="root"
      v-bind="$attrs"
      class="agent-panel"
      :class="{
        'agent-panel--floating': floating,
        'agent-panel--modal': modal,
      }"
    >
      <div
        v-if="backdrop"
        class="agent-overlay-backdrop agent-panel__backdrop"
        aria-hidden="true"
      ...
      <component
        :is="component"
        ref="panel"
        class="agent-panel__panel agent-glass-surface"
        :class="[
          sizeClass,
          `agent-panel__panel--${side}`,
          { 'agent-panel__panel--floating': floating },
        ]"
        :style="panelStyle"
        :role="effectiveRole"
