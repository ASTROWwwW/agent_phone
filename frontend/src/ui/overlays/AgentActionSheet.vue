...
const effectiveRole = computed(() => {
  ...
  return isDialog && !props.ariaLabel && !props.label && !props.ariaLabelledby
    ? 'presentation'
    : role
})
...
<template>
  <Transition
    name="agent-action-rise"
    :duration="{ enter: 300, leave: 300 }"
    appear
  >
    <div v-if="opened" ref="root" v-bind="$attrs" class="agent-action-sheet">
      <div
        v-if="backdrop"
      ...
      <component
        :is="component"
        ref="panel"
        class="agent-action-sheet__panel"
        :role="effectiveRole"
        :aria-modal="
        ...
        "
        :aria-label="
          effectiveRole ? ariaLabel || label || undefined : undefined
        "
        :aria-labelledby="
