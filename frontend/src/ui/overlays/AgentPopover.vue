...
function reportPositionError(reason: PositionError): void {
  ...
  console.warn(`[agent_phone] AgentPopover positioning failed: ${reason}.`)
...
<template>
  ...
  >
    <div
      ...
      class="agent-popover"
      :class="{
        'agent-popover--modal': modal,
        'agent-popover--positioned': position,
      }"
    >
      <div
        v-if="backdrop"
        class="agent-overlay-backdrop agent-popover__backdrop"
        aria-hidden="true"
      ...
      <component
        :is="component"
        ref="panel"
        class="agent-popover__panel agent-glass-surface"
        :class="
          position ? `agent-popover__panel--${position.placement}` : undefined
        "
        :style="panelStyle"
        :role="effectiveRole"
      ...
      >
        <div
          ...
          class="agent-popover__arrow-wrap"
          :class="[angleClass, `agent-popover__arrow-wrap--${arrowSide}`]"
          :style="arrowStyle"
          aria-hidden="true"
        >
          <span class="agent-popover__arrow"></span>
        </div>

        <div class="agent-popover__content"><slot /></div>
      </component>
    </div>
  </Transition>
</template>
