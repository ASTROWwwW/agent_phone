...
function getButtons(): HTMLElement[] {
  return links.value
    ? Array.from(links.value.querySelectorAll<HTMLElement>('.agent-tab-button'))
    : []
}
    ...
    const needsUpdate = mutations.some(
      (mutation) =>
        ...
        (mutation.type === 'attributes' &&
          ...
          mutation.target.classList.contains('agent-tab-button')),
    )
...
<template>
  <component
    ...
    class="agent-tabbar"
    :class="{
      'agent-tabbar--floating': props.floating,
      'agent-tabbar--icons': props.icons,
      'agent-tabbar--labels': props.labels,
    }"
    :aria-label="accessibleLabel"
  >
    <span class="agent-tabbar__blur" aria-hidden="true" />
    <span class="agent-tabbar__background" :class="bgClass" aria-hidden="true" />
    <div class="agent-tabbar__inner" :class="innerClass">
      <AgentGlass class="agent-tabbar__pane">
        <div ref="links" class="agent-tabbar__links">
          <slot />
        </div>
        <span
          v-if="hasLinks"
          class="agent-tabbar__highlight"
          :class="{ 'agent-tabbar__highlight--pressed': pressed }"
          :style="highlightStyle"
          aria-hidden="true"
        >
          <span class="agent-tabbar__highlight-inner" />
          <span class="agent-tabbar__highlight-thumb" />
        </span>
      </AgentGlass>
    </div>
  </component>
