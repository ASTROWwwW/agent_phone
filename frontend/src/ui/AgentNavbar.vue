...
import { provideAgentNavbar } from './navbar-context'
...
const collapseState = ref(getAgentNavbarCollapseState(0, titleHeight.value))
const collapseStyle = computed<CSSProperties>(
  () =>
    ({
      '--agent-navbar-collapse-offset': `${collapseState.value.offset}px`,
      '--agent-navbar-compact-title-opacity': `${collapseState.value.compactTitleOpacity}`,
      '--agent-navbar-extended-title-opacity': `${collapseState.value.extendedTitleOpacity}`,
    }) as CSSProperties,
)
...
function updateCollapse(): void {
  ...
  collapseState.value = isCollapsible.value
    ? getAgentNavbarCollapseState(
        observedScrollElement?.scrollTop ?? 0,
        titleHeight.value,
        hasExtendedTitle.value,
      )
    : getAgentNavbarCollapseState(0, titleHeight.value)
...
<template>
  <component
    ...
    class="agent-navbar"
    :class="[
      `agent-navbar--${resolvedVariant}`,
      {
        'agent-navbar--outline': outline,
        'agent-navbar--transparent': transparent,
        'agent-navbar--with-subnavbar': Boolean($slots.subnavbar),
        'agent-navbar--no-navigation': !hasNavigationRow(),
      },
    ]"
    :style="collapseStyle"
  >
    <div class="agent-navbar__blur" aria-hidden="true" />
    <div class="agent-navbar__background" aria-hidden="true" />

    <div
      v-if="hasNavigationRow()"
      class="agent-navbar__inner"
      :class="innerClass"
    >
      <div v-if="hasLeftContent()" class="agent-navbar__left agent-glass-surface">
        <slot name="left">
          <button
            v-if="showBack"
            type="button"
            class="agent-navbar__back"
            :class="`agent-navbar__back--${backAppearance}`"
            :aria-label="accessibleBackLabel"
            @click="emit('back')"
          >
            <svg
              class="agent-navbar__back-icon"
              aria-hidden="true"
              viewBox="0 0 12 20"
            >
              <path
            ...
            <span v-if="showBackText" class="agent-navbar__back-label">
              {{ backLabel }}
            </span>
          </button>
        </slot>
      ...
      <div v-if="!hasExtendedTitle" class="agent-navbar__heading">
        <h1 class="agent-navbar__title">
          <slot name="title">{{ title }}</slot>
        </h1>
        <p v-if="subtitle || $slots.subtitle" class="agent-navbar__subtitle">
          <slot name="subtitle">{{ subtitle }}</slot>
        </p>
      </div>

      <div v-if="hasRightContent()" class="agent-navbar__right agent-glass-surface">
        <slot name="right" />
      </div>
    </div>

    <div v-if="hasExtendedTitle" class="agent-navbar__title-container">
      <div>
        <h1 class="agent-navbar__title">
          <slot name="title">{{ title }}</slot>
        </h1>
    ...
    <div
      ...
      class="agent-navbar__collapsed-heading"
      aria-hidden="true"
    >
      <span class="agent-navbar__title">
        <slot name="title">{{ title }}</slot>
      </span>
    </div>
