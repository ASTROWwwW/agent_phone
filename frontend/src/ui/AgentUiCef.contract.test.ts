...
describe('Agent UI FiveM CEF contracts', () => {
  it('does not import transitional Konsta components into Agent UI', () => {
  ...
  it('keeps Performance hold transforms on the Chrome 103 syntax floor', () => {
    const performanceCss = performanceRules(mainCssSource)
  ...
  it('keeps optional glass blur behind a solid CEF fallback', () => {
    ...
    const violations = blurSources.flatMap(({ content, name }) => {
      ...
      if (!/background(?:-color)?\s*:[^;]*var\(--agent-glass-solid/im.test(css)) {
