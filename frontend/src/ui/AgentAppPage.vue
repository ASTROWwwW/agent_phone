...
import { provideAgentPageScroll } from './page-scroll-context'
...
const pageScroll = provideAgentPageScroll()
  ...
  effectiveAccent.value || effectiveAccentSoft.value
    ? {
        ...(effectiveAccent.value
          ? { '--agent-app-accent': effectiveAccent.value }
          : {}),
        ...(effectiveAccentSoft.value
          ? { '--agent-app-accent-soft': effectiveAccentSoft.value }
          : {}),
      }
    : undefined,
)
...
<template>
  <component
    :is="component"
    v-bind="$attrs"
    class="agent-app-page"
    :class="{ 'agent-app-page--dark': isDark }"
    :style="pageStyle"
    :aria-label="accessibleLabel"
  >
    <div class="agent-app-page__backdrop" aria-hidden="true"></div>
    <slot />
  </component>
</template>
