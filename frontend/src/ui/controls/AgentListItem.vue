...
const listContext = inject(agentListContextKey, undefined)
...
<template>
  <component
    ...
    v-if="groupTitle"
    v-bind="$attrs"
    class="agent-list-item agent-list-item--group-title"
    :class="{
      'agent-list-item--contacts': contacts,
      'agent-list-item--dividers': effectiveDividers,
    }"
  ...
  <component
    ...
    :class="{
      'agent-list-item--disabled': disabled,
      'agent-list-item--active': active,
      'agent-list-item--contacts': contacts,
      'agent-list-item--dividers': effectiveDividers,
      'agent-list-item--with-actions': hasActions,
      'agent-list-item--no-dividers': !effectiveDividers,
      'agent-list-item--label': label,
      'agent-list-item--link': isLink,
      'agent-list-item--menu': menu,
      'agent-list-item--nested': isNested,
    }"
    :role="component === 'div' ? 'listitem' : undefined"
  >
    <component
    ...
    >
      <span
        ...
        class="agent-list-item__media"
        :class="mediaClass"
      >
        <slot name="media">{{ media }}</slot>
      </span>

      <span class="agent-list-item__content" :class="innerClass">
        <small v-if="header || $slots.header" class="agent-list-item__header">
        ...
        <span
          v-if="
            ...
            (isLink && chevron && !menu)
          "
          class="agent-list-item__title-wrap"
        ...
        >
          <div
            ...
            :class="[
              titleFontSizeIos,
              { 'agent-list-item__title--strong': resolvedStrongTitle },
            ]"
          >
            <slot name="title">{{ title }}</slot>
          </div>
          <span
            ...
            class="agent-list-item__after"
          >
            <slot name="after">{{ after }}</slot>
          </span>
          <span
            v-if="isLink && chevron && !menu"
            class="agent-list-item__chevron"
            aria-hidden="true"
          />
        </span>
        <span
          v-if="subtitle || $slots.subtitle"
          class="agent-list-item__subtitle"
        >
          <slot name="subtitle">{{ subtitle }}</slot>
        </span>
        <span v-if="text || $slots.text" class="agent-list-item__text">
          <slot name="text">{{ text }}</slot>
        </span>
        <small v-if="footer || $slots.footer" class="agent-list-item__footer">
    ...
    <div v-if="hasActions" class="agent-list-item__actions">
      <slot name="actions" />
    </div>
    <slot />
  </component>
