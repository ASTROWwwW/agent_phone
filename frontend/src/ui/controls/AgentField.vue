...
<template>
  <component
    ...
    :class="[
      attrs.class,
      {
        'agent-field--disabled': disabled,
        'agent-field--error': Boolean(error),
        'agent-field--floating-label':
          Boolean(label || $slots.label) && floatingLabel,
        'agent-field--floating-raised': isFloatingRaised,
        'agent-field--has-label': Boolean(label || slots.label),
        'agent-field--inline': layout === 'inline',
        'agent-field--outline': outline,
        'agent-field--has-leading': Boolean(slots.leading),
        'agent-field--has-media': hasMedia,
      },
    ]"
    :style="attrs.style"
  >
    <span v-if="hasMedia" class="agent-field__media">
      <slot v-if="$slots.media" name="media" />
      <slot v-else name="leading" />
    </span>
    <div class="agent-field__inner">
      <label
        v-if="label || $slots.label"
        class="agent-field__label"
        :for="resolvedInputId"
      >
        <span class="agent-field__label-text"
          >{{ label }}<slot name="label"
        /></span>
      </label>
      <div class="agent-field__control">
        <slot v-if="$slots.input" name="input" />
        <textarea
          v-else-if="type === 'textarea'"
          v-bind="nativeAttrs"
          :id="resolvedInputId"
          class="agent-field__input agent-field__textarea"
          :class="inputClass"
        ...
        <select
          ...
          v-bind="nativeAttrs"
          :id="resolvedInputId"
          class="agent-field__input agent-field__select"
          :class="inputClass"
          :aria-describedby="describedBy"
          :aria-invalid="error ? true : undefined"
          :aria-label="ariaLabel || undefined"
        ...
        <input
          ...
          @input="handleInput"
        />
        <span v-if="$slots.trailing" class="agent-field__trailing">
          <slot name="trailing" />
        </span>
        <svg
          v-if="dropdown"
          class="agent-field__dropdown"
      ...
      <small v-if="help || $slots.help" :id="helpId" class="agent-field__help">
        {{ help }}<slot name="help" />
      </small>
      <small
        v-if="(error && error !== true) || $slots.error"
        :id="errorId"
        class="agent-field__error"
        role="alert"
      >
      ...
      <small
        ...
        :id="infoId"
        class="agent-field__info"
      >
        {{ info }}<slot name="info" />
      </small>
    </div>
    <span v-if="outline" class="agent-field__border" aria-hidden="true" />
