...
import AgentStatusCard from '@/ui/controls/AgentStatusCard.vue'
...
describe('AgentStatusCard', () => {
  it('renders a textual live status with a semantic tone and indicator', async () => {
    ...
    expect(html).toContain('agent-status-card--success')
    expect(html).toContain('agent-status-card__indicator')
  ...
  it('keeps an indicator in the right-hand column when no icon is present', async () => {
    ...
    expect(controlsStyles).toMatch(
      /\.agent-status-card__copy\s*\{[^}]*flex:\s*1 1 auto;/s,
    )
    expect(controlsStyles).toMatch(
      /\.agent-status-card__indicator\s*\{[^}]*flex:\s*none;/s,
    )
  })
  ...
  it('prefers a trailing slot over the indicator', async () => {
    ...
    expect(html).toContain('agent-status-card__trailing')
    expect(html).toContain('120.5 MHz')
    expect(html).not.toContain('agent-status-card__indicator')
    expect(controlsStyles).toMatch(
      /\.agent-status-card__trailing\s*\{[^}]*flex:\s*none;/s,
    )
  })
})
