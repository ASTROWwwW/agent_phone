...
import AgentCheckbox from './AgentCheckbox.vue'
...
describe('AgentCheckbox', () => {
  it('keeps its native state and accessible label wiring', async () => {
    ...
    expect(html).toContain('agent-checkbox--checked')
    expect(html).toContain('agent-checkbox--indeterminate')
    expect(html).toContain('agent-checkbox__indeterminate')
  ...
  it('matches the Konsta iOS checkbox colors without inventing a glass hold state', () => {
    ...
    const checkboxStyles = controls.slice(
      controls.indexOf('.agent-checkbox {'),
      controls.indexOf('.agent-fab {'),
    )

    expect(checkboxStyles).toContain(
      'border: 1px solid var(--agent-subtle, rgba(0, 0, 0, 0.3))',
    )
    expect(checkboxStyles).toMatch(
      /\.agent-checkbox__indeterminate\s*\{[\s\S]*?width: 75%/,
    )
    expect(checkboxStyles).not.toContain('--agent-shadow-glass')
