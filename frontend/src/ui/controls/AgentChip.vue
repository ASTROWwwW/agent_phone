...
import AgentChipDeleteIcon from './AgentChipDeleteIcon.vue'
...
<template>
  <component
    ...
    class="agent-chip"
    :class="{
      'agent-chip--outline': outline,
      'agent-chip--selected': selected,
      'agent-chip--with-delete': showDelete,
    }"
    :aria-pressed="
    ...
    @click="handleClick"
  >
    <span v-if="$slots.media" class="agent-chip__media">
      <slot name="media" />
    </span>
    <slot />
    <span
