...
const rangeStyle = computed<CSSProperties>(() => ({
  '--agent-range-progress': `${normalizedProgress.value}%`,
}))
...
<template>
  <label
    ...
    class="agent-range"
    :class="{
      'agent-range--captioned': Boolean(caption || $slots.caption),
      'agent-range--disabled': disabled,
    }"
    :for="inputId"
  >
    <span
      v-if="caption || $slots.caption"
      class="agent-range__caption"
      aria-hidden="true"
    >
    ...
    <input
      ...
      @input="handleInput"
    />
    <span v-if="$slots.default" class="agent-range__label">
      <slot />
    </span>
  </label>
</template>
