...
import AgentField from '@/ui/controls/AgentField.vue'

describe('AgentField', () => {
  it('forwards min, max, and step to the native input', async () => {
    const app = createSSRApp(AgentField, {
  ...
  it('exposes the inline settings layout without changing input semantics', async () => {
    ...
    expect(html).toContain('agent-field--inline')
  ...
  it('keeps the leading alias on the canonical Konsta media column', async () => {
    ...
    expect(html).toContain('agent-field--has-leading')
    expect(html).toContain('agent-field--has-media')
    expect(html).toContain('class="agent-field__media"')
    expect(html.indexOf('agent-field__media')).toBeLessThan(
      html.indexOf('agent-field__inner'),
    )
  })
  ...
  it('renders canonical media, inner, label, and control nesting', async () => {
    ...
    expect(html).toMatch(
      /<span class="agent-field__media">[\s\S]*?<\/span><div class="agent-field__inner"><label[\s\S]*?<div class="agent-field__control">/,
    )
  })
  ...
  it('renders a native date input without changing its accessible label', async () => {
    ...
    expect(html).toContain('<label class="agent-field__label" for="birthday"')
    expect(html).toMatch(
  ...
  it('renders typed select options and native options from the default slot', async () => {
    ...
    expect(select).toContain('class="agent-field__input agent-field__select"')
  ...
  it('renders the exact Konsta dropdown triangle', async () => {
    ...
    expect(html).toContain(
      '<svg class="agent-field__dropdown" xmlns="http://www.w3.org/2000/svg" width="8" height="5" viewBox="0 0 8 5" fill="currentColor" aria-hidden="true" focusable="false">',
    )
    expect(html).toContain(
  ...
  it('raises floating labels only after the field has a value', async () => {
    const emptyApp = createSSRApp(AgentField, {
      floatingLabel: true,
      label: 'Name',
      placeholder: 'Your name',
    })
    const valuedApp = createSSRApp(AgentField, {
      floatingLabel: true,
      label: 'Name',
      modelValue: 'Agent',
      placeholder: 'Your name',
    ...
    expect(emptyHtml).toContain('agent-field--floating-label')
    expect(emptyHtml).not.toContain('agent-field--floating-raised')
    expect(valuedHtml).toContain('agent-field--floating-label')
    expect(valuedHtml).toContain('agent-field--floating-raised')
  })
  ...
  it('locks Konsta input, outline, floating-label, and reduced-motion geometry', () => {
    ...
    expect(controls).toMatch(
      /\.agent-field__input\s*\{[^}]*height:\s*40px;[^}]*min-height:\s*40px;[^}]*font-size:\s*16px;[^}]*line-height:\s*24px/s,
    )
    expect(controls).toMatch(/\.agent-field__media,[\s\S]*?padding:\s*8px 0;/)
    expect(controls).toMatch(
      /\.agent-field__inner\s*\{[^}]*padding:\s*12px calc\(16px \+ var\(--agent-safe-area-right, 0px\)\) 12px 0/s,
    )
    expect(controls).toMatch(/\.agent-field--outline\s*\{[^}]*margin:\s*16px 0;/s)
    expect(controls).toMatch(
      /\.agent-field--floating-label:not\(\.agent-field--inline\) \.agent-field__label\s*\{[^}]*transform:\s*translateY\(16px\) scale\(1\.33\)/s,
    )
    expect(controls).toMatch(
      /\.agent-field--floating-label\.agent-field--outline:not\([\s\S]*?\.agent-field__label\s*\{[^}]*transform:\s*translateY\(24px\) scale\(1\.33\)/s,
    )
    expect(controls).toMatch(
      /@media \(prefers-reduced-motion: reduce\)\s*\{[^}]*\.agent-field__label/s,
    )
  })
  ...
  it('renders an explicitly labelled clear control only for a non-empty value', async () => {
    ...
    expect(html).toContain('class="agent-field__clear"')
  ...
  it('does not expose an unnamed clear control', async () => {
    ...
    expect(html).not.toContain('agent-field__clear')
  })

  it('declares its accessible clear event without changing existing events', () => {
    const component = AgentField as unknown as { emits: string[] }
