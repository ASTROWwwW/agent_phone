...
<template>
  <component
    ...
    :class="{
      'agent-searchbar--disabled': disabled,
      'agent-searchbar--focused': focused,
      'agent-searchbar--with-cancel': showCancel,
      'agent-searchbar--with-disable': showDisable,
    }"
    @blur.capture="emit('blur-capture', $event)"
    @focus.capture="emit('focus-capture', $event)"
  >
    <AgentGlass :highlight="false" class="agent-searchbar__control">
      <label v-if="label" class="agent-visually-hidden" :for="resolvedInputId">
        {{ label }}
      </label>
      <svg class="agent-searchbar__icon" aria-hidden="true" viewBox="0 0 24 24">
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
      ...
      <input
        ...
        @input="handleInput"
      />
      <span v-if="$slots.suffix" class="agent-searchbar__suffix">
        <slot name="suffix" />
      </span>
      <button
        v-if="clearButton && localValue"
        class="agent-searchbar__clear"
        type="button"
    ...
    <button
      v-if="showDisable"
      class="agent-searchbar__disable agent-searchbar__cancel"
      type="button"
      ...
      @click="disable"
    >
      <span class="agent-searchbar__disable-icon" aria-hidden="true" />
    </button>
  </component>
</template>
