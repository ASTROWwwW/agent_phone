...
async function renderRow(props: Record<string, unknown>): Promise<string> {
  return renderToString(createSSRApp(AgentSettingsRow, props))
}

describe('AgentSettingsRow', () => {
  it('uses the regular row padding for toggles without compacting them', () => {
    expect(settingsStyles).toMatch(
      /\.agent-settings-row__frame,\s*\.agent-settings-range-row__frame\s*\{[^}]*padding:\s*8px 16px/s,
    )
    expect(settingsStyles).not.toMatch(
      /\.agent-settings-row--toggle\s+\.agent-settings-row__frame\s*\{[^}]*padding-(?:top|bottom):/s,
    )
    expect(settingsStyles).toMatch(
      /\.agent-settings-group--compact\s+\.agent-settings-row__frame,[^}]*padding-top:\s*6px;[^}]*padding-bottom:\s*6px/s,
    )
  })
  ...
  it('renders navigation as a full-row button with value and chevron', async () => {
    ...
    expect(html).toContain('agent-settings-row__value')
    expect(html).toContain('agent-settings-row__chevron')
  ...
  it('marks a selected choice without adding a chevron', async () => {
    ...
    expect(html).toContain('agent-settings-row__check')
    expect(html).not.toContain('agent-settings-row__chevron')
  })
  ...
  it('renders destructive actions as buttons with the danger tone', async () => {
    ...
    expect(html).toContain('<button')
    expect(html).toContain('agent-settings-row--danger')
    expect(html).not.toContain('agent-settings-row__chevron')
  })
  ...
  it('keeps value and chevron when navigation supplies custom trailing content', async () => {
    ...
    const valueId = html.match(
      /<span id="([^"]+)" class="agent-settings-row__value"/,
    )?.[1]
    const trailingId = html.match(
      /<span id="([^"]+)" class="agent-settings-row__trailing"/,
    )?.[1]
