...
import AgentSpinner from '@/ui/controls/AgentSpinner.vue'
import AgentToggle from '@/ui/controls/AgentToggle.vue'
...
<template>
  <li
    ...
    :class="[
      `agent-settings-row--${kind}`,
      `agent-settings-row--${tone}`,
      {
        'agent-settings-row--disabled': disabled,
        'agent-settings-row--has-media': Boolean($slots.leading),
        'agent-settings-row--pending': pending,
        'agent-settings-row--selected': selected,
      },
    ]"
  >
    <div
      v-if="kind === 'toggle'"
      class="agent-settings-row__frame"
      :aria-busy="pending || undefined"
    >
      <span v-if="$slots.leading" class="agent-settings-row__leading">
        <slot name="leading" />
      </span>

      <span class="agent-settings-row__content">
        <label :id="titleId" class="agent-settings-row__title" :for="toggleId">
          <slot name="title">{{ title }}</slot>
        </label>
        <span
          v-if="hasDescription"
          :id="descriptionId"
          class="agent-settings-row__description"
        >
          <slot name="description">{{ description }}</slot>
        </span>
      </span>

      <span class="agent-settings-row__accessory">
        <AgentSpinner v-if="pending" :size="18" />
        <AgentToggle
          :id="toggleId"
    ...
    >
      ...
      <span class="agent-settings-row__content">
        <span :id="titleId" class="agent-settings-row__title">
          <slot name="title">{{ title }}</slot>
        </span>
        <span
          v-if="hasDescription"
      ...
      <span
        v-if="
          ...
          kind === 'navigation'
        "
        class="agent-settings-row__accessory"
      >
        <AgentSpinner v-if="pending" :size="18" />
        <template v-else>
          <span v-if="hasValue" :id="valueId" class="agent-settings-row__value">
            <slot name="value">{{ value }}</slot>
          </span>
          <span
            v-if="hasFreeTrailing"
            :id="trailingId"
            class="agent-settings-row__trailing"
          >
            <slot name="trailing" />
          </span>
          <Check
            v-if="kind === 'choice' && selected"
            class="agent-settings-row__check"
            :size="20"
            :stroke-width="2.25"
            aria-hidden="true"
          />
          <ChevronRight
            ...
            class="agent-settings-row__chevron"
            :size="18"
            :stroke-width="2"
            aria-hidden="true"
          />
