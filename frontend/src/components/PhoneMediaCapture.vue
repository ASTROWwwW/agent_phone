...
async function failUpload(
  ...
  console.error('[Agent Phone Media] Upload failed.', {
  ...
  if (!response.success) {
    console.error('[Agent Phone Media] Could not forward upload diagnostics.', {
      correlationId: debug.correlationId,
...
async function uploadReady(ready: UploadReady): Promise<void> {
  ...
  console.info('[Agent Phone Media] Capture prepared for upload.', {
    bytes: blob.size,
    correlationId: ready.correlationId,
    mimeType: blob.type,
  ...
  try {
    ...
    console.info('[Agent Phone Media] FiveManage upload responded.', {
      correlationId: ready.correlationId,
    ...
    console.info(
      '[Agent Phone Media] Upload completion forwarded to the server.',
      {
        correlationId: ready.correlationId,
      },
    )
...
function onMessage(event: MessageEvent): void {
  ...
  if (message.type === 'camera:recordStart') {
    ...
    console.info('[Agent Phone Media] Upload-ready received.', {
      correlationId: ready.correlationId,
      type: ready.mediaType,
    })
