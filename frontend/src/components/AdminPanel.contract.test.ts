...
describe('standalone admin panel contracts', () => {
  ...
  it('uses a fixed blue accent and replaces appearance controls with live statistics', () => {
    ...
    expect(source).not.toContain("'agent-phone-admin-accent'")
    expect(source).not.toContain("'agent-phone-admin-font-family'")
    expect(source).not.toContain("'agent-phone-admin-font-size'")
  ...
  it('stages app changes locally and saves them only from the toolbar action', () => {
    ...
    expect(server).toContain(
      'Bridge.Callbacks.Register("agent_phone:admin:save-apps"',
    )
  })
  ...
  it('applies global bundled-app availability to phones and admin management', () => {
    ...
    expect(phoneServer).toContain('function AgentPhone.IsAppEnabled(app_id)')
    expect(phoneServer).toContain('function AgentPhone.GetDisabledApps()')
    expect(phoneServer).toContain('disabledApps = AgentPhone.GetDisabledApps()')
    expect(server).toContain('if not AgentPhone.IsAppEnabled(app_id) then')
    expect(server).toContain('disabledApps = AgentPhone.GetDisabledApps()')
  ...
  it('protects activity views and device moderation with ownership and audit checks', () => {
    for (const endpoint of [
      ...
      expect(server).toContain(
        `Bridge.Callbacks.Register("agent_phone:admin:${endpoint}"`,
      )
    }
    ...
    expect(simServer).toContain('function AgentPhoneSim.ChangeNumber(')
    expect(simServer).toContain('UPDATE IGNORE `agent_phone_sims`')
  ...
  it('authorizes every server request without requiring a phone session', () => {
    expect(server).not.toContain('AgentPhone.RequireSession(source)')
    expect(server).toContain(
  ...
  it('opens directly from the configurable command with dedicated focus', () => {
    ...
    expect(phoneClient).toContain('RegisterNetEvent("agent_phone:admin:launch"')
    expect(phoneClient).toContain('AgentPhoneFocus.SetAdminPanel(true)')
    expect(focusClient).toContain('function AgentPhoneFocus.SetAdminPanel(open)')
  })
  ...
  it('validates ownership, app policy, and revision before a batch mutation', () => {
    ...
    expect(server).toContain('AgentPhone.RefreshDevice(data.imei)')
  })
  ...
  it('loads the SQL phone configurator before framework-owned configuration is read', () => {
    ...
    expect(configuratorServer).toContain(
      '{ __agentType = "map", entries = entries }',
    )
    ...
    expect(configuratorClient).toContain(
      'TriggerEvent("agent_phone:configurator:updated"',
    )
    expect(phoneClient).toMatch(
      /AddEventHandler\("agent_phone:configurator:updated"[\s\S]*?AgentPhoneLocales\.Resolve\(Config\.Bridge\.Locale\)[\s\S]*?AgentPhoneApps\.SendCatalog\(\)[\s\S]*?type = "device:updated"/,
    )
    expect(server).toContain(
    ...
    expect(mediaImportServer).toMatch(
      /AddEventHandler\("agent_phone:configurator:serverUpdated"[\s\S]*?if initialized then[\s\S]*?build_registry\(\)/,
    )
