...
import type {
  ...
  AgentPhoneAppBridgeRequest,
  AgentPhoneAppBridgeResponse,
  AgentPhoneAppContextV1,
} from '@/types/apps'
import {
  createCustomAppBridgeRequestHandler,
  getCustomAppFrameBootstrapMessages,
  getAgentPhoneAppCapabilities,
...
function sendResponse(response: AgentPhoneAppBridgeResponse): void {
  postToFrame({
    appId: props.app.id,
    ...response,
    protocolVersion: PROTOCOL_VERSION,
    type: 'agent-phone-app:response',
  })
}

async function handleBridgeRequest(
  request: AgentPhoneAppBridgeRequest,
): Promise<void> {
...
function onFrameMessage(event: MessageEvent): void {
  ...
  if (message.type === 'agent-phone-app:request') {
    void handleBridgeRequest(message as unknown as AgentPhoneAppBridgeRequest)
  }
}
