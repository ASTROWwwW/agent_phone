...
describe('Companies service-line message routing contract', () => {
  it('enables the police service line and persists an indexed route channel', () => {
    ...
    expect(schema).toContain(
      'name = "idx_agent_phone_company_requests_service_line"',
    )
  })
  ...
  it('resolves short service numbers before the normal SIM-number path', () => {
    ...
    const send = sourceBlock(
      messagesServer,
      'Bridge.Callbacks.Register("agent_phone:messages:send"',
      '\n\nend)',
    )

    expect(resolver).toContain('AgentPhoneCompanies.GetServiceLine(value)')
    ...
    expect(send).toContain('AgentPhoneCompanies.RouteServiceLineMessage')
  ...
  it('atomically mirrors inbound SMS into one active company request', () => {
    const route = sourceBlock(
      ...
      'Bridge.Callbacks.Register("agent_phone:companies:create-request"',
    )

    expect(route).toContain('current_device(source, false)')
    expect(route).toContain('service_line.canMessage')
    expect(route).toContain('UPDATE `agent_phone_sims`')
    expect(route).toContain("`channel` = 'service_line'")
    expect(route).toContain('AND NOT EXISTS (')
    expect(route).toContain('INSERT INTO `agent_phone_company_request_messages`')
    expect(route).toContain('INSERT INTO `agent_phone_sms_messages`')
    ...
    expect(route).toContain('"agent_phone:companies:notification"')
  })

  it('mirrors authorized company replies back to the customer SMS thread', () => {
    const sendMessage = sourceBlock(
      companiesServer,
      'Bridge.Callbacks.Register("agent_phone:companies:send-message"',
      'Bridge.Callbacks.Register("agent_phone:companies:claim-request"',
    )

    expect(sendMessage).toContain('access.row.channel == "service_line"')
    expect(
      sendMessage.match(/INSERT INTO `agent_phone_sms_messages`/g)?.length,
    ).toBe(2)
    expect(sendMessage).toContain("message.`sender_type` = 'customer'")
    expect(sendMessage).toContain("message.`sender_type` = 'company'")
    expect(sendMessage).toContain('"agent_phone:messages:changed"')
    expect(sendMessage).toContain('"agent_phone:messages:new"')
  })
