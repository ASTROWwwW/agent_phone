...
import SkyFlappyBird from '@/features/games/agent-flappy/SkyFlappyBird.vue'
import { useSkyFlappyStore } from '@/features/games/agent-flappy/store'
import type { SkyFlappyDesign, SkyFlappyObstacle } from '@/features/games/agent-flappy/types'
  ...
  >
    ...
    <section v-else-if="game" class="flappy-game">
      <div class="flappy-toolbar">
        <SkyButton glass icon-only rounded type="button" :aria-label="phone.t('Apps.agentFlappy.backToMenu')" @pointerdown.stop="flappy.showMenu()" @click.stop="flappy.showMenu()"><ChevronLeft :size="19" /></SkyButton>
        ...
        <SkyButton glass icon-only rounded type="button" :aria-label="phone.t('Apps.agentFlappy.pause')" @click="togglePause"><Pause v-if="game.status === 'playing'" :size="16" fill="currentColor" /><Play v-else :size="16" fill="currentColor" /></SkyButton>
      </div>
