...
const server = readFileSync(
  new URL('../../../../agent_phone/source/server/crypto.lua', import.meta.url),
  'utf8',
)
const passwordProvider = readFileSync(
  new URL(
    '../../../../agent_phone/source/server/crypto_password.js',
    import.meta.url,
  ),
  'utf8',
)
...
describe('VaultX crypto app contracts', () => {
  it('uses Agent UI without introducing Konsta components', () => {
  ...
  it('includes advanced market detail and persistent profile controls', () => {
    ...
    expect(server).toContain('agent_phone:crypto:update-profile')
  ...
  it('keeps registration focused and crypto-key transfers server-authoritative', () => {
    const transferServer = server.slice(
      ...
      server.indexOf('Bridge.Callbacks.Register("agent_phone:crypto:quote"'),
    )
    ...
    expect(server).toContain(
      'Bridge.Callbacks.Register("agent_phone:crypto:recipient"',
    )
    expect(server).toContain(
      'Bridge.Callbacks.Register("agent_phone:crypto:transfer"',
    )
  ...
  it('keeps all consequential calculations and state transitions on the server', () => {
    expect(server).toContain(
      'Bridge.Callbacks.Register("agent_phone:crypto:quote"',
    )
    expect(server).toContain(
      'Bridge.Callbacks.Register("agent_phone:crypto:execute"',
    )
  ...
  it('streams server-driven market movement into live portfolio values', () => {
    ...
    expect(server).toContain('TriggerClientEvent("agent_phone:crypto:changed"')
