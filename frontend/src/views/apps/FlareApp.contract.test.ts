...
describe('FlareApp profile editing contract', () => {
  ...
  it('exposes confirmed sign-out and destructive Flare account deletion', () => {
    ...
    expect(localeSource).toContain(
      'This signs the whole phone out of Agent Cloud.',
    )
    expect(localeSource).toContain(
      'Signing out affects every app that uses Agent Cloud on this phone.',
    )
  })
  ...
  it('deletes all account-owned Flare data in one server transaction', () => {
    ...
    expect(serverSource).toContain(
      'Bridge.Callbacks.Register("agent_phone:flare:delete-profile"',
    )
    expect(serverSource).toContain(
      'AgentPhone.AllowOperation(source, "flare_profile_delete", 3, 60)',
    )
    expect(serverSource).toContain('Bridge.Database.Transaction({')
    expect(serverSource).toContain('DELETE FROM `agent_phone_flare_matches`')
    expect(serverSource).toContain('DELETE FROM `agent_phone_flare_swipes`')
    expect(serverSource).toContain('DELETE FROM `agent_phone_flare_profiles`')
  })
})
