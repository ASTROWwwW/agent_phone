...
describe('FlipTokApp Agent UI contract', () => {
  ...
  it('sends reports to Discord and keeps moderation out of the phone UI', () => {
    ...
    expect(serverSource).not.toContain(
      'INSERT IGNORE INTO `agent_phone_fliptok_reports`',
    )
  })
  ...
  it('supports camera videos, photo slides, and a safe composer return path', () => {
    ...
    expect(migrationSource).toContain('agent_phone_fliptok_video_media')
  ...
  it('validates and persists custom audio links on the server', () => {
    ...
    expect(youtubeSource).toContain('function AgentPhoneYouTube.ParseId')
    expect(youtubeSource).toContain('function AgentPhoneYouTube.FetchMetadata')
