...
const exportedAgentTags = new Set(
  agentUiIndexSources
    .flatMap((source) =>
      [...source.matchAll(/default as (Agent[A-Za-z0-9]+)/g)].map(
        ([, name]) => name,
      ),
    )
...
describe('phone apps use Agent UI', () => {
  ...
  it('keeps app-specific support components on Agent UI', () => {
  ...
  it('only renders Agent UI tags that are exported by the shared library', () => {
    for (const { file, source } of appSources) {
      const tags = [...source.matchAll(/<\/?(agent-[a-z0-9-]+)/g)].map(
        ([, tag]) => tag,
      )

      for (const tag of tags)
        expect(exportedAgentTags, `${file}: ${tag}`).toContain(tag)
    }
  })
  ...
  it('does not pass obsolete Konsta color maps into Agent components', () => {
    for (const { file, source } of [...appSources, ...appSupportSources]) {
  ...
  it('uses top notifications instead of app-level toasts', () => {
    for (const { file, source } of appSources) {
      expect(source, file).not.toMatch(/\bAgentToast\b|<agent-toast|<k-toast/)
  ...
  it('uses shared liquid glass for remaining compact interaction controls', () => {
    ...
    for (const [file, minimum] of Object.entries(minimumGlassButtons)) {
      ...
      const glassButtons = source.match(
        /<(?:AgentButton|agent-button)(?=[^>]*\bglass\b)[^>]*>/g,
      )
