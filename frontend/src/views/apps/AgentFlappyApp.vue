...
import { playAgentFlappySound } from '@/features/games/agent-flappy/audio'
import AgentFlappyBird from '@/features/games/agent-flappy/AgentFlappyBird.vue'
import { useAgentFlappyStore } from '@/features/games/agent-flappy/store'
import type { AgentFlappyDesign, AgentFlappyObstacle } from '@/features/games/agent-flappy/types'
import { usePhoneStore } from '@/stores/phone'
import { AgentButton } from '@/ui'

const phone = usePhoneStore()
const flappy = useAgentFlappyStore()
const game = computed(() => flappy.game)
const designs: AgentFlappyDesign[] = ['dawn', 'neon', 'storm']
...
function obstacleStyle(obstacle: AgentFlappyObstacle): CSSProperties {
...
function runFrame(time: number): void {
  ...
  if (previousTime > 0) {
    ...
    if (game.value && game.value.score > previousScore) {
      playAgentFlappySound('point', flappy.soundEnabled)
    }
    const nextStatus: string | undefined = flappy.game?.status
    if (nextStatus === 'over') {
      playAgentFlappySound('crash', flappy.soundEnabled)
...
function flap(): void {
  ...
  flappy.flap()
  flapEffect.value += 1
  playAgentFlappySound('flap', flappy.soundEnabled)
}
...
function toggleSound(): void {
  ...
  if (enabled) playAgentFlappySound('point', true)
}
...
<template>
  ...
  >
    <header v-if="flappy.menuOpen" class="flappy-header">
      <h1 class="agent-type-display">{{ phone.t('Apps.agentFlappy.name') }}</h1>
      <AgentButton
        glass
        icon-only
        rounded
    ...
    <section v-if="flappy.menuOpen" class="flappy-menu">
      <div class="flappy-menu__hero" aria-hidden="true">
        <AgentFlappyBird class="agent-bird agent-bird--hero" />
    ...
    <section v-else-if="game" class="flappy-game">
      <div class="flappy-toolbar">
        <AgentButton glass icon-only rounded type="button" :aria-label="phone.t('Apps.agentFlappy.backToMenu')" @pointerdown.stop="flappy.showMenu()" @click.stop="flappy.showMenu()"><ChevronLeft :size="19" /></AgentButton>
        <div><span>{{ phone.t('Apps.agentFlappy.score') }}</span><strong>{{ game.score }}</strong></div>
        <div><span>{{ phone.t('Apps.agentFlappy.best') }}</span><strong>{{ flappy.highScore }}</strong></div>
        <AgentButton glass icon-only rounded type="button" :aria-label="phone.t('Apps.agentFlappy.pause')" @click="togglePause"><Pause v-if="game.status === 'playing'" :size="16" fill="currentColor" /><Play v-else :size="16" fill="currentColor" /></AgentButton>
      </div>

      <button type="button" class="flappy-stage" :class="{ 'flappy-stage--crashed': game.status === 'over' && !gameOverVisible }" :aria-label="phone.t('Apps.agentFlappy.flap')" @pointerdown.stop.prevent="flap">
...
.flappy-toolbar button{--agent-touch-target:32px;width:32px;height:32px}
