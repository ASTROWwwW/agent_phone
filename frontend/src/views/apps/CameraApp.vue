...
async function requestPhoto(): Promise<void> {
  ...
  console.info('[Agent Phone Media] Camera requested a photo upload.', {
    correlationId: id,
  })
  ...
  if (!response.success) {
    console.error('[Agent Phone Media] Photo upload request was rejected.', {
      correlationId: id,
      error: response.error,
    })
    window.postMessage(
...
function onMessage(event: MessageEvent): void {
  ...
  if (message.type === 'camera:zoom') {
    ...
    console.info('[Agent Phone Media] Camera received an upload result.', {
