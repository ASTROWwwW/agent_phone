...
import { AGENT_UI_DEMO_CATALOG } from './agent-ui-demo/catalog'

const developmentDirectory = fileURLToPath(new URL('.', import.meta.url))
const demoDirectory = join(developmentDirectory, 'agent-ui-demo')
const pagesDirectory = join(demoDirectory, 'pages')
const viewSource = readFileSync(
  join(developmentDirectory, 'AgentUiKitchenSinkView.vue'),
  'utf8',
)
const homeSource = readFileSync(
  join(demoDirectory, 'AgentUiDemoHome.vue'),
  'utf8',
)
const demoPageSource = readFileSync(
  join(demoDirectory, 'AgentUiDemoPage.vue'),
  'utf8',
)
const demoCssSource = readFileSync(join(demoDirectory, 'demo.css'), 'utf8')
const extensionsSource = readFileSync(
  join(pagesDirectory, 'AgentExtensionsDemo.vue'),
  'utf8',
)
...
const publicComponents = Array.from(
  new Set(
    publicIndexSources.flatMap((source) =>
      Array.from(
        source.matchAll(/default\s+as\s+(Agent[A-Za-z0-9_]*)/g),
        ([, component]) => component,
      ),
    ),
  ),
...
describe('development Agent UI Kitchen Sink contract', () => {
  it('mirrors the exact Konsta 5.3 Vue component submenu catalog', () => {
    expect(AGENT_UI_DEMO_CATALOG.map(({ id, title }) => [id, title])).toEqual(
      referenceCatalog,
    )
    expect(new Set(AGENT_UI_DEMO_CATALOG.map(({ id }) => id)).size).toBe(35)

    const missingPages = AGENT_UI_DEMO_CATALOG.filter(
  ...
  it('uses a real catalog and parameterized submenu navigation', () => {
    expect(homeSource).toContain('v-for="entry in AGENT_UI_DEMO_CATALOG"')
    ...
    expect(routerSource).toContain("path: '/development/agent-ui/:demo?'")
  ...
  it('keeps the shared demo shell aligned with the Konsta reference', () => {
    ...
    expect(demoCssSource).toMatch(/\.agent-ui-demo-copy\s*\{\s*margin:\s*0;\s*\}/)
    expect(homeSource).not.toMatch(/<AgentPopover[\s\S]*?\sangle(?:\s|>)/)
  ...
  it('preserves the locally expressible Konsta examples and states', () => {
    ...
    expect(cards).toContain('<AgentMediaCard')
    ...
    expect(contacts).toContain('<AgentListItem contacts group-title')
    ...
    expect(fab).toContain('fab-demo__button--right-top agent-ui-demo-color-red')
    expect(listButton).toContain('<AgentList dividers outline strong>')
    ...
    expect(notification).not.toContain('<AgentDialogButton strong')
    expect(panels).toContain('agent-ui-demo-panel-page--floating')
    expect(panels).toContain('--agent-safe-area-top: 0px;')
    expect(popover).not.toMatch(/<AgentPopover[\s\S]*?\sangle(?:\s|>)/)
    ...
    expect(tabbar).not.toContain('color: var(--agent-muted)')
    expect(toggle).toMatch(/<AgentListItem[\s\S]*?\slabel[\s\S]*?<AgentToggle/)
  ...
  it('demonstrates every public Agent component without Konsta runtime code', () => {
  ...
  it('keeps the lazy route and launcher development-only', () => {
    expect(routerSource).toMatch(
      /const developmentRoutes[^=]*=\s*import\.meta\.env\.DEV[\s\S]*?import\('@\/views\/development\/AgentUiKitchenSinkView\.vue'\)/,
    )
    expect(routerSource).toContain("name: 'development-agent-ui'")
    expect(appSource).toContain("route.name === 'development-agent-ui'")
    ...
    expect(settingsSource).toContain(
      "router.push({ name: 'development-agent-ui' })",
    )
    expect(settingsSource).not.toContain('AgentUiKitchenSinkView')
    expect(viewSource).not.toContain('<AgentProvider')
  ...
  it('centers the Konsta text-input stepper examples', () => {
    expect(stepperSource).toContain('agent-ui-demo-stepper-inputs')
    expect(stepperSource).toMatch(
      /\.agent-ui-demo-stepper-inputs\s*\{[\s\S]*?justify-items: center;/,
    )
  })
})
