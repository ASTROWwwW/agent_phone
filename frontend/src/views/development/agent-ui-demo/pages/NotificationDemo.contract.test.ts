...
describe('NotificationDemo Konsta parity', () => {
  ...
  it('uses the exact iOS close affordance and page rhythm', () => {
    const notificationBlocks = source.match(
      /<AgentNotification[\s\S]*?<\/AgentNotification>/g,
    )
    ...
    expect(source).toContain('gap: var(--agent-space-4)')
    expect(source).toMatch(
      /\.agent-ui-demo-notification__icon\s*\{[\s\S]*?width:\s*28px;[\s\S]*?height:\s*28px;/,
    )
  })
})
