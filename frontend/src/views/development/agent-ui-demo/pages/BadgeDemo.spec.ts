...
describe('BadgeDemo', () => {
  it('uses the plain Konsta-style navbar link for the profile action', () => {
    expect(source).toContain('AgentLink,')
    expect(source).toMatch(
      /<AgentLink[\s\S]*?aria-label="Profile, 5 notifications"[\s\S]*?class="badge-demo__profile-action"[\s\S]*?icon-only[\s\S]*?>/,
    )
    expect(source).not.toContain('AgentGlass,')
  })

  it('keeps the profile action circular, accessible, and token sized', () => {
    expect(source).toMatch(
      /\.badge-demo__profile-action\s*\{[^}]*width:\s*var\(--agent-touch-target, 44px\)[^}]*height:\s*var\(--agent-touch-target, 44px\)[^}]*border:\s*0[^}]*border-radius:\s*50%[^}]*color:\s*inherit/s,
    )
    expect(source).toMatch(
      /\.badge-demo__profile-action:focus-visible\s*\{[^}]*outline:\s*2px solid var\(--agent-app-accent, #007aff\)/s,
    )
    expect(source).toMatch(
  ...
  it('matches the reference badge colors and default primary CEO badge', () => {
    ...
    expect(source).toMatch(
      /title="Foo Bar"[\s\S]*?<AgentBadge class="badge-demo__badge--gray">0<\/AgentBadge>/,
    )
    expect(source).toMatch(
      /title="Ivan Petrov"[\s\S]*?<AgentBadge>CEO<\/AgentBadge>/,
    )
