...
<template>
  <AgentUiDemoPage title="Tabbar" with-tabbar>
    <AgentList inset strong>
      <AgentListItem title="Tabbar Labels">
        <template #after>
          <AgentToggle v-model="isTabbarLabels" aria-label="Show tabbar labels" />
        </template>
      </AgentListItem>
      <AgentListItem title="Tabbar Icons">
        <template #after>
          <AgentToggle v-model="isTabbarIcons" aria-label="Show tabbar icons" />
        </template>
      </AgentListItem>
    </AgentList>

    <AgentBlock aria-live="polite" class="agent-ui-demo-stack" inset strong>
      <p class="agent-ui-demo-tabbar__title">
        <b>{{ activeLabel }}</b>
      </p>
      <p class="agent-ui-demo-tabbar__copy">
      ...
      >
        ...
        >
          <template v-if="isTabbarIcons" #icon>
            <AgentIcon :size="26"><component :is="tab.icon" /></AgentIcon>
          </template>
        </AgentTabButton>
      </AgentTabBar>
    </template>
...
<style scoped>
.agent-ui-demo-tabbar__title,
.agent-ui-demo-tabbar__copy {
  margin: 0;
}
