...
const migration = readFileSync(
  new URL(
    '../../agent_phone/source/server/lb_phone_migration.lua',
    import.meta.url,
  ),
  'utf8',
)
...
describe('LB Phone social migration contract', () => {
  ...
  it('refreshes device sessions when the active LB account changed', () => {
    expect(migration).toMatch(
      /INSERT INTO `agent_phone_picstagram_sessions`[\s\S]*?ON DUPLICATE KEY UPDATE `profile_id` = VALUES\(`profile_id`\)/,
    )
    expect(migration).toMatch(
      /INSERT INTO `agent_phone_fliptok_sessions`[\s\S]*?ON DUPLICATE KEY UPDATE `profile_id` = VALUES\(`profile_id`\)/,
    )
  })
  ...
  it('imports LB Tinder data into the Flare domain', () => {
    ...
    expect(migration).toContain('INSERT IGNORE INTO `agent_phone_flare_profiles`')
    expect(migration).toContain(
      'INSERT IGNORE INTO `agent_phone_flare_profile_photos`',
    )
    expect(migration).toContain('INSERT IGNORE INTO `agent_phone_flare_swipes`')
    expect(migration).toContain('INSERT IGNORE INTO `agent_phone_flare_matches`')
  })
})
