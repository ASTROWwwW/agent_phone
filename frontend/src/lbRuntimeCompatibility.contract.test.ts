...
describe('LB runtime compatibility contracts', () => {
  it('routes legacy call and SMS actions through Agent Phone', () => {
    ...
    expect(phoneBridge).toContain(
      'AgentPhoneCompatibility.RegisterExportAlias("lb-phone", "CreateCall", create_call)',
    )
    expect(callsClient).toContain(
      'Bridge.Callbacks.Trigger("agent_phone:calls:dial"',
    )
    expect(callsServer).toContain(
      'AgentPhoneCompanies.GetServiceLineForCompany(data.company)',
    )
    expect(phoneBridge).toContain(
      'AgentPhoneCompatibility.RegisterExportAlias("lb-phone", "CreateSMS", create_sms)',
    )
  ...
  it('redirects PicChat phone identity to the Sky SIM table without deleting orphaned data', () => {
    ...
    expect(migration).toContain('"agent_phone_sims"')
    ...
    expect(migration).toContain('Agent Phone startup will continue')
