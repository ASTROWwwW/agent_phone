...
export type AgentPhoneAppCapability =
  | 'app.close'
  | 'app.open'
  | 'device.storage.get'
  | 'device.storage.set'
...
export type AgentPhoneAppBridgeMethod =
  | 'app.close'
  | 'app.open'
  | 'device.storage.get'
  | 'device.storage.set'
...
export type AgentPhoneAppBridgeRequest = {
  method: string
  payload?: unknown
  requestId: string
}

export type AgentPhoneAppBridgeResponse = {
  data?: unknown
  error?: string
  requestId: string
  success: boolean
...
export type AgentPhoneAppContextV1 = {
  appId: string
  capabilities: AgentPhoneAppCapability[]
