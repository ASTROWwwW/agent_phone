...
describe('Companies outbound service-line call contract', () => {
  ...
  it('accepts only a target number and derives the company from the live server member', () => {
    const callback = sourceBlock(
      companiesServer,
      `Bridge.Callbacks.Register(${quote}agent_phone:companies:dial-service-line${quote}`,
    ...
    expect(callback).toContain(
      'AgentPhoneCalls.StartCompanyCall(source, member.company_id, phone_number)',
    )
  ...
  it('revalidates call permission and presents the configured service number as caller ID', () => {
    const startCompanyCall = sourceBlock(
      callsServer,
      'function AgentPhoneCalls.StartCompanyCall(',
      `Bridge.Callbacks.Register(${quote}agent_phone:calls:dial${quote}`,
    )

    expect(startCompanyCall).toContain(
      'AgentPhoneCompanies.CanPlaceCompanyCall(source, company_id)',
    )
    expect(startCompanyCall).toContain(
      'AgentPhoneCompanies.GetServiceLineForCompany(company_id)',
    )
    expect(startCompanyCall).toContain(
...
describe('Companies background call availability contract', () => {
  it('keeps call availability enabled after the phone UI closes', () => {
    const closeDevice = sourceBlock(
      phoneServer,
      `Bridge.Callbacks.Register(${quote}agent_phone:device:close${quote}`,
      `Bridge.Callbacks.Register(${quote}agent_phone:device:notification-open${quote}`,
    )
  ...
  it('routes background calls only to an owned phone with the same registered SIM', () => {
    const getCallTargets = sourceBlock(
      companiesServer,
      'function AgentPhoneCompanies.GetCallTargets(',
      '\n\nlocal function profile_row(',
    )

    expect(getCallTargets).toContain('AgentPhone.LoadDevice(readiness.imei)')
    expect(getCallTargets).toContain(
      'AgentPhone.FindDeviceSlots(source, readiness.imei)',
    )
