...
describe('shared contact block contract', () => {
  it('stores the current SIM as blocker and the selected contact as blocked', () => {
    const callback = callsServer.slice(
      callsServer.indexOf('Bridge.Callbacks.Register("agent_phone:calls:block"'),
    )

    expect(callback).toContain('INSERT IGNORE INTO `agent_phone_call_blocks`')
  ...
  it('rejects SMS before insertion when the recipient blocked the sender', () => {
    ...
    const blockCheck = callback.indexOf('FROM `agent_phone_call_blocks`')
    const insert = callback.indexOf('INSERT INTO `agent_phone_sms_messages`')
  ...
  it('creates the block table during runtime upgrades with matching constraints', () => {
    const table = migration.slice(
      migration.indexOf('name = "agent_phone_call_blocks"'),
      migration.indexOf('name = "agent_phone_call_entries"'),
    )
    ...
    expect(table).toContain('idx_agent_phone_call_blocks_blocked')
...
describe('SMS batch ordering contract', () => {
  it('persists sub-second timestamps before applying the stable id tie-breaker', () => {
    expect(install).toMatch(
      /CREATE TABLE IF NOT EXISTS `agent_phone_sms_messages`[\s\S]*`created_at` DATETIME\(6\) NOT NULL DEFAULT CURRENT_TIMESTAMP\(6\)/,
    )
    expect(migration).toMatch(
      /ALTER TABLE `agent_phone_sms_messages`[\s\S]*MODIFY COLUMN `created_at` DATETIME\(6\) NOT NULL DEFAULT CURRENT_TIMESTAMP\(6\)/,
    )
