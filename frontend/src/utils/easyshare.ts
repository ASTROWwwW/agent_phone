...
export function easyShareCrewLinkInviteCode(
  ...
  if (typeof link === 'string') {
    const match = link.match(/^agentphone:\/\/crewlink\/invite\/([a-z0-9]{8})$/i)
...
export function easyShareDarkChatInviteCode(
  ...
  const match = link.match(
    /^agentphone:\/\/darkchat\/invite\/(DC-[A-Z0-9]{4}-[A-Z0-9]{4})$/i,
  )
...
export function easyShareMusicTarget(
  ...
  if (kind === 'track') {
    const match = link.match(
      /^agentphone:\/\/music\/(server|youtube)\/([^/?#]+)$/,
    )
    if (match) {
      return {
        id: match[2],
  ...
  if (kind === 'playlist') {
    const match = link.match(/^agentphone:\/\/music\/playlist\/([^/?#]+)$/)
